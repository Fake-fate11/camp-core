...
This plan is prospective only. It does not authorize a new Fresh run, change
the sealed B4 denominator, or alter
`honest_no_claim_under_frozen_preregistered_all_gate`.

| Evidence gap | Minimum new nonholdout evidence | Required binding |
|---|---|---|
| Collision severity | Qualified contact impulse/delta-v, relative velocity, dynamics, sensor and simulator validity | Per-tick actor/contact identity and calibrated units |
...
Any future claim requires a new nonholdout acquisition plan with endpoints,
directions, thresholds, multiplicity, hard gates, missing-data policy, and
sample size frozen before outcomes. The present v2 grid and 5 s TTC horizon
remain project-descriptive only.

Evaluation v2 itself is
`exploratory_posthoc_not_claim_authorizing`; it does not establish Fresh
benefit, real-road safety, broad unseen-map generalization, industrial
comfort/conformity, native-ranked Top1, promotion, deployment, online
