...
| B4-RC-014 | P0 | The declarative ABI inherited the candidate0-supplementary cross-arm identity flag into Static14D/Scene14D even though the actual CAMP producer never emits that diagnostic-only field | Declare the flag only on the candidate0-supplementary branch and keep method branches exact | implementation and focused regression pending |
