...
def train(rank, args):
    ...
    if rank == 0 and not hyperparams["debug"]:
        ...
        # DANGEROUS WANDB BUG AVOIDED:
        # Do NOT do `hyperparams = run.config`! WandB strips empty nested dicts
        # (like `"limits": {}`) during sync, which crashes the Trajectron architecture!
    ...
    if hyperparams.get("adaptive", False) and not hyperparams.get("only_k0", False):
    ...
    log_writer = None
    model_dir = None
    if not hyperparams.get("debug", False):
        ...
        if rank == 0:
            ...
            with open(os.path.join(model_dir, "config.json"), "w") as conf_json:
                # Use standard JSON serialization since we bypassed run.config
                if hasattr(hyperparams, "as_dict"):
    ...
    # BUG AVOIDED: Do NOT restrict the dataset using the archaic `.pkl` challenge index!
    # TrajData 1.5.0 uses updated SQLite Agent IDs. Overwriting the index with the 
    # old `.pkl` causes DataLoader workers to fetch `None` instances and crash!
