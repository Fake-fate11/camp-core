...
SCRIPT_PATH = (
    ...
    / "scripts"
    / "integrations"
    / "review_diffusion_planner_dp_camp_v16_nuscenes_fixed_dp_candidate_tensor_smoke_result.py"
)
HEAD = "0b8c62c8a788543dfc9850acc229d1d9fe5d6ba5"
SOURCE_ROOT_SHA = "55a55c99b65ddc24b816f60ad886c7044b3fedf60f5e0be4aa84a9df972b487f"


def _load_module():
    spec = importlib.util.spec_from_file_location("v16_nuscenes_fixed_dp_smoke_result_review", SCRIPT_PATH)
...
def test_v16_nuscenes_fixed_dp_smoke_result_review_passes_256_records(tmp_path: Path) -> None:
    module = _load_module()
    ...
    assert report["record_review"]["record_count"] == 256
    ...
    assert report["timing_summary"]["source_wall_clock_seconds"] == 1498.016563
    assert report["timing_summary"]["per_record_seconds"]["count"] == 256
...
def test_v16_nuscenes_fixed_dp_smoke_result_review_rejects_bad_candidate_count(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module, bad_record={"candidate_count": 7})

    report = module.build_report(**fixture)

    assert report["final_decision"]["passed"] is False
    assert "records_candidate_count_all_8" in report["final_decision"]["failed_checks"]


def _write_fixture(tmp_path: Path, module, bad_record: dict | None = None) -> dict:
    artifact = tmp_path / "source_retry"
    artifact.mkdir()
    docs = tmp_path / "docs"
    docs.mkdir()
    doc_text = "\n".join(
    ...
    records = [_record(module, index, bad_record if index == 123 else None) for index in range(256)]
    ...
    summary = {
        ...
        "wall_clock_seconds": 1498.016563,
    }
    _write_json(artifact / module.SOURCE_JSON_NAME, summary)
    _write(artifact / module.SOURCE_MD_NAME, "# Smoke retry\n")
    for name, text in {
        "HEADS": f"CAMP_HEAD={HEAD}\nCAMP_ORIGIN_MAIN={HEAD}\nDP_HEAD={module.FIXED_DP_HEAD}\n",
        "COMMAND": "run smoke retry\n",
        "stdout.txt": "ok\n",
        "stderr.txt": "",
        "run.exit": "0\n",
    }.items():
    ...
    ] + [f"candidates/candidate_tensor_{index:06d}.npz" for index in range(256)]
...
def _record(module, index: int, override: dict | None) -> dict:
    record = {
        "record_index": index,
        "split": "mini_val",
