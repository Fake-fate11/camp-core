...
SCRIPT_PATH = (
    ...
    / "scripts"
    / "integrations"
    / "review_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_readiness_uncertainty_coverage_evidence_package_continuation_plan_static_contract.py"
)
PLAN_SCRIPT_PATH = (
    ROOT
    / "scripts"
    / "integrations"
    / "plan_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_readiness_uncertainty_coverage_evidence_package_continuation.py"
)
PLAN_TEST_PATH = (
    ROOT
    / "camp_core"
    / "tests"
    / "test_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_readiness_uncertainty_coverage_evidence_package_continuation_plan.py"
)
ARTIFACT_HEAD = "b" * 40
CURRENT_HEAD = "3" * 40
...
def test_uncertainty_coverage_evidence_package_continuation_plan_static_review_passes(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert decision["post_closeout_continuation_plan_static_review_passed"] is True
    assert decision["promotion_authorization_boundary_plan_authorized"] is True
    assert decision["selector_promotion_authorized"] is False
    assert decision["deployment_authorized"] is False
    assert decision["safety_benefit_claim_authorized"] is False
    assert decision["camp_over_dp_top1_claim_authorized"] is False
    assert report["source_plan_summary"]["continuation_item_count"] == module.EXPECTED_CONTINUATION_ITEM_COUNT
    assert (fixture["output_dir"] / module.REVIEW_JSON_NAME).is_file()
    assert (fixture["output_dir"] / module.REVIEW_MD_NAME).is_file()
    assert (fixture["output_dir"] / "SHA256SUMS").is_file()


def test_uncertainty_coverage_evidence_package_continuation_plan_static_review_requires_enable(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert (
        ...
        == "explicit_continuation_plan_static_review_authorization_missing"
    )


def test_uncertainty_coverage_evidence_package_continuation_plan_static_review_rejects_wrong_eof(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert "audit_latest_eof_authorizes_continuation_plan_static_review" in report["final_decision"]["failed_checks"]
    assert "status_doc_latest_eof_authorizes_continuation_plan_static_review" in report["final_decision"]["failed_checks"]
    assert report["final_decision"]["failure_class"] == "v14_eof_contract_mismatch"


def test_uncertainty_coverage_evidence_package_continuation_plan_static_review_rejects_source_plan_leak(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_uncertainty_coverage_evidence_package_continuation_plan_static_review_rejects_hash_drift(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    fixture["continuation_plan_md"].write_text("# drift\n", encoding="utf-8")
...
def _write_fixture(
    ...
    *,
    ...
    artifact = tmp_path / "continuation_plan_artifact"
    ...
    plan_md = _write(plan_dir / module.SOURCE_PLAN_MD_NAME, "# Continuation Plan\n")
    plan_sha256s = _write_sha256sums(plan_dir / "SHA256SUMS", [plan_json, plan_md])
    command = _write(artifact / "COMMAND", "continuation plan command\n")
    heads = _write(
        artifact / "HEADS",
        "\n".join(
            [
    ...
    return {
        "continuation_plan_artifact_dir": artifact,
        "continuation_plan_json": plan_json,
        "continuation_plan_md": plan_md,
        "continuation_plan_sha256s": plan_sha256s,
...
def _source_plan_payload(
    ...
    *,
    ...
    return {
        ...
        "continuation_plan": [
            {"item_name": name}
            for name in module.PLAN_MODULE.EXPECTED_CONTINUATION_ITEMS
        ],
