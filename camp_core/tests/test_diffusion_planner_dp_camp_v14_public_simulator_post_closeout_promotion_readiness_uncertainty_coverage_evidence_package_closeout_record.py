...
def test_uncertainty_coverage_evidence_package_closeout_record_passes(tmp_path: Path) -> None:
    module = _load_module()
    ...
    assert decision["uncertainty_coverage_evidence_package_closeout_recorded"] is True
    assert decision["uncertainty_coverage_evidence_package_closeout_record_static_review_authorized"] is True
    assert decision["evidence_package_closed_by_this_gate"] is True
    assert decision["evidence_package_constructed_by_this_gate"] is False
    assert decision["selector_promotion_authorized"] is False
    assert decision["deployment_authorized"] is False
    assert decision["safety_benefit_claim_authorized"] is False
    assert report["closeout_record"]["final_package_state"] == "audit_evidence_only_closed_no_promotion_no_deployment_no_claim"
    assert (fixture["output_dir"] / module.RECORD_JSON_NAME).is_file()
    assert (fixture["output_dir"] / module.RECORD_MD_NAME).is_file()
    assert (fixture["output_dir"] / "SHA256SUMS").is_file()


def test_uncertainty_coverage_evidence_package_closeout_record_requires_enable(tmp_path: Path) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert (
        ...
        == "explicit_uncertainty_coverage_evidence_package_closeout_record_authorization_missing"
    )


def test_uncertainty_coverage_evidence_package_closeout_record_rejects_wrong_eof(tmp_path: Path) -> None:
    module = _load_module()
...
def test_uncertainty_coverage_evidence_package_closeout_record_rejects_source_review_leak(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_uncertainty_coverage_evidence_package_closeout_record_rejects_source_hash_drift(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
...
def _source_review_payload(
    ...
    *,
    ...
    return {
        ...
        "closeout_plan_summary": {
            "closeout_item_count": module.EXPECTED_CLOSEOUT_ITEM_COUNT,
            "all_no_closeout_record": True,
            "all_no_execution": True,
            "all_no_claim": True,
            "all_no_promotion": True,
            "all_no_deployment": True,
        },
        "blocked_actions": {name: False for name in module.BLOCKED_ACTIONS},
        "review_checks": [
            {"name": f"check_{index}", "passed": True, "observed": True, "expected": True}
            for index in range(module.EXPECTED_REVIEW_CHECK_COUNT)
        ],
