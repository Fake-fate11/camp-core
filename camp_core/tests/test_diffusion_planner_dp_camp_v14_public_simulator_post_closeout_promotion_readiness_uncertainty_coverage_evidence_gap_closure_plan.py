...
def test_uncertainty_coverage_evidence_gap_closure_plan_passes(tmp_path: Path) -> None:
    module = _load_module()
    ...
    assert decision["uncertainty_coverage_evidence_gap_closure_plan_static_review_authorized"] is True
    assert decision["direct_promotion_recommendation"] is False
    assert decision["selector_promotion_authorized"] is False
    assert decision["deployment_authorized"] is False
    assert decision["safety_benefit_claim_authorized"] is False
    assert decision["camp_over_dp_top1_claim_authorized"] is False
    assert [item["source_gap"] for item in report["evidence_gap_closure_plan"]] == list(
        module.EXPECTED_EVIDENCE_GAPS
    )
    assert {item["authorizes_execution"] for item in report["evidence_gap_closure_plan"]} == {False}
    assert {item["authorizes_claim"] for item in report["evidence_gap_closure_plan"]} == {False}
    assert (
        fixture["output_dir"]
...
def test_uncertainty_coverage_evidence_gap_closure_plan_requires_enable(tmp_path: Path) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert "evidence_gap_closure_plan_enabled" in report["final_decision"]["failed_checks"]
    assert (
        report["final_decision"]["failure_class"]
        == "explicit_uncertainty_coverage_evidence_gap_closure_plan_authorization_missing"
    )


def test_uncertainty_coverage_evidence_gap_closure_plan_rejects_wrong_eof(tmp_path: Path) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module, next_work="wrong_gate")

    report = module.build_report(**fixture)

    assert "audit_latest_eof_authorizes_closure_plan" in report["final_decision"]["failed_checks"]
    assert "status_doc_latest_eof_authorizes_closure_plan" in report["final_decision"]["failed_checks"]
    assert report["final_decision"]["failure_class"] == "v14_eof_contract_mismatch"


def test_uncertainty_coverage_evidence_gap_closure_plan_rejects_source_leak(tmp_path: Path) -> None:
    module = _load_module()
...
def test_uncertainty_coverage_evidence_gap_closure_plan_rejects_gap_count_drift(tmp_path: Path) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module, evidence_gap_count=4)

    report = module.build_report(**fixture)

    assert "source_review_summary_evidence_gap_count" in report["final_decision"]["failed_checks"]
    assert (
        report["final_decision"]["failure_class"]
        == "source_uncertainty_coverage_static_review_contract_failure"
    )


def test_uncertainty_coverage_evidence_gap_closure_plan_cli_writes_outputs(tmp_path: Path) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
...
def _write_fixture(
    ...
    *,
    ...
    review_json = _write_json(
        review_dir / module.SOURCE_STATIC_REVIEW_JSON_NAME,
...
def _source_static_review_payload(
    ...
    *,
    ...
    decision = {
        ...
        "recommendation": "plan_uncertainty_coverage_evidence_gap_closure_only",
        "immediate_action": "evidence_gap_closure_plan_only",
    }
    ...
    return {
        ...
        "source_review_summary": {
            "schema_version": "dp_camp_v14_public_simulator_post_closeout_promotion_readiness_uncertainty_coverage_review_v1",
            ...
            "review_item_count": module.EXPECTED_SOURCE["source_review_item_count"],
            "evidence_gap_count": evidence_gap_count
            if evidence_gap_count is not None
            else module.EXPECTED_SOURCE["source_evidence_gap_count"],
        },
