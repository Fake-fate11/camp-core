...
SCRIPT_PATH = (
    ...
    / "scripts"
    / "integrations"
    / "plan_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_evidence_acquisition_objective_3200_candidate_index_actual_safetycost_claim_decision.py"
)
SOURCE_HEAD = "f" * 40
CURRENT_HEAD = "1" * 40
...
def test_candidate_index_actual_safetycost_claim_decision_plan_passes(tmp_path: Path) -> None:
    module = _load_module()
    ...
    assert decision["objective_3200_candidate_index_actual_safetycost_claim_decision_plan_ready"] is True
    assert decision["objective_3200_candidate_index_actual_safetycost_claim_decision_plan_static_review_authorized"] is True
    assert decision["claim_executed_by_this_gate"] is False
    assert decision["safety_benefit_claim_authorized"] is False
    assert decision["camp_over_dp_top1_claim_authorized"] is False
    assert [item["item_name"] for item in report["claim_decision_plan"]] == list(module.CLAIM_DECISION_ITEMS)
    assert {item["executes_claim"] for item in report["claim_decision_plan"]} == {False}
    assert (fixture["output_dir"] / module.PLAN_JSON_NAME).is_file()
    assert (fixture["output_dir"] / module.PLAN_MD_NAME).is_file()
    assert (fixture["output_dir"] / "SHA256SUMS").is_file()


def test_candidate_index_actual_safetycost_claim_decision_plan_requires_enable(tmp_path: Path) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert "claim_decision_plan_enabled" in report["final_decision"]["failed_checks"]
    assert (
        report["final_decision"]["failure_class"]
        == "explicit_candidate_index_actual_safetycost_claim_decision_plan_authorization_missing"
    )


def test_candidate_index_actual_safetycost_claim_decision_plan_rejects_wrong_eof(tmp_path: Path) -> None:
    module = _load_module()
...
def test_candidate_index_actual_safetycost_claim_decision_plan_rejects_source_claim_leak(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert "source_safety_claim_false" in report["final_decision"]["failed_checks"]
    assert report["final_decision"]["safety_benefit_claim_authorized"] is False


def test_candidate_index_actual_safetycost_claim_decision_plan_rejects_hash_drift(tmp_path: Path) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    *,
    ...
    doc_text = "\n".join(
        [
            ...
            "objective_3200_candidate_index_actual_safetycost_claim_authorization_boundary_plan_static_review_passed=True",
            "objective_3200_candidate_index_actual_safetycost_claim_decision_plan_authorized=True",
