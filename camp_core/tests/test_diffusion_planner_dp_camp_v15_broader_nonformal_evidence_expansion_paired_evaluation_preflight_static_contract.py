...
HEAD = "efd70c7d8c5d75cccb8befc5bb2a8ee44fdfa9e3"


def _load_module():
    spec = importlib.util.spec_from_file_location("v15_paired_evaluation_preflight_static_review", SCRIPT_PATH)
...
def test_v15_paired_evaluation_preflight_static_review_passes(tmp_path: Path) -> None:
    module = _load_module()
    ...
    assert decision["reviewed_paired_evaluation_preflight"] is True
    assert decision["paired_evaluation_preflight_executed"] is False
...
def test_v15_paired_evaluation_preflight_static_review_requires_enable(tmp_path: Path) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
...
def test_v15_paired_evaluation_preflight_static_review_rejects_wrong_eof(tmp_path: Path) -> None:
    module = _load_module()
...
def test_v15_paired_evaluation_preflight_static_review_rejects_eval_leak(tmp_path: Path) -> None:
    module = _load_module()
...
def test_v15_paired_evaluation_preflight_static_review_is_recorded_in_audit() -> None:
    module = _load_module()
...
def _source_preflight_payload(module, timing: dict, *, source_updates: dict | None = None) -> dict:
    decision = {
        ...
        "paired_evaluation_preflight_executed": True,
    ...
    return {
        ...
        "paired_evaluation_preflight": module.PLAN_MODULE._paired_evaluation_preflight_plan(
            HEAD, module.FIXED_DP_HEAD
        ),
        "timing_contract": timing,
    }


def _timing_payload(module) -> dict:
    return {
        "online_selector_latency_required_fields": module.PLAN_MODULE.LATENCY_FIELDS,
        "fallback_latency_required_fields": module.PLAN_MODULE.LATENCY_FIELDS,
