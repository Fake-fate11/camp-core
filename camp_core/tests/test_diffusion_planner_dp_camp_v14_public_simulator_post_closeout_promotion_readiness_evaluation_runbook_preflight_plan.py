...
def _load_module():
    spec = importlib.util.spec_from_file_location(
        "v14_post_closeout_promotion_readiness_evaluation_runbook_preflight_plan",
        SCRIPT_PATH,
    )
...
def test_promotion_readiness_evaluation_runbook_preflight_plan_passes(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert decision["evaluation_runbook_preflight_plan_static_review_authorized"] is True
    assert decision["selector_promotion_authorized"] is False
    assert decision["deployment_authorized"] is False
    assert decision["safety_benefit_claim_authorized"] is False
    assert decision["camp_over_dp_top1_claim_authorized"] is False
    assert len(report["runbook_preflight_plan"]) == 6
    assert len(report["no_go_conditions"]) == 8
    assert len(report["forbidden_actions"]) == 6
    assert (
        fixture["output_dir"]
...
def test_promotion_readiness_evaluation_runbook_preflight_plan_accepts_uppercase_dp_head(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_promotion_readiness_evaluation_runbook_preflight_plan_requires_enable(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert "runbook_preflight_plan_enabled" in report["final_decision"]["failed_checks"]
    assert (
        report["final_decision"]["failure_class"]
        == "explicit_runbook_preflight_plan_authorization_missing"
    )
    assert report["final_decision"]["selector_promotion_authorized"] is False


def test_promotion_readiness_evaluation_runbook_preflight_plan_rejects_wrong_eof(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert (
        "audit_latest_eof_authorizes_runbook_preflight_plan"
        in report["final_decision"]["failed_checks"]
    )
    assert (
        "status_doc_latest_eof_authorizes_runbook_preflight_plan"
        in report["final_decision"]["failed_checks"]
    )
    assert report["final_decision"]["failure_class"] == "v14_eof_contract_mismatch"


def test_promotion_readiness_evaluation_runbook_preflight_plan_rejects_hash_drift(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    fixture["source_plan_md"].write_text("drift\n", encoding="utf-8")
...
def test_promotion_readiness_evaluation_runbook_preflight_plan_rejects_source_leak(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_promotion_readiness_evaluation_runbook_preflight_plan_cli_writes_outputs(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    exit_code = module.main(
        [
            "--plan_static_review_artifact_dir",
            str(fixture["plan_static_review_artifact_dir"]),
            "--plan_static_review_json",
            str(fixture["plan_static_review_json"]),
            "--plan_static_review_md",
            str(fixture["plan_static_review_md"]),
            "--plan_static_review_sha256s",
            str(fixture["plan_static_review_sha256s"]),
...
def _write_fixture(
    ...
    *,
    ...
    plan_command = _write(plan_artifact / "COMMAND", "plan command\n")
    plan_heads = _write(
        plan_artifact / "HEADS",
        "\n".join(
            [
    ...
    static_heads = _write(
        ...
        "\n".join(
            [
                ...
                f"source_evaluation_plan_artifact={plan_artifact.resolve()}",
                "",
            ]
        ),
    )
    ...
    return {
        "plan_static_review_artifact_dir": static_artifact,
        "plan_static_review_json": static_review_json,
        "plan_static_review_md": static_review_md,
        "plan_static_review_sha256s": static_review_sha256s,
...
def _static_review_payload(
    ...
    *,
    ...
    decision = {
        ...
        "evaluation_runbook_preflight_plan_authorized": True,
    ...
    return {
        ...
        "analysis": {
            ...
            "read_only": True,
            "plan_artifact_dir": str(source_plan_artifact.resolve()),
...
def _source_plan_payload(
    ...
    *,
    ...
    return {
        ...
        "decision_surfaces": [{"name": name} for name in module.EXPECTED_DECISION_SURFACES],
        "evidence_requirements": [{"name": name} for name in module.EXPECTED_EVIDENCE_REQUIREMENTS],
