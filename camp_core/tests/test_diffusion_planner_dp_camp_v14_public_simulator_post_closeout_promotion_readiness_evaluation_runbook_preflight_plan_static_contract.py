...
SCRIPT_PATH = (
    ...
    / "scripts"
    / "integrations"
    / "review_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_readiness_evaluation_runbook_preflight_plan_static_contract.py"
)
PLAN_SCRIPT_PATH = (
    Path(__file__).resolve().parents[2]
    / "scripts"
    / "integrations"
    / "plan_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_readiness_evaluation_runbook_preflight.py"
)
PLAN_TEST_PATH = (
    Path(__file__).resolve().parents[2]
    / "camp_core"
    / "tests"
    / "test_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_readiness_evaluation_runbook_preflight_plan.py"
)
ARTIFACT_HEAD = "e" * 40
CURRENT_HEAD = "f" * 40
...
def _load_module():
    spec = importlib.util.spec_from_file_location(
        "v14_post_closeout_promotion_readiness_evaluation_runbook_preflight_plan_static_review",
        SCRIPT_PATH,
    )
...
def test_promotion_readiness_evaluation_runbook_preflight_plan_static_review_passes(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert decision["evaluation_runbook_preflight_authorized"] is True
    assert decision["selector_promotion_authorized"] is False
    assert decision["deployment_authorized"] is False
    assert decision["safety_benefit_claim_authorized"] is False
    assert decision["camp_over_dp_top1_claim_authorized"] is False
    assert report["source_plan_summary"]["runbook_preflight_step_count"] == 6
...
def test_promotion_readiness_evaluation_runbook_preflight_plan_static_review_accepts_uppercase_dp_head(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_promotion_readiness_evaluation_runbook_preflight_plan_static_review_requires_enable(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert (
        ...
        == "explicit_runbook_preflight_plan_static_review_authorization_missing"
    )


def test_promotion_readiness_evaluation_runbook_preflight_plan_static_review_rejects_wrong_eof(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_promotion_readiness_evaluation_runbook_preflight_plan_static_review_rejects_hash_drift(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    fixture["runbook_preflight_plan_md"].write_text("drift\n", encoding="utf-8")
    ...
    assert (
        report["final_decision"]["failure_class"]
        == "runbook_preflight_plan_artifact_sha256_mismatch"
    )


def test_promotion_readiness_evaluation_runbook_preflight_plan_static_review_rejects_source_leak(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_promotion_readiness_evaluation_runbook_preflight_plan_static_review_cli_writes_outputs(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    exit_code = module.main(
        [
            "--runbook_preflight_plan_artifact_dir",
            str(fixture["runbook_preflight_plan_artifact_dir"]),
            "--runbook_preflight_plan_json",
            str(fixture["runbook_preflight_plan_json"]),
            "--runbook_preflight_plan_md",
            str(fixture["runbook_preflight_plan_md"]),
            "--runbook_preflight_plan_sha256s",
            str(fixture["runbook_preflight_plan_sha256s"]),
            "--plan_script_py",
...
def _write_fixture(
    ...
    *,
    ...
    plan_md = _write(plan_dir / module.PLAN_MD_NAME, "# Runbook Preflight Plan\n")
    plan_sha256s = _write_sha256sums(plan_dir / "SHA256SUMS", [plan_json, plan_md])
    command = _write(artifact / "COMMAND", "runbook preflight plan command\n")
    heads = _write(
        artifact / "HEADS",
        "\n".join(
            [
    ...
    return {
        "runbook_preflight_plan_artifact_dir": artifact,
        "runbook_preflight_plan_json": plan_json,
        "runbook_preflight_plan_md": plan_md,
        "runbook_preflight_plan_sha256s": plan_sha256s,
