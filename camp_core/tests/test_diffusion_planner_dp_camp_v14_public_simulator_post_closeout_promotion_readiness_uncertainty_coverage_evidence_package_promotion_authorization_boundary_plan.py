...
def test_uncertainty_coverage_evidence_package_promotion_authorization_boundary_plan_passes(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert decision["promotion_authorization_boundary_plan_ready"] is True
    assert decision["promotion_authorization_boundary_static_review_authorized"] is True
    ...
    assert [item["item_name"] for item in report["authorization_boundary_plan"]] == list(
        module.EXPECTED_BOUNDARY_ITEMS
    )
    assert {item["executes_promotion"] for item in report["authorization_boundary_plan"]} == {False}
    assert (fixture["output_dir"] / module.PLAN_JSON_NAME).is_file()
    assert (fixture["output_dir"] / module.PLAN_MD_NAME).is_file()
    assert (fixture["output_dir"] / "SHA256SUMS").is_file()


def test_uncertainty_coverage_evidence_package_promotion_authorization_boundary_plan_requires_enable(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert "authorization_boundary_plan_enabled" in report["final_decision"]["failed_checks"]
    assert (
        report["final_decision"]["failure_class"]
        == "explicit_promotion_authorization_boundary_plan_authorization_missing"
    )


def test_uncertainty_coverage_evidence_package_promotion_authorization_boundary_plan_rejects_wrong_eof(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert "audit_latest_eof_authorizes_boundary_plan" in report["final_decision"]["failed_checks"]
    assert "status_doc_latest_eof_authorizes_boundary_plan" in report["final_decision"]["failed_checks"]
    assert report["final_decision"]["failure_class"] == "v14_eof_contract_mismatch"


def test_uncertainty_coverage_evidence_package_promotion_authorization_boundary_plan_rejects_source_review_leak(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_uncertainty_coverage_evidence_package_promotion_authorization_boundary_plan_rejects_hash_drift(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
