...
SCRIPT_PATH = (
    ...
    / "scripts"
    / "integrations"
    / "preflight_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_readiness_evaluation.py"
)
ARTIFACT_HEAD = "e" * 40
CURRENT_HEAD = "f" * 40
...
def _load_module():
    spec = importlib.util.spec_from_file_location(
        "v14_post_closeout_promotion_readiness_evaluation_preflight",
        SCRIPT_PATH,
    )
...
def test_promotion_readiness_evaluation_preflight_passes(tmp_path: Path) -> None:
    module = _load_module()
    ...
    assert decision["preflight_static_review_authorized"] is True
    assert decision["selector_promotion_authorized"] is False
    assert decision["deployment_authorized"] is False
    assert decision["safety_benefit_claim_authorized"] is False
    assert decision["camp_over_dp_top1_claim_authorized"] is False
    assert len(report["evaluation_preflight"]) == 5
    assert all(item["triggered"] is False for item in report["no_go_status"])
    assert (
        fixture["output_dir"]
        / "post_closeout_promotion_readiness_evaluation_preflight.json"
    ).is_file()
    assert (
        fixture["output_dir"]
        / "post_closeout_promotion_readiness_evaluation_preflight.md"
    ).is_file()
    assert (fixture["output_dir"] / "SHA256SUMS").is_file()


def test_promotion_readiness_evaluation_preflight_requires_enable(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
...
def test_promotion_readiness_evaluation_preflight_rejects_wrong_eof(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_promotion_readiness_evaluation_preflight_rejects_source_review_leak(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_promotion_readiness_evaluation_preflight_rejects_source_plan_hash_drift(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    fixture["source_preflight_plan_json"].write_text("drift\n", encoding="utf-8")

    report = module.build_report(**fixture)

    assert report["final_decision"]["passed"] is False
    assert "source_preflight_plan_json_sha" in report["final_decision"]["failed_checks"]
    assert report["final_decision"]["failure_class"] == "source_artifact_sha256_mismatch"


def test_promotion_readiness_evaluation_preflight_cli_writes_outputs(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert exit_code == 0
    assert (
        output_dir / "post_closeout_promotion_readiness_evaluation_preflight.json"
    ).is_file()
    assert (
        output_dir / "post_closeout_promotion_readiness_evaluation_preflight.md"
    ).is_file()
    ...
    heads = _write(
        ...
        "\n".join(
            [
                ...
                f"source_preflight_plan_artifact={plan_artifact.resolve()}",
                "",
            ]
        ),
    )
...
def _static_review_payload(
    ...
    *,
    ...
    decision = {
        ...
        "promotion_readiness_evaluation_preflight_authorized": True,
    ...
    return {
        ...
        "analysis": {
            ...
            "read_only": True,
            "preflight_plan_artifact_dir": str(source_plan_artifact.resolve()),
    ...
    *,
    ...
    return {
        ...
        "preflight_plan": [
            ...
            for name in module.EXPECTED_PREFLIGHT_PLAN_NAMES
        ],
