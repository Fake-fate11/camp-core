...
HEAD = "36d623a91324ac81eb953c21789230dba9bab8ae"


def _load_module():
    spec = importlib.util.spec_from_file_location("v15_offline_training_preflight_plan", SCRIPT_PATH)
...
def test_v15_offline_training_preflight_plan_passes(tmp_path: Path) -> None:
    module = _load_module()
    ...
    assert set(report["offline_training_preflight_plan"]["required_inputs"]["splits"]) == {
        "train",
        "calibration",
        "holdout",
    }
...
def test_v15_offline_training_preflight_plan_requires_enable(tmp_path: Path) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert "offline_training_preflight_plan_enabled" in report["final_decision"]["failed_checks"]


def test_v15_offline_training_preflight_plan_rejects_wrong_eof(tmp_path: Path) -> None:
    module = _load_module()
...
def test_v15_offline_training_preflight_plan_rejects_source_training_leak(tmp_path: Path) -> None:
    module = _load_module()
...
def test_v15_offline_training_preflight_plan_is_recorded_in_audit() -> None:
    module = _load_module()
...
def _write_fixture(
    ...
    *,
    ...
    source_md.write_text("# Matrix Result Review\n", encoding="utf-8")
    for name, content in {
        "HEADS": f"CAMP_HEAD={HEAD}\nCAMP_ORIGIN_MAIN={HEAD}\nDP_HEAD={module.FIXED_DP_HEAD}\n",
        "COMMAND": "run matrix result review\n",
        "stdout.txt": "{}\n",
        "stderr.txt": "",
        "run.exit": "0\n",
    }.items():
