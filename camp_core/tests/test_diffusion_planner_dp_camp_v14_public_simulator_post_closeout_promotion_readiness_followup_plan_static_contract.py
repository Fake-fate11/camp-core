...
SCRIPT_PATH = (
    ...
    / "scripts"
    / "integrations"
    / "review_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_readiness_followup_plan_static_contract.py"
)
PLAN_SCRIPT_PATH = (
    Path(__file__).resolve().parents[2]
    / "scripts"
...
PLAN_TEST_PATH = (
    ...
    / "camp_core"
    / "tests"
    / "test_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_readiness_followup_plan.py"
)
ARTIFACT_HEAD = "d" * 40
CURRENT_HEAD = "e" * 40
...
def _load_module():
    spec = importlib.util.spec_from_file_location(
        "v14_post_closeout_promotion_readiness_followup_plan_static_review",
        SCRIPT_PATH,
    )
...
def test_post_closeout_promotion_readiness_followup_plan_static_review_passes(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert decision["uncertainty_coverage_review_plan_authorized"] is True
    assert decision["direct_promotion_recommendation"] is False
    assert decision["selector_promotion_authorized"] is False
    assert decision["deployment_authorized"] is False
    assert decision["safety_benefit_claim_authorized"] is False
    assert decision["camp_over_dp_top1_claim_authorized"] is False
    assert report["source_plan_summary"]["followup_item_count"] == 7
    assert (
        fixture["output_dir"]
        / "post_closeout_promotion_readiness_followup_plan_static_review.json"
    ).is_file()
    assert (
        fixture["output_dir"]
        / "post_closeout_promotion_readiness_followup_plan_static_review.md"
    ).is_file()
    assert (fixture["output_dir"] / "SHA256SUMS").is_file()


def test_post_closeout_promotion_readiness_followup_plan_static_review_requires_enable(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert (
        ...
        == "explicit_followup_plan_static_review_authorization_missing"
    )


def test_post_closeout_promotion_readiness_followup_plan_static_review_rejects_wrong_eof(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_post_closeout_promotion_readiness_followup_plan_static_review_rejects_source_leak(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_post_closeout_promotion_readiness_followup_plan_static_review_cli_writes_outputs(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    exit_code = module.main(
        [
            "--followup_plan_artifact_dir",
            str(fixture["followup_plan_artifact_dir"]),
            "--followup_plan_json",
            str(fixture["followup_plan_json"]),
            "--followup_plan_md",
            str(fixture["followup_plan_md"]),
            "--followup_plan_sha256s",
            str(fixture["followup_plan_sha256s"]),
            "--followup_plan_script_py",
            str(fixture["followup_plan_script_py"]),
            "--followup_plan_test_py",
            str(fixture["followup_plan_test_py"]),
            "--v14_audit_md",
    ...
    assert exit_code == 0
    assert (
        output_dir / "post_closeout_promotion_readiness_followup_plan_static_review.json"
    ).is_file()
    assert (
        output_dir / "post_closeout_promotion_readiness_followup_plan_static_review.md"
    ).is_file()
...
def _write_fixture(
    ...
    *,
    ...
    artifact = tmp_path / "followup_plan_artifact"
    ...
    plan_md = _write(plan_dir / module.PLAN_MD_NAME, "# Follow-Up Plan\n")
    plan_sha256s = _write_sha256sums(plan_dir / "SHA256SUMS", [plan_json, plan_md])
    command = _write(artifact / "COMMAND", "followup plan command\n")
    heads = _write(
        artifact / "HEADS",
        "\n".join(
            [
    ...
    return {
        "followup_plan_artifact_dir": artifact,
        "followup_plan_json": plan_json,
        "followup_plan_md": plan_md,
        "followup_plan_sha256s": plan_sha256s,
        "followup_plan_script_py": PLAN_SCRIPT_PATH,
        "followup_plan_test_py": PLAN_TEST_PATH,
...
def _source_plan_payload(
    ...
    *,
    ...
    decision = {
        ...
        "post_closeout_promotion_readiness_followup_plan_ready": True,
        "followup_plan_static_review_authorized": True,
        "direct_promotion_recommendation": False,
        "promotion_decision_plan_authorized_next": False,
    ...
    return {
        ...
        "followup_plan": [
            {"name": name, "authorizes_promotion": False}
            for name in module.EXPECTED_FOLLOWUP_ITEMS
        ],
        "blocked_actions": {name: False for name in module.BLOCKED_ACTIONS},
        "plan_checks": [{"name": "fixture_check", "passed": True}],
