...
from scripts.integrations.analyze_diffusion_planner_descriptor_separability import (
    READY_STATUS,
...
def _strong_report(status: str = "strong_progress_support_certificate_rejected") -> dict:
    return {
        "final_decision": {
            "status": status,
            "authorized_next_work": None,
        },
        "failure_gap": {"primary_gap": "certificate_blocks_beneficial_opportunities"},
...
def test_descriptor_separability_finds_progress_screen_on_toy_case() -> None:
    report = analyze_records(
        [
            {"raw": _record(harmful=True), "context": _context()},
            {"raw": _record(harmful=False), "context": _context()},
        ],
        strong_certificate_report=_strong_report(),
    ...
    assert report["failure_gap"]["primary_gap"] == "no_gap_promising_descriptor_screen_found"
    best = report["ranked_feature_screens"][0]
    assert best["promising_descriptor_screen"] is True
    assert best["harmful_block_rate"] == 1.0
    assert best["beneficial_retain_rate"] == 1.0


def test_descriptor_separability_blocks_when_source_gate_not_rejected() -> None:
    report = analyze_records(
        [{"raw": _record(harmful=True), "context": _context()}],
        strong_certificate_report=_strong_report(
            "strong_progress_support_certificate_promising"
        ),
        variants=(PREDECLARED_VARIANTS[1],),
    )
...
def test_descriptor_separability_rejects_formal_seed_when_forbidden() -> None:
...
def test_descriptor_separability_runtime_allowance_is_outcome_independent() -> None:
    ...
    base_rows = {
        ...
        for report in base_report["variants"][0]["feature_reports"]
        for row in report["threshold_screens"]
    }
    mutated_rows = {
        row["screen_name"]: (row["allowed_switches"], row["blocked_switches"])
        for report in mutated_report["variants"][0]["feature_reports"]
        for row in report["threshold_screens"]
    }
