...
def _load_module():
    spec = importlib.util.spec_from_file_location(
        "v14_post_closeout_promotion_readiness_uncertainty_coverage_preflight_plan",
        SCRIPT_PATH,
    )
...
def test_post_closeout_promotion_readiness_uncertainty_coverage_preflight_plan_passes(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert decision["uncertainty_coverage_review_preflight_plan_static_review_authorized"] is True
    assert decision["direct_promotion_recommendation"] is False
    assert decision["selector_promotion_authorized"] is False
    assert decision["deployment_authorized"] is False
    assert decision["safety_benefit_claim_authorized"] is False
    assert decision["camp_over_dp_top1_claim_authorized"] is False
    assert [item["name"] for item in report["preflight_plan"]] == list(module.EXPECTED_PREFLIGHT_ITEMS)
    assert (
        fixture["output_dir"]
...
def test_post_closeout_promotion_readiness_uncertainty_coverage_preflight_plan_requires_enable(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert (
        ...
        == "explicit_uncertainty_coverage_preflight_plan_authorization_missing"
    )


def test_post_closeout_promotion_readiness_uncertainty_coverage_preflight_plan_rejects_wrong_eof(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_post_closeout_promotion_readiness_uncertainty_coverage_preflight_plan_rejects_source_leak(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_post_closeout_promotion_readiness_uncertainty_coverage_preflight_plan_cli_writes_outputs(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
...
def _write_fixture(
    ...
    *,
    ...
    artifact = tmp_path / "uncertainty_coverage_plan_static_review_artifact"
    ...
    doc_text = "\n".join(
        [
            ...
            (
                "v14_public_simulator_post_closeout_promotion_readiness_"
                f"uncertainty_coverage_review_plan_static_review_artifact={artifact.resolve()}"
            ),
    ...
    command = _write(artifact / "COMMAND", "uncertainty coverage static review command\n")
    heads = _write(
        artifact / "HEADS",
        "\n".join(
            [
