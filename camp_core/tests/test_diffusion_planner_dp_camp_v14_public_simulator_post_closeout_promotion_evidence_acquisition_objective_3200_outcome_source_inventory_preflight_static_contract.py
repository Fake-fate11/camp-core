...
SCRIPT_PATH = (
    ...
    / "scripts"
    / "integrations"
    / "review_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_evidence_acquisition_objective_3200_outcome_source_inventory_preflight_static_contract.py"
)
PLAN_SCRIPT_PATH = (
    ROOT
    / "scripts"
...
PLAN_TEST_PATH = (
    ...
    / "camp_core"
    / "tests"
    / "test_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_evidence_acquisition_objective_3200_outcome_continuation_plan.py"
)
SOURCE_HEAD = "4" * 40
CURRENT_HEAD = "5" * 40
...
def test_objective_3200_source_inventory_preflight_static_review_passes(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert decision["objective_3200_outcome_source_inventory_preflight_static_review_passed"] is True
    assert decision["objective_3200_outcome_source_inventory_preflight_authorized"] is True
    assert decision["source_inventory_preflight_executed_by_this_gate"] is False
    assert decision["selector_promotion_authorized"] is False
    assert decision["deployment_authorized"] is False
    assert decision["safety_benefit_claim_authorized"] is False
    assert decision["camp_over_dp_top1_claim_authorized"] is False
    assert report["source_plan_summary"]["candidate_closed_loop_outcome_records"] == 0
    assert report["source_plan_summary"]["missing_candidate_closed_loop_outcome_records"] == 3200
    assert (fixture["output_dir"] / module.REVIEW_JSON_NAME).is_file()
    assert (fixture["output_dir"] / module.REVIEW_MD_NAME).is_file()
    assert (fixture["output_dir"] / "SHA256SUMS").is_file()


def test_objective_3200_source_inventory_preflight_static_review_requires_enable(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert (
        ...
        == "explicit_objective_3200_source_inventory_static_review_authorization_missing"
    )


def test_objective_3200_source_inventory_preflight_static_review_rejects_wrong_eof(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_objective_3200_source_inventory_preflight_static_review_rejects_claim_leak(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert "source_plan_decision_camp_over_dp_top1_claim_authorized" in report[
        "final_decision"
    ]["failed_checks"]
    assert report["final_decision"]["camp_over_dp_top1_claim_authorized"] is False


def test_objective_3200_source_inventory_preflight_static_review_rejects_hash_drift(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    fixture["objective_3200_outcome_continuation_plan_md"].write_text(
        "# drift\n",
        encoding="utf-8",
    )
...
def test_objective_3200_source_inventory_preflight_static_review_rejects_absent_gap(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    *,
    ...
    doc_text = "\n".join(
        [
            ...
            "objective_3200_outcome_continuation_plan_ready=True",
            "objective_3200_outcome_source_inventory_static_review_authorized=True",
    ...
    artifact = tmp_path / "source_continuation_plan_artifact"
    ...
    command = _write(artifact / "COMMAND", "python continuation_plan.py\n")
    stdout = _write(artifact / "stdout", "{}\n")
    stderr = _write(artifact / "stderr", "")
    run_exit = _write(artifact / "run.exit", "0\n")
    _write_sha256sums(
        artifact / "SHA256SUMS",
        [heads, command, stdout, stderr, run_exit, plan_json, plan_md, plan_sha256s],
        relative_to=artifact,
    )

    return {
        "objective_3200_outcome_continuation_plan_artifact_dir": artifact,
        "objective_3200_outcome_continuation_plan_json": plan_json,
        "objective_3200_outcome_continuation_plan_md": plan_md,
        "objective_3200_outcome_continuation_plan_sha256s": plan_sha256s,
        "continuation_plan_script_py": PLAN_SCRIPT_PATH,
        "continuation_plan_test_py": PLAN_TEST_PATH,
    ...
    *,
    ...
    return {
        ...
        "objective_gap_summary": {
            ...
            "closeout_does_not_satisfy_objective": True,
        },
        "preflight_plan": [{"step": step} for step in module.EXPECTED_PREFLIGHT_STEPS],
        "no_go_register": list(module.EXPECTED_NO_GO),
        "artifact_contract": {
            "nested_sha256s_required": True,
            "root_sha256s_required": True,
        },
        "plan_checks": [{"name": "fixture", "passed": True}],
