...
SCRIPT_PATH = (
    ...
    / "scripts"
    / "integrations"
    / "review_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_readiness_evaluation_runbook_preflight_static_contract.py"
)
...
PREFLIGHT_TEST_PATH = (
    ...
    / "camp_core"
    / "tests"
    / "test_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_readiness_evaluation_runbook_preflight.py"
)
ARTIFACT_HEAD = "e" * 40
CURRENT_HEAD = "f" * 40
...
def _load_module():
    spec = importlib.util.spec_from_file_location(
        "v14_post_closeout_promotion_readiness_evaluation_runbook_preflight_static_review",
        SCRIPT_PATH,
    )
...
def test_promotion_readiness_evaluation_runbook_preflight_static_review_passes(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert decision["evaluation_runbook_plan_authorized"] is True
    assert decision["evaluation_runbook_execution_authorized"] is False
    assert decision["selector_promotion_authorized"] is False
    assert decision["deployment_authorized"] is False
    assert decision["safety_benefit_claim_authorized"] is False
    assert decision["camp_over_dp_top1_claim_authorized"] is False
    assert report["source_preflight_summary"]["runbook_preflight_step_count"] == 6
...
def test_promotion_readiness_evaluation_runbook_preflight_static_review_accepts_uppercase_dp_head(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_promotion_readiness_evaluation_runbook_preflight_static_review_accepts_prior_source_head(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
...
def test_promotion_readiness_evaluation_runbook_preflight_static_review_requires_enable(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert (
        ...
        == "explicit_runbook_preflight_static_review_authorization_missing"
    )


def test_promotion_readiness_evaluation_runbook_preflight_static_review_rejects_wrong_eof(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_promotion_readiness_evaluation_runbook_preflight_static_review_rejects_hash_drift(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    fixture["runbook_preflight_md"].write_text("drift\n", encoding="utf-8")
    ...
    assert (
        report["final_decision"]["failure_class"]
        == "runbook_preflight_artifact_sha256_mismatch"
    )


def test_promotion_readiness_evaluation_runbook_preflight_static_review_rejects_source_leak(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_promotion_readiness_evaluation_runbook_preflight_static_review_cli_writes_outputs(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    exit_code = module.main(
        [
            "--runbook_preflight_artifact_dir",
            str(fixture["runbook_preflight_artifact_dir"]),
            "--runbook_preflight_json",
            str(fixture["runbook_preflight_json"]),
            "--runbook_preflight_md",
            str(fixture["runbook_preflight_md"]),
            "--runbook_preflight_sha256s",
            str(fixture["runbook_preflight_sha256s"]),
            "--preflight_script_py",
...
def _write_fixture(
    ...
    *,
    ...
    artifact = tmp_path / "runbook_preflight_artifact"
    ...
    command = _write(artifact / "COMMAND", "runbook preflight command\n")
    heads = _write(
        artifact / "HEADS",
        "\n".join(
            [
                ...
                "source_runbook_preflight_plan_static_review_artifact=/tmp/static_review",
    ...
    return {
        "runbook_preflight_artifact_dir": artifact,
        "runbook_preflight_json": preflight_json,
        "runbook_preflight_md": preflight_md,
        "runbook_preflight_sha256s": preflight_sha256s,
    ...
    *,
    ...
    return {
        ...
        "analysis": {
            ...
            "runbook_preflight_plan_static_review_artifact_dir": "/tmp/static_review",
            "source_runbook_preflight_plan_artifact_dir": "/tmp/plan",
        ...
        "source_static_review_summary": {
            ...
            "review_check_count": 139,
        },
        "source_runbook_preflight_plan_summary": {
            "schema_version": "plan",
            "status": "ready",
            "passed": True,
            "authorized_next_work": "static_review_only",
            "plan_check_count": 223,
