...
SCRIPT_PATH = (
    ...
    / "scripts"
    / "integrations"
    / "preflight_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_readiness_evaluation_runbook_preflight.py"
)
ARTIFACT_HEAD = "e" * 40
CURRENT_HEAD = "f" * 40
...
def _load_module():
    spec = importlib.util.spec_from_file_location(
        "v14_post_closeout_promotion_readiness_evaluation_runbook_preflight",
        SCRIPT_PATH,
    )
...
def test_promotion_readiness_evaluation_runbook_preflight_passes(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert decision["evaluation_runbook_preflight_static_review_authorized"] is True
    assert decision["selector_promotion_authorized"] is False
    assert decision["deployment_authorized"] is False
    assert decision["safety_benefit_claim_authorized"] is False
    assert decision["camp_over_dp_top1_claim_authorized"] is False
    assert len(report["runbook_preflight"]) == 6
    assert len(report["artifact_manifest_requirements"]) == 7
    assert all(item["triggered"] is False for item in report["no_go_status"])
    assert (
        fixture["output_dir"]
        / "post_closeout_promotion_readiness_evaluation_runbook_preflight.json"
    ).is_file()
    assert (
        fixture["output_dir"]
        / "post_closeout_promotion_readiness_evaluation_runbook_preflight.md"
    ).is_file()
    assert (fixture["output_dir"] / "SHA256SUMS").is_file()


def test_promotion_readiness_evaluation_runbook_preflight_accepts_uppercase_dp_head(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_promotion_readiness_evaluation_runbook_preflight_requires_enable(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert "runbook_preflight_enabled" in report["final_decision"]["failed_checks"]
    assert (
        report["final_decision"]["failure_class"]
        == "explicit_runbook_preflight_authorization_missing"
    )


def test_promotion_readiness_evaluation_runbook_preflight_rejects_wrong_eof(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert "audit_latest_eof_authorizes_runbook_preflight" in report["final_decision"]["failed_checks"]
    assert "status_doc_latest_eof_authorizes_runbook_preflight" in report["final_decision"]["failed_checks"]
    assert report["final_decision"]["failure_class"] == "v14_eof_contract_mismatch"


def test_promotion_readiness_evaluation_runbook_preflight_rejects_source_hash_drift(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    fixture["source_runbook_preflight_plan_md"].write_text("drift\n", encoding="utf-8")
...
def test_promotion_readiness_evaluation_runbook_preflight_rejects_source_review_leak(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_promotion_readiness_evaluation_runbook_preflight_cli_writes_outputs(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    exit_code = module.main(
        [
            "--runbook_preflight_plan_static_review_artifact_dir",
            str(fixture["runbook_preflight_plan_static_review_artifact_dir"]),
            "--runbook_preflight_plan_static_review_json",
            str(fixture["runbook_preflight_plan_static_review_json"]),
            "--runbook_preflight_plan_static_review_md",
            str(fixture["runbook_preflight_plan_static_review_md"]),
            "--runbook_preflight_plan_static_review_sha256s",
            str(fixture["runbook_preflight_plan_static_review_sha256s"]),
            "--source_runbook_preflight_plan_artifact_dir",
            str(fixture["source_runbook_preflight_plan_artifact_dir"]),
            "--source_runbook_preflight_plan_json",
            str(fixture["source_runbook_preflight_plan_json"]),
            "--source_runbook_preflight_plan_md",
            str(fixture["source_runbook_preflight_plan_md"]),
            "--source_runbook_preflight_plan_sha256s",
            str(fixture["source_runbook_preflight_plan_sha256s"]),
            "--v14_audit_md",
...
def _write_fixture(
    ...
    *,
    ...
    plan_heads = _write(
        ...
        "\n".join(
            [
                ...
                "source_plan_static_review_artifact=/tmp/plan_static_review",
                "source_evaluation_plan_artifact=/tmp/evaluation_plan",
                "",
            ]
        ),
    )
    ...
    static_heads = _write(
        ...
        "\n".join(
            [
                ...
                f"source_runbook_preflight_plan_artifact={plan_artifact.resolve()}",
                "",
            ]
        ),
    )
    ...
    return {
        "runbook_preflight_plan_static_review_artifact_dir": static_artifact,
        "runbook_preflight_plan_static_review_json": static_review_json,
        "runbook_preflight_plan_static_review_md": static_review_md,
        "runbook_preflight_plan_static_review_sha256s": static_review_sha256s,
        "source_runbook_preflight_plan_artifact_dir": plan_artifact,
        "source_runbook_preflight_plan_json": source_plan_json,
        "source_runbook_preflight_plan_md": source_plan_md,
        "source_runbook_preflight_plan_sha256s": source_plan_sha256s,
...
def _static_review_payload(
    ...
    *,
    ...
    decision = {
        ...
        "evaluation_runbook_preflight_authorized": True,
    ...
    return {
        ...
        "analysis": {
            ...
            "read_only": True,
            "runbook_preflight_plan_artifact_dir": str(source_plan_artifact.resolve()),
...
def _source_plan_payload(
    ...
    *,
    ...
    decision = {
        ...
        "evaluation_runbook_preflight_plan_static_review_authorized": True,
        "score_expression": module.SCORE_EXPRESSION,
        **{name: False for name in module.BLOCKED_ACTIONS},
        **{name: False for name in module.EXECUTION_FLAGS},
    }
    ...
    return {
        ...
        "forbidden_actions": [f"forbidden_{index}" for index in range(6)],
        "final_decision": decision,
    }
