...
from scripts.integrations.plan_diffusion_planner_candidate_set_consensus_lane_projected_jerk_progress_default_off_fixed_snapshot_screen_rerun_remediation_negative_support_followup_fixed_snapshot_screen_rerun import (
...
def _build(
    ...
    *,
    ...
    return build_report(
        ...
        execution_root=Path(DEFAULT_DEVELOPMENT_ROOT) / "unit_negative_support_execution",
        camp_head="abc",
...
def test_negative_support_fixed_snapshot_screen_rerun_plan_ready(
    tmp_path: Path,
) -> None:
    ...
    assert plan["execution_artifacts"]["output_root"].endswith(
        "unit_negative_support_execution"
    )


def test_negative_support_fixed_snapshot_screen_rerun_plan_rejects_dp_mismatch(
    tmp_path: Path,
) -> None:
...
def test_negative_support_fixed_snapshot_screen_rerun_plan_rejects_missing_post_review_authorization(
    tmp_path: Path,
) -> None:
    report = _build(
        tmp_path,
...
def test_negative_support_fixed_snapshot_screen_rerun_plan_rejects_blocked_action_leak(
    tmp_path: Path,
) -> None:
...
def test_negative_support_fixed_snapshot_screen_rerun_plan_rejects_missing_markdown_authorization(
    tmp_path: Path,
) -> None:
...
def test_negative_support_fixed_snapshot_screen_rerun_runbook_is_guarded_and_scoped(
    tmp_path: Path,
) -> None:
    report = _build(tmp_path)
    ...
    assert "--max_deceleration_mps2 3.0" in runbook
...
def test_negative_support_fixed_snapshot_screen_rerun_markdown_records_boundaries(
    tmp_path: Path,
) -> None:
    report = _build(tmp_path)
...
def test_negative_support_fixed_snapshot_screen_rerun_plan_cli_writes_outputs(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    monkeypatch.setattr(
        ...
        [
            ...
            str(root),
            "--execution_root",
            str(Path(DEFAULT_DEVELOPMENT_ROOT) / "unit_negative_support_execution"),
            "--camp_head",
            "abc",
            "--camp_origin_main",
            "abc",
