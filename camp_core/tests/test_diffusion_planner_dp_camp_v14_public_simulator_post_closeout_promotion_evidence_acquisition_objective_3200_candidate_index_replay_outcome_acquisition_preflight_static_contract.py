...
SCRIPT_PATH = (
    ...
    / "scripts"
    / "integrations"
    / "review_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_evidence_acquisition_objective_3200_candidate_index_replay_outcome_acquisition_preflight_static_contract.py"
)
...
PREFLIGHT_TEST_PATH = (
    ...
    / "camp_core"
    / "tests"
    / "test_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_evidence_acquisition_objective_3200_candidate_index_replay_outcome_acquisition_preflight.py"
)
SOURCE_HEAD = "d" * 40
CURRENT_HEAD = "e" * 40
...
def test_candidate_index_replay_outcome_acquisition_preflight_static_review_passes(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert decision["objective_3200_candidate_index_replay_outcome_acquisition_preflight_static_review_passed"] is True
    assert decision["objective_3200_candidate_index_replay_outcome_acquisition_execution_authorized"] is True
    ...
    assert summary["objective_required_records"] == module.OBJECTIVE_REQUIRED_RECORDS
    assert summary["candidate_closed_loop_outcome_records"] == 0
    assert summary["missing_candidate_closed_loop_outcome_records"] == module.OBJECTIVE_REQUIRED_RECORDS
...
def test_candidate_index_replay_outcome_acquisition_preflight_static_review_requires_enable(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert (
        ...
        == "explicit_candidate_index_replay_outcome_acquisition_preflight_static_review_authorization_missing"
    )


def test_candidate_index_replay_outcome_acquisition_preflight_static_review_rejects_wrong_eof(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_candidate_index_replay_outcome_acquisition_preflight_static_review_rejects_claim_leak(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_candidate_index_replay_outcome_acquisition_preflight_static_review_rejects_hash_drift(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    fixture["candidate_index_outcome_preflight_md"].write_text("# drift\n", encoding="utf-8")
...
def test_candidate_index_replay_outcome_acquisition_preflight_static_review_rejects_direct_execution(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(
        tmp_path,
        module,
        source_decision_updates={"direct_candidate_index_replay_execution_authorized": True},
    )

    report = module.build_report(**fixture)

    assert "source_preflight_direct_candidate_index_replay" in report["final_decision"]["failed_checks"]
    assert report["final_decision"]["direct_candidate_index_replay_execution_authorized"] is False
    ...
    *,
    ...
    doc_text = "\n".join(
        [
            ...
            "objective_3200_candidate_index_replay_outcome_acquisition_preflight_ready=True",
            "objective_3200_candidate_index_replay_outcome_acquisition_preflight_static_review_authorized=True",
    ...
    return {
        "candidate_index_outcome_preflight_artifact_dir": artifact,
        "candidate_index_outcome_preflight_json": preflight_json,
        "candidate_index_outcome_preflight_md": preflight_md,
        "candidate_index_outcome_preflight_sha256s": preflight_sha,
        "candidate_index_outcome_preflight_script_py": PREFLIGHT_SCRIPT_PATH,
        "candidate_index_outcome_preflight_test_py": PREFLIGHT_TEST_PATH,
    ...
    *,
    ...
    return {
        ...
        "future_execution_contract": {
            "required_records": module.OBJECTIVE_REQUIRED_RECORDS,
