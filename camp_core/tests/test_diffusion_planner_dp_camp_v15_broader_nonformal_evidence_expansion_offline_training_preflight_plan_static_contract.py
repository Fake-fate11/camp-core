...
HEAD = "63d26122fa32d50a9dfdcd886a283106d6f61348"


def _load_module():
    spec = importlib.util.spec_from_file_location("v15_offline_training_plan_static_review", SCRIPT_PATH)
...
def test_v15_offline_training_preflight_plan_static_review_passes(tmp_path: Path) -> None:
    module = _load_module()
...
def test_v15_offline_training_preflight_plan_static_review_requires_enable(tmp_path: Path) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
...
def test_v15_offline_training_preflight_plan_static_review_rejects_wrong_eof(tmp_path: Path) -> None:
    module = _load_module()
...
def test_v15_offline_training_preflight_plan_static_review_rejects_training_leak(tmp_path: Path) -> None:
    module = _load_module()
...
def test_v15_offline_training_preflight_plan_static_review_is_recorded_in_audit() -> None:
    module = _load_module()
...
def _write_fixture(
    ...
    *,
    ...
    _write_json(source_json, _source_plan_payload(module, source_updates=source_updates))
    source_md.write_text("# Offline Training Plan\n", encoding="utf-8")
    for name, content in {
        "HEADS": f"CAMP_HEAD={HEAD}\nCAMP_ORIGIN_MAIN={HEAD}\nDP_HEAD={module.FIXED_DP_HEAD}\n",
        "COMMAND": "run offline training plan\n",
        "stdout.txt": "{}\n",
        "stderr.txt": "",
        "run.exit": "0\n",
    }.items():
...
def _source_plan_payload(module, *, source_updates: dict | None = None) -> dict:
    decision = {
        "passed": True,
    ...
    return {
        ...
        "offline_training_preflight_plan": {
            "required_inputs": {"splits": ["train", "calibration", "holdout"]},
