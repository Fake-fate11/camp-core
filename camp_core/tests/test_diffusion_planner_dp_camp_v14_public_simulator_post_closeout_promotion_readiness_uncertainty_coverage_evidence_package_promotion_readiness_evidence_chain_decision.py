...
def test_uncertainty_coverage_evidence_package_promotion_readiness_evidence_chain_decision_passes(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert decision["promotion_readiness_evidence_chain_decision_recorded"] is True
    ...
    assert {item["promote_selector_now"] for item in report["evidence_chain_decision"]} == {False}
...
def test_uncertainty_coverage_evidence_package_promotion_readiness_evidence_chain_decision_requires_enable(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert "evidence_chain_decision_enabled" in report["final_decision"]["failed_checks"]
    assert report["final_decision"]["failure_class"] == "explicit_evidence_chain_decision_authorization_missing"


def test_uncertainty_coverage_evidence_package_promotion_readiness_evidence_chain_decision_rejects_wrong_eof(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert "audit_latest_eof_authorizes_evidence_chain_decision" in report["final_decision"]["failed_checks"]
    assert "status_doc_latest_eof_authorizes_evidence_chain_decision" in report["final_decision"]["failed_checks"]
    assert report["final_decision"]["failure_class"] == "v14_eof_contract_mismatch"


def test_uncertainty_coverage_evidence_package_promotion_readiness_evidence_chain_decision_rejects_source_review_leak(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_uncertainty_coverage_evidence_package_promotion_readiness_evidence_chain_decision_rejects_hash_drift(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
...
def _source_review_payload(
    ...
    *,
    ...
    return {
        ...
        "evidence_chain_decision_plan_summary": {
            ...
            "source_boundary_plan_check_count": module.EXPECTED_SOURCE_BOUNDARY_PLAN_CHECK_COUNT,
            "source_boundary_plan_failed_check_count": 0,
            "source_boundary_item_count": module.EXPECTED_SOURCE_BOUNDARY_ITEM_COUNT,
