...
SCRIPT_PATH = (
    ...
    / "scripts"
    / "integrations"
    / "review_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_evidence_acquisition_paired_evaluation_execution_preflight_static_contract.py"
)
...
PREFLIGHT_TEST_PATH = (
    ...
    / "camp_core"
    / "tests"
    / "test_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_evidence_acquisition_paired_evaluation_execution_preflight.py"
)
ARTIFACT_HEAD = "a" * 40
CURRENT_HEAD = "b" * 40
...
def test_post_closeout_promotion_evidence_acquisition_paired_evaluation_execution_preflight_static_review_passes(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert (
        decision[
            "post_closeout_promotion_evidence_acquisition_paired_evaluation_execution_authorized"
        ]
        is True
    )
    ...
    assert report["preflight_contract_summary"]["evidence_lock_count"] == len(module.EXPECTED_EVIDENCE_LOCKS)
    assert report["preflight_contract_summary"]["required_input_manifest_count"] == len(
        module.EXPECTED_REQUIRED_INPUT_MANIFESTS
    )
    assert report["preflight_contract_summary"]["preflight_plan_count"] == len(module.EXPECTED_PREFLIGHT_ITEMS)
    assert report["preflight_contract_summary"]["future_output_count"] == len(module.EXPECTED_FUTURE_OUTPUTS)
    assert report["preflight_contract_summary"]["no_go_count"] == len(module.EXPECTED_NO_GO)
    assert (fixture["output_dir"] / module.REVIEW_JSON_NAME).is_file()
    assert (fixture["output_dir"] / module.REVIEW_MD_NAME).is_file()
    assert (fixture["output_dir"] / "SHA256SUMS").is_file()


def test_post_closeout_promotion_evidence_acquisition_paired_evaluation_execution_preflight_static_review_requires_enable(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert (
        ...
        == "explicit_paired_evaluation_execution_preflight_static_review_authorization_missing"
    )


def test_post_closeout_promotion_evidence_acquisition_paired_evaluation_execution_preflight_static_review_rejects_wrong_eof(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_post_closeout_promotion_evidence_acquisition_paired_evaluation_execution_preflight_static_review_rejects_source_claim_leak(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_post_closeout_promotion_evidence_acquisition_paired_evaluation_execution_preflight_static_review_rejects_source_execution_leak(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(
    ...
    assert "source_preflight_no_paired_execution" in report["final_decision"]["failed_checks"]
    assert "source_preflight_execution_not_authorized_by_source" in report[
        "final_decision"
    ]["failed_checks"]
...
def test_post_closeout_promotion_evidence_acquisition_paired_evaluation_execution_preflight_static_review_rejects_hash_drift(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    fixture["paired_evaluation_execution_preflight_md"].write_text("# drift\n", encoding="utf-8")
...
def test_post_closeout_promotion_evidence_acquisition_paired_evaluation_execution_preflight_static_review_rejects_safety_score_doc_drift(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
...
def _write_fixture(
    ...
    *,
    ...
    artifact = tmp_path / "paired_evaluation_execution_preflight_artifact"
    ...
    command = _write(artifact / "COMMAND", "paired evaluation execution preflight command\n")
    heads = _write(
        artifact / "HEADS",
        "\n".join(
            [
    ...
    return {
        "paired_evaluation_execution_preflight_artifact_dir": artifact,
        "paired_evaluation_execution_preflight_json": preflight_json,
        "paired_evaluation_execution_preflight_md": preflight_md,
        "paired_evaluation_execution_preflight_sha256s": preflight_sha256s,
...
def _source_preflight_payload(
    ...
    *,
    ...
    decision = {
        ...
        "recommendation": "static_review_paired_evaluation_execution_preflight_only",
    ...
    return {
        ...
        "runtime_result_summary": {
            ...
            "selection_log_count": module.PREFLIGHT_MODULE.EXPECTED_SELECTION_LOG_COUNT,
        ...
        "delta_review_summary": {
            "record_count": module.EXPECTED_RECORD_COUNT,
            "static_objective_delta_supported": True,
            "shadow_selected_index_differs_from_executed_index_records": module.EXPECTED_SHADOW_DIFF_RECORDS,
        },
        "readiness_result_summary": {
            "passed": True,
            "direct_promotion_recommendation": False,
        ...
        "evidence_locks": [
            {"name": name, "requirement": "fixture"} for name in module.EXPECTED_EVIDENCE_LOCKS
        ],
        "required_input_manifests": [
            {"name": name, "requirement": "fixture"}
            for name in module.EXPECTED_REQUIRED_INPUT_MANIFESTS
        ],
        "preflight_plan": [
        ...
        "future_outputs": [
            ...
            for name in module.EXPECTED_FUTURE_OUTPUTS
        ],
        "no_go_register": [
