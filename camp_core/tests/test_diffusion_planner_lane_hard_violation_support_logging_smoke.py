...
from scripts.integrations.analyze_diffusion_planner_lane_hard_violation_support_logging_smoke import (
    analyze,
)
...
def _payload() -> dict:
    return build_lane_hard_violation_support_logging_payload(
...
def _record(*, payload: dict | None) -> dict:
    record = {
        ...
        "lane_hard_violation_support_logging": payload,
    }
    if payload is not None:
        record.update(payload["latency_ms"])
    else:
        record.update({key: 0.0 for key in LANE_HARD_VIOLATION_SUPPORT_LATENCY_KEYS})
    return record
...
def test_lane_hard_violation_smoke_audit_accepts_paired_payload(
    tmp_path: Path,
) -> None:
...
def test_lane_hard_violation_smoke_audit_rejects_future_payload_key(
    tmp_path: Path,
) -> None:
    payload = deepcopy(_payload())
    payload["candidate_closed_loop_outcomes"] = [{"lane_violation": True}]
    _write_run(tmp_path / "baseline", enabled=False, payload=None)
...
def test_lane_hard_violation_smoke_audit_rejects_negative_atom(
    tmp_path: Path,
) -> None:
    payload = deepcopy(_payload())
    payload["lane_hard_violation_support_atoms"][1][0] = -0.5
    payload["finite_checks"]["lane_hard_violation_support_atoms_nonnegative"] = False
    _write_run(tmp_path / "baseline", enabled=False, payload=None)
    _write_run(tmp_path / "candidate", enabled=True, payload=payload)
    ...
    assert report["final_decision"]["passed"] is False
    assert any(
        "lane_hard_violation_support_atoms_nonnegative" in error
        for error in report["errors"]
    )


def test_lane_hard_violation_smoke_plan_authorizes_paired_three_step_only() -> None:
    ...
    assert "--camp_lane_hard_violation_support_logging" not in baseline_command
    assert "--camp_lane_hard_violation_support_logging" in candidate_command
    assert candidate_command[
        candidate_command.index("--camp_lane_hard_violation_support_steps") + 1
    ] == "10"
    assert baseline_command[baseline_command.index("--steps") + 1] == "3"
    assert candidate_command[candidate_command.index("--seed") + 1] == "1"


def test_lane_hard_violation_smoke_plan_rejects_missing_payload_audit(
    tmp_path: Path,
) -> None:
