...
def _load_module():
    spec = importlib.util.spec_from_file_location("v16_nuscenes_smoke_preflight_plan", SCRIPT_PATH)
...
def test_v16_nuscenes_smoke_preflight_plan_passes(tmp_path: Path) -> None:
    module = _load_module()
...
def test_v16_nuscenes_smoke_preflight_plan_requires_enable(tmp_path: Path) -> None:
    ...
    assert "smoke_preflight_plan_enabled" in report["final_decision"]["failed_checks"]


def test_v16_nuscenes_smoke_preflight_plan_rejects_wrong_eof(tmp_path: Path) -> None:
    module = _load_module()
    ...
    assert report["final_decision"]["passed"] is False
    assert "audit_authorizes_smoke_preflight_plan" in report["final_decision"]["failed_checks"]
    assert "status_authorizes_smoke_preflight_plan" in report["final_decision"]["failed_checks"]


def test_v16_nuscenes_smoke_preflight_plan_rejects_missing_tensor_hash(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module, drop_record="candidate_tensor_sha256")

    report = module.build_report(**fixture)

    assert report["final_decision"]["passed"] is False
    assert "source_records_candidate_tensor_sha256" in report["final_decision"]["failed_checks"]


def test_v16_nuscenes_smoke_preflight_plan_is_recorded() -> None:
    module = _load_module()
...
def _write_fixture(
    ...
    *,
    ...
    source_md.write_text("# Adapter Plan Static Review\n", encoding="utf-8")
    for name, content in {
        "HEADS": f"CAMP_HEAD={HEAD}\nCAMP_ORIGIN_MAIN={HEAD}\nDP_HEAD={module.FIXED_DP_HEAD}\n",
        "COMMAND": "run adapter plan static review\n",
        "stdout.txt": "{}\n",
        "stderr.txt": "",
        "run.exit": "0\n",
    }.items():
