...
HEAD = "af32d2237fa8515aae05b9a1dec7ae8fd17aefdd"


def _load_module():
    spec = importlib.util.spec_from_file_location("v15_matrix_execution_preflight", SCRIPT_PATH)
...
def test_v15_matrix_execution_preflight_passes(tmp_path: Path) -> None:
    module = _load_module()
    ...
    assert decision["matrix_execution_preflight_executed"] is True
    ...
    assert report["matrix_execution_preflight"]["combination_count"] == 576
    assert report["timing_contract"]["timing_required"] is True
    assert "training_wall_clock_seconds" in report["timing_contract"]["offline_training_required_fields"]
...
def test_v15_matrix_execution_preflight_requires_enable(tmp_path: Path) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert "matrix_execution_preflight_enabled" in report["final_decision"]["failed_checks"]


def test_v15_matrix_execution_preflight_rejects_wrong_eof(tmp_path: Path) -> None:
    module = _load_module()
...
def test_v15_matrix_execution_preflight_rejects_source_execution_leak(tmp_path: Path) -> None:
    module = _load_module()
...
def test_v15_matrix_execution_preflight_is_recorded_in_audit() -> None:
    module = _load_module()
...
def _write_fixture(
    ...
    *,
    ...
    source_md.write_text("# Matrix Plan Static Review\n", encoding="utf-8")
    for name, content in {
        "HEADS": f"CAMP_HEAD={HEAD}\nCAMP_ORIGIN_MAIN={HEAD}\nDP_HEAD={module.FIXED_DP_HEAD}\n",
        "COMMAND": "run matrix plan static review\n",
        "stdout.txt": "{}\n",
        "stderr.txt": "",
        "run.exit": "0\n",
    }.items():
