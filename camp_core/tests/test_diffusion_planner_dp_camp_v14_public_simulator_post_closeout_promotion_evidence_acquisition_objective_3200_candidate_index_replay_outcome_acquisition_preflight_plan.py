...
def test_candidate_index_replay_outcome_acquisition_preflight_plan_passes(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert decision["objective_3200_candidate_index_replay_outcome_acquisition_preflight_plan_ready"] is True
    assert decision["objective_3200_candidate_index_replay_outcome_acquisition_preflight_plan_static_review_authorized"] is True
    ...
    assert runner["has_candidate_index_replay_flag"] is True
    assert runner["has_harness_alias_flag"] is True
    assert runner["routes_shadow_selected_index"] is True
    assert [item["name"] for item in report["outcome_acquisition_preflight_plan"]] == list(
        module.EXPECTED_PREFLIGHT_PLAN_ITEMS
    )
    assert "exactly_3200_strict_pairs" in report["strict_pairing_and_metrics_protocol"]["pass_fail_criteria"]
    assert (fixture["output_dir"] / module.PLAN_JSON_NAME).is_file()
    assert (fixture["output_dir"] / module.PLAN_MD_NAME).is_file()
    assert (fixture["output_dir"] / "SHA256SUMS").is_file()


def test_candidate_index_replay_outcome_acquisition_preflight_plan_requires_enable(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _fixture(tmp_path, module)
    ...
    assert "outcome_acquisition_preflight_plan_enabled" in report["final_decision"]["failed_checks"]
    assert (
        report["final_decision"]["failure_class"]
        == "explicit_candidate_index_replay_outcome_acquisition_preflight_plan_authorization_missing"
    )


def test_candidate_index_replay_outcome_acquisition_preflight_plan_rejects_wrong_eof(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_candidate_index_replay_outcome_acquisition_preflight_plan_rejects_missing_harness_flag(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _fixture(tmp_path, module)
    fixture["runtime_replay_script_py"].write_text(
    ...
    assert report["final_decision"]["failure_class"] == "candidate_index_replay_harness_surface_missing"


def test_candidate_index_replay_outcome_acquisition_preflight_plan_rejects_hash_drift(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _fixture(tmp_path, module)
    ...
    *,
    ...
    doc_text = "\n".join(
        [
            ...
            "objective_3200_candidate_index_replay_harness_post_implementation_static_review_passed=True",
            "objective_3200_candidate_index_replay_outcome_acquisition_preflight_plan_authorized=True",
    ...
    runtime_replay_script = _write_runtime_fixture(tmp_path)
    ...
    return _write(
        ...
        "\n".join(
            [
                ...
                "    'requires --camp_collect_closed_loop_outcomes'",
                "    'requires --camp_candidate_tensor_provenance_logging'",
                "    shadow_selected_index = 3",
...
def _source_review_report(module) -> dict[str, Any]:
    ...
    return {
        ...
        "source_implementation_summary": {
            ...
            "candidate_index_replay_flag_present": True,
            "harness_execution_executed_by_this_gate": False,
            "outcome_acquisition_executed_by_this_gate": False,
        },
