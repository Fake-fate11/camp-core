...
HEAD = "b1ca952a6f63f446fdfbfb01f92bdbe8954d68c8"


def _load_module():
    spec = importlib.util.spec_from_file_location("v15_paired_evaluation_execution_preflight", SCRIPT_PATH)
...
def test_v15_paired_evaluation_execution_preflight_passes(tmp_path: Path) -> None:
    module = _load_module()
    ...
    preflight = report["paired_evaluation_execution_preflight"]
    assert decision["passed"] is True
    assert decision["status"] == module.READY_STATUS
    assert decision["authorized_next_work"] == module.AUTHORIZED_NEXT_WORK
    assert decision["paired_evaluation_execution_preflight_executed"] is True
    assert decision["paired_evaluation_executed"] is False
    assert set(preflight["required_inputs"]["evaluation_splits"]) == {"calibration", "holdout"}
    assert preflight["paired_protocol"]["baseline"] == "dp_top1"
    assert (fixture["output_dir"] / module.PREFLIGHT_JSON_NAME).is_file()
    assert (fixture["output_dir"] / module.TIMING_JSON_NAME).is_file()


def test_v15_paired_evaluation_execution_preflight_requires_enable(tmp_path: Path) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
...
def test_v15_paired_evaluation_execution_preflight_rejects_wrong_eof(tmp_path: Path) -> None:
    module = _load_module()
...
def test_v15_paired_evaluation_execution_preflight_rejects_source_eval_leak(tmp_path: Path) -> None:
    module = _load_module()
...
def test_v15_paired_evaluation_execution_preflight_rejects_failed_source_review(tmp_path: Path) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module, source_updates={"passed": False})
...
def test_v15_paired_evaluation_execution_preflight_is_recorded_in_audit() -> None:
    module = _load_module()
...
def _write_fixture(
    ...
    *,
    ...
    source_md.write_text("# Paired Evaluation Execution Plan Static Review\n", encoding="utf-8")
    for name, content in {
        "HEADS": f"CAMP_HEAD={HEAD}\nCAMP_ORIGIN_MAIN={HEAD}\nDP_HEAD={module.FIXED_DP_HEAD}\n",
        "COMMAND": "run paired evaluation execution plan static review\n",
        "stdout.txt": "{}\n",
        "stderr.txt": "",
        "run.exit": "0\n",
    }.items():
