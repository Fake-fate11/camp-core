...
SCRIPT_PATH = (
    ...
    / "scripts"
    / "integrations"
    / "review_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_evidence_acquisition_actual_safetycost_outcome_materialization_preflight_plan_static_contract.py"
)
PLAN_SCRIPT_PATH = (
    ROOT
    / "scripts"
    / "integrations"
    / "plan_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_evidence_acquisition_actual_safetycost_outcome_materialization_preflight.py"
)
PLAN_TEST_PATH = (
    ROOT
    / "camp_core"
    / "tests"
    / "test_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_evidence_acquisition_actual_safetycost_outcome_materialization_preflight_plan.py"
)
SOURCE_HEAD = "7" * 40
CURRENT_HEAD = "8" * 40
...
def test_actual_safetycost_outcome_materialization_preflight_plan_static_review_passes(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert decision["actual_safetycost_outcome_materialization_preflight_authorized"] is True
    ...
    assert report["source_plan_summary"]["preflight_plan_check_count"] == 74
...
def test_actual_safetycost_outcome_materialization_preflight_plan_static_review_requires_enable(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert (
        ...
        == "explicit_actual_safetycost_outcome_materialization_preflight_plan_static_review_authorization_missing"
    )


def test_actual_safetycost_outcome_materialization_preflight_plan_static_review_rejects_wrong_eof(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_actual_safetycost_outcome_materialization_preflight_plan_static_review_rejects_source_claim_leak(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_actual_safetycost_outcome_materialization_preflight_plan_static_review_rejects_hash_drift(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    fixture["outcome_materialization_preflight_plan_md"].write_text(
        "# drift\n",
        encoding="utf-8",
    )
...
def _write_fixture(
    ...
    *,
    ...
    return {
        "outcome_materialization_preflight_plan_artifact_dir": artifact,
        "outcome_materialization_preflight_plan_json": plan_json,
        "outcome_materialization_preflight_plan_md": plan_md,
        "outcome_materialization_preflight_plan_sha256s": plan_sha,
        "outcome_materialization_preflight_plan_script_py": PLAN_SCRIPT_PATH,
        "outcome_materialization_preflight_plan_test_py": PLAN_TEST_PATH,
    ...
    *,
    ...
    return {
        ...
        "preflight_plan": [
            ...
            for name in plan_module.EXPECTED_PREFLIGHT_ITEMS
        ],
        "planned_outputs": [
        ...
        "source_static_review_summary": {
            ...
            "source_plan_check_count": 71,
            "source_paired_record_count": 3200,
            "source_shadow_diff_records": 2832,
        },
        "outcome_materialization_preflight_summary": {
            "actual_safetycost_v1_available": False,
            "actual_safetycost_v1_claim_rule_evaluable": False,
