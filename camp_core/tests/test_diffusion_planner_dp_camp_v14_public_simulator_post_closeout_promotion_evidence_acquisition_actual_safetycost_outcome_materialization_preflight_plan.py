...
def test_actual_safetycost_outcome_materialization_preflight_plan_passes(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert (
        decision[
            "post_closeout_promotion_evidence_acquisition_paired_evaluation_actual_safetycost_outcome_materialization_preflight_plan_static_review_authorized"
        ]
        is True
    )
    ...
    assert {item["materializes_outcomes"] for item in report["preflight_plan"]} == {
        False
    }
...
def test_actual_safetycost_outcome_materialization_preflight_plan_requires_enable(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert "outcome_materialization_preflight_plan_enabled" in report["final_decision"][
        "failed_checks"
    ]
    assert (
        report["final_decision"]["failure_class"]
        == "explicit_actual_safetycost_outcome_materialization_preflight_plan_authorization_missing"
    )


def test_actual_safetycost_outcome_materialization_preflight_plan_rejects_wrong_eof(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_actual_safetycost_outcome_materialization_preflight_plan_rejects_source_claim_leak(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_actual_safetycost_outcome_materialization_preflight_plan_rejects_hash_drift(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
