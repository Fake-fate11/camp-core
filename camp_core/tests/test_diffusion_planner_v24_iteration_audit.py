...
def test_v24_evidence_reviewer_comprehensive_production_contract_is_sealed() -> None:
    text = " ".join(AUDIT.read_text(encoding="utf-8").split())
    for phrase in (
        "## Gate 61: Reviewer Comprehensive Production-Contract Remediation",
        "`ValueError: source reviewer holdout-state/root binding mismatch`",
        ...
        "`source_arc_length_m` is the independently reviewed census/builder-cache completion denominator",
        "unmodified formal reviewer code dry-validated source-review, evidence, launch, and marker",
        "passed all `256` tests",
