...
HEAD = "f1f233092b4e52982d64e33a1ab08f44fec3283a"


def _load_module():
    spec = importlib.util.spec_from_file_location("v15_source_inventory_preflight", SCRIPT_PATH)
...
def test_v15_source_inventory_preflight_passes(tmp_path: Path) -> None:
    module = _load_module()
    ...
    assert list(report["inventory_requirements"]) == list(module.INVENTORY_REQUIREMENTS)
    assert (fixture["output_dir"] / module.PREFLIGHT_JSON_NAME).is_file()
    assert (fixture["output_dir"] / module.PREFLIGHT_MD_NAME).is_file()


def test_v15_source_inventory_preflight_requires_enable(tmp_path: Path) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
...
def test_v15_source_inventory_preflight_rejects_wrong_eof(tmp_path: Path) -> None:
    module = _load_module()
    ...
    assert report["final_decision"]["passed"] is False
    assert "audit_authorizes_inventory_preflight" in report["final_decision"]["failed_checks"]
    assert "status_authorizes_inventory_preflight" in report["final_decision"]["failed_checks"]


def test_v15_source_inventory_preflight_rejects_source_dp_mutation(tmp_path: Path) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module, source_updates={"dp_modified": True})
