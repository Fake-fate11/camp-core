...
def _load_module():
    spec = importlib.util.spec_from_file_location(
        "v14_post_closeout_promotion_readiness_evidence_chain_no_promotion_closeout_record",
        SCRIPT_PATH,
    )
...
def test_promotion_readiness_evidence_chain_no_promotion_closeout_record_passes(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert decision["promotion_readiness_evidence_chain_no_promotion_closeout_recorded"] is True
    assert decision["promotion_readiness_evidence_chain_no_promotion_closeout_record_static_review_authorized"] is True
    assert decision["evidence_chain_closed_by_this_gate"] is True
    assert decision["selector_promotion_authorized"] is False
    assert decision["deployment_authorized"] is False
    assert decision["safety_benefit_claim_authorized"] is False
    assert report["closeout_record"]["final_evidence_chain_state"] == "audit_evidence_chain_closed_no_promotion_no_deployment_no_claim"
    assert (fixture["output_dir"] / module.RECORD_JSON_NAME).is_file()
    assert (fixture["output_dir"] / module.RECORD_MD_NAME).is_file()
    assert (fixture["output_dir"] / "SHA256SUMS").is_file()


def test_promotion_readiness_evidence_chain_no_promotion_closeout_record_requires_enable(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert (
        ...
        == "explicit_evidence_chain_no_promotion_closeout_record_authorization_missing"
    )


def test_promotion_readiness_evidence_chain_no_promotion_closeout_record_rejects_wrong_eof(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert "audit_latest_eof_authorizes_closeout_record" in report["final_decision"]["failed_checks"]
    assert "status_doc_latest_eof_authorizes_closeout_record" in report["final_decision"]["failed_checks"]
    assert report["final_decision"]["failure_class"] == "v14_eof_contract_mismatch"


def test_promotion_readiness_evidence_chain_no_promotion_closeout_record_rejects_source_review_leak(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_promotion_readiness_evidence_chain_no_promotion_closeout_record_rejects_hash_drift(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
...
def _source_review_payload(
    ...
    *,
    ...
    decision = {
        ...
        "promotion_readiness_evidence_chain_decision_static_review_passed": True,
        "promotion_readiness_evidence_chain_no_promotion_closeout_authorized": True,
        "recommendation": "record_no_promotion_closeout_for_current_evidence_chain",
    ...
    return {
        ...
        "source_decision_summary": {
            "decision_check_count": module.EXPECTED_SOURCE_DECISION_CHECK_COUNT,
            "failed_check_count": 0,
        },
        "source_chain_summary": {
            "source_review_check_count": module.EXPECTED_SOURCE_STATIC_REVIEW_CHECK_COUNT,
            "source_review_failed_check_count": 0,
