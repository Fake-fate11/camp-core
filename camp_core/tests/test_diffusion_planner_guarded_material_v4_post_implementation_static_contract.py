...
MODULE = (
    ...
    "review_diffusion_planner_guarded_material_v4_post_implementation_static_contract"
)
...
status={target.READY_STATUS.replace("post_implementation_static_contract_review_complete", "implementation_complete")}
passed=True
failed_checks=[]
...
new_generator_policy={target.MATERIAL_POLICY_V4}
new_default_off_profile={target.MATERIAL_PROFILE_V4}
...
def _write_inputs(
    ...
    *,
    ...
    v4_test_text: str | None = None,
) -> tuple[Path, Path, Path, Path]:
    audit = tmp_path / "audit.md"
    source = tmp_path / "source.py"
    route_test = tmp_path / "test_route.py"
    v4_test = tmp_path / "test_v4.py"
    ...
    v4_test.write_text(
        v4_test_text if v4_test_text is not None else _read_current(target.DEFAULT_V4_TEST_PATH),
        encoding="utf-8",
    )
    return audit, source, route_test, v4_test
...
def test_guarded_material_v4_post_static_review_ready(tmp_path: Path) -> None:
    ...
    assert review["source_contract"]["contracts"]["explicit_v4_profile_policy_pair"]
    assert review["source_contract"]["contracts"][
        "v4_materializes_ready_candidates_report_only"
    ]
    assert review["v4_test_contract"]["contracts"][
        "v4_materialization_accounting_tests_present"
    ]


def test_guarded_material_v4_post_static_review_rejects_dp_mismatch(
    tmp_path: Path,
) -> None:
...
def test_guarded_material_v4_post_static_review_requires_audit_gate(
    tmp_path: Path,
) -> None:
    report = _build(
        tmp_path,
...
def test_guarded_material_v4_post_static_review_rejects_missing_v4_policy(
    tmp_path: Path,
) -> None:
    report = _build(
        tmp_path,
        source_text=_read_current(target.DEFAULT_SOURCE_PATH).replace(
            target.MATERIAL_POLICY_V4,
            "removed_material_policy_v4",
        ),
    )

    assert report["final_decision"]["status"] == target.REJECT_STATUS
    assert (
        "source_contract.explicit_v4_profile_policy_pair"
        in report["final_decision"]["failed_checks"]
    )


def test_guarded_material_v4_post_static_review_rejects_missing_generated_count_test(
    tmp_path: Path,
) -> None:
    report = _build(
        tmp_path,
        v4_test_text=_read_current(target.DEFAULT_V4_TEST_PATH).replace(
    ...
    assert report["final_decision"]["status"] == target.REJECT_STATUS
    assert (
        "v4_test_contract.v4_materialization_accounting_tests_present"
        in report["final_decision"]["failed_checks"]
    )


def test_guarded_material_v4_post_static_review_rejects_mutable_descriptor(
    tmp_path: Path,
) -> None:
    report = _build(
        tmp_path,
    ...
    assert report["final_decision"]["status"] == target.REJECT_STATUS
    assert (
        "source_contract.v4_descriptor_legality_report_only"
        in report["final_decision"]["failed_checks"]
    )


def test_guarded_material_v4_post_static_review_cli_writes_outputs(
    tmp_path: Path,
) -> None:
    audit, source, route_test, v4_test = _write_inputs(tmp_path)
