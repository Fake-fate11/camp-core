...
from scripts.integrations.plan_diffusion_planner_lane_hard_violation_support_matched_outcome_label_pass import (
...
def _broader_smoke(*, passed: bool = True, logging_ms: float = 2.2) -> dict:
    return {
        "final_decision": {
            "status": (
        ...
        "latency_ms": {
            "latency_ms_lane_hard_violation_support_logging": logging_ms,
        },
    }
...
def test_lane_hard_matched_outcome_plan_ready_for_exact_scope() -> None:
    ...
    matched_commands = [
        ...
        if item["variant"] == "matched_lane_hard_outcomes"
    ]
    assert len(baseline_commands) == 4
    assert len(matched_commands) == 4
    assert all(
        "--camp_lane_hard_violation_support_logging" not in command
        for command in baseline_commands
    )
    assert all(
        "--camp_collect_closed_loop_outcomes" not in command
        for command in baseline_commands
    )
    assert all(
        "--camp_lane_hard_violation_support_logging" in command
        for command in matched_commands
    )
    assert all(
        "--camp_collect_closed_loop_outcomes" in command
        for command in matched_commands
    )
    ...
    contract_command = report["commands"]["matched_lane_hard_contract_audit"]
...
def test_lane_hard_matched_outcome_plan_rejects_latency_blocked_source() -> None:
    ...
    assert failed == ["broader_lane_hard_support_latency_within_budget"]


def test_lane_hard_matched_outcome_plan_rejects_selector_mismatch() -> None:
...
def test_lane_hard_matched_outcome_plan_rejects_formal_seed() -> None:
...
def test_lane_hard_matched_outcome_plan_cli_writes_outputs(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    monkeypatch.setattr(
        "sys.argv",
        [
            "plan_diffusion_planner_lane_hard_violation_support_matched_outcome_label_pass.py",
    ...
    assert payload["plan_spec"]["root"] == "/tmp/lane_hard_matched"
