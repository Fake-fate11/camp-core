...
HEAD = "9217be18cf77d817028545d6cdcd24202ad1f1a9"


def _load_module():
    spec = importlib.util.spec_from_file_location("v15_matrix_execution_preflight_plan", SCRIPT_PATH)
...
def test_v15_matrix_execution_preflight_plan_passes(tmp_path: Path) -> None:
    module = _load_module()
    ...
    assert report["matrix_plan"]["combination_count"] == 576
    assert (fixture["output_dir"] / module.PLAN_JSON_NAME).is_file()
    assert (fixture["output_dir"] / module.PLAN_MD_NAME).is_file()


def test_v15_matrix_execution_preflight_plan_requires_enable(tmp_path: Path) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
...
def test_v15_matrix_execution_preflight_plan_rejects_wrong_eof(tmp_path: Path) -> None:
    module = _load_module()
...
def test_v15_matrix_execution_preflight_plan_rejects_source_training_leak(tmp_path: Path) -> None:
    module = _load_module()
...
def test_v15_matrix_execution_preflight_plan_is_recorded_in_audit() -> None:
    module = _load_module()
...
def _write_fixture(
    ...
    *,
    ...
    source_md.write_text("# Result Review\n", encoding="utf-8")
    for name, content in {
