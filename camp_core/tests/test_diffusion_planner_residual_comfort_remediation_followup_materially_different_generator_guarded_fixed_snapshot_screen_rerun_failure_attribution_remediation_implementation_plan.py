...
MODULE = (
    ...
    "snapshot_screen_rerun_failure_attribution_remediation_implementation"
)
...
def _write_inputs(
    ...
    *,
    ...
    markdown: str = "# Material Generator Remediation Design Static Contract Review\n",
) -> tuple[Path, Path]:
...
def test_material_generator_remediation_implementation_plan_ready(
    tmp_path: Path,
) -> None:
    ...
    assert "explicit_v2_profile_policy_pair" in slices
    assert "lane_red_hard_feasibility_precheck" in slices
    assert "jerk_limited_stop_and_creep_profiles" in slices
    assert "lateral_heading_continuity_projection" in slices
    assert "diagnostic_descriptor_payload_v2" in slices
    assert "default_off_and_v1_behavior_unchanged" in tests
    assert "v2_explicit_pair_required" in tests
    assert "candidate0_and_dp_rows_preserved" in tests
    assert "hard_precheck_fail_closed" in tests
    assert "finite_current_tick_inputs_only" in tests
    assert "descriptor_legality_and_affine_contract" in tests


def test_material_generator_remediation_implementation_plan_rejects_dp_mismatch(
    tmp_path: Path,
) -> None:
...
def test_material_generator_remediation_implementation_plan_rejects_missing_audit(
    tmp_path: Path,
) -> None:
...
def test_material_generator_remediation_implementation_plan_rejects_wrong_review_status(
    tmp_path: Path,
) -> None:
...
def test_material_generator_remediation_implementation_plan_rejects_wrong_next(
    tmp_path: Path,
) -> None:
...
def test_material_generator_remediation_implementation_plan_rejects_unapproved_review(
    tmp_path: Path,
) -> None:
    report = _build(
        tmp_path,
...
def test_material_generator_remediation_implementation_plan_rejects_missing_contract(
    tmp_path: Path,
) -> None:
    report = _build(
        tmp_path,
...
def test_material_generator_remediation_implementation_plan_rejects_blocked_action(
    tmp_path: Path,
) -> None:
...
def test_material_generator_remediation_implementation_plan_cli_writes_outputs(
    tmp_path: Path,
    monkeypatch,
) -> None:
