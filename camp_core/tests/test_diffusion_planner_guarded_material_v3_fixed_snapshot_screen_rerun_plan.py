...
MODULE = (
    ...
    "plan_diffusion_planner_guarded_material_v3_fixed_snapshot_screen_rerun"
)
...
def _build(
    ...
    *,
    ...
    return target.build_report(
        ...
        execution_root=Path(target.DEFAULT_EXECUTION_ROOT) / "unit_v3_rerun",
...
def test_guarded_material_v3_fixed_snapshot_screen_rerun_plan_ready(
    tmp_path: Path,
) -> None:
...
def test_guarded_material_v3_fixed_snapshot_screen_rerun_plan_rejects_dp_mismatch(
    tmp_path: Path,
) -> None:
...
def test_guarded_material_v3_fixed_snapshot_screen_rerun_plan_rejects_missing_authorization(
    tmp_path: Path,
) -> None:
    report = _build(
        tmp_path,
...
def test_guarded_material_v3_fixed_snapshot_screen_rerun_plan_rejects_blocked_action(
    tmp_path: Path,
) -> None:
...
def test_guarded_material_v3_fixed_snapshot_screen_rerun_runbook_is_guarded(
    tmp_path: Path,
) -> None:
    report = _build(tmp_path)
...
def test_guarded_material_v3_fixed_snapshot_screen_rerun_markdown_records_boundaries(
    tmp_path: Path,
) -> None:
    report = _build(tmp_path)
    markdown = target.render_markdown(report)

    assert "Guarded Material V3 Fixed-Snapshot Screen Rerun Plan" in markdown
...
def test_guarded_material_v3_fixed_snapshot_screen_rerun_plan_cli_writes_outputs(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    output_bash = tmp_path / "out" / "guarded_material_v3_runbook.sh"
    monkeypatch.setattr(
        "sys.argv",
        [
            "plan",
            ...
            str(root),
            "--execution_root",
            str(Path(target.DEFAULT_EXECUTION_ROOT) / "unit_v3_rerun"),
            "--camp_head",
            CAMP_COMMIT,
            "--camp_origin_main",
            CAMP_COMMIT,
