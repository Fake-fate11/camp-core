...
HEAD = "76c869a27ba4f7fc6698ddd7da933617f27d2e40"


def _load_module():
    spec = importlib.util.spec_from_file_location("v15_paired_evaluation_preflight", SCRIPT_PATH)
...
def test_v15_paired_evaluation_preflight_passes(tmp_path: Path) -> None:
    module = _load_module()
    ...
    assert decision["paired_evaluation_preflight_executed"] is True
    assert decision["paired_evaluation_executed"] is False
    assert decision["training_executed"] is False
    assert set(report["paired_evaluation_preflight"]["required_inputs"]["evaluation_splits"]) == {
        "calibration",
        "holdout",
    }
...
def test_v15_paired_evaluation_preflight_requires_enable(tmp_path: Path) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
...
def test_v15_paired_evaluation_preflight_rejects_wrong_eof(tmp_path: Path) -> None:
    module = _load_module()
...
def test_v15_paired_evaluation_preflight_rejects_source_eval_leak(tmp_path: Path) -> None:
    module = _load_module()
...
def test_v15_paired_evaluation_preflight_is_recorded_in_audit() -> None:
    module = _load_module()
...
def _write_fixture(
    ...
    *,
    ...
    source_md.write_text("# Paired Evaluation Plan Static Review\n", encoding="utf-8")
    for name, content in {
        "HEADS": f"CAMP_HEAD={HEAD}\nCAMP_ORIGIN_MAIN={HEAD}\nDP_HEAD={module.FIXED_DP_HEAD}\n",
        "COMMAND": "run paired evaluation plan static review\n",
        "stdout.txt": "{}\n",
        "stderr.txt": "",
        "run.exit": "0\n",
    }.items():
