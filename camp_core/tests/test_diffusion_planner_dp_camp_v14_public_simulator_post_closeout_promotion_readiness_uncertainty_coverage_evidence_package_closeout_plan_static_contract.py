...
SCRIPT_PATH = (
    ...
    / "scripts"
    / "integrations"
    / "review_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_readiness_uncertainty_coverage_evidence_package_closeout_plan_static_contract.py"
)
PLAN_SCRIPT_PATH = (
    ROOT
    / "scripts"
...
PLAN_TEST_PATH = (
    ...
    / "camp_core"
    / "tests"
    / "test_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_readiness_uncertainty_coverage_evidence_package_closeout_plan.py"
)
ARTIFACT_HEAD = "b" * 40
CURRENT_HEAD = "c" * 40
...
def _load_module():
    spec = importlib.util.spec_from_file_location(
        "v14_post_closeout_promotion_readiness_uncertainty_coverage_evidence_package_closeout_plan_static_contract",
        SCRIPT_PATH,
    )
...
def test_uncertainty_coverage_evidence_package_closeout_plan_static_contract_passes(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert decision["uncertainty_coverage_evidence_package_closeout_plan_static_review_passed"] is True
    assert decision["uncertainty_coverage_evidence_package_closeout_record_authorized"] is True
    assert decision["evidence_package_closed_by_this_gate"] is False
    assert decision["selector_promotion_authorized"] is False
    assert decision["deployment_authorized"] is False
    assert decision["safety_benefit_claim_authorized"] is False
    assert decision["camp_over_dp_top1_claim_authorized"] is False
    assert report["closeout_plan_summary"]["closeout_item_count"] == len(module.EXPECTED_CLOSEOUT_ITEMS)
    assert report["closeout_plan_summary"]["all_no_closeout_record"] is True
    assert report["closeout_plan_summary"]["all_no_execution"] is True
    assert report["closeout_plan_summary"]["all_no_claim"] is True
    assert report["closeout_plan_summary"]["all_no_promotion"] is True
    assert report["closeout_plan_summary"]["all_no_deployment"] is True
    assert (fixture["output_dir"] / module.REVIEW_JSON_NAME).is_file()
    assert (fixture["output_dir"] / module.REVIEW_MD_NAME).is_file()
    assert (fixture["output_dir"] / "SHA256SUMS").is_file()


def test_uncertainty_coverage_evidence_package_closeout_plan_static_contract_requires_enable(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert (
        ...
        == "explicit_uncertainty_coverage_evidence_package_closeout_plan_static_review_authorization_missing"
    )


def test_uncertainty_coverage_evidence_package_closeout_plan_static_contract_rejects_wrong_eof(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_uncertainty_coverage_evidence_package_closeout_plan_static_contract_rejects_source_leak(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_uncertainty_coverage_evidence_package_closeout_plan_static_contract_rejects_closeout_item_drift(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(
        tmp_path,
        module,
        closeout_plan_updates={"claim_boundary_closeout": {"authorizes_claim": True}},
    )

    report = module.build_report(**fixture)

    assert "source_closeout_plan_no_claim" in report["final_decision"]["failed_checks"]
    assert (
        report["final_decision"]["failure_class"]
        == "source_evidence_package_closeout_plan_static_review_contract_failure"
    )


def test_uncertainty_coverage_evidence_package_closeout_plan_static_contract_rejects_source_surface_gap(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
...
def test_uncertainty_coverage_evidence_package_closeout_plan_static_contract_cli_writes_outputs(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    exit_code = module.main(
        [
            "--closeout_plan_artifact_dir",
            str(fixture["closeout_plan_artifact_dir"]),
            "--closeout_plan_json",
            str(fixture["closeout_plan_json"]),
            "--closeout_plan_md",
            str(fixture["closeout_plan_md"]),
            "--closeout_plan_sha256s",
            str(fixture["closeout_plan_sha256s"]),
            "--plan_script_py",
...
def _write_fixture(
    ...
    *,
    ...
    closeout_plan_updates: dict[str, dict[str, Any]] | None = None,
) -> dict[str, Any]:
    artifact = tmp_path / "closeout_plan_artifact"
    ...
    plan_json = _write_json(
        ...
        _source_plan_payload(
            ...
            closeout_plan_updates=closeout_plan_updates,
        ),
    )
    plan_md = _write(plan_dir / module.SOURCE_PLAN_MD_NAME, "# Closeout Plan\n")
    plan_sha256s = _write_sha256sums(plan_dir / "SHA256SUMS", [plan_json, plan_md])
    command = _write(artifact / "COMMAND", "closeout plan command\n")
    heads = _write(
        artifact / "HEADS",
        "\n".join(
            [
    ...
    return {
        "closeout_plan_artifact_dir": artifact,
        "closeout_plan_json": plan_json,
        "closeout_plan_md": plan_md,
        "closeout_plan_sha256s": plan_sha256s,
...
def _source_plan_payload(
    ...
    *,
    ...
    decision = {
        ...
        "recommendation": "static_review_uncertainty_coverage_evidence_package_closeout_plan_only",
        "immediate_action": "evidence_package_closeout_plan_static_review_only",
    }
    ...
    return {
        ...
        "source_static_review_summary": {
            "schema_version": "source_static_review",
            "status": "passed",
            "passed": True,
        ...
        "closeout_plan": _closeout_plan(module, updates=closeout_plan_updates),
