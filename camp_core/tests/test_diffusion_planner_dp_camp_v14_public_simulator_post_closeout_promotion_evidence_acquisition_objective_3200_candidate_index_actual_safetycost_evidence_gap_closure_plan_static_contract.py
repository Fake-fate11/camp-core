...
SCRIPT_PATH = (
    ...
    / "scripts"
    / "integrations"
    / "review_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_evidence_acquisition_objective_3200_candidate_index_actual_safetycost_evidence_gap_closure_plan_static_contract.py"
)
PLAN_SCRIPT_PATH = (
    ROOT
    / "scripts"
...
PLAN_TEST_PATH = (
    ...
    / "camp_core"
    / "tests"
    / "test_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_evidence_acquisition_objective_3200_candidate_index_actual_safetycost_evidence_gap_closure_plan.py"
)
SOURCE_HEAD = "e" * 40
CURRENT_HEAD = "f" * 40
...
def test_candidate_index_actual_safetycost_evidence_gap_closure_plan_static_review_passes(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert (
        decision[
            "objective_3200_candidate_index_actual_safetycost_evidence_gap_closure_plan_static_review_passed"
        ]
        is True
    )
    assert (
        decision[
            "objective_3200_candidate_index_actual_safetycost_delta_materialization_preflight_plan_authorized"
        ]
        is True
    )
    ...
    assert summary["plan_check_count"] == 75
    assert summary["required_input_count"] == 8
...
def test_candidate_index_actual_safetycost_evidence_gap_closure_plan_static_review_requires_enable(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert (
        ...
        == "explicit_candidate_index_actual_safetycost_plan_static_review_authorization_missing"
    )


def test_candidate_index_actual_safetycost_evidence_gap_closure_plan_static_review_rejects_wrong_eof(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_candidate_index_actual_safetycost_evidence_gap_closure_plan_static_review_rejects_source_claim_leak(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_candidate_index_actual_safetycost_evidence_gap_closure_plan_static_review_rejects_hash_drift(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    fixture["candidate_index_actual_safetycost_plan_md"].write_text(
        "# drift\n",
        encoding="utf-8",
    )
    ...
    *,
    ...
    doc_text = "\n".join(
        [
            ...
            "objective_3200_candidate_index_actual_safetycost_evidence_gap_closure_plan_ready=True",
            "objective_3200_candidate_index_actual_safetycost_evidence_gap_closure_plan_static_review_authorized=True",
    ...
    return {
        "candidate_index_actual_safetycost_plan_artifact_dir": artifact,
        "candidate_index_actual_safetycost_plan_json": plan_json,
        "candidate_index_actual_safetycost_plan_md": plan_md,
        "candidate_index_actual_safetycost_plan_sha256s": plan_sha,
        "candidate_index_actual_safetycost_plan_script_py": PLAN_SCRIPT_PATH,
        "candidate_index_actual_safetycost_plan_test_py": PLAN_TEST_PATH,
    ...
    *,
    ...
    return {
        ...
        "closure_plan": [
            ...
            for name in module.PLAN_MODULE.EXPECTED_PLAN_ITEMS
        ],
        ...
        "plan_checks": [{"name": f"check_{idx}", "passed": True} for idx in range(75)],
