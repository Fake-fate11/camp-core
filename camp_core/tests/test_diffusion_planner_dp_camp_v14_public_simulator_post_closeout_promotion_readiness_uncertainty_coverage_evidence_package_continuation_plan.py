...
def test_uncertainty_coverage_evidence_package_continuation_plan_passes(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert decision["post_closeout_continuation_plan_ready"] is True
    assert decision["post_closeout_continuation_plan_static_review_authorized"] is True
    assert decision["selector_promotion_authorized"] is False
    assert decision["deployment_authorized"] is False
    assert decision["safety_benefit_claim_authorized"] is False
    assert decision["camp_over_dp_top1_claim_authorized"] is False
    assert [item["item_name"] for item in report["continuation_plan"]] == list(
        module.EXPECTED_CONTINUATION_ITEMS
    )
    assert {item["authorizes_promotion"] for item in report["continuation_plan"]} == {False}
    assert (fixture["output_dir"] / module.PLAN_JSON_NAME).is_file()
    assert (fixture["output_dir"] / module.PLAN_MD_NAME).is_file()
    assert (fixture["output_dir"] / "SHA256SUMS").is_file()


def test_uncertainty_coverage_evidence_package_continuation_plan_requires_enable(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert (
        ...
        == "explicit_post_closeout_continuation_authorization_missing"
    )


def test_uncertainty_coverage_evidence_package_continuation_plan_rejects_wrong_eof(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert "audit_latest_eof_is_new_authorization_boundary" in report["final_decision"]["failed_checks"]
    assert "status_doc_latest_eof_is_new_authorization_boundary" in report["final_decision"]["failed_checks"]
    assert report["final_decision"]["failure_class"] == "v14_eof_contract_mismatch"


def test_uncertainty_coverage_evidence_package_continuation_plan_rejects_source_promotion_leak(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_uncertainty_coverage_evidence_package_continuation_plan_rejects_source_hash_drift(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
