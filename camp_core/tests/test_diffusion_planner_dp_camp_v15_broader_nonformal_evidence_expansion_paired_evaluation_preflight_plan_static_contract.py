...
HEAD = "b1cf922cc7d1c370e965528ca7608c2a79c2b557"


def _load_module():
    spec = importlib.util.spec_from_file_location("v15_paired_evaluation_preflight_plan_static_review", SCRIPT_PATH)
...
def test_v15_paired_evaluation_preflight_plan_static_review_passes(tmp_path: Path) -> None:
    module = _load_module()
    ...
    assert decision["reviewed_paired_evaluation_preflight_plan"] is True
...
def test_v15_paired_evaluation_preflight_plan_static_review_requires_enable(tmp_path: Path) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
...
def test_v15_paired_evaluation_preflight_plan_static_review_rejects_wrong_eof(tmp_path: Path) -> None:
    module = _load_module()
...
def test_v15_paired_evaluation_preflight_plan_static_review_rejects_paired_eval_leak(tmp_path: Path) -> None:
    module = _load_module()
...
def test_v15_paired_evaluation_preflight_plan_static_review_rejects_train_split_eval(tmp_path: Path) -> None:
    module = _load_module()
...
def test_v15_paired_evaluation_preflight_plan_static_review_is_recorded_in_audit() -> None:
    module = _load_module()
...
def _write_fixture(
    ...
    *,
    ...
    source_md.write_text("# Paired Evaluation Preflight Plan\n", encoding="utf-8")
    for name, content in {
        "HEADS": f"CAMP_HEAD={HEAD}\nCAMP_ORIGIN_MAIN={HEAD}\nDP_HEAD={module.FIXED_DP_HEAD}\n",
        "COMMAND": "run paired evaluation preflight plan\n",
        "stdout.txt": "{}\n",
        "stderr.txt": "",
        "run.exit": "0\n",
    }.items():
...
def _source_plan_payload(
    ...
    *,
    ...
    plan = module.PLAN_MODULE._paired_evaluation_preflight_plan(HEAD, module.FIXED_DP_HEAD)
    if plan_updates:
