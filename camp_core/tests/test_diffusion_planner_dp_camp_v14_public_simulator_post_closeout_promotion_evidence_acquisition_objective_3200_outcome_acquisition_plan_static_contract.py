...
SCRIPT_PATH = (
    ...
    / "scripts"
    / "integrations"
    / "review_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_evidence_acquisition_objective_3200_outcome_acquisition_plan_static_contract.py"
)
PLAN_SCRIPT_PATH = (
    ROOT
    / "scripts"
...
PLAN_TEST_PATH = (
    ...
    / "camp_core"
    / "tests"
    / "test_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_evidence_acquisition_objective_3200_outcome_acquisition_plan.py"
)
SOURCE_HEAD = "a" * 40
CURRENT_HEAD = "b" * 40
...
def _load_module():
    spec = importlib.util.spec_from_file_location(
        "v14_post_closeout_promotion_evidence_acquisition_objective_3200_outcome_acquisition_plan_static_review",
        SCRIPT_PATH,
    )
...
def test_objective_3200_outcome_acquisition_plan_static_review_passes(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert decision["objective_3200_outcome_acquisition_plan_static_review_passed"] is True
    assert decision["objective_3200_outcome_acquisition_preflight_authorized"] is True
    assert decision["direct_replay_execution_authorized"] is False
    assert decision["selector_promotion_authorized"] is False
    assert decision["deployment_authorized"] is False
    assert decision["safety_benefit_claim_authorized"] is False
    assert decision["camp_over_dp_top1_claim_authorized"] is False
    assert report["source_plan_summary"]["plan_step_count"] == 6
...
def test_objective_3200_outcome_acquisition_plan_static_review_requires_enable(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert (
        ...
        == "explicit_objective_3200_outcome_acquisition_plan_static_review_authorization_missing"
    )


def test_objective_3200_outcome_acquisition_plan_static_review_rejects_wrong_eof(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_objective_3200_outcome_acquisition_plan_static_review_rejects_claim_leak(
    tmp_path: Path,
) -> None:
    module = _load_module()
    ...
    assert "source_plan_camp_over_dp_top1_claim_authorized" in report[
        "final_decision"
    ]["failed_checks"]
    assert report["final_decision"]["camp_over_dp_top1_claim_authorized"] is False


def test_objective_3200_outcome_acquisition_plan_static_review_rejects_direct_replay(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(
        tmp_path,
        module,
        source_decision_updates={"direct_replay_execution_authorized": True},
    )

    report = module.build_report(**fixture)

    assert "source_plan_direct_replay_execution_authorized" in report[
        "final_decision"
    ]["failed_checks"]
    assert report["final_decision"]["direct_replay_execution_authorized"] is False


def test_objective_3200_outcome_acquisition_plan_static_review_rejects_hash_drift(
    tmp_path: Path,
) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    fixture["acquisition_plan_md"].write_text("# drift\n", encoding="utf-8")
...
def _write_fixture(
    ...
    *,
    ...
    artifact = tmp_path / "source_acquisition_plan"
    ...
    return {
        "acquisition_plan_artifact_dir": artifact,
        "acquisition_plan_json": plan_json,
        "acquisition_plan_md": plan_md,
        "acquisition_plan_sha256s": plan_sha,
    ...
    *,
    ...
    return {
        ...
        "objective_gap_summary": {
            ...
            "missing_candidate_closed_loop_outcome_records": module.PLAN_MODULE.OBJECTIVE_REQUIRED_RECORDS,
            "existing_artifacts_satisfy_objective": False,
            "requires_acquisition_plan": True,
        },
        "acquisition_plan": [{"step": step} for step in module.EXPECTED_PLAN_STEPS],
        "no_go_register": list(module.EXPECTED_NO_GO),
        "strict_pairing_contract": {
            "required_rows": module.PLAN_MODULE.OBJECTIVE_REQUIRED_RECORDS,
