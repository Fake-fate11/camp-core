...
SCRIPT_PATH = (
    ...
    / "scripts"
    / "integrations"
    / "decide_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_evidence_acquisition_objective_3200_candidate_index_actual_safetycost_deployment.py"
)
SOURCE_HEAD = "2" * 40
CURRENT_HEAD = "3" * 40
...
def test_candidate_index_actual_safetycost_deployment_decision_passes(tmp_path: Path) -> None:
    module = _load_module()
    ...
    assert decision["objective_3200_candidate_index_actual_safetycost_deployment_decision_passed"] is True
    assert (
        decision["objective_3200_candidate_index_actual_safetycost_online_selector_activation_decision_plan_authorized"]
        is True
    )
    ...
    assert report["deployment_record"]["deployment_authorized"] is True
...
def test_candidate_index_actual_safetycost_deployment_decision_requires_enable(tmp_path: Path) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert "deployment_decision_enabled" in report["final_decision"]["failed_checks"]
    assert (
        report["final_decision"]["failure_class"]
        == "explicit_candidate_index_actual_safetycost_deployment_decision_authorization_missing"
    )


def test_candidate_index_actual_safetycost_deployment_decision_rejects_wrong_eof(tmp_path: Path) -> None:
    module = _load_module()
...
def test_candidate_index_actual_safetycost_deployment_decision_rejects_source_deployment_leak(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_candidate_index_actual_safetycost_deployment_decision_rejects_hash_drift(tmp_path: Path) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    *,
    ...
    doc_text = "\n".join(
        [
            ...
            "objective_3200_candidate_index_actual_safetycost_deployment_decision_authorized=True",
