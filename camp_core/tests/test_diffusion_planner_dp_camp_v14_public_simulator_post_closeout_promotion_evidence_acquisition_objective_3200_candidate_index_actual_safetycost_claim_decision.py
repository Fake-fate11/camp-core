...
SCRIPT_PATH = (
    ...
    / "scripts"
    / "integrations"
    / "decide_diffusion_planner_dp_camp_v14_public_simulator_post_closeout_promotion_evidence_acquisition_objective_3200_candidate_index_actual_safetycost_claim.py"
)
SOURCE_HEAD = "5" * 40
CURRENT_HEAD = "6" * 40
...
def test_candidate_index_actual_safetycost_claim_decision_passes(tmp_path: Path) -> None:
    module = _load_module()
    ...
    assert decision["objective_3200_candidate_index_actual_safetycost_claim_decision_passed"] is True
    ...
    assert report["claim_record"]["safety_benefit_claim_authorized"] is True
...
def test_candidate_index_actual_safetycost_claim_decision_requires_enable(tmp_path: Path) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    assert "claim_decision_enabled" in report["final_decision"]["failed_checks"]
    assert (
        report["final_decision"]["failure_class"]
        == "explicit_candidate_index_actual_safetycost_claim_decision_authorization_missing"
    )


def test_candidate_index_actual_safetycost_claim_decision_rejects_wrong_eof(tmp_path: Path) -> None:
    module = _load_module()
...
def test_candidate_index_actual_safetycost_claim_decision_rejects_source_claim_leak(
    tmp_path: Path,
) -> None:
    module = _load_module()
...
def test_candidate_index_actual_safetycost_claim_decision_rejects_hash_drift(tmp_path: Path) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
    ...
    *,
    ...
    doc_text = "\n".join(
        [
            ...
            "objective_3200_candidate_index_actual_safetycost_claim_decision_authorized=True",
