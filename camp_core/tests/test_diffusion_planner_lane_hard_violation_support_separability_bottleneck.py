...
from scripts.integrations.analyze_diffusion_planner_lane_hard_violation_support_separability_bottleneck import (
    READY_STATUS,
...
ATOM_DESCRIPTOR = f"atom_{LANE_HARD_VIOLATION_SUPPORT_ATOM_NAMES[0]}"
...
def _separability_report(status: str = "lane_hard_violation_support_descriptor_separability_rejected") -> dict:
    strict = {
...
def test_lane_hard_bottleneck_diagnoses_overlap_tradeoff() -> None:
...
def test_lane_hard_bottleneck_blocks_when_source_not_rejected() -> None:
    ...
    assert report["final_decision"]["authorized_next_work"] == (
        "fix_lane_hard_separability_source_before_bottleneck_diagnosis"
    )


def test_lane_hard_bottleneck_rejects_formal_seed_when_forbidden() -> None:
...
def test_lane_hard_bottleneck_descriptor_overlap_is_outcome_independent() -> None:
...
def test_lane_hard_bottleneck_cli_reads_selection_log(tmp_path: Path) -> None:
