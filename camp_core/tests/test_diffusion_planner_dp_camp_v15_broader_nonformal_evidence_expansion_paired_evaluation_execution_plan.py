...
HEAD = "06729792828889011c3f55893233ef9732741bb3"


def _load_module():
    spec = importlib.util.spec_from_file_location("v15_paired_evaluation_execution_plan", SCRIPT_PATH)
...
def test_v15_paired_evaluation_execution_plan_passes(tmp_path: Path) -> None:
    module = _load_module()
    ...
    assert plan["paired_protocol"]["baseline"] == "dp_top1"
    assert plan["paired_protocol"]["camp_selection_policy"] == "select_from_fixed_dp_candidate_tensor"
    assert (fixture["output_dir"] / module.PLAN_JSON_NAME).is_file()
    assert (fixture["output_dir"] / module.PLAN_MD_NAME).is_file()


def test_v15_paired_evaluation_execution_plan_requires_enable(tmp_path: Path) -> None:
    module = _load_module()
    fixture = _write_fixture(tmp_path, module)
...
def test_v15_paired_evaluation_execution_plan_rejects_wrong_eof(tmp_path: Path) -> None:
    module = _load_module()
...
def test_v15_paired_evaluation_execution_plan_rejects_eval_leak(tmp_path: Path) -> None:
    module = _load_module()
...
def test_v15_paired_evaluation_execution_plan_rejects_failed_source_review(tmp_path: Path) -> None:
    module = _load_module()
...
def test_v15_paired_evaluation_execution_plan_is_recorded_in_audit() -> None:
    module = _load_module()
...
def _write_fixture(
    ...
    *,
    ...
    source_md.write_text("# Paired Evaluation Preflight Static Review\n", encoding="utf-8")
    for name, content in {
        "HEADS": f"CAMP_HEAD={HEAD}\nCAMP_ORIGIN_MAIN={HEAD}\nDP_HEAD={module.FIXED_DP_HEAD}\n",
        "COMMAND": "run paired evaluation preflight static review\n",
        "stdout.txt": "{}\n",
        "stderr.txt": "",
        "run.exit": "0\n",
    }.items():
