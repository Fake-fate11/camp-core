...
MODULE = (
    ...
    "rerun_failure_attribution_remediation_fixed_snapshot_screen_rerun"
)
...
def _post_review_payload(
    *,
    ...
    decision: dict[str, object] = {
        ...
        "remediation_fixed_snapshot_screen_rerun_plan_authorized": plan_authorized,
    }
...
def _build(
    ...
    *,
    ...
    return target.build_report(
        ...
        execution_root=Path(target.DEFAULT_EXECUTION_ROOT) / "unit_v2_rerun",
...
def test_remediation_fixed_snapshot_screen_rerun_plan_ready(tmp_path: Path) -> None:
...
def test_remediation_fixed_snapshot_screen_rerun_plan_rejects_dp_mismatch(
    tmp_path: Path,
) -> None:
...
def test_remediation_fixed_snapshot_screen_rerun_plan_rejects_missing_authorization(
    tmp_path: Path,
) -> None:
    report = _build(
        tmp_path,
...
def test_remediation_fixed_snapshot_screen_rerun_plan_rejects_blocked_action(
    tmp_path: Path,
) -> None:
...
def test_remediation_fixed_snapshot_screen_rerun_runbook_is_guarded_and_v2_scoped(
    tmp_path: Path,
) -> None:
    report = _build(tmp_path)
    ...
    assert "--prefix_step 3" in runbook
    assert "--bridge_step 10" in runbook
    assert (
...
def test_remediation_fixed_snapshot_screen_rerun_markdown_records_boundaries(
    tmp_path: Path,
) -> None:
    report = _build(tmp_path)
    markdown = target.render_markdown(report)

    assert "Remediation Fixed-Snapshot Screen Rerun Plan" in markdown
...
def test_remediation_fixed_snapshot_screen_rerun_plan_cli_writes_outputs(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    output_bash = tmp_path / "out" / "material_generator_v2_runbook.sh"
    monkeypatch.setattr(
        "sys.argv",
        [
            "plan",
            ...
            str(root),
            "--execution_root",
            str(Path(target.DEFAULT_EXECUTION_ROOT) / "unit_v2_rerun"),
            "--camp_head",
            CAMP_COMMIT,
            "--camp_origin_main",
            CAMP_COMMIT,
