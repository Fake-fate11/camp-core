...
try:
    from trajdata import AgentBatch, AgentType, UnifiedDataset  # type: ignore
...
class NuscenesDatasetConfig:
    ...
    history_sec: Tuple[Optional[float], Optional[float]] = (0.1, 2.0)
    future_sec: Tuple[Optional[float], Optional[float]] = (6.0, 6.0)
    agent_interaction_distances: Optional[Dict[Any, float]] = None
...
def default_nuscenes_attention_radius() -> Dict[Any, float]:
...
def _radius_lookup(radius_map: Dict[Any, float], neighbor_type: Any, target_type: Any) -> float:
    keys = [
        (neighbor_type, target_type),
        (getattr(neighbor_type, "value", neighbor_type), target_type),
        (neighbor_type, getattr(target_type, "value", target_type)),
        (getattr(neighbor_type, "value", neighbor_type), getattr(target_type, "value", target_type)),
    ]
    for key in keys:
        try:
            return float(radius_map[key])
        except Exception:
            pass
    try:
        return float(radius_map.default_factory())  # type: ignore[attr-defined]
    except Exception:
        return 20.0


def _pad_or_trim_xy(xy: np.ndarray, horizon: int) -> Optional[np.ndarray]:
    ...
    if xy.shape[0] == 0:
        return None
    if xy.shape[0] >= horizon:
        return xy[:horizon]
    padding = np.tile(xy[-1], (horizon - xy.shape[0], 1))
    return np.concatenate([xy, padding], axis=0)


def _scene_cache_dynamic_obstacles(
    batch: AgentBatch,
    idx: int,
    map_api: Optional[Any],
    horizon: int,
) -> Dict[int, np.ndarray]:
    if map_api is None or not hasattr(map_api, "_data_index"):
        return {}
    if not hasattr(batch, "data_idx"):
        return {}
    ...
    try:
        ...
        from trajdata.utils.state_utils import convert_to_frame_state

        data_idx_val = batch.data_idx[idx]
        if hasattr(data_idx_val, "item"):
            data_idx_val = data_idx_val.item()
        scene_path, agent_id, ts = map_api._data_index[int(data_idx_val)]

        scene = EnvCache.load(scene_path)
        scene_utils.enforce_desired_dt(scene, getattr(map_api, "desired_dt", None))
        scene_cache = map_api.cache_class(
            map_api.cache_path,
            scene,
            getattr(map_api, "augmentations", None),
        )
        scene_cache.set_obs_format(getattr(map_api, "obs_format", "x,y,xd,yd,xdd,ydd,s,c"))
        ...
        scene_time_agent = SceneTimeAgent.from_cache(
            ...
            agent_id,
            scene_cache,
            only_types=getattr(map_api, "only_types", None),
            no_types=getattr(map_api, "no_types", None),
            incl_robot_future=getattr(map_api, "incl_robot_future", False),
        )
        agent_info = scene_time_agent.agent
        if agent_info is None:
            return {}

        if getattr(map_api, "standardize_data", True):
            raw_state = scene_cache.get_raw_state(agent_info.name, ts)
            obs_frame = convert_to_frame_state(
                raw_state,
                stationary=not getattr(map_api, "standardize_derivatives", False),
                grounded=True,
            )
            scene_cache.set_obs_frame(obs_frame)

        distances = scene_time_agent.get_agent_distances_to(agent_info)
        radius_map = getattr(map_api, "agent_interaction_distances", None)
        if radius_map is None:
            radius_map = default_nuscenes_attention_radius()

        neighbors = []
        for agent, dist in zip(scene_time_agent.agents, distances):
            if agent.name == agent_info.name or not np.isfinite(dist):
                continue
            radius = _radius_lookup(radius_map, agent.type, agent_info.type)
            if float(dist) <= radius:
                neighbors.append((float(dist), agent))

        neighbors.sort(key=lambda item: item[0])
        max_neighbor_num = getattr(map_api, "max_neighbor_num", None)
        if max_neighbor_num is not None:
            neighbors = neighbors[: int(max_neighbor_num)]

        dynamic_obstacles: Dict[int, np.ndarray] = {}
        future_sec = getattr(map_api, "future_sec", (None, None))
        for n_i, (_, agent) in enumerate(neighbors):
            obs_traj = None
            try:
                fut, _ = scene_cache.get_agent_future(agent, ts, future_sec)
                obs_traj = _pad_or_trim_xy(np.asarray(fut), horizon)
            except Exception:
                obs_traj = None

            if obs_traj is None:
                try:
                    hist, _ = scene_cache.get_agent_history(
                        agent,
                        ts,
                        getattr(map_api, "history_sec", (None, None)),
                    )
                    hist_xy = _pad_or_trim_xy(np.asarray(hist), len(np.asarray(hist)))
                    if hist_xy is None:
                        continue
                    curr = hist_xy[-1]
                    if hist_xy.shape[0] > 1:
                        vel = (hist_xy[-1] - hist_xy[-2]) / float(getattr(scene, "dt", 0.5))
                    else:
                        vel = np.zeros(2)
                    dt = float(getattr(scene, "dt", 0.5))
                    obs_traj = np.stack([curr + vel * t * dt for t in range(horizon)])
                except Exception:
                    continue
    ...
    def __init__(self, cfg: NuscenesDatasetConfig) -> None:
        ...
        unified_kwargs: Dict[str, Any] = dict(
            ...
            agent_interaction_distances=(
                cfg.agent_interaction_distances
                if cfg.agent_interaction_distances is not None
                else default_nuscenes_attention_radius()
            ),
            max_neighbor_num=cfg.max_neighbor_num,
            rebuild_cache=cfg.rebuild_cache,
            rebuild_maps=cfg.rebuild_maps,
            incl_vector_map=cfg.use_vector_map,
...
def extract_driver_context(
    ...
    if vec_map is None and map_api is not None:
        try:
            if batch.map_names is not None:
                ...
                elif hasattr(map_api, "_map_api") and map_api._map_api is not None:
                    vec_map = map_api._map_api.get_map(map_name)
    ...
    if hasattr(batch, "neigh_hist") and batch.neigh_hist is not None:
        ...
        if num_n > 0:
            ...
            agent_neighs_fut_len = None
            if hasattr(batch, "neigh_fut_len") and batch.neigh_fut_len is not None:
                agent_neighs_fut_len = batch.neigh_fut_len[idx]
            
            for n_i in range(num_n):
                # Neighbor n_i
                n_hist = agent_neighs[n_i].cpu().numpy()
                hist_xy = n_hist[:, :2]
                valid_hist_xy = hist_xy[~np.isnan(hist_xy).any(axis=1)]
                if valid_hist_xy.shape[0] == 0:
                    continue
                curr_n_pos = valid_hist_xy[-1]
                ...
                if agent_neighs_fut is not None:
                     ...
                     valid_len = n_fut.shape[0]
                     if agent_neighs_fut_len is not None:
                         valid_len = min(valid_len, int(agent_neighs_fut_len[n_i]))

                     if valid_len > 0:
                         curr = n_fut[: min(valid_len, horizon), :2]
                         curr = curr[~np.isnan(curr).any(axis=1)]
                         if curr.shape[0] > 0:
                             ...
                             if pad_len > 0:
                                 ...
                                 obs_traj = curr[:horizon]
                         
                # Method 2: Fallback to Constant Velocity
                if obs_traj is None:
                    n_vel = np.zeros(2)
                    if valid_hist_xy.shape[0] > 1:
                        diff = valid_hist_xy[-1] - valid_hist_xy[-2]
                        # dt is batch.dt
                        n_vel = diff / dt
    ...
    if not dynamic_obstacles:
        dynamic_obstacles = _scene_cache_dynamic_obstacles(batch, idx, map_api, horizon)
