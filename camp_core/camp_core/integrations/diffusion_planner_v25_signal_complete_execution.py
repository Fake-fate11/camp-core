...
PAIRED_CALIBRATION_CONFIG_SCHEMA_VERSION = (
...
_PAIRED_CALIBRATION_ARM_CONTRACT = {
    "candidate0_operational_default": {
        "native_arm": "dp",
        "fixed_k8_candidate0": True,
        "selector_role": "v25_calibration_candidate0_assets_bound_not_consumed",
        "selector_model": "DP-operational-default-candidate0",
        "static_weights_consumed": False,
        "scene_weight_provider_required": False,
    },
    "camp_static14d": {
        "native_arm": "camp",
        "fixed_k8_candidate0": False,
        "selector_role": "v25_calibration_static14d_sealed_runtime",
        "selector_model": "CAMP-Static14D",
        "static_weights_consumed": True,
        "scene_weight_provider_required": False,
    },
    "camp_scene14d_no_v2i": {
        "native_arm": "camp",
        "fixed_k8_candidate0": False,
        "selector_role": "v25_calibration_scene14d_no_v2i_sealed_runtime",
        "selector_model": "CAMP-Scene14D",
        "static_weights_consumed": False,
        "scene_weight_provider_required": True,
    },
...
def build_paired_calibration_arm_config(
    *,
    probe_template: Mapping[str, Any],
    prepared_runtime: Mapping[str, Any],
    execution_unit: Mapping[str, Any],
    ...
    """Build one preregistered three-arm calibration config.

    Calibration is outcome-bearing but is neither Fresh nor model training.  All
    model/scaler assets are already sealed, and every arm receives an independent
    reset under the same pair authority.
    """

    if type(probe_template) is not dict:
        raise ValueError("paired calibration probe template must be a mapping")
    if plan_arm not in _PAIRED_CALIBRATION_ARM_CONTRACT:
        raise ValueError("paired calibration plan arm drifted")
    prepared = copy.deepcopy(prepared_runtime)
    case = prepared.get("case")
    if type(case) is not dict or prepared.get("schema_version") != RUNTIME_SCHEMA_VERSION:
        raise ValueError("paired calibration runtime authority drifted")
    build_signal_complete_scene_adapter(prepared)
    unit = _fresh_execution_unit(execution_unit, prepared)
    arm_index = unit["ordered_arms"].index(plan_arm)
    route = _asset(route_asset, "route_asset")
    if route.get("name") != case.get("route_identity_sha256"):
        raise ValueError("paired calibration route asset identity drifted")
    authority = _calibration_runtime_selector_authority(runtime_selector_authority)
    contract = _PAIRED_CALIBRATION_ARM_CONTRACT[plan_arm]

    fixed_dp = copy.deepcopy(_mapping(probe_template, "fixed_dp"))
    fixed_dp["repo"] = str(dp_repo.resolve())
    if fixed_dp.get("head") != FIXED_DP_HEAD:
        raise ValueError("paired calibration fixed DP HEAD drifted")
    selector = copy.deepcopy(_mapping(probe_template, "selector"))
    selector.update(
        {
            "atom_scales": dict(authority["atom_scales"]),
    ...
    config = {
        "schema_version": PAIRED_CALIBRATION_CONFIG_SCHEMA_VERSION,
        "fixed_dp": fixed_dp,
        "selector": selector,
        "map": {
            "path": case["source_map_path"],
        ...
        "protocol": {
            ...
            "camp_method_outcomes_authorized": True,
    ...
    return validate_paired_calibration_arm_config(config)


def validate_paired_calibration_arm_config(
    ...
    if type(value) is not dict or set(value) != fields:
        raise ValueError("paired calibration arm config field set drifted")
    result = copy.deepcopy(value)
    if result.get("schema_version") != PAIRED_CALIBRATION_CONFIG_SCHEMA_VERSION:
        raise ValueError("paired calibration arm config schema drifted")
    fixed = _mapping(result, "fixed_dp")
    if fixed.get("head") != FIXED_DP_HEAD or type(fixed.get("repo")) is not str:
        raise ValueError("paired calibration fixed DP authority drifted")
    _asset(_mapping(fixed, "checkpoint"), "checkpoint")
    _asset(_mapping(fixed, "args_json"), "args_json")
    if type(fixed.get("native_source_sha256")) is not dict:
        raise ValueError("paired calibration fixed DP source authority is missing")

    runtime = _mapping(result, "signal_complete_runtime")
    case = build_signal_complete_scene_adapter(runtime).case
    map_asset = _asset(_mapping(result, "map"), "map")
    routes = result.get("routes")
    if type(routes) is not list or len(routes) != 1:
        raise ValueError("paired calibration requires one route asset")
    route = _asset(routes[0], "route")
    _require_sha(route.get("name"), "route.name")
    if (
        map_asset["path"] != case["source_map_path"]
        ...
        raise ValueError("paired calibration runtime assets drifted")

    authority = _calibration_runtime_selector_authority(
        _mapping(result, "runtime_selector_authority")
    )
    protocol = _mapping(result, "protocol")
    plan_arm = protocol.get("calibration_plan_arm")
    if plan_arm not in _PAIRED_CALIBRATION_ARM_CONTRACT:
        raise ValueError("paired calibration protocol arm drifted")
    contract = _PAIRED_CALIBRATION_ARM_CONTRACT[plan_arm]
    selector = _mapping(result, "selector")
    if (
        not _strict_equal(selector.get("atom_scales"), authority["atom_scales"])
        ...
        raise ValueError("paired calibration selector contract drifted")

    plan = _mapping(result, "signal_complete_plan_authority")
    if set(plan) != {
        "unit_ordinal",
        "unit_sha256",
        "scenario_identity_sha256",
        ...
        raise ValueError("paired calibration plan authority fields drifted")
    if (
        type(plan["unit_ordinal"]) is not int
        or plan["unit_ordinal"] < 0
        or plan["scenario_identity_sha256"] != runtime["scenario_identity_sha256"]
        or plan["route_identity_sha256"] != case["route_identity_sha256"]
        or plan["semantic_parameter_block_sha256"] != case["parameter_block_id"]
        or type(plan["ordered_arms"]) is not list
        or tuple(plan["ordered_arms"]) not in _FRESH_ARM_ROTATIONS
        or plan["plan_arm"] != plan_arm
        or plan["arm_order_index"] != plan["ordered_arms"].index(plan_arm)
    ):
        raise ValueError("paired calibration plan authority drifted")
    for name in ("unit_sha256", "scenario_identity_sha256", "semantic_parameter_block_sha256"):
        _require_sha(plan[name], name)
    seed = plan["seed"]
    if type(seed) is not int or seed not in {25301, 25302}:
        raise ValueError("paired calibration seed authority drifted")
    if plan["unit_sha256"] != _canonical_sha(
        {
            "scenario_identity_sha256": plan["scenario_identity_sha256"],
            "seed": seed,
            "ordered_arms": plan["ordered_arms"],
        }
    ):
        raise ValueError("paired calibration unit SHA drifted")
    expected_seeds = {
        "scenario": seed,
        "candidate": seed,
        "bootstrap": seed,
    ...
    if not _strict_equal(_mapping(result, "seeds"), expected_seeds):
        raise ValueError("paired calibration seed namespace drifted")
    spawn = _mapping(result, "spawn_config")
    critical_spawn = {
        "seed": seed,
        "max_steps": 64,
    ...
    if any(
        ...
        raise ValueError("paired calibration spawn contract drifted")
    expected_protocol = {
        "arm_order": [contract["native_arm"]],
        "calibration_plan_arm": plan_arm,
        "arm_order_index": plan["arm_order_index"],
        "route_order": [case["route_identity_sha256"]],
        "calibration_steps": 64,
        "sample_every_ticks": 1,
        "candidate_k": 8,
    ...
    if not _strict_equal(protocol, expected_protocol):
        raise ValueError("paired calibration protocol drifted")
    return result
...
def _calibration_runtime_selector_authority(
    value: Mapping[str, Any],
) -> dict[str, Any]:
    """Validate the already-trained selector assets used by calibration.

    Unlike the Fresh authority, this binding intentionally has no opening,
    qualification, or scenario-manifest roots.  Calibration consumes only the
    accepted training/review pair and its immutable runtime parameters while
    ...
    if type(value) is not dict or set(value) != fields:
        raise ValueError("paired calibration runtime selector authority fields drifted")
    result = copy.deepcopy(value)
    for name in ("training_artifact", "training_review_artifact"):
        binding = result[name]
        if type(binding) is not dict or set(binding) != {"path", "root_sha256"}:
            raise ValueError(f"paired calibration {name} binding drifted")
        if type(binding["path"]) is not str or not binding["path"]:
            raise ValueError(f"paired calibration {name} path is invalid")
        _require_sha(binding["root_sha256"], f"{name}.root_sha256")
    for name in (
        "model_registry_sha256",
        "training_scale_sha256",
