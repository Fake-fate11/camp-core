"""Neutral CAMP current-state context arithmetic.

This is the canonical implementation for the frozen 26D raw context,
availability-aware complement lift, and affine simplex weights.  Protocol
literals are intentionally retained for compatibility with existing CAMP
weights; V26 callers provide source capabilities explicitly instead of
entering a V25 policy path.
"""
...
# Historical literal retained because frozen reference assets carry it.
...
_SPEED_CAPABILITIES = frozenset({"available", "typed_missing"})
_SIGNAL_CAPABILITIES = frozenset(
    {"route_only", "available", "not_applicable", "unavailable"}
)


@dataclass(frozen=True)
class CAMPContextSourceCapabilities:
    """Authoritative availability states for optional current-state inputs."""

    speed_limit_status: str = "available"
    signal_source_state: str = "route_only"

    def __post_init__(self) -> None:
        if self.speed_limit_status not in _SPEED_CAPABILITIES:
            raise ValueError("CAMP speed-limit capability is invalid")
        if self.signal_source_state not in _SIGNAL_CAPABILITIES:
            raise ValueError("CAMP signal capability is invalid")


@dataclass(frozen=True)
class CAMPContextRecord:
    raw: np.ndarray
    ...
    def as_dict(self) -> dict[str, float]:
        ...
        return {name: float(value) for name, value in zip(RAW_FEATURE_NAMES, self.raw)}


@dataclass(frozen=True)
class CAMPContextScaler:
    q05: np.ndarray
    q95: np.ndarray
    ...
    def __post_init__(self) -> None:
        ...
        if low.shape != (RAW_FEATURE_COUNT,) or high.shape != (RAW_FEATURE_COUNT,):
            raise ValueError(f"q05/q95 must both have shape ({RAW_FEATURE_COUNT},).")
    ...
    def lift(
        self, raw: np.ndarray, *, source_complete: np.ndarray | None = None
    ) -> np.ndarray:
...
def _capabilities(value: CAMPContextSourceCapabilities | Mapping[str, Any] | None) -> CAMPContextSourceCapabilities:
    if value is None:
        return CAMPContextSourceCapabilities()
    if isinstance(value, CAMPContextSourceCapabilities):
        return value
    if type(value) is not dict or set(value) != {"speed_limit_status", "signal_source_state"}:
        raise ValueError("CAMP context source capabilities field set drifted")
    return CAMPContextSourceCapabilities(
        speed_limit_status=value["speed_limit_status"],
        signal_source_state=value["signal_source_state"],
    )


def build_camp_raw_context(
    *,
    ...
    source_capabilities: CAMPContextSourceCapabilities | Mapping[str, Any] | None = None,
) -> CAMPContextRecord:
    """Build frozen CAMP context using source capabilities, never imputation."""

    capabilities = _capabilities(source_capabilities)
    ...
    route_limits = np.asarray(causal_input["route_lanes_speed_limit"], dtype=np.float64).reshape(-1)
    route_has_limits = np.asarray(causal_input["route_lanes_has_speed_limit"], dtype=bool).reshape(-1)
    ...
    if available_limits.size:
        ...
        if capabilities.speed_limit_status != "typed_missing":
            raise ValueError("CAMP context requires a current positive route speed limit.")
    ...
    phase, signal_distance, phase_known = _resolved_signal_context(
        ...
        signal_capability=capabilities.signal_source_state,
    )
    phase_remaining, timing_known, timing_receipt = _v2i_timing_context(
        v2i_signal_timing, regulatory_signal_mapped=phase_known
    )
    ...
    consensus = build_candidate_set_consensus_payload(
        candidates=trajectories, support_steps=trajectories.shape[1]
    )
    ...
    progress_std = float(np.std(_candidate_route_progress(trajectories[:, :, :2], route_rows[:, :2])))
    phase_values = {
        "red": (1.0, 0.0, 0.0, 0.0),
    ...
    raw = np.asarray(
        [
            ego_speed, ego_longitudinal_acceleration, ego_lateral_acceleration, ego_yaw_rate,
            float(np.mean(curvature)), float(np.max(curvature)), float(np.min(lane_widths)),
            float(np.median(lane_widths)), route_speed_limit_min, route_speed_limit_current,
            *phase_values, signal_distance, phase_remaining, *neighbor_values,
            float(consensus["candidate_set_consensus_center_rms_median_m"]),
            float(consensus["candidate_set_consensus_center_rms_mad_m"]), endpoint_std,
            progress_std, float(np.mean(source_valid)),
        ], dtype=np.float64
    )
    ...
    complete = (
        *(True for _ in range(8)), speed_limit_context_complete, speed_limit_context_complete,
        *(phase_known for _ in range(5)), timing_known,
        *(neighbor_complete for _ in range(5)), *(True for _ in range(5)),
    )
    if len(complete) != RAW_FEATURE_COUNT:
        raise AssertionError("internal V25 source-completeness dimension mismatch")
    return CAMPContextRecord(
        raw=raw,
        source_complete=tuple(bool(value) for value in complete),
        source_receipt=timing_receipt,
    )


def _v2i_timing_context(
    payload: Mapping[str, Any] | None, *, regulatory_signal_mapped: bool
) -> tuple[float, bool, Mapping[str, Any]]:
    if payload is None:
        return 0.0, False, {
            "mode": "no_v2i", "phase_remaining_available": False,
            "regulatory_signal_mapped": bool(regulatory_signal_mapped),
        }
    required = {"source_id", "phase_remaining_s", "decision_timestamp_s", "source_timestamp_s", "maximum_age_s", "valid"}
    ...
    numeric_fields = ("phase_remaining_s", "decision_timestamp_s", "source_timestamp_s", "maximum_age_s")
    ...
    phase_remaining, decision_timestamp, source_timestamp, maximum_age = (float(value) for value in values)
    ...
    return phase_remaining, True, {
        "mode": "v2i_current_time", "source_id": source_id,
        "decision_timestamp_s": decision_timestamp, "source_timestamp_s": source_timestamp,
        "age_s": age, "maximum_age_s": maximum_age, "fresh": True, "valid": True,
        "phase_remaining_available": True, "regulatory_signal_mapped": True,
    }


def fit_train_context_scaler(
    raw_contexts: np.ndarray, *, source_complete: np.ndarray | None = None,
    record_weights: np.ndarray | None = None, lower_quantile: float = 0.05,
    upper_quantile: float = 0.95, minimum_span: float = 1e-6,
) -> CAMPContextScaler:
    ...
    available = np.ones(values.shape, dtype=np.bool_) if source_complete is None else _strict_context_source_complete(source_complete, values.shape)
    ...
    for feature_index in range(RAW_FEATURE_COUNT):
        ...
        if not np.any(mask):
            q05[feature_index], q95[feature_index] = 0.0, 1.0
            continue
        q05[feature_index] = _weighted_context_quantile(values[mask, feature_index], weights[mask], lower_quantile)
        q95[feature_index] = _weighted_context_quantile(values[mask, feature_index], weights[mask], upper_quantile)
    q95 = np.maximum(q95, q05 + float(minimum_span))
    return CAMPContextScaler(q05=q05, q95=q95)
...
def masked_complement_lift(unit_context: np.ndarray, source_complete: np.ndarray) -> np.ndarray:
    values = _raw_matrix(unit_context)
    if np.any(values < -1e-12) or np.any(values > 1.0 + 1e-12):
        raise ValueError("normalized V25 context must lie in [0,1].")
...
def validate_column_simplex_theta(theta: np.ndarray, *, num_atoms: int | None = None, atol: float = 1e-8) -> np.ndarray:
    values = np.asarray(theta, dtype=np.float64)
    if values.ndim != 2 or values.shape[1] != PHI_DIMENSION:
        raise ValueError(f"V25 Theta must have shape [num_atoms,{PHI_DIMENSION}].")
...
def context_weights(theta: np.ndarray, phi: np.ndarray) -> np.ndarray:
    ...
    if np.any(weights < -1e-9) or not np.allclose(weights.sum(axis=1), 1.0, rtol=0.0, atol=1e-8):
...
def _strict_context_source_complete(source_complete: np.ndarray, shape: tuple[int, int]) -> np.ndarray:
...
def _context_record_weights(record_weights: np.ndarray | None, size: int) -> np.ndarray:
    if record_weights is None:
        return np.full(size, 1.0 / size, dtype=np.float64)
    values = np.asarray(record_weights)
    if values.shape != (size,) or values.dtype.kind not in "fiu" or values.dtype.kind == "b":
...
def _weighted_context_quantile(values: np.ndarray, weights: np.ndarray, quantile: float) -> float:
...
def _candidate_route_progress(candidates_xy: np.ndarray, route_xy: np.ndarray) -> np.ndarray:
    ...
    for candidate_index, points in enumerate(candidates_xy):
        ...
        along = np.clip(np.einsum("tsd,sd->ts", relative, directions), 0.0, segment_lengths[None, :])
        projections = starts[None, :, :] + directions[None, :, :] * along[:, :, None]
        nearest = np.argmin(np.linalg.norm(points[:, None, :] - projections, axis=2), axis=1)
        projected_arc = arc_starts[nearest] + along[np.arange(points.shape[0]), nearest]
        result[candidate_index] = float(np.max(projected_arc))
    return result


def _route_signal_context(route_rows: np.ndarray, route_arc: np.ndarray) -> tuple[str, float, bool]:
    states = route_rows[:, 8:13]
    known = np.flatnonzero(np.sum(states[:, :4], axis=1) > 0.5)
    if known.size == 0:
        return "unknown", float(route_arc[-1]), False
    index = int(known[0])
    phase = ("green", "yellow", "red", "unknown")[int(np.argmax(states[index, :4]))]
    return phase, float(route_arc[index]), True


def _resolved_signal_context(
    route_rows: np.ndarray, route_arc: np.ndarray, *, causal_signal_atom_input: Mapping[str, Any] | None,
    signal_capability: str,
) -> tuple[str, float, bool]:
    phase, distance, known = _route_signal_context(route_rows, route_arc)
    if signal_capability == "route_only":
        if causal_signal_atom_input is not None:
            raise ValueError("route-only signal capability rejects a causal signal authority")
        return phase, distance, known
    if causal_signal_atom_input is None:
        raise ValueError("source-bound signal capability requires a causal signal authority")
    permitted = (signal_capability,)
    signal = validate_causal_signal_atom_input(causal_signal_atom_input, permitted_source_states=permitted)
    if signal["source_state"] != signal_capability:
        raise ValueError("CAMP signal capability and causal authority drifted")
    if signal_capability == "unavailable":
        return "unknown", float(route_arc[-1]), False
    if signal_capability == "not_applicable":
        if known:
    ...
    return current_phase, max(float(stop_line.mean(axis=0) @ tangent), 0.0), True
...
def _neighbor_context(neighbor_history: Any, ego_speed: float) -> tuple[tuple[float, ...], bool]:
    ...
    if not active.any():
        return (0.0, NO_NEIGHBOR_DISTANCE_M, NO_NEIGHBOR_TTC_S, 0.0, NO_NEIGHBOR_DISTANCE_M), True
    rows = current[active]
    positions = rows[:, :2]
    distances = np.linalg.norm(positions, axis=1)
    relative_velocity = rows[:, 4:6] - np.array([ego_speed, 0.0])
    closing = -np.einsum("ij,ij->i", positions, relative_velocity) / np.maximum(distances, 1e-6)
    ttc = np.where(closing > 1e-6, distances / closing, NO_NEIGHBOR_TTC_S)
    closest = int(np.argmin(distances))
    return (
        float(rows.shape[0]), float(np.min(distances)), float(min(np.min(ttc), NO_NEIGHBOR_TTC_S)),
        float(closing[closest]), float(np.min(np.abs(positions[:, 1]))),
    ), True
