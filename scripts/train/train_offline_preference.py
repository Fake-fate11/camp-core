...
def parse_args():
    ...
    parser.add_argument("--num_scenarios", type=int, default=50, help="Number of scenarios to use; <=0 means all")
    parser.add_argument("--cache_path", type=str, default=None, help="Optional prebuilt CAMP train cache with atoms and gt_atoms")
...
def collect_from_cache(args, atom_scales: np.ndarray):
    print(f"Loading offline preference data from cache: {args.cache_path}", flush=True)
    ...
    if args.num_scenarios > 0 and args.num_scenarios < total:
        ...
        print(f"Using first {len(scenarios)}/{total} cached scenarios.", flush=True)

    gts = []
    cands = []
    for idx, sample in enumerate(scenarios):
        if "gt_atoms" not in sample:
            raise ValueError(f"Scenario {idx} in {args.cache_path} is missing gt_atoms.")
        atoms = as_numpy(sample["atoms"]).astype(np.float32)
        gt_atoms = as_numpy(sample["gt_atoms"]).astype(np.float32)
        if atoms.ndim != 2:
            raise ValueError(f"Scenario {idx} atoms must be [K, R], got {atoms.shape}")
        if atoms.shape[1] != len(atom_scales) or gt_atoms.shape[0] != len(atom_scales):
            raise ValueError(
                f"Scenario {idx} atom dims do not match scales: atoms={atoms.shape}, gt={gt_atoms.shape}, scales={atom_scales.shape}"
            )

        atoms_norm = atoms / atom_scales.reshape(1, -1)
        gt_norm = gt_atoms / atom_scales
        atoms_norm = np.clip(np.nan_to_num(atoms_norm, nan=0.0, posinf=10.0, neginf=0.0), 0.0, 10.0)
        gt_norm = np.clip(np.nan_to_num(gt_norm, nan=0.0, posinf=10.0, neginf=0.0), 0.0, 10.0)
        gts.append(gt_norm)
        cands.append(atoms_norm)

    gts_np = np.stack(gts).astype(np.float32)
    cands_np = np.stack(cands).astype(np.float32)
    print(f"Loaded cache preference tensors: gt={gts_np.shape}, candidates={cands_np.shape}", flush=True)
    return gts_np, cands_np

def train_offline_weights_from_tensors(gts_np: np.ndarray, cands_np: np.ndarray, args, device: torch.device) -> np.ndarray:
    print("Training offline weights from prebuilt cache (vectorized BTL)...", flush=True)
    gts_t = torch.tensor(gts_np, dtype=torch.float32, device=device)
    cands_t = torch.tensor(cands_np, dtype=torch.float32, device=device)

    num_atoms = int(gts_t.shape[1])
    ...
    for epoch in range(args.epochs):
        ...
        delta = gts_t.unsqueeze(1) - cands_t
        diffs = (delta * w).sum(dim=-1)
        final_loss = torch.nn.functional.softplus(diffs).mean() + args.l1_reg * torch.norm(w, 1)
        final_loss.backward()
        optimizer.step()

        if epoch % 10 == 0:
            print(f"Epoch {epoch}: Loss {final_loss.item():.4f}", flush=True)

    final_w = torch.softmax(w_logits, dim=0).detach().cpu().numpy()
    print(f"Final Weights (R={num_atoms}): {final_w}", flush=True)
    return final_w
...
def main():
    ...
    if args.cache_path:
        gts_np, cands_np = collect_from_cache(args, atom_scales)
        final_w = train_offline_weights_from_tensors(gts_np, cands_np, args, device)
    ...
    print(f"Total Scenarios to Collect: {target_label}", flush=True)
    
    print("Starting batch loop...", flush=True)
    count = 0
    for batch in dataloader:
        if (not collect_all) and scenarios_collected >= args.num_scenarios:
            break
        ...
        for i in range(B):
            ...
            if scenarios_collected % 100 == 0:
                 print(f"Collected: {scenarios_collected}/{target_label}", flush=True)
