...
from scripts.integrations.review_diffusion_planner_candidate_set_consensus_lane_projected_jerk_progress_default_off_fixed_snapshot_screen_rerun_remediation_post_implementation_static_contract import (  # noqa: E402
...
READY_STATUS = (
    ...
    "fixed_snapshot_screen_rerun_remediation_fixed_snapshot_screen_rerun_"
    "plan_ready"
)
REJECT_STATUS = (
    "candidate_set_consensus_lane_projected_jerk_progress_support_default_off_"
...
AUTHORIZED_NEXT_WORK = (
    "candidate_set_consensus_lane_projected_jerk_progress_support_default_off_"
    "fixed_snapshot_screen_rerun_remediation_guarded_fixed_snapshot_screen_rerun_only"
)
...
DEFAULT_POST_REVIEW_ROOT = (
    ...
    "post_implementation_static_review_bff8f8b"
)
...
GUARD_ENV_VAR = (
    ...
    "REMEDIATION_FIXED_SNAPSHOT_RERUN_APPROVED"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Plan-only guarded fixed-snapshot screen rerun after the "
            "default-off remediation post-implementation static review."
        )
    )
    parser.add_argument(
        "--post_review_root",
    ...
    parser.add_argument(
        "--execution_root",
        type=Path,
        default=Path(DEFAULT_EXECUTION_ROOT),
    )
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Default-Off Remediation Fixed-Snapshot Screen Rerun Plan",
        "",
...
(cd "${{DIFFUSION_REPO}}" && git rev-parse HEAD) > "${{OUTPUT_ROOT}}/DP_HEAD.txt"
...
"${{PY}}" scripts/integrations/analyze_diffusion_planner_route_topology_candidate_screen.py \\
  ...
  --label default_off_remediation_fixed_snapshot_screen_rerun \\
...
def _screen_rerun_plan(execution_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "execution_artifacts": {
            "output_root": root,
            "screen_json": f"{root}/default_off_remediation_fixed_snapshot_screen.json",
            "screen_md": f"{root}/default_off_remediation_fixed_snapshot_screen.md",
...
def _artifact_checks(artifact: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        ...
        _check("post_review_markdown_records_next_gate", "Next Gate" in artifact["markdown_text"]),
    ]
...
def _plan_checks(plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        _check("plan_uses_remediation_policy", config["generator_policy"] == PLANNED_POLICY),
...
def _runbook_checks(runbook_text: str) -> list[dict[str, Any]]:
    return [
        ...
        _check("runbook_uses_remediation_policy", f"--generator_policy {PLANNED_POLICY}" in runbook_text),
        _check("runbook_has_snapshot_screen_command", "analyze_diffusion_planner_route_topology_candidate_screen.py" in runbook_text),
        _check("runbook_does_not_run_replay", "replay" not in runbook_text.lower()),
        _check("runbook_does_not_modify_git", "git pull" not in runbook_text.lower() and "git checkout" not in runbook_text.lower()),
    ]
...
def _boundary_checks() -> list[dict[str, Any]]:
    ...
    return [
        _check("boundary_blocks_current_execution", "screen rerun execution is not authorized in this plan gate" in text),
...
def _final_decision(
    ...
    return {
        ...
        "fixed_snapshot_screen_rerun_next_gate_authorized": passed,
...
def _blocked_boundaries() -> list[str]:
    return [
        "candidate generation and screen rerun execution is not authorized in this plan gate",
