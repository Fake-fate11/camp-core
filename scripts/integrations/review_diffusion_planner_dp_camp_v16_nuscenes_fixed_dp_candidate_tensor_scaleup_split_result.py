#!/usr/bin/env python3
"""Review the v16 fixed-DP scale-up split execution result."""
...
EXPECTED_SCENES = SOURCE_MODULE.EXPECTED_SCENES
EXPECTED_SCENE_COUNTS = SOURCE_MODULE.EXPECTED_SCENE_COUNTS
EXPECTED_RECORD_COUNTS = SOURCE_MODULE.EXPECTED_RECORD_COUNTS
...
READY_STATUS = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_split_result_review_passed"
REJECT_STATUS = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_split_result_review_rejected"
AUTHORIZED_NEXT_WORK = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_preflight_plan_only"
SCHEMA_VERSION = "dp_camp_v16_nuscenes_fixed_dp_candidate_tensor_scaleup_split_result_review_v1"
REVIEW_JSON_NAME = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_split_result_review.json"
REVIEW_MD_NAME = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_split_result_review.md"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v16_nuscenes_fixed_dp_candidate_tensor_scaleup_split_result_review",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        enabled=args.enable_v16_nuscenes_fixed_dp_candidate_tensor_scaleup_split_result_review,
    )
...
def build_report(
    *,
    ...
    source_file_checks = _source_file_checks(
        artifact,
        source_summary_json,
    ...
    review = _split_result_review(records_by_split, manifest, all(check["passed"] for check in source_file_checks))
    ...
    checks = [
        ...
        _expect("source_summary_records_10000", source_split.get("records"), EXPECTED_RECORDS),
        _expect("source_summary_scenes_50", source_split.get("scenes"), EXPECTED_SCENES),
        _expect("source_summary_unique_samples_10000", source_split.get("unique_samples"), EXPECTED_RECORDS),
        _expect("source_summary_scene_counts", source_split.get("scene_counts"), EXPECTED_SCENE_COUNTS),
        _expect("source_summary_record_counts", source_split.get("record_counts"), EXPECTED_RECORD_COUNTS),
        ...
        _expect("source_summary_unassigned_record_count", source_split.get("unassigned_record_count"), 0),
        _expect("source_summary_split_manifest_materialized", source_split.get("split_manifest_materialized"), True),
        _expect("review_records_10000", review["records"], EXPECTED_RECORDS),
        _expect("review_scenes_50", review["scenes"], EXPECTED_SCENES),
        _expect("review_unique_samples_10000", review["unique_samples"], EXPECTED_RECORDS),
        _expect("review_scene_counts", review["scene_counts"], EXPECTED_SCENE_COUNTS),
        _expect("review_record_counts", review["record_counts"], EXPECTED_RECORD_COUNTS),
        ...
        _expect("unassigned_record_count_zero", review["unassigned_record_count"], 0),
        _expect("source_artifact_files_present", review["source_artifact_files_present"], True),
    ]
    checks.extend(source_file_checks)
...
def _split_result_review(
    records_by_split: dict[str, list[dict[str, Any]]],
    manifest: dict[str, Any],
    source_artifact_files_present: bool,
) -> dict[str, Any]:
    ordered = ("train", "calibration", "holdout")
    all_records = [record for split in ordered for record in records_by_split[split]]
    scene_sets = {
        split: {str(record.get("source_scene_id") or record.get("scene_id")) for record in records_by_split[split]}
        for split in ordered
    }
    sample_sets = {
        split: {str(record.get("source_sample_id") or record.get("sample_id")) for record in records_by_split[split]}
        for split in ordered
    }
    manifest_assignment_sets = {
        split: set(manifest.get("scene_assignments", {}).get(split, []))
        for split in ordered
    }
    return {
        ...
        "scene_counts": {split: len(scene_sets[split]) for split in ordered},
        "record_counts": {split: len(records_by_split[split]) for split in ordered},
        "scene_assignments": {split: sorted(scene_sets[split]) for split in ordered},
        "scene_zero_overlap": _sets_disjoint(scene_sets.values()) and _sets_disjoint(manifest_assignment_sets.values()),
        ...
        "unassigned_record_count": manifest.get("unassigned_record_count"),
        "source_artifact_files_present": source_artifact_files_present,
    }
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# V16 nuScenes Fixed-DP Scale-Up Split Result Review",
            "",
            ...
            f"- Scene counts: `{review['scene_counts']}`",
            f"- Record counts: `{review['record_counts']}`",
            f"- Scene zero-overlap: `{review['scene_zero_overlap']}`",
            f"- Sample zero-overlap: `{review['sample_zero_overlap']}`",
            "- Boundary: result review only; no training, evaluation, claim, promotion, or deployment.",
            "",
        ]
    )
