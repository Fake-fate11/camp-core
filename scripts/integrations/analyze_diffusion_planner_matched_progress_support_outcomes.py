...
EXPECTED_PROGRESS_SUPPORT_FLAGS = {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Fail-closed audit for matched DP-CAMP progress-support payloads "
            "and candidate outcome labels collected in the same nonformal "
            "replay records. This is read-only and does not run Diffusion "
            "Planner."
        )
    )
...
def analyze(
    ...
    *,
    ...
    warnings: list[str] = []
    records_total = 0
    progress_support_records = 0
    outcome_records = 0
    formal_seed_records = 0
    candidate_rows = 0
    max_latency_ms = {key: 0.0 for key in PROGRESS_SUPPORT_LATENCY_KEYS}
    ...
    for log_path in log_paths:
        ...
        for record_index, raw in enumerate(payload):
            ...
            progress_ok = _validate_progress_support_payload(
                raw,
            ...
            progress_support_records += int(progress_ok)
            outcome_records += int(outcome_ok)
            log_progress_records += int(progress_ok)
            log_outcome_records += int(outcome_ok)
            if progress_ok and outcome_ok:
                candidate_rows += int(expected_candidates)
        per_log.append(
            {
                "selection_log": str(log_path),
                "records": len(payload),
                "progress_support_records": log_progress_records,
    ...
    if records_total and progress_support_records != records_total:
        errors.append(
            f"progress_support_records={progress_support_records} "
    ...
    return {
        "analysis": {
            "name": "dp_camp_matched_progress_support_outcome_contract_v1",
            "label": label,
            "training": False,
            ...
            "math_boundary": (
                "Progress-support descriptors are current-tick fixed "
        ...
        "counts": {
            ...
            "progress_support_records": progress_support_records,
        ...
        "final_decision": {
            "status": (
                ...
                if passed
                else "matched_progress_support_outcome_contract_rejected"
            ),
            "passed": passed,
            "authorized_next_work": (
                "offline_progress_support_descriptor_separability_screen_only"
                if passed
                else "fix_matched_progress_support_label_collection_before_separability"
            ),
...
def _validate_progress_support_payload(
    record: dict[str, Any],
    *,
    record_prefix: str,
    ...
    payload = record.get("progress_support_logging")
    if not isinstance(payload, dict):
        errors.append(f"{record_prefix}: progress_support_logging missing")
        return False
    for field, expected in EXPECTED_PROGRESS_SUPPORT_FLAGS.items():
        if payload.get(field) != expected:
            errors.append(
                f"{record_prefix}: progress_support_logging.{field}="
                f"{payload.get(field)!r}"
            )
    if "candidate_closed_loop_outcomes" in payload:
        errors.append(
            f"{record_prefix}: progress-support payload contains candidate outcomes"
        )
    ...
    if candidate_count != int(expected_candidates):
        errors.append(
            f"{record_prefix}: progress-support candidate_count={candidate_count} "
    ...
    if not isinstance(finite_checks, dict) or not finite_checks:
        errors.append(f"{record_prefix}: progress-support finite_checks missing")
    else:
        for field in (
            *PROGRESS_SUPPORT_FIELD_NAMES,
    ...
    if payload.get("progress_support_atom_names") != list(PROGRESS_SUPPORT_ATOM_NAMES):
        errors.append(f"{record_prefix}: progress_support_atom_names mismatch")
    ...
    if atoms.shape != (int(expected_candidates), len(PROGRESS_SUPPORT_ATOM_NAMES)):
        errors.append(f"{record_prefix}: progress_support_atoms shape={list(atoms.shape)}")
    elif not np.all(np.isfinite(atoms)):
        errors.append(f"{record_prefix}: progress_support_atoms not finite")
    elif not np.all(atoms >= -1e-12):
        errors.append(f"{record_prefix}: progress_support_atoms has negative values")
...
def render_markdown(report: dict[str, Any]) -> str:
    lines = [
        "# DP CAMP Matched Progress-Support Outcome Contract Audit",
        "",
        "This read-only audit requires every checked selection-log record to carry "
        "both a no-leak progress-support payload and offline candidate outcome "
