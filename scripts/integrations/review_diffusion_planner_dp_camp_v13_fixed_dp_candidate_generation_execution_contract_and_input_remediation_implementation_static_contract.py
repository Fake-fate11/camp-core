#!/usr/bin/env python3
"""Static review for fixed-DP execution contract/input remediation implementation plan."""
...
REQUIRED_PLAN_TEST_SNIPPETS = (
    "test_execution_contract_input_remediation_implementation_plan_authorizes_static_review_only",
    "test_execution_contract_input_remediation_implementation_plan_rejects_source_action_leak",
    "AUTHORIZED_NEXT_WORK",
    "INPUT_MATERIALIZER_REQUIREMENTS",
)
SOURCE_FALSE_FLAGS = (
...
def build_report(
    *,
    ...
    plan = _dict(source_payload.get("implementation_plan"))
    ...
    return {
        ...
        "source_implementation_plan": {
            "path": str(implementation_plan_json),
            "artifact_dir": str(implementation_plan_artifact_dir),
        ...
        "static_contract_review": {
            ...
            "implementation_targets": plan.get("implementation_targets"),
            "test_targets": plan.get("test_targets"),
            "runner_requirements": plan.get("runner_requirements"),
            "preflight_requirements": plan.get("preflight_requirements"),
            "input_materializer_requirements": plan.get("input_materializer_requirements"),
...
def _checks(
    *,
    ...
    checks = [
        _expect("implementation_plan_json_exists", implementation_plan_json.exists(), True),
        _expect("implementation_plan_artifact_dir_exists", implementation_plan_artifact_dir.is_dir(), True),
        _expect("implementation_plan_script_exists", implementation_plan_script.exists(), True),
        _expect("implementation_plan_test_exists", implementation_plan_test.exists(), True),
        ...
        _expect("source_dp_repo_modification_forbidden", plan.get("dp_repo_modification_allowed"), False),
    ...
    for target in REQUIRED_IMPLEMENTATION_TARGETS:
        checks.append(_expect(f"plan_implementation_target_{_slug(target)}", target in _list(plan.get("implementation_targets")), True))
    for target in REQUIRED_TEST_TARGETS:
        checks.append(_expect(f"plan_test_target_{_slug(target)}", target in _list(plan.get("test_targets")), True))
    for requirement in REQUIRED_RUNNER_REQUIREMENTS:
        checks.append(_expect(f"plan_runner_requirement_{_slug(requirement)}", requirement in _list(plan.get("runner_requirements")), True))
    for requirement in REQUIRED_PREFLIGHT_REQUIREMENTS:
        checks.append(_expect(f"plan_preflight_requirement_{_slug(requirement)}", requirement in _list(plan.get("preflight_requirements")), True))
    for requirement in REQUIRED_INPUT_MATERIALIZER_REQUIREMENTS:
        checks.append(_expect(f"plan_input_materializer_requirement_{_slug(requirement)}", requirement in _list(plan.get("input_materializer_requirements")), True))
...
def _decision(
    *,
    ...
    return {
        ...
        "fixed_dp_candidate_generation_execution_contract_and_input_remediation_implementation_static_contract_review_passed": passed,
        "fixed_dp_candidate_generation_execution_contract_and_input_remediation_implementation_authorized_next": passed,
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# Fixed-DP Execution Contract/Input Remediation Implementation Static Review",
            "",
