#!/usr/bin/env python3
"""Preflight actual-SafetyCost outcome materialization without executing it."""
...
def _load_source_review_module():
    review_path = Path(__file__).resolve().with_name(
        ...
        "preflight_plan_static_contract.py"
    )
    spec = importlib.util.spec_from_file_location(
        "v14_post_closeout_promotion_evidence_acquisition_actual_safetycost_outcome_materialization_preflight_plan_static_review",
        review_path,
    )
...
SCHEMA_VERSION = (
    ...
    "actual_safetycost_outcome_materialization_preflight_v1"
)
...
READY_STATUS = (
    ...
    "actual_safetycost_outcome_materialization_preflight_ready"
)
REJECT_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_paired_evaluation_"
    "actual_safetycost_outcome_materialization_preflight_rejected"
)
AUTHORIZED_NEXT_WORK = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_paired_evaluation_"
    "actual_safetycost_outcome_materialization_preflight_static_review_only"
)

PREFLIGHT_JSON_NAME = (
    "post_closeout_promotion_evidence_acquisition_paired_evaluation_"
    "actual_safetycost_outcome_materialization_preflight.json"
)
PREFLIGHT_MD_NAME = (
    "post_closeout_promotion_evidence_acquisition_paired_evaluation_"
    "actual_safetycost_outcome_materialization_preflight.md"
)

EXPECTED_MATERIALIZATION_INPUTS = (
    "passed_outcome_materialization_preflight_plan_static_review_artifact",
    "source_outcome_materialization_preflight_plan_artifact",
    "fixed_dp_candidate_tensor_identity_manifest",
    "paired_shadow_selected_run_key_manifest",
    "shadow_selected_non_top1_candidate_index_manifest",
    "closed_loop_runner_config_snapshot",
    "outcome_summary_schema_and_hash_contract",
    "evaluation_only_leakage_guard",
    "safetycost_v1_claim_rule_config_snapshot",
    "artifact_heads_command_stdout_stderr_contract",
)
EXPECTED_PREFLIGHT_STEPS = (
    "lock_source_static_review_and_preflight_plan_artifacts",
    "verify_fixed_dp_candidate_tensor_identity_immutability",
    "verify_paired_shadow_selected_run_key_coverage",
    "verify_runner_inputs_do_not_modify_dp_or_candidates",
    "verify_outcome_summary_schema_and_hash_manifest",
    "verify_no_closed_loop_training_or_online_input",
    "verify_full36_formal_seed_and_split_exclusions",
    "emit_static_review_ready_materialization_preflight_artifact",
)
EXPECTED_FUTURE_OUTPUTS = (
    "shadow_selected_outcome_materialization_run_manifest",
    "closed_loop_runner_invocation_manifest",
    "paired_outcome_summary_schema",
    "paired_outcome_identity_join_manifest",
    "safetycost_v1_evaluation_input_manifest",
    "materialization_no_go_report",
)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v14_post_closeout_promotion_evidence_acquisition_actual_safetycost_outcome_materialization_preflight",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        label=args.label,
        enabled=(
            args.enable_v14_post_closeout_promotion_evidence_acquisition_actual_safetycost_outcome_materialization_preflight
        ),
    )
...
def build_report(
    *,
    ...
    artifact_dir = preflight_plan_static_review_artifact_dir.resolve()
    review_json = preflight_plan_static_review_json.resolve()
    review_md = preflight_plan_static_review_md.resolve()
    review_sha256s = preflight_plan_static_review_sha256s.resolve()
    ...
    materialization_inputs = _materialization_inputs(source_review)
    ...
    return {
        ...
        "analysis": {
            ...
            "preflight_only": True,
            "read_only": True,
            "actual_safetycost_outcome_materialization_preflight_only": True,
        ...
        "inputs": {
            "preflight_plan_static_review_artifact_dir": str(artifact_dir),
            "preflight_plan_static_review_json": str(review_json),
            "preflight_plan_static_review_md": str(review_md),
            "preflight_plan_static_review_sha256s": str(review_sha256s),
        ...
        "source_preflight_plan_summary": _source_preflight_plan_summary(source_review),
        "preflight_scope_summary": _preflight_scope_summary(source_review),
        "materialization_inputs": materialization_inputs,
        "preflight_steps": preflight_steps,
        "future_outputs": future_outputs,
...
def _checks(
    *,
    ...
    scope = HELPER_MODULE._dict(source_review.get("preflight_scope_summary"))
    ...
    require("outcome_materialization_preflight_enabled", enabled)
    ...
    expect("source_review_static_review_passed", decision.get("post_closeout_promotion_evidence_acquisition_paired_evaluation_actual_safetycost_outcome_materialization_preflight_plan_static_review_passed"), True)
    expect("source_review_preflight_authorized", decision.get("actual_safetycost_outcome_materialization_preflight_authorized"), True)
    ...
    expect("source_review_no_closed_loop_training_input", decision.get("closed_loop_outcome_training_or_online_input_authorized"), False)
    ...
    expect("source_review_check_count", len(_list(source_review.get("static_review_checks"))), 82)
    ...
    expect("source_preflight_plan_count", source_plan.get("preflight_plan_count"), 8)
    ...
    expect("source_static_review_check_count", source_plan.get("source_static_review_check_count"), 82)
...
def _preflight_contract_checks(
    ...
    return [
        ...
        {
            "name": "preflight_steps_do_not_materialize_outcomes",
            "passed": sorted({item["materializes_outcomes"] for item in preflight_steps}) == [False],
            "actual": sorted({item["materializes_outcomes"] for item in preflight_steps}),
            "expected": [False],
        },
        {
...
def _materialization_inputs(source_review: dict[str, Any]) -> list[dict[str, Any]]:
    summary = _source_static_review_summary(source_review)
    return [
        {
            "name": name,
            "status": "locked_for_static_review",
            "materializes_outcomes": False,
            "source_status": summary.get("status") if index == 0 else None,
        }
...
def _preflight_steps() -> list[dict[str, Any]]:
    return [
        {"name": name, "status": "preflight_checked", "materializes_outcomes": False}
...
def _source_preflight_plan_summary(source_review: dict[str, Any]) -> dict[str, Any]:
    return HELPER_MODULE._dict(source_review.get("source_plan_summary"))


def _preflight_scope_summary(source_review: dict[str, Any]) -> dict[str, Any]:
    return HELPER_MODULE._dict(source_review.get("preflight_scope_summary"))
...
def _decision(*, passed: bool, checks: list[dict[str, Any]], source_review: dict[str, Any]) -> dict[str, Any]:
    ...
    if passed:
        failure_class = None
    elif "outcome_materialization_preflight_enabled" in failed:
        failure_class = "explicit_actual_safetycost_outcome_materialization_preflight_authorization_missing"
        ...
        failure_class = "actual_safetycost_outcome_materialization_preflight_contract_failure"
    ...
    decision = {
        ...
        "post_closeout_promotion_evidence_acquisition_paired_evaluation_actual_safetycost_outcome_materialization_preflight_ready": bool(passed),
        "post_closeout_promotion_evidence_acquisition_paired_evaluation_actual_safetycost_outcome_materialization_preflight_static_review_authorized": bool(passed),
        "actual_safetycost_outcome_materialization_preflight_executed_by_this_gate": bool(passed),
        ...
        "recommendation": "static_review_actual_safetycost_outcome_materialization_preflight_only" if passed else "repair_or_rerun_same_preflight_gate",
        "score_expression": SCORE_EXPRESSION,
    }
    for action in BLOCKED_ACTIONS:
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    summary = report["source_preflight_plan_summary"]
    scope = report["preflight_scope_summary"]
    lines = [
        "# v14 Actual-SafetyCost Outcome-Materialization Preflight",
        "",
        ...
        "",
        f"- Source preflight-plan checks: `{summary.get('preflight_plan_check_count')}`",
        f"- Source failed checks: `{summary.get('failed_check_count')}`",
        f"- Source paired records: `{summary.get('source_paired_record_count')}`",
        f"- Source shadow differs from Top-1 records: `{summary.get('source_shadow_diff_records')}`",
        "",
        "## Preflight Scope",
        "",
        ...
        "## Boundary",
        "",
        "- Preflight only: no outcome materialization, replay, training, candidate generation, DP modification, promotion, deployment, online selector activation, or claim.",
