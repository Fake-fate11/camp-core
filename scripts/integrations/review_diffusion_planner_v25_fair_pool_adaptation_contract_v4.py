"""Seal a separate-role literal review of fair-pool contract v4."""
...
from camp_core.integrations.diffusion_planner_v25_fair_pool_adaptation_review_v4 import (  # noqa: E402
    review_contract_literal_v4,
)
...
def review(
    *,
    ...
    verify_complete_seal(source, source_root, label="fair-pool contract v4")
    ...
    literal = review_contract_literal_v4(contract)
    if (
        source_report.get("status")
        != "sealed_provenance_design_only_acquisition_unauthorized"
    ...
    try:
        report = {
            "schema_version": (
                ...
                "independent_review_v4"
            ),
            "status": (
                "passed_independent_provenance_semantic_review_v4"
            ),
            "source": {"path": str(source), "root_sha256": source_root},
            "contract_sha256": hashlib.sha256(contract_bytes).hexdigest(),
            "literal_review": literal,
            "reviewer_imported_v4_producer_contract_module": False,
            "reviewer_imported_input_manifest_module": False,
            "reviewer_imported_fairness_or_selector_module": False,
            "reviewer_local_trust_artifact_threshold_receipt_oracle": True,
            "caller_threshold_status_or_within_boolean_used": False,
        ...
        root = seal_artifact(
            staging,
            label="V25 fair-pool adaptation contract review v4",
        )
