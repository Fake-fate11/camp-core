#!/usr/bin/env python3
"""Plan-only promotion-readiness evidence-chain decision gate."""
...
def _load_source_review_module():
    review_path = Path(__file__).resolve().with_name(
        ...
        "uncertainty_coverage_evidence_package_promotion_authorization_boundary_plan_static_contract.py"
    )
    spec = importlib.util.spec_from_file_location(
        "v14_post_closeout_promotion_readiness_uncertainty_coverage_evidence_package_promotion_authorization_boundary_static_review",
        review_path,
    )
...
SCHEMA_VERSION = (
    ...
    "uncertainty_coverage_evidence_package_promotion_readiness_evidence_chain_decision_plan_v1"
)
...
REJECT_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_readiness_uncertainty_coverage_evidence_package_promotion_readiness_evidence_chain_decision_plan_rejected"
)
...
PLAN_JSON_NAME = (
    "post_closeout_promotion_readiness_uncertainty_coverage_evidence_package_promotion_readiness_evidence_chain_decision_plan.json"
)
PLAN_MD_NAME = (
    "post_closeout_promotion_readiness_uncertainty_coverage_evidence_package_promotion_readiness_evidence_chain_decision_plan.md"
)

EXPECTED_SOURCE_REVIEW_CHECK_COUNT = 132
EXPECTED_SOURCE_PLAN_CHECK_COUNT = 124
EXPECTED_DECISION_ITEMS = (
    "fixed_dp_and_candidate_source_lock",
    "selector_evidence_package_closed_state",
    "masked_objective_delta_scope",
    "uncertainty_coverage_manifest_scope",
    "promotion_vs_deployment_vs_claim_separation",
    "fail_closed_decision_outputs",
)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v14_post_closeout_promotion_readiness_uncertainty_coverage_evidence_package_promotion_readiness_evidence_chain_decision_plan",
        action="store_true",
        help="Explicit opt-in for plan-only promotion-readiness evidence-chain decision planning.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        label=args.label,
        enabled=args.enable_v14_post_closeout_promotion_readiness_uncertainty_coverage_evidence_package_promotion_readiness_evidence_chain_decision_plan,
    )
...
def build_report(
    *,
    ...
    checks: list[dict[str, Any]] = [
        BASE_MODULE._expect("evidence_chain_decision_plan_enabled", enabled, True),
    ...
    decision_plan = _decision_plan(label=label, source_review=source_review)
    checks.extend(_decision_plan_checks(decision_plan))
    ...
    return {
        ...
        "boundary_plan_summary": _boundary_plan_summary(source_review),
        "evidence_chain_decision_plan": decision_plan,
...
def _markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Post-Closeout Promotion-Readiness Evidence-Chain Decision Plan",
        "",
        ...
        "",
        "## Evidence-Chain Decision Plan",
    ]
    for item in report["evidence_chain_decision_plan"]:
...
def _source_review_contract_checks(source_review: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    checks = [
        ...
        BASE_MODULE._expect("source_review_boundary_static_review_passed", decision.get("promotion_authorization_boundary_static_review_passed"), True),
        BASE_MODULE._expect("source_review_evidence_chain_decision_plan_authorized", decision.get("promotion_readiness_evidence_chain_decision_plan_authorized"), True),
        BASE_MODULE._expect("source_review_user_future_authorization_recorded", decision.get("user_authorized_future_promotion_deployment_online_selector_and_claim_gates"), True),
        ...
        BASE_MODULE._expect("source_boundary_item_count", source_plan.get("boundary_item_count"), 5),
...
def _audit_checks(v14_text: str, status_text: str) -> list[dict[str, Any]]:
    return [
        BASE_MODULE._expect("audit_latest_status_is_boundary_static_review", BASE_MODULE._latest_value(v14_text, "current_v14_status"), SOURCE_REVIEW_STATUS),
        BASE_MODULE._expect("audit_latest_eof_authorizes_evidence_chain_decision_plan", BASE_MODULE._latest_value(v14_text, "next_work_target"), AUTHORIZED_CURRENT_WORK),
        BASE_MODULE._expect("audit_boundary_static_review_passed", BASE_MODULE._latest_value(v14_text, "promotion_authorization_boundary_static_review_passed"), "True"),
        BASE_MODULE._expect("audit_evidence_chain_decision_plan_authorized", BASE_MODULE._latest_value(v14_text, "promotion_readiness_evidence_chain_decision_plan_authorized"), "True"),
        ...
        BASE_MODULE._expect("status_doc_latest_status_is_boundary_static_review", BASE_MODULE._latest_value(status_text, "current_v14_status"), SOURCE_REVIEW_STATUS),
        BASE_MODULE._expect("status_doc_latest_eof_authorizes_evidence_chain_decision_plan", BASE_MODULE._latest_value(status_text, "next_work_target"), AUTHORIZED_CURRENT_WORK),
    ]


def _decision_plan(label: str | None, source_review: dict[str, Any]) -> list[dict[str, Any]]:
    source_summary = _source_static_review_summary(source_review)
    return [
        {
            "item_name": "fixed_dp_and_candidate_source_lock",
            "purpose": "require the decision gate to prove fixed DP head and fixed DP candidate tensor provenance before any promotion decision",
        ...
        },
        {
            ...
            "purpose": "carry the closed no-promotion evidence-package state as source context rather than treating it as a promotion recommendation",
            "source_status": source_summary["status"],
            "decision_gate_required": True,
            "authorizes_promotion_now": False,
            "authorizes_deployment_now": False,
        ...
        },
        {
            ...
            "purpose": "separate static masked-objective delta evidence from safety-benefit or CAMP-over-DP claims",
            "source_status": source_summary["status"],
            "decision_gate_required": True,
            "authorizes_promotion_now": False,
            "authorizes_deployment_now": False,
        ...
        },
        {
            ...
            "purpose": "require the decision gate to consume uncertainty and coverage manifests as audit evidence, not as online inputs",
            "source_status": source_summary["status"],
            "decision_gate_required": True,
            "authorizes_promotion_now": False,
            "authorizes_deployment_now": False,
        ...
        },
        {
            ...
            "purpose": "keep selector promotion, deployment, online activation, deployable checkpoint claims, and safety/CAMP-over-DP claims as separate decisions",
            "source_status": source_summary["status"],
            "decision_gate_required": True,
            "authorizes_promotion_now": False,
            "authorizes_deployment_now": False,
        ...
        },
        {
            ...
            "purpose": "require explicit no-promotion/no-deployment/no-claim outputs when any evidence-chain prerequisite is incomplete",
            "source_status": source_summary["status"],
            "decision_gate_required": True,
            "authorizes_promotion_now": False,
            "authorizes_deployment_now": False,
...
def _decision_plan_checks(plan: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [
        BASE_MODULE._expect("decision_item_names", [item.get("item_name") for item in plan], list(EXPECTED_DECISION_ITEMS)),
        BASE_MODULE._expect("decision_items_gate_required", sorted({item.get("decision_gate_required") for item in plan}), [True]),
        BASE_MODULE._expect("decision_items_no_promotion_now", sorted({item.get("authorizes_promotion_now") for item in plan}), [False]),
        BASE_MODULE._expect("decision_items_no_deployment_now", sorted({item.get("authorizes_deployment_now") for item in plan}), [False]),
        BASE_MODULE._expect("decision_items_no_claim_now", sorted({item.get("authorizes_claim_now") for item in plan}), [False]),
    ]
...
def _boundary_plan_summary(source_review: dict[str, Any]) -> dict[str, Any]:
    summary = BASE_MODULE._dict(source_review.get("source_plan_summary"))
    return {
        "plan_check_count": summary.get("plan_check_count"),
        "failed_check_count": summary.get("failed_check_count"),
        "boundary_item_count": summary.get("boundary_item_count"),
    }
...
def _decision(passed: bool, checks: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if passed:
        failure_class = None
    elif "evidence_chain_decision_plan_enabled" in failed:
        failure_class = "explicit_evidence_chain_decision_plan_authorization_missing"
    ...
    decision: dict[str, Any] = {
        ...
        "promotion_readiness_evidence_chain_decision_plan_ready": passed,
        ...
        "score_expression": SCORE_EXPRESSION,
        "recommendation": "static_review_evidence_chain_decision_plan_before_any_decision" if passed else "repair_contract_before_rerun",
    }
    for action in BLOCKED_ACTIONS:
