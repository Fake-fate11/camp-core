...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Create a dp_camp_v9_13d training-log view by appending the logged "
            "shadow red stopping-margin cost to certified dp_camp_v8_12d atoms."
        )
    )
...
def augment_record(
    ...
    *,
    ...
    if not np.all(np.isfinite(atoms)) or np.any(atoms < 0.0):
        raise ValueError(f"{log_path} record {record_idx} has invalid v8 atoms.")

    margin = np.asarray(
        record.get("candidate_red_stopping_margin_cost"),
        dtype=np.float64,
    ).reshape(-1)
    if margin.shape != (atoms.shape[0],):
        raise ValueError(
            f"{log_path} record {record_idx} has red stopping-margin shape "
            f"{margin.shape}; expected ({atoms.shape[0]},)."
        )
    if not np.all(np.isfinite(margin)) or np.any(margin < 0.0):
        raise ValueError(
            f"{log_path} record {record_idx} has invalid red stopping-margin costs."
        )
    ...
    augmented["atoms"] = np.concatenate([atoms, margin.reshape(-1, 1)], axis=1).tolist()
    augmented["red_stopping_margin_used_as_atom"] = False
    augmented["offline_augmentation"] = {
        "name": "dp_camp_v9_red_stopping_margin_from_shadow",
        "source_field": "candidate_red_stopping_margin_cost",
        "target_atom": "red_stopping_margin_cost",
        "source_log": str(log_path),
        "no_closed_loop_label_source": True,
        "selection_scores_remain_source_v8": True,
    }
    for stale_field in (
        "normalized_atoms",
...
def augment_records(
    ...
    *,
    ...
    return [
        augment_record(record, log_path=log_path, record_idx=record_idx)
...
def augment_logs(
    ...
    *,
    ...
    return {
        "augmentation": "dp_camp_v9_red_stopping_margin_from_shadow",
        "schema": {
        ...
        "source_schema": {
            "version": atom_schema_for_dimension(len(DP_CAMP_ATOM_NAMES_V8))[0],
            "atom_names": list(DP_CAMP_ATOM_NAMES_V8),
            "num_atoms": len(DP_CAMP_ATOM_NAMES_V8),
        },
        "counts": {
            "logs": len(reports),
...
def main() -> None:
    ...
    manifest_path = args.output_root / "v9_red_stopping_augmentation_manifest.json"
