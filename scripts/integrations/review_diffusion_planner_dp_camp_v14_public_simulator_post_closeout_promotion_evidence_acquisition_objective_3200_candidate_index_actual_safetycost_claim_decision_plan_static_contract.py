#!/usr/bin/env python3
"""Read-only static review for the objective-3200 claim decision plan."""
...
def _load_plan_module():
    plan_path = Path(__file__).resolve().with_name(
        ...
        "actual_safetycost_claim_decision.py"
    )
    spec = importlib.util.spec_from_file_location(
        "v14_candidate_index_actual_safetycost_claim_decision_plan",
        plan_path,
    )
...
CLAIM_DECISION_ITEMS = PLAN_MODULE.CLAIM_DECISION_ITEMS

SCHEMA_VERSION = (
    "dp_camp_v14_public_simulator_post_closeout_promotion_evidence_acquisition_"
    "objective_3200_candidate_index_actual_safetycost_claim_decision_plan_static_review_v1"
)
READY_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_claim_decision_plan_static_review_passed"
)
REJECT_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_claim_decision_plan_static_review_rejected"
)
AUTHORIZED_NEXT_WORK = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_claim_decision_only"
)

REVIEW_JSON_NAME = (
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_claim_decision_plan_static_review.json"
)
REVIEW_MD_NAME = (
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_claim_decision_plan_static_review.md"
)


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--source_claim_decision_plan_artifact_dir", type=Path, required=True)
    parser.add_argument("--source_claim_decision_plan_json", type=Path, required=True)
    parser.add_argument("--source_claim_decision_plan_md", type=Path, required=True)
    parser.add_argument("--source_claim_decision_plan_sha256s", type=Path, required=True)
    parser.add_argument("--claim_decision_plan_script", type=Path, required=True)
    parser.add_argument("--claim_decision_plan_test", type=Path, required=True)
    ...
    parser.add_argument(
        "--enable_v14_post_closeout_promotion_evidence_acquisition_objective_3200_candidate_index_actual_safetycost_claim_decision_plan_static_review",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        source_claim_decision_plan_artifact_dir=args.source_claim_decision_plan_artifact_dir,
        source_claim_decision_plan_json=args.source_claim_decision_plan_json,
        source_claim_decision_plan_md=args.source_claim_decision_plan_md,
        source_claim_decision_plan_sha256s=args.source_claim_decision_plan_sha256s,
        claim_decision_plan_script=args.claim_decision_plan_script,
        claim_decision_plan_test=args.claim_decision_plan_test,
        ...
        enabled=(
            args.enable_v14_post_closeout_promotion_evidence_acquisition_objective_3200_candidate_index_actual_safetycost_claim_decision_plan_static_review
        ),
    )
...
def build_report(
    *,
    source_claim_decision_plan_artifact_dir: Path,
    source_claim_decision_plan_json: Path,
    source_claim_decision_plan_md: Path,
    source_claim_decision_plan_sha256s: Path,
    claim_decision_plan_script: Path,
    claim_decision_plan_test: Path,
    v14_audit_md: Path,
    ...
    artifact_dir = source_claim_decision_plan_artifact_dir.resolve()
    paths = {
        "source_claim_decision_plan_json": source_claim_decision_plan_json.resolve(),
        "source_claim_decision_plan_md": source_claim_decision_plan_md.resolve(),
        "source_claim_decision_plan_sha256s": source_claim_decision_plan_sha256s.resolve(),
        "claim_decision_plan_script": claim_decision_plan_script.resolve(),
        "claim_decision_plan_test": claim_decision_plan_test.resolve(),
        "v14_audit_md": v14_audit_md.resolve(),
        "current_status_md": current_status_md.resolve(),
    }
    files = _artifact_files(artifact_dir)
    source_plan = BASE_MODULE._read_json_dict(paths["source_claim_decision_plan_json"])
    ...
    nested_sha256s = BASE_MODULE._read_sha256sums(paths["source_claim_decision_plan_sha256s"])
    script_text = BASE_MODULE._read_text(paths["claim_decision_plan_script"])
    test_text = BASE_MODULE._read_text(paths["claim_decision_plan_test"])
    checks = _checks(
        enabled=enabled,
    ...
    return {
        ...
        "inputs": {
            "source_claim_decision_plan_artifact_dir": str(artifact_dir),
        ...
        "source_claim_decision_plan_summary": _source_claim_decision_plan_summary(source_plan),
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    source = report["source_claim_decision_plan_summary"]
    failed = decision["failed_checks"] or ["none"]
    return "\n".join(
        [
            "# Objective-3200 Candidate-Index Actual-SafetyCost Claim Decision Plan Static Review",
            "",
            ...
            f"- Claim decision item count: `{source['claim_decision_item_count']}`",
            "",
            "This static review authorizes only the next claim decision gate.",
            "",
        ]
    )
...
def _checks(
    *,
    ...
    claim_plan = _list(source_plan.get("claim_decision_plan"))
    checks = [
        BASE_MODULE._expect("static_review_enabled", enabled, True),
        BASE_MODULE._check("source_claim_decision_plan_artifact_dir_exists", artifact_dir.is_dir(), str(artifact_dir), "directory"),
        BASE_MODULE._expect("source_claim_decision_plan_json_path_matches_artifact", paths["source_claim_decision_plan_json"], files["plan_json"]),
        BASE_MODULE._expect("source_claim_decision_plan_md_path_matches_artifact", paths["source_claim_decision_plan_md"], files["plan_md"]),
        BASE_MODULE._expect("source_claim_decision_plan_sha256s_path_matches_artifact", paths["source_claim_decision_plan_sha256s"], files["plan_sha256s"]),
        ...
        BASE_MODULE._expect("audit_plan_ready", BASE_MODULE._latest_value(v14_text, "objective_3200_candidate_index_actual_safetycost_claim_decision_plan_ready"), "True"),
        BASE_MODULE._expect("audit_static_review_authorized", BASE_MODULE._latest_value(v14_text, "objective_3200_candidate_index_actual_safetycost_claim_decision_plan_static_review_authorized"), "True"),
        BASE_MODULE._expect("audit_safety_claim_false", BASE_MODULE._latest_value(v14_text, "safety_benefit_claim_authorized"), "False"),
        BASE_MODULE._expect("audit_camp_claim_false", BASE_MODULE._latest_value(v14_text, "camp_over_dp_top1_claim_authorized"), "False"),
        ...
        BASE_MODULE._expect("source_plan_static_review_authorized", decision.get("objective_3200_candidate_index_actual_safetycost_claim_decision_plan_static_review_authorized"), True),
        ...
        BASE_MODULE._expect("claim_decision_item_count", len(claim_plan), len(CLAIM_DECISION_ITEMS)),
        BASE_MODULE._expect("claim_decision_item_names", [item.get("item_name") for item in claim_plan], list(CLAIM_DECISION_ITEMS)),
        BASE_MODULE._expect("claim_decision_items_no_claim_execution", sorted({item.get("executes_claim") for item in claim_plan}), [False]),
        BASE_MODULE._expect("claim_decision_items_no_promotion", sorted({item.get("executes_promotion") for item in claim_plan}), [False]),
        BASE_MODULE._expect("claim_decision_items_no_deployment", sorted({item.get("executes_deployment") for item in claim_plan}), [False]),
        BASE_MODULE._check("plan_script_schema_token", PLAN_SCHEMA in script_text or "objective_3200_candidate_index_actual_safetycost_claim_decision_plan_v1" in script_text, PLAN_SCHEMA, "present"),
        BASE_MODULE._check("plan_script_static_review_next_token", AUTHORIZED_CURRENT_WORK in script_text or "candidate_index_actual_safetycost_claim_decision_plan_static_review_only" in script_text, AUTHORIZED_CURRENT_WORK, "present"),
        ...
        BASE_MODULE._check("plan_test_pass_test", "claim_decision_plan_passes" in test_text, "claim_decision_plan_passes", "present"),
        BASE_MODULE._check("plan_test_hash_drift_test", "rejects_hash_drift" in test_text, "rejects_hash_drift", "present"),
        BASE_MODULE._check("plan_test_claim_leak_test", "rejects_source_claim_leak" in test_text, "rejects_source_claim_leak", "present"),
    ]
...
def _source_claim_decision_plan_summary(source_plan: dict[str, Any]) -> dict[str, Any]:
    decision = BASE_MODULE._dict(source_plan.get("final_decision"))
    return {
        ...
        "claim_decision_item_count": len(_list(source_plan.get("claim_decision_plan"))),
    }
...
def _decision(*, passed: bool, checks: list[dict[str, Any]], source_plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if passed:
        ...
        failure_class = "explicit_candidate_index_actual_safetycost_claim_decision_plan_static_review_authorization_missing"
        ...
        failure_class = "source_claim_decision_plan_contract_failure"
    else:
        failure_class = "claim_decision_plan_static_review_contract_failure"
    return {
        "passed": bool(passed),
        ...
        "objective_3200_candidate_index_actual_safetycost_claim_decision_plan_static_review_passed": bool(passed),
        "objective_3200_candidate_index_actual_safetycost_claim_decision_authorized": bool(passed),
        "source_plan_passed": source_decision.get("passed"),
        "source_plan_ready": source_decision.get("objective_3200_candidate_index_actual_safetycost_claim_decision_plan_ready"),
