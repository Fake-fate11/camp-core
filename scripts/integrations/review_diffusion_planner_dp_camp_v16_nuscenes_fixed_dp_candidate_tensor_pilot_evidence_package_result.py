#!/usr/bin/env python3
"""Review the v16 fixed-DP pilot evidence-package construction result."""
...
def _load_construction_module():
    ...
    spec = importlib.util.spec_from_file_location("v16_pilot_evidence_package_construction", path)
...
READY_STATUS = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_evidence_package_result_review_passed"
REJECT_STATUS = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_evidence_package_result_review_rejected"
AUTHORIZED_NEXT_WORK = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_plan_only"
SCHEMA_VERSION = "dp_camp_v16_nuscenes_fixed_dp_candidate_tensor_pilot_evidence_package_result_review_v1"
...
REVIEW_JSON_NAME = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_evidence_package_result_review.json"
REVIEW_MD_NAME = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_evidence_package_result_review.md"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v16_nuscenes_fixed_dp_candidate_tensor_pilot_evidence_package_result_review",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        enabled=args.enable_v16_nuscenes_fixed_dp_candidate_tensor_pilot_evidence_package_result_review,
    )
...
def build_report(
    *,
    ...
    checks = [
        ...
        _expect("source_package_schema", manifest.get("schema_version"), "dp_camp_v16_nuscenes_fixed_dp_candidate_tensor_pilot_evidence_package_v1"),
        ...
        _expect("source_artifact_count", review["source_artifact_count"], 10),
        _expect("source_artifact_ids", review["source_artifact_ids"], EXPECTED_SOURCE_IDS),
        _expect("manifest_source_artifact_count", manifest.get("source_artifact_count"), 10),
        ...
        _expect("boundary_smoke_only", review["no_claim_boundary"].get("smoke_only"), True),
        _expect("boundary_scene_count_4", review["no_claim_boundary"].get("scene_count"), 4),
        _expect("boundary_calibration_rows_14", review["no_claim_boundary"].get("calibration_rows"), 14),
        _expect("boundary_holdout_rows_147", review["no_claim_boundary"].get("holdout_rows"), 147),
        ...
        _expect("primary_eval_rows_161", review["smoke_metrics_summary"].get("primary_eval_rows"), 161),
        _expect("better_tie_worse", review["smoke_metrics_summary"].get("better_tie_worse"), {"better": 158, "tie": 3, "worse": 0}),
        _approx("mean_delta", review["smoke_metrics_summary"].get("mean_delta"), -0.0729566626154565),
        _expect("ci95", review["smoke_metrics_summary"].get("ci95"), {"high": -0.03979996775908021, "low": -0.10611335747183279}),
        _approx("oracle_gap_closed", review["smoke_metrics_summary"].get("oracle_gap_closed"), 0.9993321161828008),
        _expect("report_has_smoke_only_scope", review["package_report_has_smoke_only_scope"], True),
        _expect("report_has_four_scene_limit", review["package_report_has_four_scene_limit"], True),
        _expect("report_has_calibration_holdout_limit", review["package_report_has_calibration_holdout_limit"], True),
        _expect("report_has_paired_eval_summary", review["package_report_has_paired_eval_summary"], True),
        _expect("report_has_no_claim_boundary", review["package_report_has_no_claim_boundary"], True),
        _expect("report_has_scaleup_recommendation", review["package_report_has_scaleup_recommendation"], True),
        _expect("no_claims_authorized", review["no_claims_authorized"], True),
        _expect("no_scaleup_execution_authorized", review["no_scaleup_execution_authorized"], True),
    ...
    return _stable(
        {
            ...
            "pilot_evidence_package_result_review": review,
            "checks": checks,
            "final_decision": {
                "passed": passed,
                "status": READY_STATUS if passed else REJECT_STATUS,
...
def _package_result_review(
    ...
    paired = summaries.get("paired_evaluation_result_review", {}).get("paired_evaluation_result_review", {})
    training = summaries.get("training_result_review", {}).get("training_result_review", {})
    split = summaries.get("split_result_review", {}).get("split_result_review", {})
    ...
    return {
        ...
        # ponytail: construction keeps full file paths and hashes in source_index, not duplicated in the compact manifest.
        ...
        "smoke_metrics_summary": manifest.get("smoke_metrics_summary", {}),
        "recommended_next_path": manifest.get("recommended_next_path", {}),
        "package_report_has_smoke_only_scope": "smoke-only" in report_lower,
        "package_report_has_four_scene_limit": "4 scenes" in report_lower,
        "package_report_has_calibration_holdout_limit": "calibration" in report_lower and "holdout" in report_lower,
        "package_report_has_paired_eval_summary": "paired eval" in report_lower or "paired-eval" in report_lower,
        ...
        "package_report_has_scaleup_recommendation": "scale-up plan" in report_lower and "10000" in report_lower,
        "no_claims_authorized": not any(manifest.get(field) for field in ("authorizes_claim", "authorizes_promotion", "authorizes_deployment")),
        "no_scaleup_execution_authorized": manifest.get("authorizes_scale_up_execution") is False,
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    metrics = review["smoke_metrics_summary"]
    return "\n".join(
        [
            ...
            f"- Scope: smoke-only, 4 scenes, calibration `{review['no_claim_boundary'].get('calibration_rows')}`, holdout `{review['no_claim_boundary'].get('holdout_rows')}`.",
            f"- Paired eval summary: `{metrics}`",
            "- Result-review only. No claim, promotion, deployment, or scale-up execution is authorized.",
            f"- K/candidate_count: `{review['k_candidate_count']}`",
            f"- Next recommended path: `{review['recommended_next_path']}`",
            "",
        ]
    )
...
def _approx(name: str, actual: Any, expected: Any, *, abs_tol: float = 1e-12) -> dict[str, Any]:
    try:
        ...
        passed = False
    return _check(name, passed, actual, expected)
