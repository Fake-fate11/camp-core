#!/usr/bin/env python3
"""Plan the objective-3200 fixed-candidate-index replay harness preflight.

This gate is read-only and plan-only. It consumes the audited failure
remediation static review and preregisters the contract for a future CAMP-side
candidate-index replay harness preflight. It does not implement the harness,
run replay, acquire outcomes, modify Diffusion Planner, promote, deploy, enable
an online selector, or make claims.
"""
...
def _load_source_review_module():
    review_path = Path(__file__).resolve().with_name(
        ...
        "failure_remediation_plan_static_contract.py"
    )
    spec = importlib.util.spec_from_file_location(
        "v14_objective_3200_failure_remediation_plan_static_review",
        review_path,
    )
...
BASE_MODULE = SOURCE_REVIEW_MODULE.PLAN_MODULE
...
OBJECTIVE_REQUIRED_RECORDS = BASE_MODULE.OBJECTIVE_REQUIRED_RECORDS
...
SCHEMA_VERSION = (
    ...
    "objective_3200_candidate_index_replay_harness_preflight_plan_v1"
)
...
READY_STATUS = (
    ...
    "candidate_index_replay_harness_preflight_plan_ready"
)
REJECT_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_replay_harness_preflight_plan_rejected"
)
AUTHORIZED_NEXT_WORK = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_replay_harness_preflight_plan_static_review_only"
)

PLAN_JSON_NAME = (
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_replay_harness_preflight_plan.json"
)
PLAN_MD_NAME = (
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_replay_harness_preflight_plan.md"
)

EXPECTED_HARNESS_PLAN_ITEMS = (
    "lock_source_remediation_static_review",
    "pre_register_candidate_index_binding_manifest",
    "pre_register_camp_side_evaluation_only_runner_surface",
    "pre_register_fixed_dp_candidate_tensor_identity_checks",
    "pre_register_candidate_index_replay_no_go_checks",
...
EXPECTED_NO_GO = (
    ...
    "direct_replay_execution_before_static_review",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v14_post_closeout_promotion_evidence_acquisition_objective_3200_candidate_index_replay_harness_preflight_plan",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        enabled=(
            args.enable_v14_post_closeout_promotion_evidence_acquisition_objective_3200_candidate_index_replay_harness_preflight_plan
        ),
    )
    ...
    *,
    ...
    return {
        ...
        "analysis": {
            "read_only": True,
            "plan_only": True,
            "candidate_index_replay_harness_preflight_plan_only": True,
        ...
        "harness_preflight_plan": _harness_preflight_plan(runner_surface),
        "candidate_index_binding_contract": _candidate_index_binding_contract(),
        "artifact_contract": _artifact_contract(),
        "no_go_register": [{"name": name} for name in EXPECTED_NO_GO],
        "plan_checks": checks,
        "final_decision": _decision(passed=passed, checks=checks, source_summary=source_summary, runner_surface=runner_surface),
    }
...
def _checks(
    *,
    ...
    checks = [
        _expect("harness_preflight_plan_enabled", enabled, True),
    ...
    checks.extend(
        [
            ...
            _expect("source_static_review_candidate_harness_plan_authorized", decision.get("objective_3200_outcome_acquisition_candidate_index_replay_harness_preflight_plan_authorized"), True),
            ...
            _expect("source_candidate_index_flag_present", source_summary["candidate_index_replay_flag_present"], False),
            _expect("runner_has_collect_closed_loop_flag", runner_surface["has_collect_closed_loop_outcomes_flag"], True),
            _expect("runner_has_compute_outcomes_import", runner_surface["has_compute_candidate_closed_loop_outcomes_import"], True),
            _expect("runner_current_candidate_index_replay_flag", runner_surface["has_candidate_index_replay_flag"], False),
            _expect("runner_logs_selected_index", runner_surface["logs_selected_index"], True),
            _expect("runner_logs_executed_output_policy_dp_top1", runner_surface["logs_executed_output_policy_dp_top1"], True),
        ]
    )
...
def _source_review_summary(source_review: dict[str, Any]) -> dict[str, Any]:
    decision = _dict(source_review.get("final_decision"))
    source_plan = _dict(source_review.get("source_plan_summary"))
    return {
        "passed": decision.get("passed"),
        "status": decision.get("status"),
        "authorized_next_work": decision.get("authorized_next_work"),
        "objective_required_records": int(source_plan.get("objective_required_records") or OBJECTIVE_REQUIRED_RECORDS),
        "candidate_closed_loop_outcome_records": int(source_plan.get("candidate_closed_loop_outcome_records") or 0),
        "missing_candidate_closed_loop_outcome_records": int(
            source_plan.get("missing_candidate_closed_loop_outcome_records") or OBJECTIVE_REQUIRED_RECORDS
        ),
        "candidate_index_replay_flag_present": bool(source_plan.get("candidate_index_replay_flag_present")),
    }
...
def _runner_surface(text: str) -> dict[str, Any]:
    ...
    return {
        "has_collect_closed_loop_outcomes_flag": "--camp_collect_closed_loop_outcomes" in text,
        "has_compute_candidate_closed_loop_outcomes_import": "compute_candidate_closed_loop_outcomes" in text,
        "has_candidate_index_replay_flag": any(flag in text for flag in candidate_index_flags),
        "candidate_index_flags_checked": list(candidate_index_flags),
        "logs_selected_index": "\"selected_index\"" in text or "'selected_index'" in text,
        "logs_shadow_selected_index": "shadow_selected_index" in text,
        "logs_executed_output_policy_dp_top1": "executed_output_policy" in text and "dp_top1" in text,
        "paper_faithful_boundary_present": "PAPER_FAITHFUL_BOUNDARY_ERROR" in text,
        "current_conclusion": "future_preflight_must_plan_or_add_camp_side_candidate_index_binding",
    }


def _harness_preflight_plan(runner_surface: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        {
            "name": "lock_source_remediation_static_review",
            "executes_harness": False,
            "purpose": "Use the passed remediation static review as the only source for continuing the 3200 objective.",
        },
        {
            "name": "pre_register_candidate_index_binding_manifest",
            "executes_harness": False,
            "purpose": "Define the per-record manifest binding record key, fixed DP candidate tensor digest, top1 index, and shadow_selected_index.",
        },
        {
            "name": "pre_register_camp_side_evaluation_only_runner_surface",
            "executes_harness": False,
            "purpose": "Plan a CAMP-side evaluation-only runner surface if no audited candidate-index replay flag exists.",
            "candidate_index_replay_flag_present": runner_surface["has_candidate_index_replay_flag"],
        },
        {
            "name": "pre_register_fixed_dp_candidate_tensor_identity_checks",
            "executes_harness": False,
            "purpose": "Require pre/post tensor hash equality and reject any DP code, config, weight, checkpoint, or candidate tensor mutation.",
        },
        {
            "name": "pre_register_candidate_index_replay_no_go_checks",
            "executes_harness": False,
            "purpose": "Fail closed on Full36, formal seeds 11/12/13, closed-loop training input, non-affine scoring, non-simplex weights, postselection, and claims.",
        },
        {
            "name": "authorize_static_review_only",
            "executes_harness": False,
            "purpose": "Review this plan before any harness implementation or replay execution gate.",
        },
    ]


def _candidate_index_binding_contract() -> dict[str, Any]:
    return {
        "required_records": OBJECTIVE_REQUIRED_RECORDS,
        "binding_source": "shadow_selected_index from audited default-off CAMP reranking over fixed DP candidates",
        "allowed_candidate_domain": "indices of the immutable fixed DP candidate tensor for the same record key",
        "top1_reference_index": 0,
        "candidate_tensor_identity": "pre-CAMP and post-CAMP selector tensor hashes must match",
        "closed_loop_outcome_usage": "offline evaluation evidence only",
...
def _decision(
    *,
    ...
    if passed:
        failure_class = None
    elif "harness_preflight_plan_enabled" in failed:
        failure_class = "explicit_candidate_index_replay_harness_preflight_plan_authorization_missing"
        ...
        failure_class = "candidate_index_replay_harness_preflight_plan_contract_failure"
    decision = {
        "passed": bool(passed),
        ...
        "objective_3200_candidate_index_replay_harness_preflight_plan_ready": bool(passed),
        "objective_3200_candidate_index_replay_harness_preflight_plan_static_review_authorized": bool(passed),
        ...
        "runtime_has_candidate_index_replay_flag": runner_surface["has_candidate_index_replay_flag"],
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# Objective-3200 Candidate-Index Replay Harness Preflight Plan",
            "",
            ...
            f"- Closed-loop outcome helper present: `{runner['has_compute_candidate_closed_loop_outcomes_import']}`",
            "",
            "This gate authorizes static review only; it does not implement or execute a harness.",
            "",
        ]
    )
