#!/usr/bin/env python3
"""Static-review the v16 fixed-DP scale-up paired-evaluation preflight plan."""
...
def _load_plan_module():
    ...
    spec = importlib.util.spec_from_file_location("v16_scaleup_paired_eval_preflight_plan", path)
...
READY_STATUS = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_paired_evaluation_preflight_plan_static_review_passed"
REJECT_STATUS = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_paired_evaluation_preflight_plan_static_review_rejected"
AUTHORIZED_NEXT_WORK = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_paired_evaluation_preflight_only"
SCHEMA_VERSION = "dp_camp_v16_nuscenes_fixed_dp_candidate_tensor_scaleup_paired_evaluation_preflight_plan_static_review_v1"
REVIEW_JSON_NAME = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_paired_evaluation_preflight_plan_static_review.json"
REVIEW_MD_NAME = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_paired_evaluation_preflight_plan_static_review.md"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v16_nuscenes_fixed_dp_candidate_tensor_scaleup_paired_evaluation_preflight_plan_static_review",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        enabled=args.enable_v16_nuscenes_fixed_dp_candidate_tensor_scaleup_paired_evaluation_preflight_plan_static_review,
    )
...
def build_report(
    *,
    ...
    checks = [
        ...
        _expect("calibration_rows_2156", rows.get("calibration"), PLAN_MODULE.EXPECTED_COUNTS["calibration"]),
        _expect("holdout_rows_1581", rows.get("holdout"), PLAN_MODULE.EXPECTED_COUNTS["holdout"]),
        _expect("primary_eval_rows_3737", rows.get("primary_eval_total"), 3737),
        ...
        _expect("train_reporting_only_rows_6263", rows.get("train_reporting_only"), PLAN_MODULE.EXPECTED_COUNTS["train"]),
        ...
        _expect("no_train_leakage_into_primary_eval", pass_fail.get("no_train_leakage_into_primary_eval"), True),
        ...
        _expect("scaleup_evidence_only", plan.get("scaleup_evidence_only"), True),
        ...
        _check(
            "claim_boundary_requires_execution_and_result_review",
            "no claim until execution and result review pass" in str(claims.get("reason", "")),
            claims.get("reason"),
            "no claim until execution and result review pass",
        ),
    ]
    ...
    return _stable(
        {
            ...
            "plan_static_review": {
                ...
                "scaleup_evidence_only": plan.get("scaleup_evidence_only"),
                "claims": claims,
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# V16 nuScenes Fixed-DP Scale-Up Paired-Evaluation Preflight Plan Static Review",
            "",
            ...
            f"- Scale-up evidence only: `{review['scaleup_evidence_only']}`",
            "",
        ]
    )
