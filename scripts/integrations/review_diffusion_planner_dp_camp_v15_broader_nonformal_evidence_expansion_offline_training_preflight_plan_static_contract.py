#!/usr/bin/env python3
"""Static-review the v15 offline training preflight plan."""
...
def _load_plan_module():
    path = Path(__file__).resolve().with_name(
        "plan_diffusion_planner_dp_camp_v15_broader_nonformal_evidence_expansion_offline_training_preflight.py"
    )
    spec = importlib.util.spec_from_file_location("v15_offline_training_preflight_plan", path)
...
SCHEMA_VERSION = "dp_camp_v15_broader_nonformal_evidence_expansion_offline_training_preflight_plan_static_review_v1"
AUTHORIZED_CURRENT_WORK = PLAN_MODULE.AUTHORIZED_NEXT_WORK
READY_STATUS = "v15_broader_nonformal_evidence_expansion_offline_training_preflight_plan_static_review_passed"
REJECT_STATUS = "v15_broader_nonformal_evidence_expansion_offline_training_preflight_plan_static_review_rejected"
AUTHORIZED_NEXT_WORK = "v15_broader_nonformal_evidence_expansion_offline_training_preflight_only"
REVIEW_JSON_NAME = "v15_broader_nonformal_evidence_expansion_offline_training_preflight_plan_static_review.json"
REVIEW_MD_NAME = "v15_broader_nonformal_evidence_expansion_offline_training_preflight_plan_static_review.md"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v15_broader_nonformal_evidence_expansion_offline_training_preflight_plan_static_review",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        enabled=args.enable_v15_broader_nonformal_evidence_expansion_offline_training_preflight_plan_static_review,
    )
...
def build_report(
    *,
    ...
    plan = source["offline_training_preflight_plan"]
    ...
    checks = [
        ...
        _check("plan_has_splits", set(plan["required_inputs"]["splits"]) == {"train", "calibration", "holdout"}, plan["required_inputs"]["splits"], "three splits"),
        ...
        _expect("plan_timing_required", plan["timing_contract"]["timing_required"], True),
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# V15 Offline Training Preflight Plan Static Review",
            "",
