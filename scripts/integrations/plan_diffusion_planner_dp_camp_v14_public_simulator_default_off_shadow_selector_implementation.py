#!/usr/bin/env python3
"""Plan-only v14 default-off shadow selector implementation gate.

This gate reads the v14 default-off shadow selector static integration
contract plan and current source surfaces. It plans a future implementation
without editing selector wiring, training, replaying, generating candidates,
modifying DP, promoting, deploying, or making safety/CAMP-over-DP claims.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        label=args.label,
        enabled=args.enable_v14_default_off_shadow_selector_implementation_plan,
    )
...
def build_report(
    *,
    ...
    source_plan = _read_json_dict(static_contract_plan_json)
    ...
    return {
        ...
        "analysis": {
            ...
            "static_contract_plan_json": str(static_contract_plan_json.resolve()),
        ...
        "static_review_requirements": _static_review_requirements(),
...
def write_outputs(output_dir: Path, report: dict[str, Any]) -> None:
    ...
    _write_json(output_dir / "default_off_shadow_selector_implementation_plan.json", report)
    (output_dir / "default_off_shadow_selector_implementation_plan.md").write_text(
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# V14 Default-Off Shadow Selector Implementation Plan",
        "",
        ...
        f"- Selector phase: `{plan['selector_phase']}`",
        f"- Runtime effect: `{plan['runtime_effect']}`",
        f"- Selection rule: `{plan['selection_rule']}`",
        f"- Fail-closed policy: `{plan['fail_closed_policy']}`",
        "",
        "## Future Target Files",
        "",
    ]
    for item in plan["future_target_files"]:
        lines.append(f"- `{item}`")
    lines.extend(["", "## Required Steps", ""])
    for item in plan["required_steps"]:
        lines.append(f"- `{item}`")
    lines.extend(["", "## Static Review Requirements", ""])
    for item in report["static_review_requirements"]:
        lines.append(f"- `{item}`")
...
def _source_plan_checks(payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        _expect("source_static_contract_plan_status_ready", decision.get("status"), SOURCE_PLAN_STATUS),
        _expect("source_static_contract_plan_passed", decision.get("passed"), True),
        _expect("source_static_contract_plan_failed_checks_empty", decision.get("failed_checks"), []),
        _expect(
            "source_static_contract_authorizes_this_plan",
            decision.get("authorized_next_work"),
            SOURCE_AUTHORIZED_NEXT_WORK,
        ),
        _expect("source_static_contract_plan_ready", decision.get("static_contract_plan_ready"), True),
        _expect(
        ...
        _expect("source_contract_status", contract.get("status"), "plan_ready_no_implementation"),
        ...
        _expect(
            "source_contract_candidate_source",
            contract.get("candidate_source"),
        ...
        _expect("source_contract_kill_switch_required", contract.get("kill_switch_required"), True),
        _expect(
            "source_contract_trajectory_mutation_forbidden",
            contract.get("trajectory_mutation_authorized"),
            False,
        ),
        _expect(
            "source_contract_postselection_forbidden",
            contract.get("postselection_authorized"),
            False,
        ),
        _expect(
            "source_contract_formal_seed_usage_forbidden",
            contract.get("formal_seed_usage_authorized"),
            False,
        ),
        *[_expect(f"source_static_contract_{name}_false", decision.get(name), False) for name in BLOCKED_ACTIONS],
    ]
...
def _source_summary(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "source_training_records": source.get("training_records"),
        "source_num_candidates": source.get("num_candidates"),
...
def _implementation_plan() -> dict[str, Any]:
    return {
        ...
        "fail_closed_policy": (
            ...
            "nonfinite scores, source mismatch, or unavailable candidates, "
            "execute DP Top-1 and log no shadow selection"
        ),
        "future_target_files": [
            ...
            "camp_core/tests/test_diffusion_planner_v14_iteration_audit.py",
        ],
        "required_steps": [
            ...
            "derive shadow_selected_index with argmin over finite feasible candidate rows",
...
def _failure_class(failed: list[str]) -> str:
    ...
    if "plan_enabled" in failed_set:
        return "explicit_implementation_plan_authorization_missing"
    if {"audit_latest_status", "audit_latest_next_work"} & failed_set:
        return "v14_eof_contract_mismatch"
    if any(name.startswith("source_static_contract") or name.startswith("source_contract") for name in failed):
        return "source_static_contract_plan_failure"
    ...
    return "default_off_shadow_selector_implementation_plan_failure"
