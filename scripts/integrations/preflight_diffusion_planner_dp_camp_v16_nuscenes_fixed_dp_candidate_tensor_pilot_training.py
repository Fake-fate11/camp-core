#!/usr/bin/env python3
"""Preflight the v16 fixed-DP pilot static CAMP training gate."""
...
def _load_source_review_module():
    ...
    spec = importlib.util.spec_from_file_location("v16_pilot_training_preflight_plan_static_review", path)
...
READY_STATUS = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_training_preflight_ready"
REJECT_STATUS = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_training_preflight_rejected"
AUTHORIZED_NEXT_WORK = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_training_execution_only"
SCHEMA_VERSION = "dp_camp_v16_nuscenes_fixed_dp_candidate_tensor_pilot_training_preflight_v1"
PREFLIGHT_JSON_NAME = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_training_preflight.json"
PREFLIGHT_MD_NAME = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_training_preflight.md"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v16_nuscenes_fixed_dp_candidate_tensor_pilot_training_preflight",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        enabled=args.enable_v16_nuscenes_fixed_dp_candidate_tensor_pilot_training_preflight,
    )
...
def build_report(
    *,
    ...
    plan_body = plan.get("pilot_training_preflight_plan", {})
    ...
    checks = [
        ...
        _expect("train_records_863", record_summary["counts"]["train"], PLAN_MODULE.EXPECTED_COUNTS["train"]),
        _expect("calibration_records_14", record_summary["counts"]["calibration"], PLAN_MODULE.EXPECTED_COUNTS["calibration"]),
        _expect("holdout_records_147", record_summary["counts"]["holdout"], PLAN_MODULE.EXPECTED_COUNTS["holdout"]),
        _expect("plan_train_records_863", plan_body.get("training_inputs", {}).get("train_records"), PLAN_MODULE.EXPECTED_COUNTS["train"]),
        _expect("review_train_records_863", review_body.get("train_records"), PLAN_MODULE.EXPECTED_COUNTS["train"]),
        ...
        _expect("training_command_uses_train_only", str(source_train_records_jsonl) in command and "calibration_records.jsonl" not in command and "holdout_records.jsonl" not in command, True),
    ...
    return _stable(
        {
            ...
            "pilot_training_preflight": preflight,
            "checks": checks,
            "final_decision": {
                "passed": passed,
                "status": READY_STATUS if passed else REJECT_STATUS,
