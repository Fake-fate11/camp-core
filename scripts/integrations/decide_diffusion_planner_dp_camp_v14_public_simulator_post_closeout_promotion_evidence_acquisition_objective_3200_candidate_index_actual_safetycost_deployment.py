#!/usr/bin/env python3
"""Audited deployment decision for objective-3200 SafetyCost evidence."""
...
def _load_source_review_module():
    review_path = Path(__file__).resolve().with_name(
        ...
        "actual_safetycost_deployment_decision_plan_static_contract.py"
    )
    spec = importlib.util.spec_from_file_location(
        "v14_candidate_index_actual_safetycost_deployment_decision_plan_static_review",
        review_path,
    )
...
SCHEMA_VERSION = (
    ...
    "objective_3200_candidate_index_actual_safetycost_deployment_decision_v1"
)
READY_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_deployment_decision_passed"
)
REJECT_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_deployment_decision_rejected"
)
AUTHORIZED_NEXT_WORK = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_online_selector_activation_decision_plan_only"
)
DECISION_JSON_NAME = (
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_deployment_decision.json"
)
DECISION_MD_NAME = (
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_deployment_decision.md"
)
EXPECTED_SOURCE_REVIEW_CHECK_COUNT = 83
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v14_post_closeout_promotion_evidence_acquisition_objective_3200_candidate_index_actual_safetycost_deployment_decision",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        enabled=args.enable_v14_post_closeout_promotion_evidence_acquisition_objective_3200_candidate_index_actual_safetycost_deployment_decision,
    )
...
def build_report(
    *,
    ...
    deployment_record = _deployment_record(source_review)
    checks = _checks(
        enabled=enabled,
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    record = report["deployment_record"]
    return "\n".join(
        [
            "# Objective-3200 Candidate-Index Actual-SafetyCost Deployment Decision",
            "",
...
def _checks(
    *,
    ...
    checks = [
        BASE_MODULE._expect("deployment_decision_enabled", enabled, True),
        ...
        BASE_MODULE._expect("audit_deployment_decision_authorized", BASE_MODULE._latest_value(v14_text, "objective_3200_candidate_index_actual_safetycost_deployment_decision_authorized"), "True"),
        ...
        BASE_MODULE._expect("source_deployment_decision_authorized", decision.get("objective_3200_candidate_index_actual_safetycost_deployment_decision_authorized"), True),
        ...
        BASE_MODULE._expect("deployment_record_authorizes_deployment", deployment_record.get("deployment_authorized"), True),
        BASE_MODULE._expect("deployment_record_online_false", deployment_record.get("online_selector_change_authorized"), False),
        BASE_MODULE._check("deployment_scope_mentions_default_off", "default-off" in deployment_record.get("scope_limit", ""), deployment_record.get("scope_limit"), "default-off"),
    ]
...
def _deployment_record(source_review: dict[str, Any]) -> dict[str, Any]:
    source_decision = BASE_MODULE._dict(source_review.get("final_decision"))
    return {
        "decision_text": "Authorize default-off deployment packaging for the audited objective-3200 fixed-DP candidate-tensor selector.",
        "scope_limit": "This deployment decision is default-off and does not authorize online selector activation, trajectory generation, or DP modification.",
...
def _source_static_review_summary(source_review: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "deployment_decision_authorized": decision.get("objective_3200_candidate_index_actual_safetycost_deployment_decision_authorized"),
    }
...
def _decision(*, passed: bool, checks: list[dict[str, Any]], source_review: dict[str, Any]) -> dict[str, Any]:
    ...
    if passed:
        failure_class = None
    elif "deployment_decision_enabled" in failed:
        failure_class = "explicit_candidate_index_actual_safetycost_deployment_decision_authorization_missing"
        ...
        failure_class = "source_deployment_plan_static_review_contract_failure"
    else:
        failure_class = "deployment_decision_contract_failure"
    return {
        "passed": bool(passed),
        ...
        "objective_3200_candidate_index_actual_safetycost_deployment_decision_passed": bool(passed),
        "objective_3200_candidate_index_actual_safetycost_online_selector_activation_decision_plan_authorized": bool(passed),
        "source_static_review_passed": source_decision.get("passed"),
        "selector_promotion_authorized": source_decision.get("selector_promotion_authorized"),
        "deployment_authorized": bool(passed),
        "online_selector_change_authorized": False,
        "safety_benefit_claim_authorized": source_decision.get("safety_benefit_claim_authorized"),
        "camp_over_dp_top1_claim_authorized": source_decision.get("camp_over_dp_top1_claim_authorized"),
    }
