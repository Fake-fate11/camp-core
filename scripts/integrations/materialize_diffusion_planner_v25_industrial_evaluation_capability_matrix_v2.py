...
from camp_core.integrations.diffusion_planner_v25_industrial_evaluation_contract_v2 import (  # noqa: E402
    ...
    validate_capability_matrix_v2,
    validate_evaluation_contract_v2,
)
...
def materialize_v2(
    ...
    verify_complete_seal(review_dir, review_root, label="industrial v2 contract review")
    review = object_from(review_dir / "report.json")
    if review.get("status") != (
        "passed_independent_literal_industrial_evaluation_contract_review_v2"
    ):
        raise ValueError("industrial v2 contract review did not pass")
    contract = validate_evaluation_contract_v2(
        object_from(contract_dir / "report.json")["contract"]
    )
    matrix = validate_capability_matrix_v2(
        capability_matrix_v2(contract, source_dirs), contract, source_dirs
    )
    head = git_head()
    report = {
        "schema_version": "camp_dp_v25_industrial_oriented_evaluation_capability_artifact_v2",
        ...
        "source_dirs": {key: str(value.resolve()) for key, value in sorted(source_dirs.items())},
    ...
    return write_atomic(
        ...
        report,
        {
            "role": "industrial_evaluation_scalar_leaf_capability_audit_v2",
            "implementation_head": head,
            "contract_root_sha256": contract_root,
            "contract_review_root_sha256": review_root,
        },
        label="V25 industrial evaluation scalar leaf capability audit v2",
    )
...
def main() -> int:
    ...
    source_dirs = {
        key: getattr(args, f"{key}_dir") for key in SEALED_SOURCES
    }
    print(
        materialize_v2(
            args.output,
