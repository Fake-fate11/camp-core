...
PAYLOAD_AUDIT = (
    ROOT / "scripts/integrations/analyze_diffusion_planner_progress_support_logging_smoke.py"
)
...
READY_STATUS = "progress_support_logging_nonformal_smoke_plan_ready"
REJECT_STATUS = "progress_support_logging_nonformal_smoke_plan_rejected"
SOURCE_STATUS = "progress_support_logging_implementation_unit_tested"


@dataclass(frozen=True)
class SmokeSpec:
    root: str = "/root/autodl-tmp/camp_dp_progress_support_logging_smoke"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Design-only preflight for the first default-off progress-support "
            "logging smoke. It emits paired nonformal replay commands and does "
    ...
    *,
    ...
    decision = {
        ...
        "authorized_next_work": (
            "default_off_progress_support_logging_paired_three_step_smoke_only"
            if passed
            else None
        ),
    ...
    return {
        "analysis": {
            "name": "dp_camp_progress_support_logging_nonformal_smoke_plan_v1",
            "label": label,
            ...
            "math_boundary": (
                ...
                "fixed finite-candidate progress-support descriptors and "
                "nonnegative candidate atom coefficients. It must not change "
...
def _source_checks(
    *,
    ...
    return [
        _check_tokens(
            "replay_default_off_progress_support_logging_cli",
            replay_text,
            (
        ...
        _check_tokens(
            ...
            payload_text,
            (
                "dp_camp_progress_support_logging_smoke_audit_v1",
...
def _plan_checks(smoke: SmokeSpec) -> list[dict[str, Any]]:
    ...
    return [
        ...
        {
            "name": "progress_support_horizon_valid",
            "passed": smoke.progress_support_steps >= 2,
            "value": smoke.progress_support_steps,
        },
        {
            "name": "progress_support_dt_valid",
            "passed": smoke.progress_support_dt_s > 0.0,
    ...
    if logging:
        command.extend(
            [
                ...
                str(smoke.progress_support_steps),
                "--camp_progress_support_dt_s",
                str(smoke.progress_support_dt_s),
            ]
        )
    return command
...
def _payload_audit_command(
    ...
    return [
        ...
        str(smoke.num_candidates),
        "--output_json",
        f"{audit_dir}/progress_support_logging_smoke.json",
        "--output_md",
        f"{audit_dir}/progress_support_logging_smoke.md",
        "--require_pass",
    ]
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Progress-Support Logging Smoke Plan",
        "",
