...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Audit a paired default-off progress+lane/hard context logging "
            "smoke. The baseline run must keep logging disabled and the "
            "candidate run must enable context logging without collecting "
...
def analyze(
    *,
    ...
    return {
        "analysis": {
            "name": "dp_camp_progress_lane_hard_context_logging_smoke_audit_v1",
            ...
            "math_boundary": (
                "Progress+lane/hard context payloads are current-tick "
                "finite-candidate geometry diagnostics. Logged atoms are fixed "
                "nonnegative candidate coefficients and preserve affine CAMP "
                "scores in weights."
            ),
        },
        "counts": {
...
def _discover_logs(root: Path) -> dict[str, Path]:
    if not root.is_dir():
        return {}
    return {str(path.parent.relative_to(root)): path for path in root.rglob(LOG_NAME)}
...
def _validate_summary(
    ...
    *,
    ...
    if not isinstance(metadata, dict):
        errors.append(f"{key}: missing {SUMMARY_KEY} summary")
        return
    expected = {
        "schema_version": PROGRESS_LANE_HARD_CONTEXT_LOGGING_SCHEMA_VERSION,
    ...
    if expected_enabled and int(metadata.get("records", -1)) <= 0:
        errors.append(f"{key}: enabled summary has no context records")
...
def _validate_payload(
    ...
    *,
    ...
    if not isinstance(finite_checks, dict):
        ...
        missing = sorted(set(PROGRESS_LANE_HARD_CONTEXT_FIELD_NAMES) - set(finite_checks))
        if missing:
    ...
    if payload.get(ATOM_NAMES_KEY) != list(PROGRESS_LANE_HARD_CONTEXT_ATOM_NAMES):
        errors.append(f"{prefix}: {ATOM_NAMES_KEY} mismatch")
    atoms = np.asarray(payload.get(ATOMS_KEY), dtype=np.float64)
    if atoms.shape != (
        int(expected_candidates),
        len(PROGRESS_LANE_HARD_CONTEXT_ATOM_NAMES),
    ):
        errors.append(f"{prefix}: {ATOMS_KEY} shape={list(atoms.shape)}")
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Progress+Lane/Hard Context Logging Smoke Audit",
        "",
