#!/usr/bin/env python3
"""Static review for the v14 runtime manifest materialization plan-only gate."""
...
REJECT_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_"
    "runtime_artifact_manifest_materialization_static_contract_review_rejected"
)
DISABLED_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_"
    "runtime_artifact_manifest_materialization_static_contract_review_default_off_disabled"
)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--runtime_artifact_manifest_materialization_plan_json", type=Path, required=True)
    parser.add_argument("--runtime_artifact_manifest_materialization_plan_script_py", type=Path, required=True)
    parser.add_argument("--runtime_artifact_manifest_materialization_plan_test_py", type=Path, required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        runtime_artifact_manifest_materialization_plan_script_py=(
            args.runtime_artifact_manifest_materialization_plan_script_py
        ),
        runtime_artifact_manifest_materialization_plan_test_py=(
            args.runtime_artifact_manifest_materialization_plan_test_py
        ),
        ...
        label=args.label,
        enabled=(
            args.enable_v14_public_simulator_runtime_artifact_manifest_materialization_static_contract_review
        ),
    )
...
def build_report(
    *,
    ...
    runtime_artifact_manifest_materialization_plan_script_py: Path,
    runtime_artifact_manifest_materialization_plan_test_py: Path,
    ...
    paths = {
        "runtime_artifact_manifest_materialization_plan": (
            runtime_artifact_manifest_materialization_plan_json
        ),
        "runtime_artifact_manifest_materialization_plan_script": (
            runtime_artifact_manifest_materialization_plan_script_py
        ),
        "runtime_artifact_manifest_materialization_plan_test": (
            runtime_artifact_manifest_materialization_plan_test_py
        ),
    ...
    plan_payload = _dict(payloads.get("runtime_artifact_manifest_materialization_plan"))
...
def write_outputs(output_dir: Path, report: dict[str, Any]) -> None:
    ...
    _write_json(
        output_dir / "runtime_artifact_manifest_materialization_static_contract_review.json",
        report,
    )
    (
        output_dir
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# V14 Runtime Manifest Materialization Static Contract Review",
        "",
        f"- Status: `{decision['status']}`",
        f"- Passed: `{decision['passed']}`",
        f"- Authorized next work: `{decision['authorized_next_work']}`",
        f"- Materialization implementation plan authorized: `{decision['runtime_artifact_manifest_materialization_implementation_plan_authorized']}`",
        ...
        f"- Runtime manifest written by source plan: `{summary.get('runtime_manifest_written_by_this_gate')}`",
    ...
    *,
    ...
    return {
        ...
        "analysis": {
            ...
            "required_dp_head": required_dp_head,
            "math_boundary": (
                "The source materialization plan must be a plan artifact only. "
                "The future runtime manifest may pin immutable paths and "
                "hashes, but CAMP remains fixed-DP-candidate reranking with "
                "affine score_k(w)=a_k^T w and executed output DP Top-1."
            ),
        },
        "source_hashes": {},
        "contract_summary": {},
        ...
        "final_decision": {
            ...
            "runtime_artifact_manifest_materialization_static_contract_review_passed": False,
            "runtime_artifact_manifest_materialization_implementation_plan_authorized": False,
            ...
            "failed_checks": [],
            "failure_class": "materialization_static_review_gate_disabled",
        },
    }
...
def _source_plan_checks(payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    checks = [
        ...
        _expect("source_plan_ready_flag", decision.get("runtime_artifact_manifest_materialization_plan_ready"), True),
        _expect(
            "source_plan_static_review_authorized",
            decision.get("runtime_artifact_manifest_materialization_static_contract_review_authorized"),
            True,
        ),
        ...
        _check(
            "source_plan_planned_path_absent",
        ...
        _expect("source_plan_runtime_manifest_written_false", plan.get("runtime_manifest_written_by_this_gate"), False),
        _expect("source_plan_runtime_manifest_materialized_false", plan.get("runtime_manifest_materialized_by_this_gate"), False),
        _expect("source_plan_runtime_execution_enabled_false", plan.get("runtime_execution_enabled_by_this_gate"), False),
        ...
        _expect("future_selection_effect_false", future.get("selection_effect"), False),
        _expect("future_online_selector_change_false", future.get("online_selector_change"), False),
        ...
        _expect("future_dp_head_at_plan", future.get("dp_head_at_plan"), FIXED_DP_HEAD),
        ...
        _expect("future_atom_alias", aliases.get("atom_scales"), atom_entry.get("sha256")),
        _expect("future_weight_alias", aliases.get("static_weights"), weights_entry.get("sha256")),
        _contains_in_list("future_runtime_invocation_manifest_path", plan.get("future_runtime_invocation_template"), "--camp_shadow_artifact_manifest"),
        _contains_in_list("future_runtime_invocation_atom_hash", plan.get("future_runtime_invocation_template"), "--camp_shadow_expected_atom_scales_sha256"),
        _contains_in_list("future_runtime_invocation_weight_hash", plan.get("future_runtime_invocation_template"), "--camp_shadow_expected_static_weights_sha256"),
    ]
...
def _source_surface_checks(texts: dict[str, str]) -> list[dict[str, Any]]:
    script = texts.get("runtime_artifact_manifest_materialization_plan_script", "")
    test = texts.get("runtime_artifact_manifest_materialization_plan_test", "")
    runner = texts.get("replay_runner", "")
    return [
        _contains("script_schema_constant", script, "SCHEMA_VERSION"),
        _contains_all(
            "script_v14_materialization_plan_schema",
            script,
            (
                "dp_camp_v14_public_simulator_default_off_shadow_selector_",
                "runtime_artifact_manifest_materialization_plan_v1",
        ...
        _contains("script_blocks_manifest_write", script, '"runtime_manifest_written_by_this_gate": False'),
        _contains("script_blocks_manifest_materialization", script, '"runtime_manifest_materialized_by_this_gate": False'),
        _contains("script_future_artifacts", script, '"artifacts": {'),
        _contains("script_atom_scales_entry", script, '"atom_scales"'),
        _contains("script_static_weights_entry", script, '"static_weights"'),
        ...
        _contains("test_ready_case", test, "test_runtime_artifact_manifest_materialization_plan_ready_without_writing_manifest"),
        _contains("test_disabled_case", test, "test_runtime_artifact_manifest_materialization_plan_is_disabled_until_enabled"),
        _contains("test_rejects_authorization_leak", test, "test_runtime_artifact_manifest_materialization_plan_rejects_authorization_leak"),
        _contains("test_rejects_planned_path_drift", test, "test_runtime_artifact_manifest_materialization_plan_rejects_planned_path_drift"),
        _contains("test_rejects_wrong_eof", test, "test_runtime_artifact_manifest_materialization_plan_rejects_wrong_eof"),
...
def _audit_contract_checks(v14_text: str, status_text: str) -> list[dict[str, Any]]:
    ...
    return [
        _check(
            ...
            "pending materialization static-review gate or completed review gate",
        ),
        _check(
            "current_status_boundary_matches_materialization_static_review_gate",
            status_pending or status_complete,
            {"pending": status_pending, "complete": status_complete},
            "pending materialization static-review gate or completed review gate",
        ),
        _contains("audit_records_materialization_plan_ready", eof, "runtime_artifact_manifest_materialization_plan_ready=True"),
        _contains("audit_authorizes_materialization_static_review", eof, "runtime_artifact_manifest_materialization_static_contract_review_authorized=True"),
...
def _contract_summary(
    ...
    return {
        ...
        "runtime_manifest_written_by_this_gate": plan.get("runtime_manifest_written_by_this_gate"),
...
def _review_scope() -> list[str]:
    return [
        "prove the source output is a materialization plan and not a runtime manifest",
        "prove no runtime manifest file was written by the plan-only gate",
        "prove the future manifest pins the v14 schema, public simulator source scope, K=8, and 9 approved atoms",
        "prove future runtime entries are exactly atom_scales and static_weights with sha256 aliases",
...
def _forbidden_paths() -> list[str]:
    return [
        "writing the runtime manifest in this static-review gate",
        "using the source materialization plan JSON as a runtime manifest",
...
def _decision(passed: bool, checks: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "runtime_artifact_manifest_materialization_static_contract_review_passed": bool(passed),
        "runtime_artifact_manifest_materialization_implementation_plan_authorized": bool(passed),
        "runtime_artifact_manifest_materialization_authorized": False,
        "default_off_shadow_selector_runtime_execution_authorized": False,
        "training_authorized": False,
        "training_execution_authorized": False,
...
def _failure_class(failed: list[str]) -> str:
    ...
    return "runtime_artifact_manifest_materialization_static_contract_failure"
