...
from scripts.integrations.analyze_diffusion_planner_progress_lane_hard_joint_descriptor_separability import (  # noqa: E402
    ...
    REJECT_STATUS as JOINT_SEPARABILITY_REJECT_STATUS,
    _lane_hard_candidate_rows,
    _lane_hard_descriptor_specs,
    _load_json,
    _merge_record_rows,
    _path_seeds,
    _progress_candidate_rows,
    _progress_descriptor_specs,
)


READY_STATUS = "progress_lane_hard_joint_separability_bottleneck_diagnosed"
SOURCE_BLOCKED_STATUS = (
    "progress_lane_hard_joint_separability_bottleneck_source_not_ready"
)

SOURCE_PRIMARY_GAP = "progress_lane_hard_joint_descriptors_do_not_separate_candidates"
SOURCE_NEXT_WORK = "diagnose_progress_lane_hard_joint_descriptor_bottleneck_before_retraining"
NEXT_WORK = "reject_current_joint_support_descriptor_family_or_preflight_new_default_off_state_logging"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Read-only bottleneck diagnosis for a rejected progress+lane/hard "
            "joint descriptor separability screen. It reuses existing matched "
    ...
    parser.add_argument("--joint_separability_json", type=Path, required=True)
...
def main() -> None:
    ...
    report = analyze(
        paths,
        joint_separability_report=_load_json(args.joint_separability_json),
        label=args.label,
...
def analyze(
    paths: list[Path],
    *,
    joint_separability_report: dict[str, Any],
    label: str | None = None,
    ...
    return analyze_records(
        items,
        joint_separability_report=joint_separability_report,
        label=label,
...
def analyze_records(
    ...
    *,
    ...
    source = _source_gate(joint_separability_report)
    best = _best_screen(joint_separability_report)
    rows, formal_seed_records, descriptor_families = _rows_from_items(items)
    ...
    tradeoff = _screen_tradeoff(joint_separability_report)
    ...
    final = {
        ...
        "authorized_next_work": (
            ...
            else "fix_joint_separability_source_before_bottleneck_diagnosis"
        ),
        **{key: False for key in BLOCKED_ACTIONS},
    }
    return {
        "analysis": {
            "name": "dp_camp_progress_lane_hard_joint_separability_bottleneck_v1",
            "label": label,
            "training": False,
            ...
            "predeclared_hypothesis": (
                "The rejected joint descriptor family is bottlenecked by "
                "descriptor/atom overlap: strict safe thresholds overblock "
                "beneficial candidates, while high-retain thresholds would "
                "allow harmful candidates."
            ),
            "descriptor_families": descriptor_families,
            "math_boundary": (
                "The diagnosis reuses fixed current-tick progress-support "
                "and lane/hard support descriptors plus the rejected offline "
                "screen. Candidate outcomes explain offline error modes only; "
                "they are not runtime selector inputs. If these descriptors "
                "are later atomized, they remain fixed coefficients in an "
                "affine score_k(w)=a_k^T w for the simplex/CVaR/L2 convex "
                "master. No DP-side classical Benders master/subproblem, "
                "dual, or cut is introduced."
            ),
        },
        "source_separability_gate": source,
        "source_records": _source_records(joint_separability_report),
        "best_screen": best,
...
def _rows_from_items(
    ...
    return rows, formal_seed_records, {
        "progress_support": [spec.name for spec in progress_specs],
        "lane_hard_support": [spec.name for spec in lane_specs],
    }
...
def _source_gate(report: dict[str, Any]) -> dict[str, Any]:
    ...
    passed = (
        ...
        and status == JOINT_SEPARABILITY_REJECT_STATUS
...
def _diagnosis(
    ...
    if strict_overblocks and high_retain_allows_harmful:
        ...
        primary = "strict_joint_screen_overblocks_beneficial"
    elif high_retain_allows_harmful:
        primary = "high_retain_joint_screens_allow_harmful"
    else:
        primary = "joint_descriptor_overlap_or_missing_state_requires_review"
    return {
        "primary_gap": primary,
        ...
        "current_joint_descriptor_family_recommended_for_certificate": False,
        "camp_retraining_recommended": False,
        "next_evidence_needed": (
            "new no-leak state/support descriptors or a default-off logging "
            "preflight, not CAMP retraining from the rejected descriptor family"
        ),
    }
...
def _descriptor_overlap(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for name in descriptors:
        ...
        summaries.append(
            {
                "descriptor": name,
                "family": _descriptor_family(name),
                "beneficial": bq,
                "harmful": hq,
...
def _summary_for_rows(
    ...
    return {
        ...
        "dominant_contribution_family_counts": _dominant_contribution_family_counts(rows),
        "reason_counts": _reason_counts(rows),
        "top_examples": _examples(rows, names),
    }


def _dominant_contribution_family_counts(rows: list[dict[str, Any]]) -> dict[str, int]:
    counts: dict[str, int] = {}
    for row in rows:
        ...
        if not contributions:
            ...
            name = max(contributions, key=lambda key: abs(float(contributions[key])))
            family = _descriptor_family(str(name))
        counts[family] = counts.get(family, 0) + 1
    return dict(sorted(counts.items()))


def _descriptor_family(name: str) -> str:
    progress_names = {spec.name for spec in _progress_descriptor_specs()}
    lane_names = {spec.name for spec in _lane_hard_descriptor_specs()}
    if name in progress_names:
        return "progress_support"
    if name in lane_names:
        return "lane_hard_support"
    return "unknown"
...
def _reasons(row: dict[str, Any]) -> list[str]:
    ...
    if not reasons:
        reasons.append("beneficial_or_neutral_support_overlap")
    return reasons
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Progress + Lane/Hard Joint Separability Bottleneck Diagnosis",
        "",
