#!/usr/bin/env python3
"""Execute the v16 fixed-DP pilot paired-evaluation gate."""
...
AUTHORIZED_NEXT_WORK = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_paired_evaluation_result_review_only"
READY_STATUS = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_paired_evaluation_execution_passed"
REJECT_STATUS = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_paired_evaluation_execution_rejected"
SCHEMA_VERSION = "dp_camp_v16_nuscenes_fixed_dp_candidate_tensor_pilot_paired_evaluation_execution_v1"
EXECUTION_JSON_NAME = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_paired_evaluation_execution.json"
EXECUTION_MD_NAME = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_paired_evaluation_execution.md"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--source_pilot_corpus_artifact_dir", type=Path, required=True)
    ...
    parser.add_argument(
        "--enable_v16_nuscenes_fixed_dp_candidate_tensor_pilot_paired_evaluation_execution",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = run_execution(
        ...
        source_pilot_corpus_artifact_dir=args.source_pilot_corpus_artifact_dir,
        ...
        enabled=args.enable_v16_nuscenes_fixed_dp_candidate_tensor_pilot_paired_evaluation_execution,
        command=sys.argv,
    )
...
def build_report(
    *,
    ...
    source_pilot_corpus_artifact_dir: Path,
    v16_audit_md: Path,
    ...
    source_artifacts = {
        ...
        "pilot_corpus": _source_artifact(source_pilot_corpus_artifact_dir, expected_pilot_corpus_root_sha256),
    }
    primary_splits = preflight.get("pilot_paired_evaluation_preflight", {}).get("primary_eval_splits", [])
    reporting_splits = preflight.get("pilot_paired_evaluation_preflight", {}).get("reporting_only_splits", [])
    primary_source_records = _tag_records(calibration_records, "calibration") + _tag_records(holdout_records, "holdout")
    enriched_records, atom_derivation = _enrich_eval_records(primary_source_records)
    paired_rows, latency = _paired_rows(enriched_records, model, current_camp_head, current_dp_head)
    split_metrics = _split_metrics(paired_rows)
    record_summary = _record_summary(train_records, calibration_records, holdout_records, paired_rows)
    checks = [
        ...
        _expect(
            "source_preflight_authorizes_execution",
            preflight.get("final_decision", {}).get("authorized_next_work"),
        ...
        _expect("train_records_reporting_only_863", len(train_records), EXPECTED_COUNTS["train"]),
        ...
        _expect("primary_eval_rows_161", len(paired_rows), EXPECTED_COUNTS["calibration"] + EXPECTED_COUNTS["holdout"]),
        ...
        _check("model_weights_match_atoms", _weights_match_records(model, paired_rows), model.get("weights"), "weights match atoms"),
    ...
    return _stable(
        {
            ...
            "pilot_paired_evaluation_execution": {
                ...
                "claims": {
                    ...
                    "reason": "pilot calibration=14 and holdout=147 are smoke-only",
                },
            },
...
def _enrich_eval_records(records: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    if all(record.get("atoms") is not None and record.get("feasible_mask") is not None for record in records):
        return records, {
            "attempted": False,
            "records_already_with_atoms": len(records),
            "records_enriched": 0,
            "failed_records": 0,
            "errors": [],
            "source": "existing_record_atoms",
        }
    return TRAINING_MODULE._derive_missing_train_atoms(records)
...
def _paired_rows(
    ...
    weights = np.asarray(model.get("weights") or [], dtype=np.float64)
    scales = _atom_scales(model, len(weights))
    rows = []
    latencies = []
    for index, record in enumerate(records):
        started = time.perf_counter()
        row = _paired_row(record, index, weights, scales, camp_head, dp_head)
        latencies.append((time.perf_counter() - started) * 1000.0)
        rows.append(row)
    return rows, _latency_summary(latencies)
...
def _paired_row(
    ...
    atoms = np.asarray(record.get("atoms") or [], dtype=np.float64)
    feasible = np.asarray(record.get("feasible_mask") or [], dtype=bool)
    if atoms.ndim != 2 or weights.shape != (atoms.shape[1],):
        scores = np.full((EXPECTED_K,), float("inf"))
    else:
        scores = np.nan_to_num(atoms / scales.reshape(1, -1), nan=0.0, posinf=0.0, neginf=0.0) @ weights
    if feasible.shape != (atoms.shape[0],) or not feasible.any():
        feasible = np.ones((atoms.shape[0],), dtype=bool)
    masked_scores = np.where(feasible, scores, np.inf)
    selected_index = int(np.argmin(masked_scores)) if masked_scores.size else -1
    dp_top1_index = int(record.get("dp_top1_index", 0))
    dp_metric = _score_at(scores, dp_top1_index)
    camp_metric = _score_at(scores, selected_index)
    oracle_metric = float(np.min(scores)) if scores.size else None
    delta = None if dp_metric is None or camp_metric is None else float(camp_metric - dp_metric)
    return {
        "row_index": row_index,
        "split": str(record.get("eval_split") or record.get("split")),
        "scene_id": record.get("scene_id"),
        "sample_id": record.get("sample_id"),
        "camp_head": camp_head,
        "fixed_dp_head": dp_head,
        "source_camp_head": record.get("CAMP_HEAD"),
        "source_dp_head": record.get("DP_HEAD"),
        "k": record.get("K"),
        "candidate_count": record.get("candidate_count"),
        "candidate_tensor_shape": record.get("candidate_tensor_shape"),
        "adapter_input_shape": record.get("adapter_input_shape"),
        "candidate_tensor_sha256": record.get("candidate_tensor_sha256"),
        "candidate_tensor_sha256_present": bool(record.get("candidate_tensor_sha256")),
        "candidate_tensor_unchanged_by_camp": record.get("candidate_tensor_unchanged_by_camp") is True,
        "atom_count": int(atoms.shape[1]) if atoms.ndim == 2 else 0,
        "dp_top1_index": dp_top1_index,
        "selected_index": selected_index,
        "selected_index_in_range": 0 <= selected_index < int(record.get("candidate_count", 0) or 0),
        "dp_top1_metric": dp_metric,
        "camp_selected_metric": camp_metric,
        "oracle_metric": oracle_metric,
        "delta": delta,
        "outcome": _outcome(delta),
        "non_top1_selection": selected_index != dp_top1_index,
        "score_expression": SCORE_EXPRESSION,
        "scores": [float(value) for value in scores.tolist()] if scores.size else [],
...
def _score_at(scores: np.ndarray, index: int) -> float | None:
...
def _split_metrics(rows: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "primary": _metric_payload(rows),
        "calibration": _metric_payload([row for row in rows if row["split"] == "calibration"]),
        "holdout": _metric_payload([row for row in rows if row["split"] == "holdout"]),
        "train": _metric_payload([]),
    }


def _metric_payload(rows: list[dict[str, Any]]) -> dict[str, Any]:
    deltas = [float(row["delta"]) for row in rows if row["delta"] is not None]
    dp_metrics = [float(row["dp_top1_metric"]) for row in rows if row["dp_top1_metric"] is not None]
    camp_metrics = [float(row["camp_selected_metric"]) for row in rows if row["camp_selected_metric"] is not None]
    gap_den = 0.0
    gap_num = 0.0
    for row in rows:
        dp = row["dp_top1_metric"]
        camp = row["camp_selected_metric"]
        oracle = row["oracle_metric"]
        if dp is None or camp is None or oracle is None:
            continue
        gap = max(float(dp) - float(oracle), 0.0)
        gap_den += gap
        gap_num += min(max(float(dp) - float(camp), 0.0), gap)
    return {
        "row_count": len(rows),
        "better_tie_worse": {
        ...
        "ci95": _ci95(deltas),
        "dp_top1_metric": _summary(dp_metrics),
        "camp_selected_metric": _summary(camp_metrics),
        "non_top1_selection_rate": (
            sum(1 for row in rows if row["non_top1_selection"]) / len(rows) if rows else None
        ),
        "oracle_gap_closed": gap_num / gap_den if gap_den > 0.0 else None,
        "performance_claim": False,
    }
...
def _record_summary(
    ...
    source_records = calibration_records + holdout_records
    all_records = train_records + source_records
    scene_sets = {
        "train": {str(record.get("scene_id")) for record in train_records},
        "calibration": {str(record.get("scene_id")) for record in calibration_records},
        "holdout": {str(record.get("scene_id")) for record in holdout_records},
    }
    sample_sets = {
        "train": {str(record.get("sample_id")) for record in train_records},
        "calibration": {str(record.get("sample_id")) for record in calibration_records},
        "holdout": {str(record.get("sample_id")) for record in holdout_records},
    }
    return {
        "scene_zero_overlap": _sets_disjoint(scene_sets.values()),
        "sample_zero_overlap": _sets_disjoint(sample_sets.values()),
        "train_rows_in_primary_eval": sum(1 for row in paired_rows if row["split"] == "train"),
        ...
        "missing_candidate_tensor_sha256": sum(1 for record in source_records if not record.get("candidate_tensor_sha256")),
        "candidate_tensor_mutated_count": sum(
            1 for record in source_records if record.get("candidate_tensor_unchanged_by_camp") is not True
        ),
        "selected_index_out_of_range_count": sum(1 for row in paired_rows if not row["selected_index_in_range"]),
    }


def _atom_scales(model: dict[str, Any], size: int) -> np.ndarray:
    path = Path(str((model.get("trainer_summary") or {}).get("atom_scales_path") or ""))
    if path.is_file():
        raw = _read_json(path)
        scales = np.asarray(raw.get("scales") or [], dtype=np.float64)
    else:
        scales = np.ones((size,), dtype=np.float64)
    if scales.shape != (size,):
        scales = np.ones((size,), dtype=np.float64)
    return np.maximum(np.nan_to_num(scales, nan=1.0, posinf=1.0, neginf=1.0), 1e-6)


def _tag_records(records: list[dict[str, Any]], split: str) -> list[dict[str, Any]]:
    return [dict(record, eval_split=split) for record in records]


def _weights_match_records(model: dict[str, Any], rows: list[dict[str, Any]]) -> bool:
    weights = model.get("weights") or []
    if not rows:
        return False
    return bool(weights) and all(row.get("atom_count") == len(weights) for row in rows)


def _ci95(values: list[float]) -> dict[str, float | None]:
    if not values:
...
def _decision(passed: bool, failed: list[str], check_count: int) -> dict[str, Any]:
    return {
        ...
        "authorized_next_work": AUTHORIZED_NEXT_WORK if passed else AUTHORIZED_CURRENT_WORK,
        "paired_evaluation_executed": passed,
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- Smoke only: `{paired['pilot_eval_smoke_only']}`",
