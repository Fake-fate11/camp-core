#!/usr/bin/env python3
"""Audited online selector activation decision for objective-3200 SafetyCost evidence."""
...
def _load_source_review_module():
    review_path = Path(__file__).resolve().with_name(
        ...
        "actual_safetycost_online_selector_activation_decision_plan_static_contract.py"
    )
...
READY_STATUS = (
    ...
    "candidate_index_actual_safetycost_online_selector_activation_decision_passed"
)
REJECT_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_online_selector_activation_decision_rejected"
)
AUTHORIZED_NEXT_WORK = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_online_selector_activation_execution_plan_only"
)
DECISION_JSON_NAME = (
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_online_selector_activation_decision.json"
)
DECISION_MD_NAME = (
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_online_selector_activation_decision.md"
)
EXPECTED_SOURCE_REVIEW_CHECK_COUNT = 84
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v14_post_closeout_promotion_evidence_acquisition_objective_3200_candidate_index_actual_safetycost_online_selector_activation_decision",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        enabled=args.enable_v14_post_closeout_promotion_evidence_acquisition_objective_3200_candidate_index_actual_safetycost_online_selector_activation_decision,
    )
...
def build_report(
    *,
    ...
    activation_record = _activation_record(source_review)
    checks = _checks(
        enabled=enabled,
    ...
    return {
        ...
        "analysis": {
            "online_selector_activation_decision_gate": True,
        ...
        "online_selector_activation_record": activation_record,
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    record = report["online_selector_activation_record"]
    return "\n".join(
        [
            "# Objective-3200 Candidate-Index Actual-SafetyCost Online Selector Activation Decision",
            "",
            ...
            "",
            "## Online Selector Activation Decision",
            "",
...
def _checks(
    *,
    ...
    checks = [
        BASE_MODULE._expect("online_selector_activation_decision_enabled", enabled, True),
        ...
        BASE_MODULE._expect("audit_online_selector_activation_decision_authorized", BASE_MODULE._latest_value(v14_text, "objective_3200_candidate_index_actual_safetycost_online_selector_activation_decision_authorized"), "True"),
        ...
        BASE_MODULE._expect("source_online_selector_activation_decision_authorized", decision.get("objective_3200_candidate_index_actual_safetycost_online_selector_activation_decision_authorized"), True),
        ...
        BASE_MODULE._expect("activation_record_authorizes_online", activation_record.get("online_selector_change_authorized"), True),
        BASE_MODULE._expect("activation_record_dp_modification_false", activation_record.get("dp_modification_authorized"), False),
        BASE_MODULE._expect("activation_record_candidate_mutation_false", activation_record.get("candidate_mutation_authorized"), False),
        BASE_MODULE._check("activation_scope_mentions_no_execution", "does not execute" in activation_record.get("scope_limit", ""), activation_record.get("scope_limit"), "does not execute"),
    ]
...
def _activation_record(source_review: dict[str, Any]) -> dict[str, Any]:
    source_decision = BASE_MODULE._dict(source_review.get("final_decision"))
    return {
        "decision_text": "Authorize a later execution-planning gate for enabling the audited objective-3200 CAMP selector online.",
        "scope_limit": "This decision does not execute online selector activation, DP modification, candidate mutation, training, or replay.",
...
def _source_static_review_summary(source_review: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "online_selector_activation_decision_authorized": decision.get("objective_3200_candidate_index_actual_safetycost_online_selector_activation_decision_authorized"),
    }
...
def _decision(*, passed: bool, checks: list[dict[str, Any]], source_review: dict[str, Any]) -> dict[str, Any]:
    ...
    if passed:
        failure_class = None
    elif "online_selector_activation_decision_enabled" in failed:
        failure_class = "explicit_candidate_index_actual_safetycost_online_selector_activation_decision_authorization_missing"
        ...
        failure_class = "source_online_selector_activation_plan_static_review_contract_failure"
    else:
        failure_class = "online_selector_activation_decision_contract_failure"
    return {
        "passed": bool(passed),
        ...
        "objective_3200_candidate_index_actual_safetycost_online_selector_activation_decision_passed": bool(passed),
        "objective_3200_candidate_index_actual_safetycost_online_selector_activation_execution_plan_authorized": bool(passed),
        "source_static_review_passed": source_decision.get("passed"),
        "selector_promotion_authorized": source_decision.get("selector_promotion_authorized"),
        "deployment_authorized": source_decision.get("deployment_authorized"),
        "online_selector_change_authorized": bool(passed),
        "safety_benefit_claim_authorized": source_decision.get("safety_benefit_claim_authorized"),
        "camp_over_dp_top1_claim_authorized": source_decision.get("camp_over_dp_top1_claim_authorized"),
    }
