...
DEFAULT_POST_REVIEW_ROOT = (
    ...
    "guarded_rerun_failure_attribution_remediation_v3_post_static_review_bff8f8b"
)
DEFAULT_EXECUTION_ROOT = (
    "/root/autodl-tmp/camp_dp_material_generator_failure_attribution_remediation_"
    "guarded_rerun_failure_attribution_remediation_v3_fixed_snapshot_screen_rerun_bff8f8b"
)
...
POST_REVIEW_JSON = "material_generator_v3_post_implementation_static_contract_review.json"
POST_REVIEW_MD = "material_generator_v3_post_implementation_static_contract_review.md"
...
GUARD_ENV_VAR = "CAMP_MATERIAL_GENERATOR_V3_FIXED_SNAPSHOT_RERUN_APPROVED"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Plan-only guarded fixed-snapshot screen rerun for guarded material "
            "generator v3."
        )
    )
    parser.add_argument(
    ...
    return {
        "analysis": {
            ...
            "math_boundary": (
                ...
                "benefit, claim CAMP over DP Top-1, or claim a DP-side "
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Guarded Material V3 Fixed-Snapshot Screen Rerun Plan",
        "",
...
"${{PY}}" scripts/integrations/analyze_diffusion_planner_route_topology_candidate_screen.py \\
  ...
  --label guarded_material_v3_fixed_snapshot_screen \\
...
cd "${{OUTPUT_ROOT}}"
sha256sum \\
  guarded_material_v3_fixed_snapshot_screen.json \\
  guarded_material_v3_fixed_snapshot_screen.md \\
...
def _screen_rerun_plan(execution_root: Path) -> dict[str, Any]:
    ...
    return {
        "selection_type": "guarded_material_v3_fixed_snapshot_screen_rerun_plan_only",
        ...
        "execution_artifacts": {
            "output_root": root,
            "screen_json": f"{root}/guarded_material_v3_fixed_snapshot_screen.json",
            "screen_md": f"{root}/guarded_material_v3_fixed_snapshot_screen.md",
            "heads": f"{root}/HEADS.txt",
            "sha256sums": f"{root}/SHA256SUMS",
            "runbook": f"{root}/guarded_material_v3_fixed_snapshot_screen_runbook.sh",
        },
        "accept_criteria": [
            ...
            "future guarded rerun must use only the fixed nonformal seed-2 snapshot corpus",
            "future evidence must report hard/precheck, progress, comfort, and v3 descriptor coverage",
            "future evidence must not claim safety benefit or CAMP-over-DP Top-1 superiority",
        ],
        "blocked_boundaries": [
...
def _post_review_summary(payload: dict[str, Any], markdown_text: str) -> dict[str, Any]:
    ...
    return {
        ...
        "plan_authorized": bool(decision.get("fixed_snapshot_screen_rerun_plan_authorized")),
        "blocked_action_conflicts": [
            key for key in BLOCKED_ACTIONS if bool(decision.get(key))
        ],
        "markdown_records_authorized_next_work": (
            "Authorized next work" in markdown_text
            and POST_REVIEW_AUTHORIZED_NEXT_WORK in markdown_text
        ),
    }
...
def _source_checks(source: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        ...
        _check("post_review_plan_authorized", source["plan_authorized"] is True),
        ...
        _check(
            ...
            source["markdown_records_authorized_next_work"],
        ),
    ]
...
def _plan_checks(plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        _check(
            "plan_selection_type",
            plan["selection_type"] == "guarded_material_v3_fixed_snapshot_screen_rerun_plan_only",
        ),
        _check("plan_selects_guarded_rerun_next", plan["selected_next_work"] == AUTHORIZED_NEXT_WORK),
        _check("plan_nonformal_seed2", scope["seed"] == 2 and scope["formal_seed"] is False),
        _check("plan_formal_seeds_frozen", scope["formal_seeds_frozen"] == [11, 12, 13]),
        _check("plan_expected_snapshot_count", scope["expected_snapshot_count"] == DEFAULT_EXPECTED_SNAPSHOT_COUNT),
        _check("plan_uses_v3_policy", config["generator_policy"] == PLANNED_POLICY),
        _check("plan_uses_v3_profile", config["default_off_remediation_profile"] == REMEDIATION_PROFILE),
        _check("plan_uses_comfort_first_hard_support_prefix", config["prefix_steps"] == [1]),
        _check("plan_uses_comfort_first_hard_support_bridge", config["bridge_steps"] == [0]),
        _check("plan_pins_candidate_cap", config["max_remediation_candidates"] == DEFAULT_MAX_REMEDIATION_CANDIDATES),
        _check("plan_keeps_v3_budget_floors_off", config["command_jerk_worse_budget_mps3"] == 0.0),
        _check("plan_keeps_rollout_comfort_budgets_exact", config["rollout_jerk_worse_budget_mps3"] == 0.0 and config["rollout_lateral_worse_budget_mps2"] == 0.0),
    ]
...
def _runbook_checks(runbook_text: str) -> list[dict[str, Any]]:
    ...
    return [
        ...
        _check("runbook_uses_camp_env_python", DEFAULT_REMOTE_PYTHON in runbook_text),
        _check("runbook_uses_v3_policy", f"--generator_policy {PLANNED_POLICY}" in runbook_text),
        _check("runbook_uses_v3_profile", f"--default_off_remediation_profile {REMEDIATION_PROFILE}" in runbook_text),
        _check("runbook_pins_candidate_cap", f"--max_remediation_candidates {DEFAULT_MAX_REMEDIATION_CANDIDATES}" in runbook_text),
        _check("runbook_has_v3_prefix_bridge", "--prefix_step 1" in runbook_text and "--bridge_step 0" in runbook_text),
        _check("runbook_has_screen_command", "analyze_diffusion_planner_route_topology_candidate_screen.py" in runbook_text),
...
def _boundary_checks() -> list[dict[str, Any]]:
    ...
    return [
        _check("boundary_authorizes_guarded_rerun_next", decision["guarded_fixed_snapshot_screen_rerun_next_gate_authorized"] is True),
        _check("boundary_blocks_candidate_generation", decision["candidate_generation_execution_authorized"] is False),
        _check("boundary_blocks_screen_rerun_now", decision["fixed_snapshot_screen_rerun_authorized"] is False),
