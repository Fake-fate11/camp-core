#!/usr/bin/env python3
"""Static review for the v14 runbook plan-only artifact."""
...
REJECT_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_readiness_evaluation_runbook_plan_static_review_rejected"
)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--runbook_plan_artifact_dir", type=Path, required=True)
    parser.add_argument("--runbook_plan_json", type=Path, required=True)
    parser.add_argument("--runbook_plan_md", type=Path, required=True)
    parser.add_argument("--runbook_plan_sha256s", type=Path, required=True)
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Explicit opt-in for static review of the runbook plan.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        runbook_plan_artifact_dir=args.runbook_plan_artifact_dir,
        runbook_plan_json=args.runbook_plan_json,
        runbook_plan_md=args.runbook_plan_md,
        runbook_plan_sha256s=args.runbook_plan_sha256s,
        ...
        label=args.label,
        enabled=args.enable_v14_post_closeout_promotion_readiness_evaluation_runbook_plan_static_review,
    )
...
def build_report(
    *,
    runbook_plan_artifact_dir: Path,
    runbook_plan_json: Path,
    runbook_plan_md: Path,
    runbook_plan_sha256s: Path,
    plan_script_py: Path,
    plan_test_py: Path,
    v14_audit_md: Path,
    ...
    artifact_dir = runbook_plan_artifact_dir.resolve()
    paths = {
        "runbook_plan_json": runbook_plan_json.resolve(),
        "runbook_plan_md": runbook_plan_md.resolve(),
        "runbook_plan_sha256s": runbook_plan_sha256s.resolve(),
    ...
    source_plan = _read_json_dict(paths["runbook_plan_json"])
    root_sha256s = _read_sha256sums(artifact_files["root_sha256s"])
    plan_sha256s = _read_sha256sums(paths["runbook_plan_sha256s"])
    ...
    checks: list[dict[str, Any]] = [
        ...
        _check("runbook_plan_artifact_dir_exists", artifact_dir.is_dir(), str(artifact_dir), "directory"),
    ]
    ...
    checks.extend(
        [
            _expect("plan_json_matches_artifact_layout", paths["runbook_plan_json"], artifact_files["plan_json"]),
            _expect("plan_md_matches_artifact_layout", paths["runbook_plan_md"], artifact_files["plan_md"]),
            _expect("plan_sha256s_matches_artifact_layout", paths["runbook_plan_sha256s"], artifact_files["plan_sha256s"]),
        ]
    )
    ...
    return {
        ...
        "analysis": {
            ...
            "read_only": True,
            "runbook_plan_artifact_dir": str(artifact_dir),
            "runbook_plan_json": str(paths["runbook_plan_json"]),
            "runbook_plan_md": str(paths["runbook_plan_md"]),
            "runbook_plan_sha256s": str(paths["runbook_plan_sha256s"]),
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Source runbook steps: `{summary.get('runbook_step_count')}`",
...
def _source_surface_checks(script: str, test: str) -> list[dict[str, Any]]:
    return [
        _contains("source_surface_script_schema", script, "promotion_readiness_evaluation_runbook_plan_v1"),
        _contains("source_surface_script_static_review_next", script, "runbook_plan_static_review_only"),
        ...
        _contains("source_surface_test_pass_case", test, "test_promotion_readiness_evaluation_runbook_plan_passes"),
        _contains("source_surface_test_requires_enable", test, "test_promotion_readiness_evaluation_runbook_plan_requires_enable"),
        _contains("source_surface_test_rejects_wrong_eof", test, "test_promotion_readiness_evaluation_runbook_plan_rejects_wrong_eof"),
        _contains("source_surface_test_accepts_uppercase_dp_head", test, "test_promotion_readiness_evaluation_runbook_plan_accepts_uppercase_dp_head"),
    ]
...
def _audit_checks(v14_text: str, status_text: str) -> list[dict[str, Any]]:
    ...
    return [
        ...
        _expect("audit_runbook_plan_ready", _latest_value(v14_text, "post_closeout_promotion_readiness_evaluation_runbook_plan_ready"), "True"),
        _expect("audit_runbook_plan_static_review_authorized", _latest_value(v14_text, "post_closeout_promotion_readiness_evaluation_runbook_plan_static_review_authorized"), "True"),
...
def _decision(passed: bool, checks: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    decision = {
        ...
        "post_closeout_promotion_readiness_evaluation_runbook_plan_static_review_passed": bool(passed),
        "evaluation_runbook_execution_preflight_authorized": bool(passed),
        "evaluation_runbook_execution_authorized": False,
        "recommendation": "preflight_read_only_promotion_readiness_evaluation_runbook_execution_only",
        "immediate_action": "preflight_promotion_readiness_evaluation_runbook_execution_only",
...
def _failure_class(failed: list[str]) -> str:
    ...
    if "static_review_enabled" in failed_set:
        return "explicit_runbook_plan_static_review_authorization_missing"
    ...
    if any(name.endswith("_sha") or name.endswith("_root_sha") for name in failed):
        return "runbook_plan_artifact_sha256_mismatch"
    ...
    return "promotion_readiness_evaluation_runbook_plan_static_review_failure"
