#!/usr/bin/env python3
"""Plan-only online selector activation decision after audited deployment decision."""
...
def _load_source_decision_module():
    decision_path = Path(__file__).resolve().with_name(
        ...
        "actual_safetycost_deployment.py"
    )
    spec = importlib.util.spec_from_file_location(
        "v14_candidate_index_actual_safetycost_deployment_decision",
        decision_path,
    )
...
READY_STATUS = (
    ...
    "candidate_index_actual_safetycost_online_selector_activation_decision_plan_ready"
)
REJECT_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_online_selector_activation_decision_plan_rejected"
)
AUTHORIZED_NEXT_WORK = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_online_selector_activation_decision_plan_static_review_only"
)
PLAN_JSON_NAME = (
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_online_selector_activation_decision_plan.json"
)
PLAN_MD_NAME = (
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_online_selector_activation_decision_plan.md"
)
EXPECTED_SOURCE_DECISION_CHECK_COUNT = 68
ONLINE_SELECTOR_ACTIVATION_DECISION_ITEMS = (
    "source_deployment_decision_scope_binding",
    "default_off_online_activation_boundary",
    "no_dp_or_candidate_mutation_boundary",
    "online_activation_artifact_inventory",
    "fail_closed_rollback_and_disable_controls",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--source_deployment_decision_artifact_dir", type=Path, required=True)
    parser.add_argument("--source_deployment_decision_json", type=Path, required=True)
    parser.add_argument("--source_deployment_decision_md", type=Path, required=True)
    parser.add_argument("--source_deployment_decision_sha256s", type=Path, required=True)
    ...
    parser.add_argument(
        "--enable_v14_post_closeout_promotion_evidence_acquisition_objective_3200_candidate_index_actual_safetycost_online_selector_activation_decision_plan",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        source_deployment_decision_artifact_dir=args.source_deployment_decision_artifact_dir,
        source_deployment_decision_json=args.source_deployment_decision_json,
        source_deployment_decision_md=args.source_deployment_decision_md,
        source_deployment_decision_sha256s=args.source_deployment_decision_sha256s,
        ...
        enabled=(
            args.enable_v14_post_closeout_promotion_evidence_acquisition_objective_3200_candidate_index_actual_safetycost_online_selector_activation_decision_plan
        ),
    )
...
def build_report(
    *,
    source_deployment_decision_artifact_dir: Path,
    source_deployment_decision_json: Path,
    source_deployment_decision_md: Path,
    source_deployment_decision_sha256s: Path,
    v14_audit_md: Path,
    ...
    artifact_dir = source_deployment_decision_artifact_dir.resolve()
    paths = {
        "source_deployment_decision_json": source_deployment_decision_json.resolve(),
        "source_deployment_decision_md": source_deployment_decision_md.resolve(),
        "source_deployment_decision_sha256s": source_deployment_decision_sha256s.resolve(),
        "v14_audit_md": v14_audit_md.resolve(),
        "current_status_md": current_status_md.resolve(),
    }
    files = _artifact_files(artifact_dir)
    source_decision_report = BASE_MODULE._read_json_dict(paths["source_deployment_decision_json"])
    ...
    nested_sha256s = BASE_MODULE._read_sha256sums(paths["source_deployment_decision_sha256s"])
    v14_text = BASE_MODULE._read_text(paths["v14_audit_md"])
    status_text = BASE_MODULE._read_text(paths["current_status_md"])
    activation_plan = _online_selector_activation_decision_plan(source_decision_report)
    checks = _checks(
        enabled=enabled,
    ...
    return {
        ...
        "inputs": {
            "source_deployment_decision_artifact_dir": str(artifact_dir),
        ...
        "source_deployment_decision_summary": _source_decision_summary(source_decision_report),
        "online_selector_activation_decision_plan": activation_plan,
        "plan_checks": checks,
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Objective-3200 Candidate-Index Actual-SafetyCost Online Selector Activation Decision Plan",
        "",
        ...
        "",
        "## Online Selector Activation Decision Items",
        "",
    ]
    for item in report["online_selector_activation_decision_plan"]:
...
def _checks(
    *,
    ...
    activation_plan: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    decision = BASE_MODULE._dict(source_decision_report.get("final_decision"))
    checks = [
        BASE_MODULE._expect("online_selector_activation_decision_plan_enabled", enabled, True),
        BASE_MODULE._check("source_deployment_decision_artifact_dir_exists", artifact_dir.is_dir(), str(artifact_dir), "directory"),
        BASE_MODULE._expect("source_deployment_decision_json_path_matches_artifact", paths["source_deployment_decision_json"], files["decision_json"]),
        BASE_MODULE._expect("source_deployment_decision_md_path_matches_artifact", paths["source_deployment_decision_md"], files["decision_md"]),
        BASE_MODULE._expect("source_deployment_decision_sha256s_path_matches_artifact", paths["source_deployment_decision_sha256s"], files["decision_sha256s"]),
        ...
        BASE_MODULE._expect(
            "audit_online_selector_activation_decision_plan_authorized",
            BASE_MODULE._latest_value(
                v14_text,
                "objective_3200_candidate_index_actual_safetycost_online_selector_activation_decision_plan_authorized",
            ),
            "True",
        ),
        ...
        BASE_MODULE._expect("source_deployment_decision_run_exit", BASE_MODULE._read_text(files["run_exit"]).strip(), "0"),
        BASE_MODULE._expect("source_deployment_decision_schema", source_decision_report.get("schema_version"), SOURCE_DECISION_SCHEMA),
        BASE_MODULE._expect("source_deployment_decision_status", decision.get("status"), SOURCE_DECISION_STATUS),
        BASE_MODULE._expect("source_deployment_decision_passed", decision.get("passed"), True),
        BASE_MODULE._expect("source_deployment_decision_failure_class", decision.get("failure_class"), None),
        BASE_MODULE._expect("source_deployment_decision_failed_checks", decision.get("failed_checks"), []),
        BASE_MODULE._expect("source_deployment_decision_check_count", decision.get("check_count"), EXPECTED_SOURCE_DECISION_CHECK_COUNT),
        BASE_MODULE._expect("source_deployment_decision_failed_check_count", decision.get("failed_check_count"), 0),
        BASE_MODULE._expect("source_deployment_decision_authorized_next_work", decision.get("authorized_next_work"), AUTHORIZED_CURRENT_WORK),
        BASE_MODULE._expect("source_deployment_decision_passed_flag", decision.get("objective_3200_candidate_index_actual_safetycost_deployment_decision_passed"), True),
        BASE_MODULE._expect(
            "source_online_selector_activation_decision_plan_authorized",
            decision.get("objective_3200_candidate_index_actual_safetycost_online_selector_activation_decision_plan_authorized"),
            True,
        ),
        ...
        BASE_MODULE._expect(
            "online_selector_activation_decision_item_names",
            [item.get("item_name") for item in activation_plan],
            list(ONLINE_SELECTOR_ACTIVATION_DECISION_ITEMS),
        ),
...
def _online_selector_activation_decision_plan(source_decision_report: dict[str, Any]) -> list[dict[str, Any]]:
    decision = BASE_MODULE._dict(source_decision_report.get("final_decision"))
    return [
        {
            "item_name": "source_deployment_decision_scope_binding",
            "purpose": "bind any later online selector activation decision to the audited deployment decision and fixed-DP candidate tensor scope",
            "source_deployment_authorized": decision.get("deployment_authorized"),
            "authorizes_online_selector_now": False,
            "authorizes_dp_modification": False,
            "authorizes_candidate_mutation": False,
        },
        {
            "item_name": "default_off_online_activation_boundary",
            "purpose": "require online activation to remain disabled until a later static review and activation decision gate pass",
            "authorizes_online_selector_now": False,
            "authorizes_dp_modification": False,
            "authorizes_candidate_mutation": False,
        },
        {
            "item_name": "no_dp_or_candidate_mutation_boundary",
            "purpose": "keep fixed DP code, config, weights, checkpoints, and candidate tensors immutable for activation planning",
        ...
        {
            "item_name": "online_activation_artifact_inventory",
            "purpose": "predefine the manifest, runtime switch, fail-closed evidence, and rollback artifacts needed before any online activation decision",
            "authorizes_online_selector_now": False,
            "authorizes_dp_modification": False,
            "authorizes_candidate_mutation": False,
        },
        {
            "item_name": "fail_closed_rollback_and_disable_controls",
            "purpose": "require reversible disable controls and fail-closed DP Top-1 fallback before any online selector can be enabled later",
            "authorizes_online_selector_now": False,
            "authorizes_dp_modification": False,
            "authorizes_candidate_mutation": False,
        },
        {
            ...
            "purpose": "require a read-only static review before any online selector activation decision can execute",
...
def _decision(
    *,
    ...
    if passed:
        failure_class = None
    elif "online_selector_activation_decision_plan_enabled" in failed:
        failure_class = "explicit_candidate_index_actual_safetycost_online_selector_activation_decision_plan_authorization_missing"
        ...
        failure_class = "source_deployment_decision_contract_failure"
    else:
        failure_class = "online_selector_activation_decision_plan_contract_failure"
    return {
        "passed": bool(passed),
        ...
        "objective_3200_candidate_index_actual_safetycost_online_selector_activation_decision_plan_ready": bool(passed),
        "objective_3200_candidate_index_actual_safetycost_online_selector_activation_decision_plan_static_review_authorized": bool(passed),
        "source_deployment_decision_passed": source_decision.get("passed"),
