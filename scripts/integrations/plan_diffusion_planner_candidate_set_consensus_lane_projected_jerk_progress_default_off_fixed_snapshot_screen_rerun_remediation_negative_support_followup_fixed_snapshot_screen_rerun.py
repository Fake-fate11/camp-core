...
from scripts.integrations.review_diffusion_planner_candidate_set_consensus_lane_projected_jerk_progress_default_off_fixed_snapshot_screen_rerun_remediation_negative_support_followup_post_implementation_static_contract import (  # noqa: E402
...
DEFAULT_POST_REVIEW_ROOT = (
    ...
    "negative_support_followup_post_implementation_static_review_bff8f8b"
)
...
GUARD_ENV_VAR = (
    "CANDIDATE_SET_CONSENSUS_NEGATIVE_SUPPORT_FOLLOWUP_"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Plan-only guarded fixed-snapshot screen rerun for the "
            "negative-support follow-up policy."
        )
    )
    parser.add_argument(
        "--post_review_root",
    ...
    return {
        "analysis": {
            "name": (
                ...
                "support_followup_fixed_snapshot_screen_rerun_plan_v1"
            ),
            "label": label,
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Negative-Support Follow-Up Fixed-Snapshot Screen Rerun Plan",
        "",
...
"${{PY}}" scripts/integrations/analyze_diffusion_planner_route_topology_candidate_screen.py \\
  ...
  --label negative_support_followup_fixed_snapshot_screen \\
...
cd "${{OUTPUT_ROOT}}"
sha256sum \\
  negative_support_followup_fixed_snapshot_screen.json \\
  negative_support_followup_fixed_snapshot_screen.md \\
...
def _screen_rerun_plan(execution_root: Path) -> dict[str, Any]:
    ...
    return {
        "selection_type": "negative_support_followup_fixed_snapshot_screen_rerun_plan_only",
        ...
        "execution_artifacts": {
            "output_root": root,
            "screen_json": f"{root}/negative_support_followup_fixed_snapshot_screen.json",
            "screen_md": f"{root}/negative_support_followup_fixed_snapshot_screen.md",
            "heads": f"{root}/HEADS.txt",
            "sha256sums": f"{root}/SHA256SUMS",
            "runbook": f"{root}/fixed_snapshot_screen_rerun_guarded_runbook.sh",
        },
        "command_contract": [
            "requires guard env before any screen rerun",
            "records CAMP HEAD, origin/main, DP HEAD, snapshot count, and output root",
            "uses only the existing nonformal fixed-snapshot corpus with seed 2",
            "uses the negative-support follow-up generator policy",
            "keeps current-tick finite feature and fail-closed contracts owned by the implementation",
            "does not call replay, training, online selector, atom promotion, or DP modification commands",
            "records SHA256SUMS after a future guarded rerun",
        ],
        "accept_criteria": [
            "plan artifact and runbook are complete",
...
def _source_checks(source: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        ...
        _check(
            ...
            source["fixed_snapshot_screen_rerun_plan_authorized"] is True,
        ),
...
def _plan_checks(plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        _check("plan_uses_negative_support_policy", config["generator_policy"] == PLANNED_POLICY),
...
def _runbook_checks(runbook_text: str) -> list[dict[str, Any]]:
    ...
    return [
        ...
        _check("runbook_uses_camp_python", DEFAULT_REMOTE_PYTHON in runbook_text),
        _check("runbook_uses_negative_support_policy", f"--generator_policy {PLANNED_POLICY}" in runbook_text),
