#!/usr/bin/env python3
"""Execute the v16 fixed-DP scale-up CAMP training gate."""
...
from scripts.integrations.preflight_diffusion_planner_dp_camp_v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training import (  # noqa: E402
    FIXED_DP_HEAD,
...
SCHEMA_VERSION = "dp_camp_v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_execution_v1"
AUTHORIZED_CURRENT_WORK = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_execution_only"
AUTHORIZED_NEXT_WORK = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_result_review_only"
READY_STATUS = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_execution_passed"
FAILED_STATUS = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_execution_failed"
REPORT_JSON_NAME = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_execution.json"
REPORT_MD_NAME = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_execution.md"
SCORE_EXPRESSION = "score_k(w)=a_k^T w"
EXPECTED_COUNTS = {"train": 6263, "calibration": 2156, "holdout": 1581}
EXPECTED_K = 8
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_execution",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = run_execution(
        ...
        epochs=args.epochs,
        enabled=args.enable_v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_execution,
        command=sys.argv,
    )
...
def run_execution(
    *,
    ...
    report = build_report(
        source_corpus_artifact_dir=source_corpus_artifact_dir,
        ...
        expected_corpus_root_sha256=expected_corpus_root_sha256,
...
def build_report(
    *,
    ...
    preflight_body = preflight.get("scaleup_training_preflight", {})
    audit_text = _read_text(v16_audit_md)
    status_text = _read_text(current_status_md).split("## Current V15 Status", 1)[0]
    source_artifacts = {
        "scaleup_corpus": _source_artifact(source_corpus_artifact_dir, expected_corpus_root_sha256),
    ...
    checks = [
        ...
        _contains(
            ...
            status_text,
            f"v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_preflight_status={SOURCE_PREFLIGHT_STATUS}",
        ),
        ...
        _check("training_output_root_absent", not output_dir.exists(), str(output_dir), "absent"),
        _expect("train_records_6263", len(train_records), EXPECTED_COUNTS["train"]),
        ...
        _expect("preflight_train_records_6263", preflight_body.get("train_records"), EXPECTED_COUNTS["train"]),
...
def _run_trainer(
    ...
    *,
    ...
    report["scaleup_training_execution"].update(
        {
...
def _model_from_summary(summary: dict[str, Any], report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "approved_atoms": report["scaleup_training_execution"]["atom_summary"].get("atom_names", []),
        "approved_atoms_only": atom_names == report["scaleup_training_execution"]["atom_summary"].get("atom_names", []),
        "score_expression": SCORE_EXPRESSION,
...
def _model_checks(model: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        _expect("model_train_records_6263", model.get("train_records"), EXPECTED_COUNTS["train"]),
...
def write_outputs(output_dir: Path, report: dict[str, Any]) -> None:
    ...
    (output_dir / "scaleup_training_config.json").write_text(
        json.dumps(report["training_config"], indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    timing = {
        "training_start": report["scaleup_training_execution"]["training_start"],
        "training_end": report["scaleup_training_execution"]["training_end"],
        "offline_training_wall_clock_seconds": report["scaleup_training_execution"]["offline_training_wall_clock_seconds"],
    }
    (output_dir / "scaleup_training_timing.json").write_text(json.dumps(timing, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    (output_dir / "scaleup_training_timing.md").write_text(
        "\n".join(
            [
                "# Scale-Up Training Timing",
                "",
    ...
    training_log = "".join(json.dumps(row, sort_keys=True) + "\n" for row in report.get("training_log", []))
    (output_dir / "scaleup_training.log").write_text(training_log, encoding="utf-8")
    (output_dir / "training_log.jsonl").write_text(training_log, encoding="utf-8")
...
def _fail_after_training(report: dict[str, Any], failed: list[str]) -> None:
    ...
    report["final_decision"] = _decision(False, failed, training_executed=report["scaleup_training_execution"]["training_executed"])
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# V16 nuScenes Fixed-DP Scale-Up Training Execution",
            "",
