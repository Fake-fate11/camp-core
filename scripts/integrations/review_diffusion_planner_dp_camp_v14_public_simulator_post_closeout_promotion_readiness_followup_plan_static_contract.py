#!/usr/bin/env python3
"""Static review for the v14 post-closeout promotion-readiness follow-up plan."""
...
REJECT_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_readiness_followup_plan_static_review_rejected"
)
...
REVIEW_JSON_NAME = "post_closeout_promotion_readiness_followup_plan_static_review.json"
REVIEW_MD_NAME = "post_closeout_promotion_readiness_followup_plan_static_review.md"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--followup_plan_artifact_dir", type=Path, required=True)
    parser.add_argument("--followup_plan_json", type=Path, required=True)
    parser.add_argument("--followup_plan_md", type=Path, required=True)
    parser.add_argument("--followup_plan_sha256s", type=Path, required=True)
    parser.add_argument("--followup_plan_script_py", type=Path, required=True)
    parser.add_argument("--followup_plan_test_py", type=Path, required=True)
    ...
    parser.add_argument(
        "--enable_v14_post_closeout_promotion_readiness_followup_plan_static_review",
        action="store_true",
        help="Explicit opt-in for static review of the read-only follow-up plan.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        followup_plan_artifact_dir=args.followup_plan_artifact_dir,
        followup_plan_json=args.followup_plan_json,
        followup_plan_md=args.followup_plan_md,
        followup_plan_sha256s=args.followup_plan_sha256s,
        followup_plan_script_py=args.followup_plan_script_py,
        followup_plan_test_py=args.followup_plan_test_py,
        ...
        label=args.label,
        enabled=args.enable_v14_post_closeout_promotion_readiness_followup_plan_static_review,
    )
...
def build_report(
    *,
    followup_plan_artifact_dir: Path,
    followup_plan_json: Path,
    followup_plan_md: Path,
    followup_plan_sha256s: Path,
    followup_plan_script_py: Path,
    followup_plan_test_py: Path,
    v14_audit_md: Path,
    ...
    artifact_dir = followup_plan_artifact_dir.resolve()
    paths = {
        "followup_plan_json": followup_plan_json.resolve(),
        "followup_plan_md": followup_plan_md.resolve(),
        "followup_plan_sha256s": followup_plan_sha256s.resolve(),
        "followup_plan_script_py": followup_plan_script_py.resolve(),
        "followup_plan_test_py": followup_plan_test_py.resolve(),
    ...
    source_plan = _read_json_dict(paths["followup_plan_json"])
    root_sha256s = _read_sha256sums(artifact_files["root_sha256s"])
    plan_sha256s = _read_sha256sums(paths["followup_plan_sha256s"])
    heads = _parse_key_values(_read_text(artifact_files["heads"]))
    script_text = _read_text(paths["followup_plan_script_py"])
    test_text = _read_text(paths["followup_plan_test_py"])
    ...
    checks: list[dict[str, Any]] = [
        ...
        _check("followup_plan_artifact_dir_exists", artifact_dir.is_dir(), str(artifact_dir), "directory"),
    ]
    ...
    checks.extend(
        [
            _expect("followup_plan_json_matches_artifact_layout", paths["followup_plan_json"], artifact_files["plan_json"]),
            _expect("followup_plan_md_matches_artifact_layout", paths["followup_plan_md"], artifact_files["plan_md"]),
            _expect("followup_plan_sha256s_matches_artifact_layout", paths["followup_plan_sha256s"], artifact_files["plan_sha256s"]),
        ]
    )
    ...
    return {
        ...
        "analysis": {
            ...
            "read_only": True,
            "followup_plan_artifact_dir": str(artifact_dir),
            "followup_plan_json": str(paths["followup_plan_json"]),
...
def _source_plan_contract_checks(source_plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    followup = _list(source_plan.get("followup_plan"))
    checks = [
        ...
        _expect("source_plan_static_review_authorized", decision.get("followup_plan_static_review_authorized"), True),
        ...
        _expect("source_plan_followup_items", [item.get("name") for item in followup], list(EXPECTED_FOLLOWUP_ITEMS)),
        _expect("source_plan_followup_item_count", len(followup), len(EXPECTED_FOLLOWUP_ITEMS)),
        _expect("source_plan_followup_no_promotion", [item.get("authorizes_promotion") for item in followup], [False] * len(EXPECTED_FOLLOWUP_ITEMS)),
    ]
...
def _source_surface_checks(script: str, test: str) -> list[dict[str, Any]]:
    return [
        _contains("source_surface_script_schema", script, "promotion_readiness_followup_plan_v1"),
        _contains("source_surface_script_authorized_next", script, "followup_plan_static_review_only"),
        _contains("source_surface_script_blocks_promotion", script, '"direct_promotion_recommendation": False'),
        ...
        _contains("source_surface_script_gap_item", script, "uncertainty_and_coverage_gap"),
        _contains("source_surface_test_pass_case", test, "test_post_closeout_promotion_readiness_followup_plan_passes"),
        _contains("source_surface_test_requires_enable", test, "test_post_closeout_promotion_readiness_followup_plan_requires_enable"),
        _contains("source_surface_test_rejects_wrong_eof", test, "test_post_closeout_promotion_readiness_followup_plan_rejects_wrong_eof"),
        _contains("source_surface_test_rejects_source_leak", test, "test_post_closeout_promotion_readiness_followup_plan_rejects_source_promotion_leak"),
    ]
...
def _audit_checks(v14_text: str, status_text: str) -> list[dict[str, Any]]:
    ...
    return [
        ...
        _expect("audit_followup_plan_ready", _latest_value(v14_text, "post_closeout_promotion_readiness_followup_plan_ready"), "True"),
        _expect("audit_followup_plan_static_review_authorized", _latest_value(v14_text, "post_closeout_promotion_readiness_followup_plan_static_review_authorized"), "True"),
...
def _source_summary(source_plan: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "followup_item_count": len(_list(source_plan.get("followup_plan"))),
...
def _decision(passed: bool, checks: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    decision = {
        ...
        "post_closeout_promotion_readiness_followup_plan_static_review_passed": bool(passed),
        "uncertainty_coverage_review_plan_authorized": bool(passed),
        "direct_promotion_recommendation": False,
        "promotion_decision_plan_authorized_next": False,
        "recommendation": "plan_read_only_uncertainty_coverage_review_only",
        "immediate_action": "plan_uncertainty_coverage_review_only",
...
def _failure_class(failed: list[str]) -> str:
    ...
    if "static_review_enabled" in failed_set:
        return "explicit_followup_plan_static_review_authorization_missing"
    ...
    if any(name.endswith("_sha") or name.endswith("_root_sha") for name in failed):
        return "followup_plan_artifact_sha256_mismatch"
    if any(name.startswith("source_plan") for name in failed):
        return "source_followup_plan_contract_failure"
    return "promotion_readiness_followup_plan_static_review_failure"
