...
READY_STATUS = "lane_hard_violation_support_broader_nonformal_smoke_plan_ready"
REJECT_STATUS = "lane_hard_violation_support_broader_nonformal_smoke_plan_rejected"
SOURCE_STATUS = "lane_hard_violation_support_logging_smoke_passed"
AUTHORIZED_NEXT_WORK = "lane_hard_violation_support_broader_nonformal_paired_smoke_only"
...
class BroaderSmokeSpec:
    root: str = "/root/autodl-tmp/camp_dp_lane_hard_violation_support_broader_nonformal_smoke"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "default-off lane/hard-violation support logging."
        )
    )
...
def build_report(
    *,
    ...
    return {
        "analysis": {
            "name": "dp_camp_lane_hard_violation_support_broader_nonformal_smoke_plan_v1",
            "label": label,
            ...
            "math_boundary": (
                ...
                "smoke after the lane/hard-violation support hook passed "
                "selector-equivalence and dataset audits. Runtime payloads "
                "must remain fixed current-tick finite-candidate diagnostics "
                "computed before closed-loop outcome evaluation. If later "
                "atomized, these diagnostics enter as fixed candidate "
        ...
        "final_decision": {
            ...
            "paired_smoke_execution_scope": (
                "next gate only: paired nonformal lane/hard-violation support "
...
def _source_artifact_checks(
    *,
    ...
    logging_ms = _float(
        latency.get("latency_ms_lane_hard_violation_support_logging")
    )
    return [
        {
...
def _coverage_targets(spec: BroaderSmokeSpec) -> dict[str, Any]:
    return {
        ...
        "max_broader_lane_hard_violation_support_logging_ms": (
...
def _payload_audit_command(
    ...
    return [
        ...
        str(spec.num_candidates),
        "--output_json",
        f"{audit_root}/lane_hard_violation_support_logging_smoke.json",
        "--output_md",
        f"{audit_root}/lane_hard_violation_support_logging_smoke.md",
        "--require_pass",
    ]
...
def _accept_criteria(spec: BroaderSmokeSpec) -> list[str]:
    return [
        ...
        "baseline summaries report camp_lane_hard_violation_support_logging.enabled=false",
        "logging-enabled summaries report camp_lane_hard_violation_support_logging.enabled=true",
        "candidate records contain non-null lane_hard_violation_support_logging payloads",
        "lane_hard_violation_support atoms are finite and nonnegative for all candidates",
        ...
        f"payload audit max latency_ms_lane_hard_violation_support_logging <= {MAX_BROADER_LOGGING_MS} ms",
...
def _reject_criteria() -> list[str]:
    return [
        ...
        "the smoke is expanded beyond the paired 4-run x 12-step nonformal scope",
    ]
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Lane/Hard-Violation Support Broader Nonformal Smoke Plan",
        "",
