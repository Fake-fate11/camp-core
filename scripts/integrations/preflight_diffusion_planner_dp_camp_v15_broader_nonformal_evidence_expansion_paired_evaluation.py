#!/usr/bin/env python3
"""Preflight the v15 paired-evaluation gate."""
...
def _load_static_review_module():
    path = Path(__file__).resolve().with_name(
        "review_diffusion_planner_dp_camp_v15_broader_nonformal_evidence_expansion_paired_evaluation_preflight_plan_static_contract.py"
    )
    spec = importlib.util.spec_from_file_location("v15_paired_evaluation_plan_static_review", path)
...
SCHEMA_VERSION = "dp_camp_v15_broader_nonformal_evidence_expansion_paired_evaluation_preflight_v1"
AUTHORIZED_CURRENT_WORK = STATIC_REVIEW_MODULE.AUTHORIZED_NEXT_WORK
READY_STATUS = "v15_broader_nonformal_evidence_expansion_paired_evaluation_preflight_ready"
REJECT_STATUS = "v15_broader_nonformal_evidence_expansion_paired_evaluation_preflight_rejected"
AUTHORIZED_NEXT_WORK = "v15_broader_nonformal_evidence_expansion_paired_evaluation_preflight_static_review_only"
PREFLIGHT_JSON_NAME = "v15_broader_nonformal_evidence_expansion_paired_evaluation_preflight.json"
PREFLIGHT_MD_NAME = "v15_broader_nonformal_evidence_expansion_paired_evaluation_preflight.md"
TIMING_JSON_NAME = "paired_evaluation_timing_contract.json"
TIMING_MD_NAME = "paired_evaluation_timing_contract.md"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v15_broader_nonformal_evidence_expansion_paired_evaluation_preflight",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        enabled=args.enable_v15_broader_nonformal_evidence_expansion_paired_evaluation_preflight,
    )
...
def build_report(
    *,
    ...
    preflight = PLAN_MODULE._paired_evaluation_preflight_plan(current_camp_head, current_dp_head)
    ...
    checks = [
        ...
        _expect("source_reviewed_plan", decision.get("reviewed_paired_evaluation_preflight_plan"), True),
        _expect("source_plan_not_executed_by_review", decision.get("paired_evaluation_preflight_plan_executed"), False),
        ...
        _expect("timing_online_latency_fields", tuple(timing["online_selector_latency_required_fields"]), PLAN_MODULE.LATENCY_FIELDS),
        _expect("timing_fallback_latency_fields", tuple(timing["fallback_latency_required_fields"]), PLAN_MODULE.LATENCY_FIELDS),
    ...
    return _stable(
        {
            ...
            "paired_evaluation_preflight": preflight,
            ...
            "final_decision": {
                ...
                "paired_evaluation_preflight_executed": not failed,
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# V15 Paired Evaluation Preflight",
            "",
            f"- Status: `{decision['status']}`",
            f"- Passed: `{decision['passed']}`",
            f"- Authorized next work: `{decision['authorized_next_work']}`",
            "- This gate preflights paired evaluation only; it does not evaluate.",
            "",
        ]
    )
...
def _render_timing_markdown(timing: dict[str, Any]) -> str:
    lines = ["# V15 Paired Evaluation Timing Contract", ""]
