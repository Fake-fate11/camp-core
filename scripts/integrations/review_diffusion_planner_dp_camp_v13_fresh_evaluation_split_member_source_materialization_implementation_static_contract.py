#!/usr/bin/env python3
"""Static review for v13 fresh member-source materializer implementation plan.

This gate consumes the materialization implementation plan and checks whether a
future materializer implementation may be coded. It does not implement the
materializer, materialize inputs, run the member-source builder, select fresh
members, run DP, generate fixed-DP candidates, replay, prepare data, train
...
REQUIRED_PLAN_TEST_TERMS = (
    "test_fresh_member_source_materialization_implementation_plan_ready",
    "test_fresh_member_source_materialization_implementation_plan_rejects_wrong_audit_target",
    "test_fresh_member_source_materialization_implementation_plan_rejects_source_action_leak",
    "test_fresh_member_source_materialization_implementation_plan_rejects_missing_contracts",
    "test_fresh_member_source_materialization_implementation_plan_rejects_dp_head_drift",
)


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Static-review the v13 fresh member-source materializer implementation plan."
    )
    parser.add_argument("--materialization_implementation_plan_json", type=Path, required=True)
    parser.add_argument("--materialization_implementation_plan_script_py", type=Path, required=True)
    parser.add_argument("--materialization_implementation_plan_test_py", type=Path, required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        materialization_implementation_plan_json=args.materialization_implementation_plan_json,
        materialization_implementation_plan_script_py=args.materialization_implementation_plan_script_py,
        materialization_implementation_plan_test_py=args.materialization_implementation_plan_test_py,
...
def build_report(
    *,
    materialization_implementation_plan_json: Path,
    materialization_implementation_plan_script_py: Path,
    materialization_implementation_plan_test_py: Path,
    v13_audit_md: Path,
    ...
    plan_path = materialization_implementation_plan_json.resolve()
    script_path = materialization_implementation_plan_script_py.resolve()
    test_path = materialization_implementation_plan_test_py.resolve()
    ...
    return {
        ...
        "inputs": {
            "materialization_implementation_plan_json": str(plan_path),
            "materialization_implementation_plan_script_py": str(script_path),
            "materialization_implementation_plan_test_py": str(test_path),
            "v13_audit_md": str(audit_path),
        },
        "source_hashes": {
            "materialization_implementation_plan_json_sha256": _sha256(plan_path),
            "materialization_implementation_plan_script_py_sha256": _sha256(script_path),
            "materialization_implementation_plan_test_py_sha256": _sha256(test_path),
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# V13 Fresh Member-Source Materialization Implementation Static Review",
        "",
        f"- Status: `{decision['status']}`",
        f"- Passed: `{decision['passed']}`",
        f"- Authorized next work: `{decision['authorized_next_work']}`",
        f"- Materialization implementation authorized: `{decision['materialization_implementation_authorized_next']}`",
        ...
        f"- Future materializer script: `{review['future_materializer_script']}`",
        f"- Future materializer test: `{review['future_materializer_test']}`",
        "",
        "This static review authorizes only a future code implementation gate. "
        "It does not materialize inputs or run any evaluation/training path.",
        "",
    ]
    return "\n".join(lines)
...
def _static_contract_review(source_plan: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "implementation_static_contract_review_only": True,
        "implementation_performed_by_this_gate": False,
        "materialization_performed_by_this_gate": False,
        "future_materializer_script": plan.get(
            "future_materializer_script", EXPECTED_FUTURE_MATERIALIZER_SCRIPT
        ),
        "future_materializer_test": plan.get(
            "future_materializer_test", EXPECTED_FUTURE_MATERIALIZER_TEST
        ),
        "required_contract_groups": [
            "source_inputs_and_registry_fail_closed",
            "zero_intersections_all_required",
            "split_root_zero_alone_rejected",
            "formal_seed_and_full36_exclusion",
            "forbidden_execution_and_dp_modification",
            "affine_score_nonnegative_simplex_convex_master",
            "output_manifest_and_sha256sums",
        ],
        ...
        "forbidden_paths": [
            "materialization_execution_by_this_static_review_gate",
            "member_source_builder_execution_by_this_static_review_gate",
            "fresh_member_selection_by_this_static_review_gate",
            "fixed_dp_candidate_generation_by_this_static_review_gate",
...
def _checks(
    *,
    ...
    return [
        ...
        _check("source_plan_script_exists", script_path.is_file(), str(script_path), "file exists"),
        _check("source_plan_test_exists", test_path.is_file(), str(test_path), "file exists"),
        ...
        _expect("audit_implementation_plan_ready", _latest_value(audit_text, "fresh_evaluation_split_member_source_materialization_implementation_plan_ready"), "True"),
        _expect("audit_static_review_authorized", _latest_value(audit_text, "materialization_implementation_static_contract_review_authorized_next"), "True"),
        *[
        ...
        _expect("source_authorizes_this_static_review", decision.get("authorized_next_work"), authorized_current_work),
        _expect("source_static_review_authorized", decision.get("materialization_implementation_static_contract_review_authorized_next"), True),
        *[
            _expect(f"source_blocks_{flag}", decision.get(flag), False)
            for flag in SOURCE_BLOCKED_FLAGS
        ],
        _expect("source_plan_does_not_implement", plan.get("implementation_performed_by_this_gate"), False),
        _expect("source_plan_does_not_materialize", plan.get("materialization_performed_by_this_gate"), False),
        _expect("source_future_script_expected", plan.get("future_materializer_script"), EXPECTED_FUTURE_MATERIALIZER_SCRIPT),
        _expect("source_future_test_expected", plan.get("future_materializer_test"), EXPECTED_FUTURE_MATERIALIZER_TEST),
        _check("source_lists_required_inputs", set(REQUIRED_SOURCE_INPUTS) <= set(_list(plan.get("required_source_inputs"))), plan.get("required_source_inputs"), "required source inputs"),
        _check("source_lists_future_outputs", set(FUTURE_OUTPUTS) <= set(_list(plan.get("future_outputs"))), plan.get("future_outputs"), "future outputs"),
        _check("source_lists_required_behavior", set(REQUIRED_FUTURE_BEHAVIOR) <= set(_list(plan.get("required_future_materializer_behavior"))), plan.get("required_future_materializer_behavior"), "required behavior"),
        _check("source_lists_static_review_requirements", set(FUTURE_STATIC_REVIEW_REQUIREMENTS) <= set(_list(plan.get("future_static_contract_review_requirements"))), plan.get("future_static_contract_review_requirements"), "static review requirements"),
        _check("source_requires_zero_intersections", all(_dict(plan.get("required_zero_intersections")).get(key) == 0 for key in ZERO_INTERSECTION_KEYS), plan.get("required_zero_intersections"), "all zero"),
        _check("source_requires_all_registries", all(_dict(plan.get("required_registry_inputs")).values()), plan.get("required_registry_inputs"), "all True"),
        _expect("source_score_affine", _dict(plan.get("math_boundary")).get("score_expression"), SCORE_EXPRESSION),
        _expect("source_nonnegative_simplex", _dict(plan.get("math_boundary")).get("nonnegative_simplex_weights_only"), True),
        _expect("source_master_convex", _dict(plan.get("math_boundary")).get("master_problem_remains_convex"), True),
        _expect("source_executed_dp_top1", _dict(plan.get("math_boundary")).get("executed_trajectory_remains_dp_top1"), True),
        _check("review_has_contract_groups", len(_list(review.get("required_contract_groups"))) == 7, review.get("required_contract_groups"), "7 contract groups"),
        _check("review_requires_all_inputs", set(REQUIRED_SOURCE_INPUTS) <= set(review["required_source_inputs"]), review["required_source_inputs"], "required inputs"),
        _check("review_requires_future_outputs", set(FUTURE_OUTPUTS) <= set(review["future_outputs"]), review["future_outputs"], "future outputs"),
        _check("review_requires_all_behavior", set(REQUIRED_FUTURE_BEHAVIOR) <= set(review["required_future_materializer_behavior"]), review["required_future_materializer_behavior"], "required behavior"),
        _check("review_requires_static_review_requirements", set(FUTURE_STATIC_REVIEW_REQUIREMENTS) <= set(review["future_static_review_requirements"]), review["future_static_review_requirements"], "static review requirements"),
        _check("review_requires_zero_intersections", all(review["required_zero_intersections"].get(key) == 0 for key in ZERO_INTERSECTION_KEYS), review["required_zero_intersections"], "all zero"),
        _check("review_requires_all_registries", all(review["required_registry_inputs"].values()), review["required_registry_inputs"], "all True"),
        _expect("review_score_affine", review["math_boundary"]["score_expression"], SCORE_EXPRESSION),
        _expect("review_nonnegative_simplex", review["math_boundary"]["nonnegative_simplex_weights_only"], True),
        _expect("review_master_convex", review["math_boundary"]["master_problem_remains_convex"], True),
        _expect("review_executed_dp_top1", review["math_boundary"]["executed_trajectory_remains_dp_top1"], True),
        _check("plan_script_terms_present", all(term in script_text for term in REQUIRED_PLAN_SCRIPT_TERMS), "script terms", "required terms"),
        _check("plan_test_terms_present", all(term in test_text for term in REQUIRED_PLAN_TEST_TERMS), "test terms", "required tests"),
    ]
...
def _decision(
    *,
    ...
    return {
        ...
        "materialization_implementation_static_contract_review_passed": passed,
        "materialization_implementation_authorized_next": passed,
...
def _source_plan_summary(source_plan: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "required_behavior_count": len(_list(plan.get("required_future_materializer_behavior"))),
        "static_review_requirement_count": len(_list(plan.get("future_static_contract_review_requirements"))),
    }
