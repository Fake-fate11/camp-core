...
READY_STATUS = (
    ...
    "fixed_snapshot_screen_rerun_remediation_negative_support_followup_"
    "residual_comfort_failure_diagnostic_remediation_followup_fixed_snapshot_"
    "screen_rerun_plan_ready"
)
REJECT_STATUS = (
    "candidate_set_consensus_lane_projected_jerk_progress_support_default_off_"
    "fixed_snapshot_screen_rerun_remediation_negative_support_followup_"
    "residual_comfort_failure_diagnostic_remediation_followup_fixed_snapshot_"
    "screen_rerun_plan_rejected"
)
AUTHORIZED_NEXT_WORK = (
    "candidate_set_consensus_lane_projected_jerk_progress_support_default_off_"
    "fixed_snapshot_screen_rerun_remediation_negative_support_followup_"
    "residual_comfort_failure_diagnostic_remediation_followup_guarded_fixed_"
    "snapshot_screen_rerun_only"
)
...
DEFAULT_POST_REVIEW_ROOT = (
    ...
    "remediation_followup_post_implementation_static_contract_review_bff8f8b"
)
...
REMEDIATION_PROFILE = "lane_projected_jerk_progress_support_v1"
GUARD_ENV_VAR = (
    "CANDIDATE_SET_CONSENSUS_RESIDUAL_COMFORT_REMEDIATION_FOLLOWUP_"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Plan-only guarded fixed-snapshot screen rerun after follow-up "
            "post-implementation static review."
        )
    )
    parser.add_argument(
        "--post_review_root",
    ...
    return {
        "analysis": {
            "name": (
                ...
                "remediation_followup_fixed_snapshot_screen_rerun_plan_v1"
            ),
            "label": label,
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Residual Comfort Remediation Follow-Up Fixed-Snapshot Screen Rerun Plan",
        "",
        ...
        "## Scope",
        "",
        f"- Snapshot dir: `{plan['snapshot_scope']['snapshot_dir']}`",
        f"- Expected snapshots: `{plan['snapshot_scope']['expected_snapshot_count']}`",
        f"- Generator policy: `{plan['candidate_config']['generator_policy']}`",
        f"- Remediation profile: `{plan['candidate_config']['default_off_remediation_profile']}`",
        "",
        "## Forbidden Work",
        "",
    ]
...
"${{PY}}" scripts/integrations/analyze_diffusion_planner_route_topology_candidate_screen.py \\
  ...
  --label residual_comfort_remediation_followup_fixed_snapshot_screen \\
...
cd "${{OUTPUT_ROOT}}"
sha256sum \\
  residual_comfort_remediation_followup_fixed_snapshot_screen.json \\
  residual_comfort_remediation_followup_fixed_snapshot_screen.md \\
...
def _screen_rerun_plan(execution_root: Path) -> dict[str, Any]:
    ...
    return {
        "selection_type": "residual_comfort_remediation_followup_fixed_snapshot_screen_rerun_plan_only",
        ...
        "execution_artifacts": {
            "output_root": root,
            "screen_json": f"{root}/residual_comfort_remediation_followup_fixed_snapshot_screen.json",
            "screen_md": f"{root}/residual_comfort_remediation_followup_fixed_snapshot_screen.md",
...
def _plan_checks(plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    config = plan["candidate_config"]
    return [
        _check("plan_selection_type", plan["selection_type"] == "residual_comfort_remediation_followup_fixed_snapshot_screen_rerun_plan_only"),
        ...
        _check("plan_uses_followup_policy", config["generator_policy"] == PLANNED_POLICY),
        _check("plan_uses_support_profile", config["default_off_remediation_profile"] == REMEDIATION_PROFILE),
...
def _runbook_checks(runbook_text: str) -> list[dict[str, Any]]:
    ...
    return [
        ...
        _check("runbook_uses_followup_policy", f"--generator_policy {PLANNED_POLICY}" in runbook_text),
        _check("runbook_uses_support_profile", f"--default_off_remediation_profile {REMEDIATION_PROFILE}" in runbook_text),
