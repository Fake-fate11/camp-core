#!/usr/bin/env python3
"""Preflight the v15 broader non-formal offline training gate."""
...
def _load_static_review_module():
    path = Path(__file__).resolve().with_name(
        "review_diffusion_planner_dp_camp_v15_broader_nonformal_evidence_expansion_offline_training_preflight_plan_static_contract.py"
    )
    spec = importlib.util.spec_from_file_location("v15_offline_training_plan_static_review", path)
...
SCHEMA_VERSION = "dp_camp_v15_broader_nonformal_evidence_expansion_offline_training_preflight_v1"
AUTHORIZED_CURRENT_WORK = STATIC_REVIEW_MODULE.AUTHORIZED_NEXT_WORK
READY_STATUS = "v15_broader_nonformal_evidence_expansion_offline_training_preflight_ready"
REJECT_STATUS = "v15_broader_nonformal_evidence_expansion_offline_training_preflight_rejected"
AUTHORIZED_NEXT_WORK = "v15_broader_nonformal_evidence_expansion_offline_training_preflight_static_review_only"
PREFLIGHT_JSON_NAME = "v15_broader_nonformal_evidence_expansion_offline_training_preflight.json"
PREFLIGHT_MD_NAME = "v15_broader_nonformal_evidence_expansion_offline_training_preflight.md"
TIMING_JSON_NAME = "v15_broader_nonformal_evidence_expansion_offline_training_preflight_timing_contract.json"
TIMING_MD_NAME = "v15_broader_nonformal_evidence_expansion_offline_training_preflight_timing_contract.md"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v15_broader_nonformal_evidence_expansion_offline_training_preflight",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        enabled=args.enable_v15_broader_nonformal_evidence_expansion_offline_training_preflight,
    )
...
def build_report(
    *,
    ...
    plan = PLAN_MODULE._training_preflight_plan(current_camp_head, current_dp_head)
    timing = plan["timing_contract"]

    checks = [
        _expect("offline_training_preflight_enabled", enabled, True),
        ...
        _check("preflight_has_training_command", bool(plan["planned_training_command"]), plan["planned_training_command"], "command"),
        _check("preflight_has_split_inputs", set(plan["required_inputs"]["splits"]) == {"train", "calibration", "holdout"}, plan["required_inputs"]["splits"], "three splits"),
        _expect("preflight_full36_used", plan["blocked_inputs"]["Full36"], False),
        _expect("preflight_formal_seed_used", plan["blocked_inputs"]["formal_seeds_11_12_13"], False),
        _expect("preflight_dp_modified", plan["mutations"]["dp_modified"], False),
        _expect("preflight_candidate_tensor_modified", plan["mutations"]["candidate_tensor_modified"], False),
        _expect("preflight_trajectory_modified", plan["mutations"]["trajectory_modified"], False),
    ...
    return _stable(
        {
            ...
            "offline_training_preflight": plan,
            ...
            "final_decision": {
                ...
                "offline_training_preflight_executed": not failed,
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# V15 Offline Training Preflight",
            "",
            f"- Status: `{decision['status']}`",
            f"- Passed: `{decision['passed']}`",
            f"- Authorized next work: `{decision['authorized_next_work']}`",
            "- This gate preflights offline training only; it does not train.",
            "",
        ]
    )
...
def _render_timing_markdown(timing: dict[str, Any]) -> str:
    lines = [
        "# V15 Offline Training Preflight Timing Contract",
        "",
