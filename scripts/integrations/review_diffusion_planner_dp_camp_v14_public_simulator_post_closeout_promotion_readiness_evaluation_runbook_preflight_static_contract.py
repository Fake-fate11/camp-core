#!/usr/bin/env python3
"""Static review for the v14 promotion-readiness runbook/preflight artifact."""
...
REJECT_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_readiness_evaluation_runbook_preflight_static_review_rejected"
)
...
EXPECTED_PREFLIGHT_STEPS = (
    ...
    "fixed_dp_candidate_boundary",
    "split_seed_zero_overlap_boundary",
    "default_off_shadow_selector_boundary",
    "metric_uncertainty_boundary",
...
EXPECTED_FUTURE_REQUIREMENTS = (
    "runbook_preflight_static_review",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--runbook_preflight_artifact_dir", type=Path, required=True)
    parser.add_argument("--runbook_preflight_json", type=Path, required=True)
    parser.add_argument("--runbook_preflight_md", type=Path, required=True)
    parser.add_argument("--runbook_preflight_sha256s", type=Path, required=True)
    ...
    parser.add_argument(
        "--enable_v14_post_closeout_promotion_readiness_evaluation_runbook_preflight_static_review",
        action="store_true",
        help="Explicit opt-in for static review of the runbook/preflight artifact.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        runbook_preflight_artifact_dir=args.runbook_preflight_artifact_dir,
        runbook_preflight_json=args.runbook_preflight_json,
        runbook_preflight_md=args.runbook_preflight_md,
        runbook_preflight_sha256s=args.runbook_preflight_sha256s,
        ...
        label=args.label,
        enabled=args.enable_v14_post_closeout_promotion_readiness_evaluation_runbook_preflight_static_review,
    )
...
def build_report(
    *,
    runbook_preflight_artifact_dir: Path,
    runbook_preflight_json: Path,
    runbook_preflight_md: Path,
    runbook_preflight_sha256s: Path,
    ...
    artifact_dir = runbook_preflight_artifact_dir.resolve()
    paths = {
        "runbook_preflight_json": runbook_preflight_json.resolve(),
        "runbook_preflight_md": runbook_preflight_md.resolve(),
        "runbook_preflight_sha256s": runbook_preflight_sha256s.resolve(),
    ...
    source_preflight = _read_json_dict(paths["runbook_preflight_json"])
    root_sha256s = _read_sha256sums(artifact_files["root_sha256s"])
    preflight_sha256s = _read_sha256sums(paths["runbook_preflight_sha256s"])
    ...
    checks: list[dict[str, Any]] = [
        ...
        _check("runbook_preflight_artifact_dir_exists", artifact_dir.is_dir(), str(artifact_dir), "directory"),
    ]
    ...
    checks.extend(
        [
            _expect("preflight_json_matches_artifact_layout", paths["runbook_preflight_json"], artifact_files["preflight_json"]),
            _expect("preflight_md_matches_artifact_layout", paths["runbook_preflight_md"], artifact_files["preflight_md"]),
            _expect("preflight_sha256s_matches_artifact_layout", paths["runbook_preflight_sha256s"], artifact_files["preflight_sha256s"]),
        ]
    )
    ...
    return {
        ...
        "analysis": {
            ...
            "read_only": True,
            "runbook_preflight_artifact_dir": str(artifact_dir),
            "runbook_preflight_json": str(paths["runbook_preflight_json"]),
            "runbook_preflight_md": str(paths["runbook_preflight_md"]),
            "runbook_preflight_sha256s": str(paths["runbook_preflight_sha256s"]),
...
def _heads_checks(heads: dict[str, str], source_preflight: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        _expect("artifact_heads_source_static_review_path", heads.get("source_runbook_preflight_plan_static_review_artifact"), analysis.get("runbook_preflight_plan_static_review_artifact_dir")),
        _expect("artifact_heads_source_plan_path", heads.get("source_runbook_preflight_plan_artifact"), analysis.get("source_runbook_preflight_plan_artifact_dir")),
    ]
...
def _source_preflight_contract_checks(source_preflight: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    checks = [
        ...
        _expect("source_preflight_steps", _names(source_preflight.get("runbook_preflight")), list(EXPECTED_PREFLIGHT_STEPS)),
        ...
        _expect("source_static_review_check_count", _dict(source_preflight.get("source_static_review_summary")).get("review_check_count"), 139),
        _expect("source_plan_check_count", _dict(source_preflight.get("source_runbook_preflight_plan_summary")).get("plan_check_count"), 223),
        _expect("source_plan_runbook_preflight_step_count", _dict(source_preflight.get("source_runbook_preflight_plan_summary")).get("runbook_preflight_step_count"), 6),
        _expect("source_plan_no_go_condition_count", _dict(source_preflight.get("source_runbook_preflight_plan_summary")).get("no_go_condition_count"), 8),
    ]
...
def _source_surface_checks(script: str, test: str) -> list[dict[str, Any]]:
    return [
        _contains("source_surface_script_schema", script, "promotion_readiness_evaluation_runbook_preflight_v1"),
        _contains("source_surface_script_static_review_next", script, "runbook_preflight_static_review_only"),
        _contains("source_surface_script_future_static_review_requirement", script, "runbook_preflight_static_review"),
        ...
        _contains("source_surface_test_pass_case", test, "test_promotion_readiness_evaluation_runbook_preflight_passes"),
        _contains("source_surface_test_requires_enable", test, "test_promotion_readiness_evaluation_runbook_preflight_requires_enable"),
        _contains("source_surface_test_rejects_wrong_eof", test, "test_promotion_readiness_evaluation_runbook_preflight_rejects_wrong_eof"),
        _contains("source_surface_test_accepts_uppercase_dp_head", test, "test_promotion_readiness_evaluation_runbook_preflight_accepts_uppercase_dp_head"),
    ]
...
def _audit_checks(v14_text: str, status_text: str) -> list[dict[str, Any]]:
    ...
    return [
        ...
        _expect("audit_runbook_preflight_ready", _latest_value(v14_text, "post_closeout_promotion_readiness_evaluation_runbook_preflight_ready"), "True"),
        _expect("audit_runbook_preflight_static_review_authorized", _latest_value(v14_text, "post_closeout_promotion_readiness_evaluation_runbook_preflight_static_review_authorized"), "True"),
...
def _decision(passed: bool, checks: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    decision = {
        ...
        "post_closeout_promotion_readiness_evaluation_runbook_preflight_static_review_passed": bool(passed),
        "evaluation_runbook_plan_authorized": bool(passed),
        "evaluation_runbook_execution_authorized": False,
        "recommendation": "plan_read_only_promotion_readiness_evaluation_runbook_only",
        "immediate_action": "plan_promotion_readiness_evaluation_runbook_only",
...
def _failure_class(failed: list[str]) -> str:
    ...
    if "static_review_enabled" in failed_set:
        return "explicit_runbook_preflight_static_review_authorization_missing"
    ...
    if any(name.endswith("_sha") or name.endswith("_root_sha") for name in failed):
        return "runbook_preflight_artifact_sha256_mismatch"
    ...
    return "promotion_readiness_evaluation_runbook_preflight_static_review_failure"
