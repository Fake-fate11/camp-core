#!/usr/bin/env python3
"""Static-review the v15 matrix execution preflight plan."""
...
def _load_plan_module():
    path = Path(__file__).resolve().with_name(
        "plan_diffusion_planner_dp_camp_v15_broader_nonformal_evidence_expansion_matrix_execution_preflight.py"
    )
    spec = importlib.util.spec_from_file_location("v15_matrix_preflight_plan", path)
...
SCHEMA_VERSION = "dp_camp_v15_broader_nonformal_evidence_expansion_matrix_execution_preflight_plan_static_review_v1"
AUTHORIZED_CURRENT_WORK = PLAN_MODULE.AUTHORIZED_NEXT_WORK
READY_STATUS = "v15_broader_nonformal_evidence_expansion_matrix_execution_preflight_plan_static_review_passed"
REJECT_STATUS = "v15_broader_nonformal_evidence_expansion_matrix_execution_preflight_plan_static_review_rejected"
AUTHORIZED_NEXT_WORK = "v15_broader_nonformal_evidence_expansion_matrix_execution_preflight_only"
REVIEW_JSON_NAME = "v15_broader_nonformal_evidence_expansion_matrix_execution_preflight_plan_static_review.json"
REVIEW_MD_NAME = "v15_broader_nonformal_evidence_expansion_matrix_execution_preflight_plan_static_review.md"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v15_broader_nonformal_evidence_expansion_matrix_execution_preflight_plan_static_review",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        enabled=args.enable_v15_broader_nonformal_evidence_expansion_matrix_execution_preflight_plan_static_review,
    )
...
def build_report(
    *,
    ...
    sha256s = _read_sha256s(source_plan_sha256s)
    ...
    checks = [
        ...
        _expect("matrix_combination_count", matrix.get("combination_count"), 576),
        _expect("matrix_timing_required", matrix.get("timing_required"), True),
        _expect("matrix_protocol", matrix.get("paired_protocol"), "camp_selected_fixed_dp_candidate_vs_dp_top1"),
    ]
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# V15 Matrix Execution Preflight Plan Static Review",
            "",
