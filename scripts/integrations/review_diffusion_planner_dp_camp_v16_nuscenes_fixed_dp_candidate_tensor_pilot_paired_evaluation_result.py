#!/usr/bin/env python3
"""Review the v16 fixed-DP pilot paired-evaluation execution result."""
...
SOURCE_MODULE = _load_module(
    ...
    "v16_pilot_paired_eval_execution",
)
...
READY_STATUS = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_paired_evaluation_result_review_passed"
REJECT_STATUS = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_paired_evaluation_result_review_rejected"
AUTHORIZED_NEXT_WORK = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_evidence_package_plan_only"
SCHEMA_VERSION = "dp_camp_v16_nuscenes_fixed_dp_candidate_tensor_pilot_paired_evaluation_result_review_v1"
...
REVIEW_JSON_NAME = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_paired_evaluation_result_review.json"
REVIEW_MD_NAME = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_paired_evaluation_result_review.md"
EXPECTED_BETTER_TIE_WORSE = {"better": 158, "tie": 3, "worse": 0}
EXPECTED_MEAN_DELTA = -0.0729566626154565
EXPECTED_NON_TOP1_SELECTION_RATE = 0.9813664596273292
EXPECTED_ORACLE_GAP_CLOSED = 0.9993321161828008
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v16_nuscenes_fixed_dp_candidate_tensor_pilot_paired_evaluation_result_review",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        enabled=args.enable_v16_nuscenes_fixed_dp_candidate_tensor_pilot_paired_evaluation_result_review,
    )
...
def build_report(
    *,
    ...
    source_execution = source.get("pilot_paired_evaluation_execution", {})
    ...
    checks = [
        ...
        _expect("primary_eval_rows_161", review["primary_eval_rows"], EXPECTED_COUNTS["calibration"] + EXPECTED_COUNTS["holdout"]),
        _expect("calibration_rows_14", review["calibration_rows"], EXPECTED_COUNTS["calibration"]),
        _expect("holdout_rows_147", review["holdout_rows"], EXPECTED_COUNTS["holdout"]),
        _expect("train_reporting_only_rows_863", review["train_reporting_only_rows"], EXPECTED_COUNTS["train"]),
        _expect("train_rows_excluded_from_primary_eval", review["train_rows_in_primary_eval"], 0),
        _expect("source_primary_rows_161", source_execution.get("paired_rows_by_split", {}).get("primary_eval_total"), 161),
        _expect("source_calibration_rows_14", source_execution.get("paired_rows_by_split", {}).get("calibration"), 14),
        _expect("source_holdout_rows_147", source_execution.get("paired_rows_by_split", {}).get("holdout"), 147),
        _expect("source_train_reporting_only_863", source_execution.get("paired_rows_by_split", {}).get("train_reporting_only"), 863),
        ...
        _expect("latency_count_161", review["latency_summary"].get("count"), 161),
        ...
        _expect("smoke_only_result", review["smoke_only_result"], True),
    ]
...
def _paired_evaluation_result_review(
    ...
    execution = source.get("pilot_paired_evaluation_execution", {})
    ...
    return {
        ...
        "smoke_only_result": execution.get("pilot_eval_smoke_only") is True,
...
def _no_claim_checks(final: dict[str, Any], execution: dict[str, Any]) -> list[dict[str, Any]]:
    checks = [
        ...
        _expect("source_smoke_only_true", execution.get("pilot_eval_smoke_only"), True),
    ]
    return checks
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "- Smoke-only result. No performance, safety, or CAMP-over-DP claim is made.",
            "- No promotion or deployment is authorized.",
            "- Recommended next gate: pilot evidence package / scale-up plan only.",
            "",
        ]
    )
