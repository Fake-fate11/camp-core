...
PAYLOAD_AUDIT = (
    ROOT / "scripts/integrations/analyze_diffusion_planner_temporal_consistency_payload_smoke.py"
)
...
RUNTIME_READY_STATUS = "temporal_consistency_payload_runtime_preflight_ready"
RUNTIME_READY_NEXT_WORK = "default_off_temporal_consistency_tiny_paired_smoke_plan_only"
READY_STATUS = "temporal_consistency_payload_nonformal_smoke_plan_ready"
REJECT_STATUS = "temporal_consistency_payload_nonformal_smoke_plan_rejected"
AUTHORIZED_NEXT_WORK = "default_off_temporal_consistency_paired_three_step_smoke_only"
...
class SmokeSpec:
    ...
    root: str = "/root/autodl-tmp/camp_dp_temporal_consistency_payload_smoke"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Design-only preflight for a default-off temporal consistency "
            "payload smoke. It emits paired nonformal replay commands and does "
            "not run Diffusion Planner."
        )
    )
    parser.add_argument("--runtime_preflight_json", type=Path, required=True)
...
def main() -> None:
    args = parse_args()
    report = build_report(
        runtime_preflight=_load_json(args.runtime_preflight_json),
        label=args.label,
        ...
        paths={"runtime_preflight_json": str(args.runtime_preflight_json)},
    )
...
def build_report(
    *,
    ...
    runtime = _runtime_summary(runtime_preflight)
    ...
    plan_checks = [*_runtime_checks(runtime), *_plan_checks(smoke)]
    ...
    return {
        "analysis": {
            "name": "dp_camp_temporal_consistency_payload_nonformal_smoke_plan_v1",
            "label": label,
            "training": False,
            ...
            "math_boundary": (
                ...
                "temporal consistency descriptors derived from fixed DP "
                "candidates and previous selected-plan memory before selection. "
                "It must not change CAMP scores, feasibility, selected indices, "
                "DP candidates, or PerfectTracker execution. If later atomized, "
                "previous_plan_temporal_consistency_rms_m is a fixed finite "
                "nonnegative coefficient preserving affine score_k(w)=a_k^T w "
                "and the simplex/CVaR/L2 convex master. This is not a DP-side "
                "classical Benders decomposition."
            ),
        },
        "runtime_preflight_summary": runtime,
...
def _runtime_summary(report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "runtime_preflight_ready": bool(decision.get("runtime_preflight_ready")),
        "authorized_next_work": decision.get("authorized_next_work"),
        "blocked_action_conflicts": conflicts,
    }


def _runtime_checks(runtime: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        _check_equal("runtime_status", runtime["status"], RUNTIME_READY_STATUS),
        _check_equal("runtime_preflight_passed", runtime["passed"], True),
        _check_equal("runtime_preflight_ready", runtime["runtime_preflight_ready"], True),
        _check_equal(
            "runtime_authorizes_smoke_plan",
            runtime["authorized_next_work"],
            RUNTIME_READY_NEXT_WORK,
        ),
        _check_empty("runtime_no_blocked_actions", runtime["blocked_action_conflicts"]),
    ]
...
def _source_checks(
    *,
    ...
    return [
        _check_tokens(
            "replay_default_off_temporal_consistency_payload_cli",
            replay_text,
            (
        ...
        _check_order(
            "replay_payload_before_selection_memory_update",
            replay_text,
            "build_temporal_consistency_payload(",
        ...
        _check_tokens(
            ...
            payload_text,
            (
                "dp_camp_temporal_consistency_payload_smoke_audit_v1",
                "temporal_consistency_payload_logging",
...
def _plan_checks(smoke: SmokeSpec) -> list[dict[str, Any]]:
    return [
        ...
        _check_equal("payload_dt_valid", smoke.payload_dt_s > 0.0, True),
        _check_equal("payload_elapsed_steps_valid", smoke.payload_elapsed_steps >= 0, True),
        _check_equal(
            "payload_min_overlap_valid",
            smoke.payload_min_overlap_steps >= 2,
            True,
        ),
        _check_equal("requires_two_available_records", smoke.min_available_records, 2),
        _check_equal(
            "requires_one_first_tick_fail_closed",
            smoke.expected_first_tick_fail_closed,
            1,
        ),
    ...
    if logging:
        command.extend(
            [
                ...
                str(smoke.payload_elapsed_steps),
                "--camp_temporal_consistency_payload_min_overlap_steps",
                str(smoke.payload_min_overlap_steps),
            ]
        )
    return command
...
def _payload_audit_command(
    ...
    return [
        ...
        str(smoke.expected_first_tick_fail_closed),
        "--output_json",
        f"{audit_dir}/temporal_consistency_payload_smoke.json",
        "--output_md",
        f"{audit_dir}/temporal_consistency_payload_smoke.md",
        "--require_pass",
    ]
...
def _accept_criteria(smoke: SmokeSpec) -> list[str]:
    return [
        ...
        "exactly one first-tick previous-plan-missing fail-closed record is present",
        f"at least {smoke.min_available_records} later records have available finite temporal consistency coefficients",
...
def _reject_criteria() -> list[str]:
    return [
        ...
        "missing previous-plan memory produces a neutral zero atom instead of fail-closed unavailable payload",
        "fewer than two available temporal consistency records are produced in the 3-step smoke",
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Temporal Consistency Payload Smoke Plan",
        "",
...
def render_bash(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "set -euo pipefail",
        "",
        "# Auto-generated from dp_camp_temporal_consistency_payload_nonformal_smoke_plan_v1.",
    ...
    lines.append('echo "temporal_consistency_payload_paired_three_step_smoke_complete"')
    lines.append("")
    return "\n".join(lines)
