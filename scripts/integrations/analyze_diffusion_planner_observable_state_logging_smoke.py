...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Audit a paired default-off observable-state logging smoke. The "
            "baseline run must keep logging disabled and the candidate run must "
            "enable logging without collecting future outcome labels."
        )
    )
...
def analyze(
    *,
    ...
    forbidden_seeds = sorted(
        _path_seeds(path)
        for path in [*baseline_logs.values(), *candidate_logs.values()]
    )
    forbidden_seen = sorted(
        {seed for seeds in forbidden_seeds for seed in seeds if seed in FORBIDDEN_SEEDS}
    )
    if forbidden_seen:
    ...
    for key in sorted(set(baseline_logs) & set(candidate_logs)):
        ...
        for idx, (baseline, candidate) in enumerate(zip(baseline_rows, candidate_rows)):
            if not isinstance(baseline, dict) or not isinstance(candidate, dict):
                ...
                continue
            baseline_payload = baseline.get("observable_state_logging")
            candidate_payload = candidate.get("observable_state_logging")
            ...
            record_reports.append(
                {
                    ...
                    "red_route_point_count": candidate_payload.get(
                        "red_route_point_count"
                    ),
                    "route_segment_count": candidate_payload.get(
                        "route_segment_count"
                    ),
                }
            )
    ...
    return {
        "analysis": {
            "name": "dp_camp_observable_state_logging_smoke_audit_v1",
        ...
        "final_decision": {
            ...
            "authorized_next_work": (
                "observable_state_logging_smoke_result_documentation_only"
                if not errors
                else None
            ),
...
def _validate_summary(
    ...
    *,
    ...
    metadata = summary.get("camp_observable_state_logging")
    if not isinstance(metadata, dict):
        errors.append(f"{key}: missing camp_observable_state_logging summary")
        return
    expected = {
        "schema_version": SCHEMA_VERSION,
    ...
    if expected_enabled and int(metadata.get("records", -1)) <= 0:
        errors.append(f"{key}: enabled summary has no observable records")
    fields = metadata.get("fields")
    if expected_enabled and fields != list(EXPECTED_FIELDS):
        errors.append(f"{key}: summary field list mismatch")
    latency_fields = metadata.get("latency_fields")
    if expected_enabled and latency_fields != list(LATENCY_FIELDS):
...
def _validate_payload(
    ...
    *,
    ...
    if not isinstance(shapes, dict):
        ...
        for field in EXPECTED_FIELDS:
            if field not in shapes:
                errors.append(f"{prefix}: field_shapes missing {field}")
    ...
    red_count = int(payload.get("red_route_point_count", -1))
    if red_count == 0:
        for field in (
            "candidate_red_stopline_distance_m",
            "candidate_red_heading_alignment",
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Observable State Logging Smoke Audit",
        "",
