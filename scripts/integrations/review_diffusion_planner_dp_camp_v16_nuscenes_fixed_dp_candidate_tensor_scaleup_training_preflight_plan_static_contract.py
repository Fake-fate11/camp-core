#!/usr/bin/env python3
"""Static-review the v16 fixed-DP scale-up training preflight plan."""
...
def _load_plan_module():
    ...
    spec = importlib.util.spec_from_file_location("v16_scaleup_training_preflight_plan", path)
...
READY_STATUS = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_preflight_plan_static_review_passed"
REJECT_STATUS = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_preflight_plan_static_review_rejected"
AUTHORIZED_NEXT_WORK = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_preflight_only"
SCHEMA_VERSION = "dp_camp_v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_preflight_plan_static_review_v1"
REVIEW_JSON_NAME = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_preflight_plan_static_review.json"
REVIEW_MD_NAME = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_preflight_plan_static_review.md"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_preflight_plan_static_review",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        enabled=args.enable_v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_preflight_plan_static_review,
    )
...
def build_report(
    *,
    ...
    plan = source.get("scaleup_training_preflight_plan", {})
    ...
    checks = [
        ...
        _expect("train_records_6263", inputs.get("train_records"), PLAN_MODULE.EXPECTED_RECORD_COUNTS["train"]),
        _expect("calibration_records_2156", inputs.get("calibration_records_available"), PLAN_MODULE.EXPECTED_RECORD_COUNTS["calibration"]),
        _expect("holdout_records_1581", inputs.get("holdout_records_available"), PLAN_MODULE.EXPECTED_RECORD_COUNTS["holdout"]),
        _expect("training_split_train_only", inputs.get("training_splits"), ["train"]),
        _expect("calibration_holdout_forbidden", inputs.get("forbidden_training_splits"), ["calibration", "holdout"]),
        ...
        _expect("no_closed_loop_outcomes_as_training_input", math.get("no_closed_loop_outcomes_as_training_input"), True),
        _expect("no_candidate_tensor_mutation", math.get("no_candidate_tensor_mutation"), True),
        _expect("no_dp_modification", math.get("no_dp_modification"), True),
    ...
    return _stable(
        {
            ...
            "plan_static_review": {
                ...
                "training_splits": inputs.get("training_splits"),
                "forbidden_training_splits": inputs.get("forbidden_training_splits"),
                ...
                "no_closed_loop_outcomes_as_training_input": math.get("no_closed_loop_outcomes_as_training_input"),
                "no_candidate_tensor_mutation": math.get("no_candidate_tensor_mutation"),
                "no_dp_modification": math.get("no_dp_modification"),
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# V16 nuScenes Fixed-DP Scale-Up Training Preflight Plan Static Review",
            "",
            ...
            f"- Training splits: `{review['training_splits']}`",
            f"- Forbidden training splits: `{review['forbidden_training_splits']}`",
...
def _required_stop_condition_checks(stop_conditions: list[str]) -> list[dict[str, Any]]:
    ...
    return [
        _check(
            f"stop_condition_{condition}",
            condition in stop_conditions,
            condition if condition in stop_conditions else "missing",
            condition,
        )
...
def _source_file_checks(
    ...
    for name in REQUIRED_SOURCE_FILES:
        ...
        if name not in ("SHA256SUMS", "ROOT_SHA256SUMS"):
            checks.append(
                _check(
                    f"source_sha256s_has_{name}",
                    _manifest_has(source_plan_sha256s, name),
                    "present" if _manifest_has(source_plan_sha256s, name) else "missing",
                    name,
                )
            )
