#!/usr/bin/env python3
"""Static review for the post-closeout paired-evaluation execution plan."""
...
SCHEMA_VERSION = (
    ...
    "promotion_evidence_acquisition_paired_evaluation_execution_plan_static_review_v1"
)
...
REJECT_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_paired_evaluation_execution_plan_static_review_rejected"
)
...
REVIEW_JSON_NAME = (
    "post_closeout_promotion_evidence_acquisition_paired_evaluation_execution_plan_static_review.json"
)
REVIEW_MD_NAME = (
    "post_closeout_promotion_evidence_acquisition_paired_evaluation_execution_plan_static_review.md"
)

EXPECTED_PLAN_CHECK_COUNT = 144
EXPECTED_REQUIRED_INPUT_COUNT = len(PLAN_MODULE.EXPECTED_REQUIRED_INPUTS)
EXPECTED_EXECUTION_PLAN_COUNT = len(PLAN_MODULE.EXPECTED_EXECUTION_PLAN_ITEMS)
EXPECTED_PLANNED_OUTPUT_COUNT = len(PLAN_MODULE.EXPECTED_PLANNED_OUTPUTS)
EXPECTED_NO_GO_COUNT = len(PLAN_MODULE.SOURCE_REVIEW_MODULE.EXPECTED_NO_GO)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--execution_plan_artifact_dir", type=Path, required=True)
    parser.add_argument("--execution_plan_json", type=Path, required=True)
    parser.add_argument("--execution_plan_md", type=Path, required=True)
    parser.add_argument("--execution_plan_sha256s", type=Path, required=True)
    ...
    parser.add_argument(
        "--enable_v14_post_closeout_promotion_evidence_acquisition_paired_evaluation_execution_plan_static_review",
        action="store_true",
        help="Explicit opt-in for static review of the paired-evaluation execution plan.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        execution_plan_artifact_dir=args.execution_plan_artifact_dir,
        execution_plan_json=args.execution_plan_json,
        execution_plan_md=args.execution_plan_md,
        execution_plan_sha256s=args.execution_plan_sha256s,
        ...
        label=args.label,
        enabled=args.enable_v14_post_closeout_promotion_evidence_acquisition_paired_evaluation_execution_plan_static_review,
    )
...
def build_report(
    *,
    execution_plan_artifact_dir: Path,
    execution_plan_json: Path,
    execution_plan_md: Path,
    execution_plan_sha256s: Path,
    plan_script_py: Path,
    plan_test_py: Path,
    ...
    artifact_dir = execution_plan_artifact_dir.resolve()
    paths = {
        "execution_plan_json": execution_plan_json.resolve(),
        "execution_plan_md": execution_plan_md.resolve(),
        "execution_plan_sha256s": execution_plan_sha256s.resolve(),
    ...
    source_plan = BASE_MODULE._read_json_dict(paths["execution_plan_json"])
    root_sha256s = BASE_MODULE._read_sha256sums(artifact_files["root_sha256s"])
    plan_sha256s = BASE_MODULE._read_sha256sums(paths["execution_plan_sha256s"])
    ...
    checks: list[dict[str, Any]] = [
        ...
        BASE_MODULE._check("execution_plan_artifact_dir_exists", artifact_dir.is_dir(), str(artifact_dir), "directory"),
    ]
    ...
    checks.extend(
        [
            BASE_MODULE._expect("plan_json_matches_artifact_layout", paths["execution_plan_json"], artifact_files["plan_json"]),
            BASE_MODULE._expect("plan_md_matches_artifact_layout", paths["execution_plan_md"], artifact_files["plan_md"]),
            BASE_MODULE._expect("plan_sha256s_matches_artifact_layout", paths["execution_plan_sha256s"], artifact_files["plan_sha256s"]),
        ]
    )
    ...
    return {
        ...
        "analysis": {
            ...
            "source_execution_plan_artifact_dir": str(artifact_dir),
            "source_execution_plan_json": str(paths["execution_plan_json"]),
...
def _markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Post-Closeout Promotion Evidence Acquisition Paired Evaluation Execution Plan Static Review",
        "",
...
def _artifact_hash_checks(
    ...
    return [
        ...
        BASE_MODULE._sha256sums_expect("source_plan_json_nested_sha", artifact_files["plan_json"], plan_sha256s, (SOURCE_PLAN_JSON_NAME, f"./{SOURCE_PLAN_JSON_NAME}")),
        BASE_MODULE._sha256sums_expect("source_plan_md_nested_sha", artifact_files["plan_md"], plan_sha256s, (SOURCE_PLAN_MD_NAME, f"./{SOURCE_PLAN_MD_NAME}")),
...
def _source_plan_contract_checks(source_plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    required_inputs = BASE_MODULE._list(source_plan.get("required_inputs"))
    execution_plan = BASE_MODULE._list(source_plan.get("execution_plan"))
    planned_outputs = BASE_MODULE._list(source_plan.get("planned_outputs"))
    no_go = BASE_MODULE._list(source_plan.get("no_go_register"))
    checks = [
        ...
        BASE_MODULE._expect("source_plan_ready", decision.get("post_closeout_promotion_evidence_acquisition_paired_evaluation_execution_plan_ready"), True),
        BASE_MODULE._expect("source_plan_static_review_authorized", decision.get("post_closeout_promotion_evidence_acquisition_paired_evaluation_execution_plan_static_review_authorized"), True),
        BASE_MODULE._expect("source_plan_no_paired_execution", decision.get("paired_evaluation_executed_by_this_gate"), False),
        ...
        BASE_MODULE._expect("source_review_status", source_review.get("status"), PLAN_MODULE.SOURCE_REVIEW_STATUS),
        BASE_MODULE._expect("source_review_check_count", source_review.get("review_check_count"), PLAN_MODULE.EXPECTED_SOURCE_REVIEW_CHECK_COUNT),
        BASE_MODULE._expect("source_review_failed_count", source_review.get("failed_check_count"), 0),
        BASE_MODULE._expect("source_review_authorized_next", source_review.get("authorized_next_work"), PLAN_MODULE.AUTHORIZED_CURRENT_WORK),
        BASE_MODULE._expect("source_required_input_names", [item.get("name") for item in required_inputs], list(PLAN_MODULE.EXPECTED_REQUIRED_INPUTS)),
        BASE_MODULE._expect("source_execution_plan_names", [item.get("name") for item in execution_plan], list(PLAN_MODULE.EXPECTED_EXECUTION_PLAN_ITEMS)),
        BASE_MODULE._expect("source_execution_plan_no_execution", sorted({item.get("executes_paired_evaluation") for item in execution_plan}), [False]),
        BASE_MODULE._expect("source_planned_output_names", [item.get("name") for item in planned_outputs], list(PLAN_MODULE.EXPECTED_PLANNED_OUTPUTS)),
        BASE_MODULE._expect("source_no_go_names", [item.get("name") for item in no_go], list(PLAN_MODULE.SOURCE_REVIEW_MODULE.EXPECTED_NO_GO)),
...
def _source_surface_checks(script_text: str, test_text: str, safety_score_text: str) -> list[dict[str, Any]]:
    return [
        BASE_MODULE._check("plan_script_schema_token", "promotion_evidence_acquisition_paired_evaluation_execution_plan_v1" in script_text, "promotion_evidence_acquisition_paired_evaluation_execution_plan_v1", "present"),
        BASE_MODULE._check("plan_script_static_review_next", "paired_evaluation_execution_plan_static_review_only" in script_text, "paired_evaluation_execution_plan_static_review_only", "present"),
        BASE_MODULE._check("plan_script_required_inputs", "EXPECTED_REQUIRED_INPUTS" in script_text, "EXPECTED_REQUIRED_INPUTS", "present"),
        BASE_MODULE._check("plan_script_no_execution_flag", "paired_evaluation_executed_by_this_gate" in script_text, "paired_evaluation_executed_by_this_gate", "present"),
        BASE_MODULE._check("plan_test_rejects_hash_drift", "rejects_hash_drift" in test_text, "rejects_hash_drift", "present"),
        BASE_MODULE._check("plan_test_rejects_execution_leak", "rejects_source_execution_leak" in test_text, "rejects_source_execution_leak", "present"),
        BASE_MODULE._check("plan_test_requires_enable", "requires_enable" in test_text, "requires_enable", "present"),
...
def _audit_checks(v14_text: str, status_text: str) -> list[dict[str, Any]]:
    return [
        BASE_MODULE._expect("audit_latest_status_is_execution_plan_ready", BASE_MODULE._latest_value(v14_text, "current_v14_status"), SOURCE_PLAN_STATUS),
        ...
        BASE_MODULE._expect("audit_execution_plan_ready", BASE_MODULE._latest_value(v14_text, "post_closeout_promotion_evidence_acquisition_paired_evaluation_execution_plan_ready"), "True"),
        BASE_MODULE._expect("audit_execution_plan_static_review_authorized", BASE_MODULE._latest_value(v14_text, "post_closeout_promotion_evidence_acquisition_paired_evaluation_execution_plan_static_review_authorized"), "True"),
        ...
        BASE_MODULE._expect("status_doc_latest_status_is_execution_plan_ready", BASE_MODULE._latest_value(status_text, "current_v14_status"), SOURCE_PLAN_STATUS),
...
def _contract_summary(source_plan: dict[str, Any]) -> dict[str, Any]:
    return {
        "required_input_count": len(BASE_MODULE._list(source_plan.get("required_inputs"))),
        "execution_plan_count": len(BASE_MODULE._list(source_plan.get("execution_plan"))),
        "planned_output_count": len(BASE_MODULE._list(source_plan.get("planned_outputs"))),
...
def _decision(passed: bool, checks: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if passed:
        ...
        failure_class = "explicit_paired_evaluation_execution_plan_static_review_authorization_missing"
        ...
        failure_class = "source_paired_evaluation_execution_plan_static_review_contract_failure"
    ...
    decision: dict[str, Any] = {
        ...
        "post_closeout_promotion_evidence_acquisition_paired_evaluation_execution_plan_static_review_passed": passed,
        "post_closeout_promotion_evidence_acquisition_paired_evaluation_execution_preflight_authorized": passed,
        "paired_evaluation_executed_by_this_gate": False,
        "previous_no_promotion_closeout_preserved": True,
        "direct_promotion_recommendation": False,
        "recommendation": "plan_paired_evaluation_execution_preflight_only" if passed else "repair_contract_before_rerun",
        "score_expression": SCORE_EXPRESSION,
    }
    for action in BLOCKED_ACTIONS:
