#!/usr/bin/env python3
"""Static-review the v16 fixed-DP pilot evidence-package plan."""
...
def _load_plan_module():
    ...
    spec = importlib.util.spec_from_file_location("v16_pilot_evidence_package_plan", path)
...
READY_STATUS = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_evidence_package_plan_static_review_passed"
REJECT_STATUS = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_evidence_package_plan_static_review_rejected"
AUTHORIZED_NEXT_WORK = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_evidence_package_construction_only"
SCHEMA_VERSION = "dp_camp_v16_nuscenes_fixed_dp_candidate_tensor_pilot_evidence_package_plan_static_review_v1"
REVIEW_JSON_NAME = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_evidence_package_plan_static_review.json"
REVIEW_MD_NAME = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_evidence_package_plan_static_review.md"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v16_nuscenes_fixed_dp_candidate_tensor_pilot_evidence_package_plan_static_review",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        enabled=args.enable_v16_nuscenes_fixed_dp_candidate_tensor_pilot_evidence_package_plan_static_review,
    )
...
def build_report(
    *,
    ...
    plan = source.get("pilot_evidence_package_plan", {})
    source_artifacts = plan.get("source_artifacts", [])
    boundary = plan.get("no_claim_boundary", {})
    pass_checks = plan.get("pass_checks", {})
    next_path = plan.get("recommended_next_path", {})
    checks = [
        ...
        _expect("source_artifact_ids", [item.get("id") for item in source_artifacts], list(PLAN_MODULE.EXPECTED_SOURCE_ARTIFACT_IDS)),
        _expect("boundary_smoke_only", boundary.get("smoke_only"), True),
        _expect("boundary_scene_count_4", boundary.get("scene_count"), 4),
        _expect("boundary_calibration_rows_14", boundary.get("calibration_rows"), 14),
        _expect("boundary_holdout_rows_147", boundary.get("holdout_rows"), 147),
        _expect("boundary_no_performance_claim", boundary.get("no_performance_claim"), True),
        _expect("boundary_no_safety_claim", boundary.get("no_safety_claim"), True),
        _expect("boundary_no_camp_over_dp_claim", boundary.get("no_camp_over_dp_claim"), True),
        _expect("boundary_no_promotion_or_deployment", boundary.get("no_promotion_or_deployment"), True),
        ...
        _expect("no_dp_modification", pass_checks.get("no_dp_modification"), True),
        _expect("no_candidate_tensor_mutation", pass_checks.get("no_candidate_tensor_mutation"), True),
        ...
        _expect("next_gate_scale_up_plan", next_path.get("next_gate"), "scale-up plan"),
        _expect("increase_scene_diversity", next_path.get("increase_scene_diversity"), True),
        _expect("target_records_10000", next_path.get("target_records"), 10000),
        _expect("pilot_result_not_usable_for_claim", next_path.get("pilot_result_usable_for_claim"), False),
    ]
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    boundary = review["no_claim_boundary"]
    return "\n".join(
        [
            ...
            "- Static review only; no package construction, claim, promotion, deployment, or scale-up execution.",
            "",
        ]
    )
