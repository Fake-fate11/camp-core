#!/usr/bin/env python3
"""Record the v14 promotion-readiness evidence-chain decision."""
...
def _load_source_review_module():
    review_path = Path(__file__).resolve().with_name(
        ...
        "uncertainty_coverage_evidence_package_promotion_readiness_evidence_chain_decision_plan_static_contract.py"
    )
...
SCHEMA_VERSION = (
    ...
    "uncertainty_coverage_evidence_package_promotion_readiness_evidence_chain_decision_v1"
)
...
REJECT_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_readiness_uncertainty_coverage_evidence_package_promotion_readiness_evidence_chain_decision_rejected"
)
...
DECISION_JSON_NAME = (
    "post_closeout_promotion_readiness_uncertainty_coverage_evidence_package_promotion_readiness_evidence_chain_decision.json"
)
DECISION_MD_NAME = (
    "post_closeout_promotion_readiness_uncertainty_coverage_evidence_package_promotion_readiness_evidence_chain_decision.md"
)

EXPECTED_SOURCE_REVIEW_CHECK_COUNT = 138
EXPECTED_SOURCE_PLAN_CHECK_COUNT = 125
EXPECTED_SOURCE_STATIC_REVIEW_CHECK_COUNT = 132
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v14_post_closeout_promotion_readiness_uncertainty_coverage_evidence_package_promotion_readiness_evidence_chain_decision",
        action="store_true",
        help="Explicit opt-in for the evidence-chain decision record.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        label=args.label,
        enabled=args.enable_v14_post_closeout_promotion_readiness_uncertainty_coverage_evidence_package_promotion_readiness_evidence_chain_decision,
    )
...
def build_report(
    *,
    ...
    checks: list[dict[str, Any]] = [
        BASE_MODULE._expect("evidence_chain_decision_enabled", enabled, True),
    ...
    evidence_decision = _evidence_chain_decision(label=label, source_review=source_review)
    checks.extend(_evidence_chain_decision_checks(evidence_decision))
    ...
    return {
        ...
        "source_decision_plan_summary": _source_decision_plan_summary(source_review),
        "evidence_chain_decision": evidence_decision,
...
def write_outputs(output_dir: Path, report: dict[str, Any]) -> None:
    ...
    BASE_MODULE._write_json(output_dir / DECISION_JSON_NAME, report)
    (output_dir / DECISION_MD_NAME).write_text(_markdown(report), encoding="utf-8")
...
def _markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Post-Closeout Promotion-Readiness Evidence-Chain Decision",
        "",
    ...
    for item in report["evidence_chain_decision"]:
        lines.append(f"- `{item['item_name']}`: `{item['decision']}`")
    lines.extend(["", "## Checks"])
    for check in report["decision_checks"]:
        lines.append(
...
def _source_review_contract_checks(source_review: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    evidence_summary = BASE_MODULE._dict(source_review.get("evidence_chain_decision_plan_summary"))
    checks = [
        ...
        BASE_MODULE._expect("source_review_static_review_passed", decision.get("promotion_readiness_evidence_chain_decision_plan_static_review_passed"), True),
        BASE_MODULE._expect("source_review_decision_authorized", decision.get("promotion_readiness_evidence_chain_decision_authorized"), True),
        BASE_MODULE._expect("source_review_user_authorization_recorded", decision.get("user_authorized_future_promotion_deployment_online_selector_and_claim_gates"), True),
        ...
        BASE_MODULE._expect("source_static_review_check_count", evidence_summary.get("source_static_review_check_count"), EXPECTED_SOURCE_STATIC_REVIEW_CHECK_COUNT),
        BASE_MODULE._expect("source_boundary_plan_check_count", evidence_summary.get("source_boundary_plan_check_count"), EXPECTED_SOURCE_BOUNDARY_PLAN_CHECK_COUNT),
        BASE_MODULE._expect("source_boundary_item_count", evidence_summary.get("source_boundary_item_count"), EXPECTED_SOURCE_BOUNDARY_ITEM_COUNT),
        BASE_MODULE._expect("source_decision_item_count", evidence_summary.get("decision_item_count"), EXPECTED_DECISION_ITEM_COUNT),
        BASE_MODULE._expect("source_score_expression", decision.get("score_expression"), SCORE_EXPRESSION),
    ]
...
def _audit_checks(v14_text: str, status_text: str) -> list[dict[str, Any]]:
    return [
        BASE_MODULE._expect("audit_latest_status_is_decision_plan_static_review", BASE_MODULE._latest_value(v14_text, "current_v14_status"), SOURCE_REVIEW_STATUS),
        BASE_MODULE._expect("audit_latest_eof_authorizes_evidence_chain_decision", BASE_MODULE._latest_value(v14_text, "next_work_target"), AUTHORIZED_CURRENT_WORK),
        BASE_MODULE._expect("audit_decision_plan_static_review_passed", BASE_MODULE._latest_value(v14_text, "promotion_readiness_evidence_chain_decision_plan_static_review_passed"), "True"),
        BASE_MODULE._expect("audit_decision_authorized", BASE_MODULE._latest_value(v14_text, "promotion_readiness_evidence_chain_decision_authorized"), "True"),
        ...
        BASE_MODULE._expect("status_doc_latest_status_is_decision_plan_static_review", BASE_MODULE._latest_value(status_text, "current_v14_status"), SOURCE_REVIEW_STATUS),
        BASE_MODULE._expect("status_doc_latest_eof_authorizes_evidence_chain_decision", BASE_MODULE._latest_value(status_text, "next_work_target"), AUTHORIZED_CURRENT_WORK),
    ]


def _evidence_chain_decision(label: str | None, source_review: dict[str, Any]) -> list[dict[str, Any]]:
    source_status = BASE_MODULE._dict(source_review.get("final_decision")).get("status")
    return [
        {
            "item_name": "fixed_dp_and_candidate_source_lock",
            "decision": "satisfied_for_audit_decision",
        ...
        {
            "item_name": "selector_evidence_package_closed_state",
            "decision": "closed_evidence_package_is_not_a_promotion_recommendation",
            "source_status": source_status,
            "promote_selector_now": False,
            "deploy_now": False,
            "claim_now": False,
        ...
        {
            "item_name": "masked_objective_delta_scope",
            "decision": "static_delta_evidence_is_not_a_safety_or_camp_over_dp_claim",
            "source_status": source_status,
            "promote_selector_now": False,
            "deploy_now": False,
            "claim_now": False,
        ...
        {
            "item_name": "uncertainty_coverage_manifest_scope",
            "decision": "uncertainty_and_coverage_manifests_remain_audit_evidence_only",
            "source_status": source_status,
            "promote_selector_now": False,
            "deploy_now": False,
            "claim_now": False,
        ...
        {
            "item_name": "promotion_vs_deployment_vs_claim_separation",
            "decision": "separate_future_gates_required_before_any_effective_action_or_claim",
            "source_status": source_status,
            "promote_selector_now": False,
            "deploy_now": False,
            "claim_now": False,
        ...
        {
            "item_name": "fail_closed_decision_outputs",
            "decision": "fail_closed_to_no_promotion_no_deployment_no_claim",
            "source_status": source_status,
            "promote_selector_now": False,
            "deploy_now": False,
            "claim_now": False,
...
def _evidence_chain_decision_checks(decision_items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [
        BASE_MODULE._expect("decision_item_names", [item.get("item_name") for item in decision_items], list(SOURCE_REVIEW_MODULE.PLAN_MODULE.EXPECTED_DECISION_ITEMS)),
        BASE_MODULE._expect("decision_items_no_selector_promotion_now", sorted({item.get("promote_selector_now") for item in decision_items}), [False]),
        BASE_MODULE._expect("decision_items_no_deployment_now", sorted({item.get("deploy_now") for item in decision_items}), [False]),
        BASE_MODULE._expect("decision_items_no_claim_now", sorted({item.get("claim_now") for item in decision_items}), [False]),
    ]
...
def _source_decision_plan_summary(source_review: dict[str, Any]) -> dict[str, Any]:
    source_plan = BASE_MODULE._dict(source_review.get("source_plan_summary"))
    evidence_summary = BASE_MODULE._dict(source_review.get("evidence_chain_decision_plan_summary"))
    return {
        "source_plan_check_count": source_plan.get("plan_check_count"),
        "source_plan_failed_check_count": source_plan.get("failed_check_count"),
        "source_static_review_check_count": evidence_summary.get("source_static_review_check_count"),
        "source_boundary_plan_check_count": evidence_summary.get("source_boundary_plan_check_count"),
        "source_boundary_item_count": evidence_summary.get("source_boundary_item_count"),
        "decision_item_count": evidence_summary.get("decision_item_count"),
    }
...
def _decision(passed: bool, checks: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if passed:
        failure_class = None
    elif "evidence_chain_decision_enabled" in failed:
        failure_class = "explicit_evidence_chain_decision_authorization_missing"
        ...
        failure_class = "evidence_chain_decision_source_contract_failure"
    ...
    decision: dict[str, Any] = {
        ...
        "promotion_readiness_evidence_chain_decision_recorded": passed,
        "promotion_readiness_evidence_chain_decision_static_review_authorized": passed,
        "user_authorized_future_promotion_deployment_online_selector_and_claim_gates": True,
        "recommendation": "do_not_promote_deploy_or_claim_from_current_evidence_chain",
        "immediate_action": "static_review_evidence_chain_decision_record",
