...
READY_STATUS = "observable_state_logging_nonformal_smoke_plan_ready"
REJECT_STATUS = "observable_state_logging_nonformal_smoke_plan_rejected"
SOURCE_STATUS = "observable_state_logging_preflight_implementation_unit_tested"
...
class SmokeSpec:
    root: str = (
        ...
        "observable_state_logging_smoke_2aa74f5"
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Design-only preflight for the first default-off observable-state "
            "logging smoke. It emits paired nonformal replay commands and "
    ...
    *,
    ...
    decision = {
        ...
        "authorized_next_work": (
            "default_off_observable_state_logging_paired_three_step_smoke_only"
            if passed
            else None
        ),
    ...
    return {
        "analysis": {
            "name": "dp_camp_observable_state_logging_nonformal_smoke_plan_v1",
            "label": label,
            ...
            "math_boundary": (
                ...
                "fixed finite-candidate descriptors. It must not change CAMP "
                "scores, feasibility, selected indices, DP candidates, or "
                "PerfectTracker execution. If descriptors are later atomized, "
                "they enter fixed coefficients a_k and preserve affine scores "
                "a_k^T w and the simplex/CVaR/L2 convex master."
            ),
        },
        ...
        "accept_criteria": [
            ...
            "payload schema, shapes, finite checks, and latency fields pass audit",
...
def _payload_audit_command(
    ...
    return [
        ...
        str(smoke.num_candidates),
        "--output_json",
        f"{audit_dir}/observable_state_logging_smoke.json",
        "--output_md",
        f"{audit_dir}/observable_state_logging_smoke.md",
        "--require_pass",
    ]
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Observable State Logging Smoke Plan",
        "",
