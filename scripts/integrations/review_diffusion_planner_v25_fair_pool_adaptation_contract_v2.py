"""Seal a separate-role literal review of fair-pool contract v2."""
...
from camp_core.integrations.diffusion_planner_v25_fair_pool_adaptation_review_v2 import (  # noqa: E402
...
def review(
    *,
    ...
    verify_complete_seal(source, source_root, label="fair-pool contract v2")
    ...
    literal = review_contract_literal_v2(contract)
    if (
    ...
    try:
        report = {
            ...
            "reviewer_local_manifest_schema_oracle": True,
            "reviewer_local_endpoint_threshold_decision_oracle": True,
        ...
        root = seal_artifact(
            staging, label="V25 fair-pool adaptation contract review v2"
        )
        os.replace(staging, output)
        verify_complete_seal(
            output,
            root,
            label="V25 fair-pool adaptation contract review v2",
        )
        return root
    finally:
        if staging.exists():
