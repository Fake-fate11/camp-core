"""Seal an independent literal review of the batch8-primary contract."""
...
from camp_core.integrations.diffusion_planner_v25_batch8_primary_generator_contract_review import (  # noqa: E402
    independent_literal_review,
)


def review(*, source: Path, source_root: str, output: Path) -> str:
    verify_complete_seal(source, source_root, label="batch8-primary contract")
    ...
    if (
        ...
        != "sealed_outcome_independent_batch8_primary_contract"
    ...
    contract = _object_value(source_report.get("contract"), "contract")
    implementation = _object_value(
    ...
    report = {
        **literal,
        ...
        "status": "passed_independent_batch8_primary_contract_review",
        "source": {
    ...
    return _atomic(output, report, "V25 batch8-primary contract review")
...
def _atomic(output: Path, report: dict[str, Any], label: str) -> str:
    ...
    try:
        ...
        (staging / "HEADS.json").write_bytes(
            _bytes(
                {
                    "role": "batch8_primary_generator_contract_review",
...
def _object(path: Path) -> dict[str, Any]:
    return _object_value(json.loads(path.read_text(encoding="utf-8")), str(path))
