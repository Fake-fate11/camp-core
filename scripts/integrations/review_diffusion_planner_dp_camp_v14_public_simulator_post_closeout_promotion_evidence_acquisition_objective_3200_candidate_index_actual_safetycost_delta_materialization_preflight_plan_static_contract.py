#!/usr/bin/env python3
"""Static review for the candidate-index SafetyCost delta preflight plan."""
...
def _load_plan_module():
    plan_path = Path(__file__).resolve().with_name(
        ...
        "actual_safetycost_delta_materialization_preflight.py"
    )
...
SCHEMA_VERSION = (
    ...
    "objective_3200_candidate_index_actual_safetycost_delta_materialization_preflight_plan_static_review_v1"
)
...
READY_STATUS = (
    ...
    "candidate_index_actual_safetycost_delta_materialization_preflight_plan_static_review_passed"
)
REJECT_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_delta_materialization_preflight_plan_static_review_rejected"
)
AUTHORIZED_NEXT_WORK = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_delta_materialization_preflight_only"
)

REVIEW_JSON_NAME = (
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_delta_materialization_preflight_plan_static_review.json"
)
REVIEW_MD_NAME = (
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_delta_materialization_preflight_plan_static_review.md"
)


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--delta_materialization_preflight_plan_artifact_dir", type=Path, required=True)
    parser.add_argument("--delta_materialization_preflight_plan_json", type=Path, required=True)
    parser.add_argument("--delta_materialization_preflight_plan_md", type=Path, required=True)
    parser.add_argument("--delta_materialization_preflight_plan_sha256s", type=Path, required=True)
    parser.add_argument("--delta_materialization_preflight_plan_script_py", type=Path, required=True)
    parser.add_argument("--delta_materialization_preflight_plan_test_py", type=Path, required=True)
    ...
    parser.add_argument(
        "--enable_v14_post_closeout_promotion_evidence_acquisition_objective_3200_candidate_index_actual_safetycost_delta_materialization_preflight_plan_static_review",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        delta_materialization_preflight_plan_artifact_dir=(
            args.delta_materialization_preflight_plan_artifact_dir
        ),
        delta_materialization_preflight_plan_json=args.delta_materialization_preflight_plan_json,
        delta_materialization_preflight_plan_md=args.delta_materialization_preflight_plan_md,
        delta_materialization_preflight_plan_sha256s=args.delta_materialization_preflight_plan_sha256s,
        delta_materialization_preflight_plan_script_py=args.delta_materialization_preflight_plan_script_py,
        delta_materialization_preflight_plan_test_py=args.delta_materialization_preflight_plan_test_py,
        ...
        enabled=(
            args.enable_v14_post_closeout_promotion_evidence_acquisition_objective_3200_candidate_index_actual_safetycost_delta_materialization_preflight_plan_static_review
        ),
    )
...
def build_report(
    *,
    delta_materialization_preflight_plan_artifact_dir: Path,
    delta_materialization_preflight_plan_json: Path,
    delta_materialization_preflight_plan_md: Path,
    delta_materialization_preflight_plan_sha256s: Path,
    delta_materialization_preflight_plan_script_py: Path,
    delta_materialization_preflight_plan_test_py: Path,
    v14_audit_md: Path,
    ...
    artifact_dir = delta_materialization_preflight_plan_artifact_dir.resolve()
    plan_json = delta_materialization_preflight_plan_json.resolve()
    plan_md = delta_materialization_preflight_plan_md.resolve()
    plan_sha256s = delta_materialization_preflight_plan_sha256s.resolve()
    script_py = delta_materialization_preflight_plan_script_py.resolve()
    test_py = delta_materialization_preflight_plan_test_py.resolve()
    ...
    return {
        ...
        "analysis": {
            ...
            "read_only": True,
            "candidate_index_actual_safetycost_delta_preflight_plan_static_review_only": True,
        ...
        "inputs": {
            "delta_materialization_preflight_plan_artifact_dir": str(artifact_dir),
            "delta_materialization_preflight_plan_json": str(plan_json),
            "delta_materialization_preflight_plan_md": str(plan_md),
            "delta_materialization_preflight_plan_sha256s": str(plan_sha256s),
            "delta_materialization_preflight_plan_script_py": str(script_py),
            "delta_materialization_preflight_plan_test_py": str(test_py),
        ...
        "delta_materialization_preflight_summary": source_plan.get(
...
def _checks(
    *,
    ...
    delta = HELPER_MODULE._dict(source_plan.get("delta_materialization_preflight_summary"))
    ...
    expect(
        "source_plan_ready",
        decision.get("objective_3200_candidate_index_actual_safetycost_delta_materialization_preflight_plan_ready"),
        True,
    )
    expect(
        "source_plan_static_review_authorized",
        decision.get("objective_3200_candidate_index_actual_safetycost_delta_materialization_preflight_plan_static_review_authorized"),
        True,
    )
    ...
    expect("source_plan_check_count", len(_list(source_plan.get("plan_checks"))), 66)
    ...
    expect(
        "source_preflight_plan_no_delta_materialization",
        sorted({item.get("materializes_safetycost_deltas") for item in _list(source_plan.get("preflight_plan"))}),
        [False],
    )
    ...
    expect("delta_actual_safetycost_available", delta.get("actual_safetycost_v1_available"), False)
    expect("delta_claim_rule_evaluable", delta.get("actual_safetycost_v1_claim_rule_evaluable"), False)
    expect(
        "delta_next_evidence_need",
        delta.get("next_evidence_need"),
...
def _source_surface_checks(script_text: str, test_text: str) -> list[dict[str, Any]]:
    ...
    expected_test_tokens = [
        "test_candidate_index_actual_safetycost_delta_materialization_preflight_plan_passes",
        "test_candidate_index_actual_safetycost_delta_materialization_preflight_plan_rejects_source_claim_leak",
        "test_candidate_index_actual_safetycost_delta_materialization_preflight_plan_rejects_hash_drift",
    ]
    checks = []
...
def _decision(*, passed: bool, checks: list[dict[str, Any]], source_plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if passed:
        ...
        failure_class = "explicit_candidate_index_delta_preflight_plan_static_review_authorization_missing"
        ...
        failure_class = "source_candidate_index_delta_preflight_plan_contract_failure"
    elif any(name.startswith(("source_", "delta_")) for name in failed):
        failure_class = "source_candidate_index_delta_preflight_evidence_contract_failure"
    elif any(name.startswith(("nested_", "root_", "artifact_")) for name in failed):
        failure_class = "artifact_hash_contract_failure"
    else:
        failure_class = "candidate_index_delta_preflight_plan_static_review_contract_failure"
    ...
    decision = {
        ...
        "objective_3200_candidate_index_actual_safetycost_delta_materialization_preflight_plan_static_review_passed": bool(passed),
        "objective_3200_candidate_index_actual_safetycost_delta_materialization_preflight_authorized": bool(passed),
        ...
        "recommendation": "candidate_index_actual_safetycost_delta_materialization_preflight_only" if passed else "repair_or_rerun_same_static_review_gate",
        "score_expression": SCORE_EXPRESSION,
    }
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Objective-3200 Candidate-Index Actual-SafetyCost Delta Preflight Plan Static Review",
        "",
        ...
        "## Delta Preflight",
        "",
        f"- Actual SafetyCost_v1 available: `{delta.get('actual_safetycost_v1_available')}`",
        f"- Claim rule evaluable: `{delta.get('actual_safetycost_v1_claim_rule_evaluable')}`",
        f"- Next evidence need: `{delta.get('next_evidence_need')}`",
        "",
        "## Boundary",
        "",
