...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    for role in ROOT_ROLES:
        parser.add_argument(f"--{role.replace('_', '-')}-artifact", type=Path, required=True)
        parser.add_argument(f"--{role.replace('_', '-')}-root-sha256", required=True)
...
def main(argv: list[str] | None = None) -> None:
    ...
    if _git("rev-parse", "HEAD") != args.pointer_head_at_release:
        raise ValueError("release pointer HEAD is not the live CAMP HEAD")
    bindings = {
        role: {
