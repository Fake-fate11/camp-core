#!/usr/bin/env python3
"""Plan the v15 paired-evaluation preflight gate."""
...
def _load_source_review_module():
    ...
    spec = importlib.util.spec_from_file_location("v15_offline_training_execution_result_review", path)
...
SCHEMA_VERSION = "dp_camp_v15_broader_nonformal_evidence_expansion_paired_evaluation_preflight_plan_v1"
AUTHORIZED_CURRENT_WORK = SOURCE_REVIEW_MODULE.AUTHORIZED_NEXT_WORK
READY_STATUS = "v15_broader_nonformal_evidence_expansion_paired_evaluation_preflight_plan_ready"
REJECT_STATUS = "v15_broader_nonformal_evidence_expansion_paired_evaluation_preflight_plan_rejected"
AUTHORIZED_NEXT_WORK = "v15_broader_nonformal_evidence_expansion_paired_evaluation_preflight_plan_static_review_only"
PLAN_JSON_NAME = "v15_broader_nonformal_evidence_expansion_paired_evaluation_preflight_plan.json"
PLAN_MD_NAME = "v15_broader_nonformal_evidence_expansion_paired_evaluation_preflight_plan.md"
LATENCY_FIELDS = ("count", "mean", "median", "p95", "p99", "max")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v15_broader_nonformal_evidence_expansion_paired_evaluation_preflight_plan",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        enabled=args.enable_v15_broader_nonformal_evidence_expansion_paired_evaluation_preflight_plan,
    )
...
def build_report(
    *,
    ...
    plan = _paired_evaluation_preflight_plan(current_camp_head, current_dp_head)

    checks = [
        _expect("paired_evaluation_preflight_plan_enabled", enabled, True),
        ...
        _expect("source_reviewed_offline_training_execution", decision.get("reviewed_offline_training_execution"), True),
        _expect("source_training_executed", decision.get("source_training_executed"), True),
        ...
        _check("plan_eval_splits_registered", set(plan["required_inputs"]["evaluation_splits"]) == {"calibration", "holdout"}, plan["required_inputs"]["evaluation_splits"], "calibration and holdout"),
        ...
        _expect("plan_online_latency_fields", tuple(plan["timing_contract"]["online_selector_latency_required_fields"]), LATENCY_FIELDS),
        _expect("plan_fallback_latency_fields", tuple(plan["timing_contract"]["fallback_latency_required_fields"]), LATENCY_FIELDS),
...
def _paired_evaluation_preflight_plan(camp_head: str, dp_head: str) -> dict[str, Any]:
    return {
        "camp_head": camp_head,
        ...
        "required_inputs": {
            ...
            "offline_training_model_manifest": EXECUTION_MODULE.MODEL_MANIFEST_JSON_NAME,
            ...
            "scenario_buckets": tuple(EXECUTION_MODULE.MATRIX_MODULE.ROOT_PLAN_MODULE.SCENARIO_BUCKETS),
        },
        "paired_protocol": {
            "baseline": "dp_top1",
        ...
        "timing_contract": {
            "online_selector_latency_required_fields": LATENCY_FIELDS,
            "fallback_latency_required_fields": LATENCY_FIELDS,
            "instrumentation_changes_selector_behavior": False,
        },
        "planned_command": (
            "python scripts/integrations/preflight_diffusion_planner_dp_camp_v15_broader_nonformal_evidence_expansion_paired_evaluation.py"
        ),
        "planned_outputs": (
            "paired_evaluation_preflight.json",
            "paired_evaluation_timing_contract.json",
            "paired_evaluation_timing_contract.md",
            "SHA256SUMS",
        ),
        "blocked_inputs": {
            "Full36": False,
        ...
        "no_go_conditions": (
            "missing fixed DP head",
            "Full36 or formal seed usage",
            "candidate tensor mutation",
            "Diffusion Planner modification",
            "performance claim before paired evaluation execution",
        ),
    }
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# V15 Paired Evaluation Preflight Plan",
            "",
            f"- Status: `{decision['status']}`",
            f"- Passed: `{decision['passed']}`",
            f"- Evaluation splits: `{', '.join(plan['required_inputs']['evaluation_splits'])}`",
            f"- Authorized next work: `{decision['authorized_next_work']}`",
            "- This gate plans paired evaluation preflight only; it does not evaluate.",
            "",
        ]
    )
