#!/usr/bin/env python3
"""Preflight a future paired evaluation evidence acquisition gate."""
...
def _load_source_review_module():
    review_path = Path(__file__).resolve().with_name(
        ...
        "promotion_evidence_acquisition_continuation_preflight_plan_static_contract.py"
    )
...
SCHEMA_VERSION = (
    ...
    "promotion_evidence_acquisition_paired_evaluation_preflight_v1"
)
...
REJECT_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_paired_evaluation_preflight_rejected"
)
...
PREFLIGHT_JSON_NAME = "post_closeout_promotion_evidence_acquisition_paired_evaluation_preflight.json"
PREFLIGHT_MD_NAME = "post_closeout_promotion_evidence_acquisition_paired_evaluation_preflight.md"

EXPECTED_SOURCE_REVIEW_CHECK_COUNT = 144
EXPECTED_SOURCE_PREFLIGHT_CHECK_COUNT = 136
EXPECTED_INPUT_REQUIREMENTS = (
    "source_static_review_artifact_lock",
    "fixed_dp_candidate_tensor_manifest",
    "shadow_selection_log_manifest",
    "strict_paired_run_key_index",
    "safetycost_v1_contract",
    "scenario_bucket_manifest",
    "coverage_uncertainty_manifest",
    "artifact_hash_contract",
)
EXPECTED_PREFLIGHT_ITEMS = (
    "verify_fixed_dp_heads_and_candidate_tensor_identity",
    "verify_shadow_selection_rows_and_dp_top1_index",
    "verify_no_formal_seed_or_full36_scope",
    "verify_no_closed_loop_outcome_training_or_online_input",
    "predeclare_safetycost_v1_and_hard_gate_metrics",
    "predeclare_coverage_uncertainty_and_bucket_outputs",
    "predeclare_fail_closed_no_go_conditions",
    "emit_static_review_ready_preflight_artifact",
)
EXPECTED_NO_GO = SOURCE_REVIEW_MODULE.PLAN_MODULE.EXPECTED_NO_GO
EXPECTED_METRICS = SOURCE_REVIEW_MODULE.PLAN_MODULE.EXPECTED_METRICS
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v14_post_closeout_promotion_evidence_acquisition_paired_evaluation_preflight",
        action="store_true",
        help="Explicit opt-in for read-only paired evaluation evidence acquisition preflight.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        label=args.label,
        enabled=args.enable_v14_post_closeout_promotion_evidence_acquisition_paired_evaluation_preflight,
    )
...
def build_report(
    *,
    ...
    checks: list[dict[str, Any]] = [
        BASE_MODULE._expect("paired_evaluation_preflight_enabled", enabled, True),
    ...
    input_requirements = _input_requirements(label=label, source_review=source_review)
    preflight_plan = _preflight_plan()
    no_go = _no_go_register()
    checks.extend(_preflight_contract_checks(input_requirements, preflight_plan, no_go))
    ...
    return {
        ...
        "source_protocol_summary": _source_protocol_summary(source_review),
...
def _markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Post-Closeout Promotion Evidence Acquisition Paired Evaluation Preflight",
        "",
    ...
    for item in report["input_requirements"]:
...
def _source_review_contract_checks(source_review: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    protocol = BASE_MODULE._dict(source_review.get("protocol_summary"))
    checks = [
        ...
        BASE_MODULE._expect("source_review_static_review_passed", decision.get("post_closeout_promotion_evidence_acquisition_continuation_preflight_plan_static_review_passed"), True),
        BASE_MODULE._expect("source_review_paired_preflight_authorized", decision.get("post_closeout_promotion_evidence_acquisition_paired_evaluation_preflight_authorized"), True),
        ...
        BASE_MODULE._expect("source_plan_preflight_check_count", source_plan.get("preflight_check_count"), EXPECTED_SOURCE_PREFLIGHT_CHECK_COUNT),
        BASE_MODULE._expect("source_protocol_count", protocol.get("protocol_item_count"), 8),
        BASE_MODULE._expect("source_metrics_count", protocol.get("metrics_count"), 8),
        BASE_MODULE._expect("source_no_go_count", protocol.get("no_go_count"), 8),
...
def _audit_checks(v14_text: str, status_text: str) -> list[dict[str, Any]]:
    return [
        ...
        BASE_MODULE._expect("audit_latest_eof_authorizes_paired_preflight", BASE_MODULE._latest_value(v14_text, "next_work_target"), AUTHORIZED_CURRENT_WORK),
        BASE_MODULE._expect("audit_static_review_passed", BASE_MODULE._latest_value(v14_text, "post_closeout_promotion_evidence_acquisition_continuation_preflight_plan_static_review_passed"), "True"),
        BASE_MODULE._expect("audit_paired_preflight_authorized", BASE_MODULE._latest_value(v14_text, "post_closeout_promotion_evidence_acquisition_paired_evaluation_preflight_authorized"), "True"),
        ...
        BASE_MODULE._expect("status_doc_latest_eof_authorizes_paired_preflight", BASE_MODULE._latest_value(status_text, "next_work_target"), AUTHORIZED_CURRENT_WORK),
    ]
...
def _input_requirements(label: str | None, source_review: dict[str, Any]) -> list[dict[str, Any]]:
    source_summary = _source_static_review_summary(source_review)
    return [
        {"name": "source_static_review_artifact_lock", "requirement": "consume the passed static-review artifact and root SHA256SUMS as immutable source evidence", "source_status": source_summary["status"], "label": label},
        {"name": "fixed_dp_candidate_tensor_manifest", "requirement": "require fixed DP candidate tensor identity and SHA/provenance before any paired evaluation"},
        {"name": "shadow_selection_log_manifest", "requirement": "require CAMP shadow_selected_index logs generated without executed-output effect"},
        {"name": "strict_paired_run_key_index", "requirement": "require identical run keys for DP Top-1 candidate index 0 and CAMP shadow-selected candidate"},
        {"name": "safetycost_v1_contract", "requirement": "require frozen SafetyCost v1 and hard-gate claim rule before any claim language"},
        {"name": "scenario_bucket_manifest", "requirement": "require explicit buckets; do not infer bucket labels from metrics"},
        {"name": "coverage_uncertainty_manifest", "requirement": "require coverage, fallback, eligibility, bucket, and confidence interval reporting"},
        {"name": "artifact_hash_contract", "requirement": "require JSON, MD, nested SHA256SUMS, root SHA256SUMS, HEADS, COMMAND, stdout, stderr, and run.exit"},
    ]
...
def _preflight_contract_checks(
    ...
    return [
        BASE_MODULE._expect("input_requirement_names", [item.get("name") for item in input_requirements], list(EXPECTED_INPUT_REQUIREMENTS)),
        BASE_MODULE._expect("preflight_item_names", [item.get("name") for item in preflight_plan], list(EXPECTED_PREFLIGHT_ITEMS)),
        BASE_MODULE._expect("preflight_items_do_not_execute_evaluation", sorted({item.get("executes_paired_evaluation") for item in preflight_plan}), [False]),
...
def _source_protocol_summary(source_review: dict[str, Any]) -> dict[str, Any]:
    protocol = BASE_MODULE._dict(source_review.get("protocol_summary"))
    return {
        "protocol_item_count": protocol.get("protocol_item_count"),
        "metrics_count": protocol.get("metrics_count"),
        "no_go_count": protocol.get("no_go_count"),
    }
...
def _decision(passed: bool, checks: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if passed:
        failure_class = None
    elif "paired_evaluation_preflight_enabled" in failed:
        failure_class = "explicit_paired_evaluation_preflight_authorization_missing"
    ...
    elif any(name.startswith(("input_", "preflight_", "no_go_")) for name in failed):
        failure_class = "paired_evaluation_preflight_contract_failure"
    ...
    decision: dict[str, Any] = {
        ...
        "post_closeout_promotion_evidence_acquisition_paired_evaluation_preflight_ready": passed,
        "post_closeout_promotion_evidence_acquisition_paired_evaluation_preflight_static_review_authorized": passed,
        "paired_evaluation_executed_by_this_gate": False,
        "previous_no_promotion_closeout_preserved": True,
        "direct_promotion_recommendation": False,
        "recommendation": "static_review_paired_evaluation_preflight_only" if passed else "repair_contract_before_rerun",
        "score_expression": SCORE_EXPRESSION,
    }
    for action in BLOCKED_ACTIONS:
