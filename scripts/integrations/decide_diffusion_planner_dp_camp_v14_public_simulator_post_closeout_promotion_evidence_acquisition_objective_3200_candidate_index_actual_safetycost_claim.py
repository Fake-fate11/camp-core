#!/usr/bin/env python3
"""Audited claim decision for objective-3200 actual SafetyCost evidence."""
...
def _load_source_review_module():
    review_path = Path(__file__).resolve().with_name(
        ...
        "actual_safetycost_claim_decision_plan_static_contract.py"
    )
...
SCHEMA_VERSION = (
    ...
    "objective_3200_candidate_index_actual_safetycost_claim_decision_v1"
)
READY_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_claim_decision_passed"
)
REJECT_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_claim_decision_rejected"
)
AUTHORIZED_NEXT_WORK = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_selector_promotion_decision_plan_only"
)
DECISION_JSON_NAME = (
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_claim_decision.json"
)
DECISION_MD_NAME = (
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_claim_decision.md"
)
EXPECTED_SOURCE_REVIEW_CHECK_COUNT = 86
CLAIM_TEXT = (
    "In the audited public-simulator objective-3200 paired SafetyCost_v1 evidence, "
    "CAMP shadow selection over fixed Diffusion Planner candidate tensors achieved lower "
    "SafetyCost_v1 than DP Top-1 under the preregistered paired criterion."
)
CLAIM_SCOPE_LIMIT = (
    "This claim is limited to the audited fixed-DP candidate-tensor shadow-selection "
    "evidence and is not a deployment, online-selector, closed-loop-safety, Full36, "
    "formal-seed, or trajectory-generation claim."
)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v14_post_closeout_promotion_evidence_acquisition_objective_3200_candidate_index_actual_safetycost_claim_decision",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        enabled=(
            args.enable_v14_post_closeout_promotion_evidence_acquisition_objective_3200_candidate_index_actual_safetycost_claim_decision
        ),
    )
...
def build_report(
    *,
    ...
    claim_record = _claim_record(source_review)
    checks = _checks(
        enabled=enabled,
    ...
    return {
        ...
        "analysis": {
            "claim_decision_gate": True,
            "claim_authorization_only": True,
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# Objective-3200 Candidate-Index Actual-SafetyCost Claim Decision",
            "",
...
def _checks(
    *,
    ...
    checks = [
        BASE_MODULE._expect("claim_decision_enabled", enabled, True),
        ...
        BASE_MODULE._expect("audit_claim_decision_authorized", BASE_MODULE._latest_value(v14_text, "objective_3200_candidate_index_actual_safetycost_claim_decision_authorized"), "True"),
        ...
        BASE_MODULE._expect("source_claim_decision_authorized", decision.get("objective_3200_candidate_index_actual_safetycost_claim_decision_authorized"), True),
        ...
        BASE_MODULE._expect("claim_record_authorizes_safety", claim_record.get("safety_benefit_claim_authorized"), True),
        BASE_MODULE._expect("claim_record_authorizes_camp_over_dp", claim_record.get("camp_over_dp_top1_claim_authorized"), True),
        BASE_MODULE._expect("claim_record_promotion_false", claim_record.get("selector_promotion_authorized"), False),
        BASE_MODULE._expect("claim_record_deployment_false", claim_record.get("deployment_authorized"), False),
        BASE_MODULE._expect("claim_record_online_false", claim_record.get("online_selector_change_authorized"), False),
        BASE_MODULE._check("claim_text_scope_mentions_public_simulator", "public-simulator" in claim_record.get("claim_text", ""), claim_record.get("claim_text"), "public-simulator"),
        BASE_MODULE._check("claim_scope_limit_mentions_not_deployment", "not a deployment" in claim_record.get("scope_limit", ""), claim_record.get("scope_limit"), "not a deployment"),
    ]
...
def _claim_record(source_review: dict[str, Any]) -> dict[str, Any]:
    source_decision = BASE_MODULE._dict(source_review.get("final_decision"))
    return {
        "claim_text": CLAIM_TEXT,
        "scope_limit": CLAIM_SCOPE_LIMIT,
...
def _source_static_review_summary(source_review: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "claim_decision_authorized": decision.get("objective_3200_candidate_index_actual_safetycost_claim_decision_authorized"),
    }
...
def _decision(*, passed: bool, checks: list[dict[str, Any]], source_review: dict[str, Any]) -> dict[str, Any]:
    ...
    if passed:
        failure_class = None
    elif "claim_decision_enabled" in failed:
        failure_class = "explicit_candidate_index_actual_safetycost_claim_decision_authorization_missing"
        ...
        failure_class = "source_claim_decision_static_review_contract_failure"
    else:
        failure_class = "claim_decision_contract_failure"
    return {
        "passed": bool(passed),
        ...
        "objective_3200_candidate_index_actual_safetycost_claim_decision_passed": bool(passed),
        "objective_3200_candidate_index_actual_safetycost_selector_promotion_decision_plan_authorized": bool(passed),
        "source_static_review_passed": source_decision.get("passed"),
        "claim_executed_by_this_gate": bool(passed),
        "safety_benefit_claim_authorized": bool(passed),
        "camp_over_dp_top1_claim_authorized": bool(passed),
        "selector_promotion_authorized": False,
        "deployment_authorized": False,
        "online_selector_change_authorized": False,
    }
