...
READY_STATUS = (
    ...
    "fixed_snapshot_screen_rerun_remediation_negative_support_followup_"
    "residual_comfort_failure_diagnostic_remediation_followup_materially_"
    "different_generator_fixed_snapshot_screen_rerun_plan_ready"
)
REJECT_STATUS = (
    "candidate_set_consensus_lane_projected_jerk_progress_support_default_off_"
    "fixed_snapshot_screen_rerun_remediation_negative_support_followup_"
    "residual_comfort_failure_diagnostic_remediation_followup_materially_"
    "different_generator_fixed_snapshot_screen_rerun_plan_rejected"
)
AUTHORIZED_NEXT_WORK = (
    "candidate_set_consensus_lane_projected_jerk_progress_support_default_off_"
    "fixed_snapshot_screen_rerun_remediation_negative_support_followup_"
    "residual_comfort_failure_diagnostic_remediation_followup_materially_"
    "different_generator_guarded_fixed_snapshot_screen_rerun_only"
)

DEFAULT_POST_REVIEW_ROOT = (
    "/root/autodl-tmp/camp_dp_material_generator_post_impl_static_review_bff8f8b"
)
...
DEFAULT_REMOTE_PYTHON = "/root/miniconda3/envs/camp/bin/python"
...
GUARD_ENV_VAR = "CAMP_MATERIAL_GENERATOR_FIXED_SNAPSHOT_RERUN_APPROVED"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Plan-only guarded fixed-snapshot screen rerun for the residual "
            "comfort follow-up materially different generator."
        )
    )
    parser.add_argument(
        "--post_review_root",
    ...
    return {
        "analysis": {
            "name": (
                ...
                "remediation_followup_materially_different_generator_fixed_"
                "snapshot_screen_rerun_plan_v1"
            ),
            "label": label,
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Material Generator Fixed-Snapshot Screen Rerun Plan",
        "",
...
"${{PY}}" scripts/integrations/analyze_diffusion_planner_route_topology_candidate_screen.py \\
  ...
  --label material_generator_fixed_snapshot_screen \\
...
cd "${{OUTPUT_ROOT}}"
sha256sum \\
  material_generator_fixed_snapshot_screen.json \\
  material_generator_fixed_snapshot_screen.md \\
...
def _screen_rerun_plan(execution_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "execution_artifacts": {
            "output_root": root,
            "screen_json": f"{root}/material_generator_fixed_snapshot_screen.json",
            "screen_md": f"{root}/material_generator_fixed_snapshot_screen.md",
            "heads": f"{root}/HEADS.txt",
            "sha256sums": f"{root}/SHA256SUMS",
            "runbook": f"{root}/material_generator_fixed_snapshot_screen_runbook.sh",
        },
        "accept_criteria": [
            ...
            "future evidence must report generated support, hard/progress/comfort failures, and material descriptor coverage",
...
def _plan_checks(plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    config = plan["candidate_config"]
    return [
        _check("plan_selection_type", plan["selection_type"] == "materially_different_generator_fixed_snapshot_screen_rerun_plan_only"),
        ...
        _check("plan_uses_material_policy", config["generator_policy"] == PLANNED_POLICY),
        _check("plan_uses_material_profile", config["default_off_remediation_profile"] == REMEDIATION_PROFILE),
...
def _runbook_checks(runbook_text: str) -> list[dict[str, Any]]:
    ...
    return [
        ...
        _check("runbook_uses_python", DEFAULT_REMOTE_PYTHON in runbook_text),
        _check("runbook_uses_material_policy", f"--generator_policy {PLANNED_POLICY}" in runbook_text),
        _check("runbook_uses_material_profile", f"--default_off_remediation_profile {REMEDIATION_PROFILE}" in runbook_text),
