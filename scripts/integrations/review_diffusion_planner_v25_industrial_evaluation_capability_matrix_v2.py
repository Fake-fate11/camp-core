...
def review_matrix_v2(
    ...
    verify_complete_seal(
        contract_review_dir, contract_review_root, label="industrial v2 contract review"
    )
    verify_complete_seal(matrix_dir, matrix_root, label="industrial v2 capability")
    contract = object_from(contract_dir / "report.json")["contract"]
    matrix = object_from(matrix_dir / "report.json")["capability_matrix"]
    review_capability_v2_literal(matrix, contract, source_dirs)
    head = git_head()
    report = {
        "schema_version": "camp_dp_v25_industrial_oriented_evaluation_capability_review_artifact_v2",
        "status": "passed_independent_scalar_leaf_capability_audit_review",
        "contract_binding": {"path": str(contract_dir.resolve()), "root_sha256": contract_root},
        "contract_review_binding": {
            "path": str(contract_review_dir.resolve()),
            "root_sha256": contract_review_root,
        },
        "capability_binding": {"path": str(matrix_dir.resolve()), "root_sha256": matrix_root},
