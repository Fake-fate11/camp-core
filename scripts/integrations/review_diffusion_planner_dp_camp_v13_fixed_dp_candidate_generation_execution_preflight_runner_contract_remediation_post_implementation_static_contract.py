#!/usr/bin/env python3
"""Post-implementation static review for the fixed-DP runner-contract gate.

This read-only gate reviews the landed CAMP-owned runner/preflight contract
and the runner implementation artifact. It can authorize only the fixed-DP
candidate generation execution preflight. It does not run Diffusion Planner,
...
FORBIDDEN_RUNNER_SCRIPT_SNIPPETS = (
...
def _checks(
    *,
    ...
    checks: list[dict[str, Any]] = [
        ...
        _expect("source_implementation_complete", source_decision.get("runner_contract_remediation_implementation_complete"), True),
        _expect(
    ...
    for snippet in FORBIDDEN_RUNNER_SCRIPT_SNIPPETS:
        checks.append(_expect(f"runner_script_forbids_{_slug(snippet)}", snippet in text_inputs["runner_script"], False))
...
def _decision(
    *,
    ...
    return {
        ...
        "runner_contract_remediation_post_implementation_static_contract_review_passed": passed,
        "fixed_dp_candidate_generation_execution_preflight_authorized_next": passed,
