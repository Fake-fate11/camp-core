#!/usr/bin/env python3
"""Preflight the v15 broader non-formal matrix execution gate."""
...
def _load_static_review_module():
    ...
    spec = importlib.util.spec_from_file_location("v15_matrix_plan_static_review", path)
...
SCHEMA_VERSION = "dp_camp_v15_broader_nonformal_evidence_expansion_matrix_execution_preflight_v1"
AUTHORIZED_CURRENT_WORK = STATIC_REVIEW_MODULE.AUTHORIZED_NEXT_WORK
READY_STATUS = "v15_broader_nonformal_evidence_expansion_matrix_execution_preflight_ready"
REJECT_STATUS = "v15_broader_nonformal_evidence_expansion_matrix_execution_preflight_rejected"
AUTHORIZED_NEXT_WORK = "v15_broader_nonformal_evidence_expansion_matrix_execution_preflight_static_review_only"
PREFLIGHT_JSON_NAME = "v15_broader_nonformal_evidence_expansion_matrix_execution_preflight.json"
PREFLIGHT_MD_NAME = "v15_broader_nonformal_evidence_expansion_matrix_execution_preflight.md"
TIMING_JSON_NAME = "v15_broader_nonformal_evidence_expansion_matrix_execution_preflight_timing_contract.json"
TIMING_MD_NAME = "v15_broader_nonformal_evidence_expansion_matrix_execution_preflight_timing_contract.md"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v15_broader_nonformal_evidence_expansion_matrix_execution_preflight",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        enabled=args.enable_v15_broader_nonformal_evidence_expansion_matrix_execution_preflight,
    )
...
def build_report(
    *,
    ...
    preflight = _matrix_execution_preflight(current_camp_head, current_dp_head)
    timing = preflight["timing_contract"]

    checks = [
        _expect("matrix_execution_preflight_enabled", enabled, True),
        ...
        _expect("source_matrix_preflight_not_executed", decision.get("matrix_execution_preflight_executed"), False),
        ...
        _expect("preflight_combination_count", preflight["combination_count"], 576),
        _expect("preflight_full36_used", preflight["blocked_inputs"]["Full36"], False),
        _expect("preflight_formal_seed_used", preflight["blocked_inputs"]["formal_seeds_11_12_13"], False),
        _expect("preflight_dp_modified", preflight["mutations"]["dp_modified"], False),
        _expect("preflight_candidate_tensor_modified", preflight["mutations"]["candidate_tensor_modified"], False),
        _expect("preflight_trajectory_modified", preflight["mutations"]["trajectory_modified"], False),
        _expect("timing_required", timing["timing_required"], True),
        _expect("timing_changes_selector_behavior", timing["instrumentation_changes_selector_behavior"], False),
        _check("offline_training_timing_registered", len(timing["offline_training_required_fields"]) >= 8, timing["offline_training_required_fields"], "offline timing fields"),
        _check("online_latency_timing_registered", len(timing["online_selector_latency_required_fields"]) == 6, timing["online_selector_latency_required_fields"], "six latency fields"),
        _check("fallback_latency_timing_registered", len(timing["fallback_latency_required_fields"]) == 6, timing["fallback_latency_required_fields"], "six fallback fields"),
    ]
    ...
    return _stable(
        {
            ...
            "matrix_execution_preflight": preflight,
            ...
            "final_decision": {
                ...
                "matrix_execution_preflight_executed": not failed,
...
def _matrix_execution_preflight(camp_head: str, dp_head: str) -> dict[str, Any]:
    matrix = PLAN_MODULE._matrix_plan()
    return {
        "camp_head": camp_head,
        "fixed_dp_head": dp_head,
        "splits": matrix["splits"],
        "all_seeds": matrix["all_seeds"],
        "route_count": matrix["route_count"],
        "combination_count": matrix["combination_count"],
        "scenario_buckets": matrix["scenario_buckets"],
        "zero_overlap_keys": matrix["zero_overlap_keys"],
        "paired_protocol": matrix["paired_protocol"],
        ...
        "no_go_conditions": (
            "Full36_or_formal_seed_11_12_13_usage",
            "DP_code_config_weight_checkpoint_change",
            "CAMP_candidate_tensor_or_trajectory_mutation",
            "reference_blend_guidance_postprocess_or_postselection",
            "affine_simplex_convex_boundary_break",
        ),
    }
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    preflight = report["matrix_execution_preflight"]
    return "\n".join(
        [
            "# V15 Matrix Execution Preflight",
            "",
            f"- Status: `{decision['status']}`",
            f"- Passed: `{decision['passed']}`",
            f"- Planned combinations: `{preflight['combination_count']}`",
            f"- Authorized next work: `{decision['authorized_next_work']}`",
            "- This gate preflights the matrix execution only; it does not run the matrix.",
            "",
        ]
    )
...
def _render_timing_markdown(timing: dict[str, Any]) -> str:
    lines = [
        "# V15 Matrix Execution Timing Contract",
        "",
        f"- Timing required: `{timing['timing_required']}`",
        f"- Instrumentation changes selector behavior: `{timing['instrumentation_changes_selector_behavior']}`",
        "",
        "## Offline Training Fields",
        "",
    ]
    lines.extend(f"- `{item}`" for item in timing["offline_training_required_fields"])
    lines.extend(["", "## Online Selector Latency Fields", ""])
    lines.extend(f"- `{item}`" for item in timing["online_selector_latency_required_fields"])
    lines.extend(["", "## Fallback Latency Fields", ""])
    lines.extend(f"- `{item}`" for item in timing["fallback_latency_required_fields"])
    lines.append("")
    return "\n".join(lines)
