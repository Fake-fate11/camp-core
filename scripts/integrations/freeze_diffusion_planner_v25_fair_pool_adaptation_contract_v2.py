"""Seal the executable outcome-independent fair-pool adaptation contract v2."""
...
from camp_core.integrations.diffusion_planner_v25_fair_pool_adaptation_contract_v2 import (  # noqa: E402
    FIXED_DP_HEAD,
...
def freeze(*, output: Path, fixed_dp_repo: Path) -> str:
    ...
    contract = validate_contract_v2(adaptation_contract_v2())
    ...
    try:
        report = {
            "schema_version": (
                "camp_dp_v25_fair_pool_adaptation_contract_artifact_v2"
            ),
            "status": "sealed_executable_design_only_acquisition_unauthorized",
            "contract": contract,
        ...
        root = seal_artifact(
            staging, label="V25 fair-pool adaptation contract v2"
        )
        os.replace(staging, output)
        verify_complete_seal(
            output, root, label="V25 fair-pool adaptation contract v2"
        )
        return root
    finally:
        if staging.exists():
