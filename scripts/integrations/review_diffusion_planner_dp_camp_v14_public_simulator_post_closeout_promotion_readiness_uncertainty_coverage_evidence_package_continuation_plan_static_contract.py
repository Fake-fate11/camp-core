#!/usr/bin/env python3
"""Static review for the v14 uncertainty/coverage package continuation plan."""
...
def _load_plan_module():
    plan_path = Path(__file__).resolve().with_name(
        ...
        "uncertainty_coverage_evidence_package_continuation.py"
    )
    spec = importlib.util.spec_from_file_location(
        "v14_post_closeout_promotion_readiness_uncertainty_coverage_evidence_package_continuation_plan",
        plan_path,
    )
...
BASE_MODULE = PLAN_MODULE.PLAN_MODULE
...
SCHEMA_VERSION = (
    ...
    "uncertainty_coverage_evidence_package_continuation_plan_static_review_v1"
)
...
REJECT_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_readiness_uncertainty_coverage_evidence_package_continuation_plan_static_review_rejected"
)
...
REVIEW_JSON_NAME = (
    "post_closeout_promotion_readiness_uncertainty_coverage_evidence_package_continuation_plan_static_review.json"
)
REVIEW_MD_NAME = (
    "post_closeout_promotion_readiness_uncertainty_coverage_evidence_package_continuation_plan_static_review.md"
)

EXPECTED_PLAN_CHECK_COUNT = 122
EXPECTED_SOURCE_STATIC_REVIEW_CHECK_COUNT = 136
EXPECTED_CONTINUATION_ITEM_COUNT = 5
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--continuation_plan_artifact_dir", type=Path, required=True)
    parser.add_argument("--continuation_plan_json", type=Path, required=True)
    parser.add_argument("--continuation_plan_md", type=Path, required=True)
    parser.add_argument("--continuation_plan_sha256s", type=Path, required=True)
    ...
    parser.add_argument(
        "--enable_v14_post_closeout_promotion_readiness_uncertainty_coverage_evidence_package_continuation_plan_static_review",
        action="store_true",
        help="Explicit opt-in for static review of the continuation plan.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        continuation_plan_artifact_dir=args.continuation_plan_artifact_dir,
        continuation_plan_json=args.continuation_plan_json,
        continuation_plan_md=args.continuation_plan_md,
        continuation_plan_sha256s=args.continuation_plan_sha256s,
        ...
        label=args.label,
        enabled=args.enable_v14_post_closeout_promotion_readiness_uncertainty_coverage_evidence_package_continuation_plan_static_review,
    )
...
def build_report(
    *,
    continuation_plan_artifact_dir: Path,
    continuation_plan_json: Path,
    continuation_plan_md: Path,
    continuation_plan_sha256s: Path,
    plan_script_py: Path,
    plan_test_py: Path,
    v14_audit_md: Path,
    ...
    artifact_dir = continuation_plan_artifact_dir.resolve()
    paths = {
        "continuation_plan_json": continuation_plan_json.resolve(),
        "continuation_plan_md": continuation_plan_md.resolve(),
        "continuation_plan_sha256s": continuation_plan_sha256s.resolve(),
    ...
    source_plan = BASE_MODULE._read_json_dict(paths["continuation_plan_json"])
    root_sha256s = BASE_MODULE._read_sha256sums(artifact_files["root_sha256s"])
    plan_sha256s = BASE_MODULE._read_sha256sums(paths["continuation_plan_sha256s"])
    ...
    checks: list[dict[str, Any]] = [
        ...
        BASE_MODULE._check("continuation_plan_artifact_dir_exists", artifact_dir.is_dir(), str(artifact_dir), "directory"),
    ]
    ...
    checks.extend(
        [
            BASE_MODULE._expect("plan_json_matches_artifact_layout", paths["continuation_plan_json"], artifact_files["plan_json"]),
            BASE_MODULE._expect("plan_md_matches_artifact_layout", paths["continuation_plan_md"], artifact_files["plan_md"]),
            BASE_MODULE._expect("plan_sha256s_matches_artifact_layout", paths["continuation_plan_sha256s"], artifact_files["plan_sha256s"]),
        ]
    )
    ...
    return {
        ...
        "analysis": {
            ...
            "read_only": True,
            "source_continuation_plan_artifact_dir": str(artifact_dir),
            "source_continuation_plan_json": str(paths["continuation_plan_json"]),
...
def _markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Post-Closeout Promotion-Readiness Uncertainty/Coverage Evidence Package Continuation Plan Static Review",
        "",
...
def _source_plan_contract_checks(source_plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    source_summary = BASE_MODULE._dict(source_plan.get("source_static_review_summary"))
    ...
    checks = [
        ...
        BASE_MODULE._expect("source_plan_ready", decision.get("post_closeout_continuation_plan_ready"), True),
        BASE_MODULE._expect("source_plan_static_review_authorized", decision.get("post_closeout_continuation_plan_static_review_authorized"), True),
        ...
        BASE_MODULE._expect("source_static_review_check_count", source_summary.get("review_check_count"), EXPECTED_SOURCE_STATIC_REVIEW_CHECK_COUNT),
        BASE_MODULE._expect("source_static_review_failed_check_count", source_summary.get("failed_check_count"), 0),
        BASE_MODULE._expect("source_continuation_item_count", len(BASE_MODULE._list(source_plan.get("continuation_plan"))), EXPECTED_CONTINUATION_ITEM_COUNT),
        BASE_MODULE._expect("source_continuation_item_names", [item.get("item_name") for item in BASE_MODULE._list(source_plan.get("continuation_plan"))], list(PLAN_MODULE.EXPECTED_CONTINUATION_ITEMS)),
...
def _source_surface_checks(script_text: str, test_text: str) -> list[dict[str, Any]]:
    return [
        BASE_MODULE._check("plan_script_schema_token", "continuation_plan_v1" in script_text, "continuation_plan_v1", "present"),
        BASE_MODULE._check("plan_script_static_review_next", "continuation_plan_static_review_only" in script_text, "continuation_plan_static_review_only", "present"),
        BASE_MODULE._check("plan_script_no_reopen", "evidence_package_reopened_by_this_gate" in script_text, "evidence_package_reopened_by_this_gate", "present"),
        BASE_MODULE._check("plan_test_rejects_source_promotion_leak", "rejects_source_promotion_leak" in test_text, "rejects_source_promotion_leak", "present"),
        BASE_MODULE._check("plan_test_rejects_source_hash_drift", "rejects_source_hash_drift" in test_text, "rejects_source_hash_drift", "present"),
    ]


def _audit_checks(v14_text: str, status_text: str) -> list[dict[str, Any]]:
    return [
        BASE_MODULE._expect("audit_latest_status_is_continuation_plan", BASE_MODULE._latest_value(v14_text, "current_v14_status"), SOURCE_PLAN_STATUS),
        BASE_MODULE._expect("audit_latest_eof_authorizes_continuation_plan_static_review", BASE_MODULE._latest_value(v14_text, "next_work_target"), AUTHORIZED_CURRENT_WORK),
        BASE_MODULE._expect("audit_continuation_plan_ready", BASE_MODULE._latest_value(v14_text, "post_closeout_continuation_plan_ready"), "True"),
        BASE_MODULE._expect("audit_continuation_plan_static_review_authorized", BASE_MODULE._latest_value(v14_text, "post_closeout_continuation_plan_static_review_authorized"), "True"),
        ...
        BASE_MODULE._expect("status_doc_latest_status_is_continuation_plan", BASE_MODULE._latest_value(status_text, "current_v14_status"), SOURCE_PLAN_STATUS),
        BASE_MODULE._expect("status_doc_latest_eof_authorizes_continuation_plan_static_review", BASE_MODULE._latest_value(status_text, "next_work_target"), AUTHORIZED_CURRENT_WORK),
    ]
...
def _source_plan_summary(source_plan: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "continuation_item_count": len(BASE_MODULE._list(source_plan.get("continuation_plan"))),
    }
...
def _decision(passed: bool, checks: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if passed:
        ...
        failure_class = "explicit_continuation_plan_static_review_authorization_missing"
        ...
        failure_class = "source_continuation_plan_static_review_contract_failure"
    ...
    decision: dict[str, Any] = {
        ...
        "post_closeout_continuation_plan_static_review_passed": passed,
        "promotion_authorization_boundary_plan_authorized": passed,
        ...
        "score_expression": SCORE_EXPRESSION,
        "recommendation": "plan_promotion_authorization_boundary_before_any_promotion_or_claim" if passed else "repair_contract_before_rerun",
    }
    for action in BLOCKED_ACTIONS:
