...
from scripts.integrations.analyze_diffusion_planner_candidate_set_consensus_lane_projected_jerk_progress_failure_attribution import (  # noqa: E402
...
READY_STATUS = (
    ...
    "remediation_design_plan_ready"
)
REJECT_STATUS = (
    "candidate_set_consensus_lane_projected_jerk_progress_support_"
    "remediation_design_plan_rejected"
)
...
DEFAULT_ANALYSIS_ROOT = (
    ...
    "jerk_progress_failure_attribution_read_only_analysis_dbaf8c1"
)
ANALYSIS_JSON = (
    "candidate_set_consensus_lane_projected_jerk_progress_"
    "failure_attribution_read_only_analysis.json"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Plan-only remediation design after read-only failure attribution "
            "identified comfort, hard-feasibility, and latency blockers."
        )
    )
...
def build_report(
    *,
    ...
    artifact = _artifact_summary(
        analysis_root,
        required_files=(ANALYSIS_JSON, SHA256SUMS, EXIT_CODE, HEADS),
    )
    ...
    checks = [
        ...
        *_source_authorization_checks(source),
    ...
    return {
        "analysis": {
            "name": (
                ...
                "jerk_progress_remediation_design_plan_v1"
            ),
            "label": label,
            ...
            "math_boundary": (
                ...
                "artifact and fixed-head audit. It does not implement a new "
                "candidate generator, create candidates, rerun the screen, run "
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines.extend(["", "## Remediation Threads", ""])
    ...
    lines.extend(
        [
            ...
            (
                "Only "
                "`candidate_set_consensus_lane_projected_jerk_progress_support_"
                "remediation_static_contract_review_only` is authorized if all "
                "checks pass."
            ),
            "",
        ]
...
def _remediation_design(source: dict[str, Any]) -> dict[str, Any]:
    ...
    latency = _top_name(source["latency_ranking"])
    return {
        ...
        "design_priorities": [
            {
                "priority": 1,
                "focus": "restore relative comfort support before any replay",
                "evidence": comfort,
            },
            {
                "priority": 2,
                "focus": "separate DP hard-feasibility blockers from CAMP policy blockers",
                "evidence": hard,
            },
            {
                "priority": 3,
                "focus": "bound candidate-build and total latency before screen rerun",
                "evidence": latency,
            },
        ],
        "remediation_threads": [
            {
                ...
                "scope": (
                    "static review of jerk, lateral, progress-loss, and "
                    "smoothness thresholds implied by the existing candidate "
                    "policy and screen evaluator"
                ),
                "static_contract": (
                    "predeclare which candidate metadata or current-tick "
                    "features may change, and prove no future labels or DP "
                    "internals are introduced"
                ),
                "rejection_boundary": (
                    "reject if the proposal requires outcome labels, online "
                    "lambda selection, DP code changes, or screen execution"
                ),
            },
            {
                "name": "hard_feasibility_contract",
                "scope": (
                    "static review of kinematic, lane-crossing, road-border, "
                    "and red-light blockers to classify which are immutable DP "
                    "candidate limitations versus candidate-policy constraints"
                ),
                "static_contract": (
                    "predeclare nonnegative or hinge/signed-split feature "
                    "definitions if a later atom is proposed, preserving "
                    "score_k(w)=a_k^T w"
                ),
                "rejection_boundary": (
                    "reject if the proposal depends on modifying DP, calling "
                    "DP differently, or claiming classical Benders cuts"
                ),
            },
            {
                "name": "latency_contract",
                "scope": (
                    "static review of candidate-build and total p95 sources "
                    "to identify cache, vectorization, or pruning contracts "
                    "that can be unit-tested before any screen rerun"
                ),
                "static_contract": (
                    "predeclare any latency fix as default-off and unit-tested "
                    "against deterministic synthetic inputs first"
                ),
                "rejection_boundary": (
                    "reject if the proposal needs broader replay, GPU timing "
                    "claims, or hidden changes to DP reward/tracker code"
                ),
            },
        ],
        "next_gate_checks": [
...
def _artifact_checks(artifact: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        ...
        _check_equal("analysis_exit_code_zero", artifact["exit_code"], "0"),
    ]
...
def _source_authorization_checks(source: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        ...
        _check_equal(
            "analysis_candidate_generation_not_authorized",
            source["candidate_generation_execution_authorized"],
            False,
        ),
        _check_equal(
            "analysis_screen_rerun_not_authorized",
            source["fixed_snapshot_screen_rerun_authorized"],
            False,
        ),
        _check_equal("analysis_replay_not_authorized", source["new_replay_authorized"], False),
        _check_equal("analysis_formal_seeds_not_authorized", source["formal_seeds_authorized"], False),
        _check_equal("analysis_full36_not_authorized", source["full36_authorized"], False),
        _check_equal("analysis_online_selector_not_authorized", source["online_selector_authorized"], False),
        _check_equal("analysis_atom_promotion_not_authorized", source["atom_promotion_authorized"], False),
        _check_equal("analysis_retraining_not_authorized", source["camp_retraining_authorized"], False),
        _check_equal("analysis_dp_modification_not_authorized", source["dp_modification_authorized"], False),
        _check_equal(
            "analysis_benders_claim_not_authorized",
...
def _source_evidence_checks(source: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        _check_equal(
            "primary_blocker_family_relative_comfort",
        ...
        _check_equal("latency_ranking_present", bool(source["latency_ranking"]), True),
    ]
...
def _design_checks(design: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        _check_equal("design_has_three_priorities", len(design["design_priorities"]), 3),
        _check_equal("design_has_three_threads", len(design["remediation_threads"]), 3),
        _check_equal("design_mentions_relative_comfort", "relative comfort" in text, True),
        _check_equal("design_mentions_hard_feasibility", "hard-feasibility" in text, True),
        ...
        _check_equal("design_mentions_convex_master", "convex master" in text, True),
    ]
