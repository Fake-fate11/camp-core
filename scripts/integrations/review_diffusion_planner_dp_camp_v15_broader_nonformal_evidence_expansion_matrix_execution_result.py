#!/usr/bin/env python3
"""Review the v15 broader non-formal matrix execution artifact."""
...
SCHEMA_VERSION = "dp_camp_v15_broader_nonformal_evidence_expansion_matrix_execution_result_review_v1"
AUTHORIZED_CURRENT_WORK = EXECUTION_MODULE.AUTHORIZED_NEXT_WORK
READY_STATUS = "v15_broader_nonformal_evidence_expansion_matrix_execution_result_review_passed"
REJECT_STATUS = "v15_broader_nonformal_evidence_expansion_matrix_execution_result_review_rejected"
AUTHORIZED_NEXT_WORK = "v15_broader_nonformal_evidence_expansion_offline_training_preflight_plan_only"
REVIEW_JSON_NAME = "v15_broader_nonformal_evidence_expansion_matrix_execution_result_review.json"
REVIEW_MD_NAME = "v15_broader_nonformal_evidence_expansion_matrix_execution_result_review.md"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--source_zero_overlap_validation", type=Path, required=True)
    ...
    parser.add_argument(
        "--enable_v15_broader_nonformal_evidence_expansion_matrix_execution_result_review",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        source_zero_overlap_validation=args.source_zero_overlap_validation,
        ...
        enabled=args.enable_v15_broader_nonformal_evidence_expansion_matrix_execution_result_review,
    )
...
def build_report(
    *,
    ...
    source_zero_overlap_validation: Path,
    ...
    zero_overlap = _read_json(source_zero_overlap_validation)
    scenario_manifest = _read_json(source_scenario_bucket_manifest)
    ...
    checks = [
        ...
        _expect("source_matrix_executed", decision.get("matrix_execution_executed"), True),
        ...
        _expect("execution_row_count", execution["matrix_execution"].get("row_count"), 576),
        _expect("rows_jsonl_row_count", len(rows), 576),
        _expect("split_manifest_row_count", split_manifest.get("total_row_count"), 576),
        _expect("zero_overlap_duplicate_count", zero_overlap.get("duplicate_count"), 0),
        _expect("scenario_manifest_row_count", scenario_manifest.get("total_row_count"), 576),
        _expect("timing_training_not_executed", timing["offline_training"].get("executed"), False),
        _expect("timing_selector_latency_not_executed", timing["online_selector_latency"].get("executed"), False),
    ...
    for path in (
        ...
        source_zero_overlap_validation,
    ...
    report = {
        ...
        "final_decision": {
            ...
            "reviewed_matrix_execution": True,
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# V15 Matrix Execution Result Review",
            "",
