#!/usr/bin/env python3
"""Plan-only claim decision gate after objective-3200 claim boundary review."""
...
def _load_source_review_module():
    review_path = Path(__file__).resolve().with_name(
        ...
        "actual_safetycost_claim_authorization_boundary_plan_static_contract.py"
    )
    spec = importlib.util.spec_from_file_location(
        "v14_candidate_index_actual_safetycost_claim_boundary_plan_static_review",
        review_path,
    )
...
SCHEMA_VERSION = (
    ...
    "objective_3200_candidate_index_actual_safetycost_claim_decision_plan_v1"
)
READY_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_claim_decision_plan_ready"
)
REJECT_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_claim_decision_plan_rejected"
)
AUTHORIZED_NEXT_WORK = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_claim_decision_plan_static_review_only"
)
PLAN_JSON_NAME = (
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_claim_decision_plan.json"
)
PLAN_MD_NAME = (
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_claim_decision_plan.md"
)
EXPECTED_SOURCE_REVIEW_CHECK_COUNT = 92
EXPECTED_SOURCE_PLAN_CHECK_COUNT = 107
OBJECTIVE_RECORD_COUNT = 3200
CLAIM_DECISION_ITEMS = (
    "claim_scope_evidence_binding",
    "allowed_claim_wording_template",
    "forbidden_claim_scope",
    "fixed_dp_candidate_tensor_limit",
    "next_gate_decision_checks",
)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v14_post_closeout_promotion_evidence_acquisition_objective_3200_candidate_index_actual_safetycost_claim_decision_plan",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        enabled=(
            args.enable_v14_post_closeout_promotion_evidence_acquisition_objective_3200_candidate_index_actual_safetycost_claim_decision_plan
        ),
    )
...
def build_report(
    *,
    ...
    claim_plan = _claim_decision_plan(source_review)
    checks = _checks(
        enabled=enabled,
    ...
    return {
        ...
        "claim_decision_plan": claim_plan,
        "plan_checks": checks,
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Objective-3200 Candidate-Index Actual-SafetyCost Claim Decision Plan",
        "",
        ...
        "",
        "## Claim Decision Items",
        "",
    ]
    for item in report["claim_decision_plan"]:
        lines.append(f"- `{item['item_name']}`: `{item['purpose']}`")
    lines.extend(["", "No claim is executed by this plan-only gate.", ""])
    return "\n".join(lines)
...
def _checks(
    *,
    ...
    checks = [
        BASE_MODULE._expect("claim_decision_plan_enabled", enabled, True),
        ...
        BASE_MODULE._expect("source_claim_decision_plan_authorized", decision.get("objective_3200_candidate_index_actual_safetycost_claim_decision_plan_authorized"), True),
        BASE_MODULE._expect("source_claim_executed_false", decision.get("claim_executed_by_this_gate"), False),
        BASE_MODULE._expect("source_safety_claim_false", decision.get("safety_benefit_claim_authorized"), False),
        BASE_MODULE._expect("source_camp_claim_false", decision.get("camp_over_dp_top1_claim_authorized"), False),
...
def _claim_decision_plan(source_review: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        {
            "item_name": "claim_scope_evidence_binding",
            "purpose": "bind any later claim decision to the audited objective-3200 paired SafetyCost_v1 evidence only",
            "required_records": OBJECTIVE_RECORD_COUNT,
            "executes_claim": False,
            "executes_promotion": False,
            "executes_deployment": False,
        },
        {
            "item_name": "allowed_claim_wording_template",
            "purpose": "prepare wording for a later decision gate: CAMP shadow selection over fixed DP candidate tensors reduced paired SafetyCost_v1 versus DP Top-1 in the audited public-simulator objective-3200 evidence",
            "requires_future_decision_gate": True,
            "executes_claim": False,
            "executes_promotion": False,
            "executes_deployment": False,
        },
        {
            "item_name": "forbidden_claim_scope",
            "purpose": "forbid online, deployment, closed-loop safety, Full36, formal-seed, or trajectory-generation generalizations",
            "executes_claim": False,
            "executes_promotion": False,
            "executes_deployment": False,
        },
        {
            "item_name": "fixed_dp_candidate_tensor_limit",
            "purpose": "require claim text to preserve fixed DP commit and CAMP rerank/select-only boundaries",
        ...
        {
            "item_name": "next_gate_decision_checks",
            "purpose": "require static review before any actual safety-benefit or CAMP-over-DP Top-1 claim authorization",
            "executes_claim": False,
            "executes_promotion": False,
            "executes_deployment": False,
        },
...
def _decision(*, passed: bool, checks: list[dict[str, Any]], source_review: dict[str, Any]) -> dict[str, Any]:
    ...
    if passed:
        failure_class = None
    elif "claim_decision_plan_enabled" in failed:
        failure_class = "explicit_candidate_index_actual_safetycost_claim_decision_plan_authorization_missing"
        ...
        failure_class = "source_claim_boundary_static_review_contract_failure"
    else:
        failure_class = "claim_decision_plan_contract_failure"
    return {
        "passed": bool(passed),
        ...
        "objective_3200_candidate_index_actual_safetycost_claim_decision_plan_ready": bool(passed),
        "objective_3200_candidate_index_actual_safetycost_claim_decision_plan_static_review_authorized": bool(passed),
