...
SOURCE_STATUS = "targeted_safety_intervention_scenario_manifest_predeclared"
SOURCE_NEXT_WORK = "targeted_candidate_branch_oracle_input_readiness_gate"

READY_STATUS = "targeted_candidate_branch_oracle_input_readiness_ready"
INCOMPLETE_STATUS = "targeted_candidate_branch_oracle_input_readiness_incomplete"
SOURCE_BLOCKED_STATUS = "targeted_candidate_branch_oracle_input_readiness_source_blocked"
CONFLICT_STATUS = "targeted_candidate_branch_oracle_input_readiness_source_conflict"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Targeted candidate-branch oracle input-readiness gate. It consumes "
            "the targeted scenario manifest gate plus the read-only candidate "
            "availability input audit."
        )
    )
    parser.add_argument("--targeted_manifest_gate_json", type=Path, required=True)
...
def main() -> None:
    args = parse_args()
    report = build_report(
        targeted_manifest_gate=_load_json(args.targeted_manifest_gate_json),
        candidate_readiness=_load_json(args.candidate_readiness_json),
        label=args.label,
        paths={
            "targeted_manifest_gate_json": str(args.targeted_manifest_gate_json),
...
def build_report(
    *,
    targeted_manifest_gate: dict[str, Any],
    candidate_readiness: dict[str, Any],
    label: str | None = None,
    paths: dict[str, str] | None = None,
) -> dict[str, Any]:
    manifest_source = _manifest_source(targeted_manifest_gate)
    readiness_source = _readiness_source(candidate_readiness, manifest_source)
    conflicts = _source_conflicts(targeted_manifest_gate, candidate_readiness)
    decision = _decision(manifest_source, readiness_source, conflicts)
    return {
        "analysis": {
            "name": "dp_camp_targeted_candidate_oracle_input_readiness_gate_v1",
            "label": label,
            "role": (
                "current-chain gate that verifies outcome-labeled fixed DP "
                "candidate logs are ready for the targeted SafetyCost oracle"
            ),
            "training": False,
            ...
            "future_outcome_leakage": (
                ...
                "oracle labels and are forbidden as online selector inputs"
            ),
            "paths": paths or {},
            "math_boundary": (
                "This readiness gate audits existing finite-candidate logs. "
                "Outcome labels are offline evidence only. Current-tick proxy "
                "fields and atom fallbacks are fixed candidate constants; a "
                "later CAMP score must still be affine score_k(w)=a_k^T w with "
                "a convex simplex/CVaR/L2 robust master. This gate does not "
                "construct a DP-side classical Benders master/subproblem, "
                "dual, or cut."
            ),
        },
        "targeted_manifest_source": manifest_source,
...
def _manifest_source(report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "target_missing_buckets": list(matrix.get("target_missing_buckets") or []),
        "guard_missing_buckets": list(matrix.get("guard_missing_buckets") or []),
        "formal_seeds": list(matrix.get("formal_seeds") or []),
...
def _readiness_source(
    ...
    planned_logs = manifest_source.get("planned_run_count")
...
def _decision(
    ...
    if conflicts:
    ...
    elif not manifest_source["passed"]:
        status = SOURCE_BLOCKED_STATUS
        reasons = ["targeted_manifest_gate_not_ready"]
        next_step = "Repair or rerun the targeted scenario manifest gate."
        ...
        next_step = (
            ...
            "running the targeted SafetyCost oracle."
        )
    else:
        status = READY_STATUS
        reasons = [
        ...
        next_step = (
            "Run the targeted SafetyCost oracle audit with the same scenario "
            "manifest before any training, replay promotion, or formal seeds."
        )
    return {
        "status": status,
        ...
        "recommended_first_action": (
            "targeted_safety_cost_oracle_audit"
            if status == READY_STATUS
            else "repair_targeted_candidate_oracle_inputs"
        ),
        "reasons": reasons,
        "next_step": next_step,
...
def _readiness_failure_reasons(source: dict[str, Any]) -> list[str]:
    ...
    if not source["log_count_matches_plan"]:
        reasons.append("readiness_log_count_does_not_match_manifest_plan")
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    manifest = report["targeted_manifest_source"]
    readiness = report["candidate_readiness_source"]
    lines = [
        "# Targeted Candidate Oracle Input Readiness",
        "",
        ...
        f"- Planned logs: `{manifest['planned_run_count']}`",
        f"- Target missing buckets: `{', '.join(manifest['target_missing_buckets']) or 'none'}`",
        f"- Guard missing buckets: `{', '.join(manifest['guard_missing_buckets']) or 'none'}`",
        "",
        "## Readiness Source",
        "",
