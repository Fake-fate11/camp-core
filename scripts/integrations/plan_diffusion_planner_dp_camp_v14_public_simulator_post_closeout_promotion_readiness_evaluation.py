#!/usr/bin/env python3
"""Plan-only v14 post-closeout promotion-readiness evaluation gate.

This gate consumes the audited promotion-readiness evaluation preflight static
review and its source preflight artifact. It emits only a read-only plan for
future promotion-readiness evidence evaluation. It does not run evaluation,
...
SOURCE_STATIC_REVIEW_SCHEMA = (
    ...
    "promotion_readiness_evaluation_preflight_static_review_v1"
)
...
SCHEMA_VERSION = (
    "dp_camp_v14_public_simulator_post_closeout_"
    "promotion_readiness_evaluation_plan_v1"
)
SOURCE_STATIC_REVIEW_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_readiness_evaluation_preflight_static_review_passed"
)
...
AUTHORIZED_CURRENT_WORK = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_readiness_evaluation_plan_only"
)
READY_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_readiness_evaluation_plan_ready"
)
REJECT_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_readiness_evaluation_plan_rejected"
)
AUTHORIZED_NEXT_WORK = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_readiness_evaluation_plan_static_review_only"
)

STATIC_REVIEW_JSON_NAME = (
    "post_closeout_promotion_readiness_evaluation_preflight_static_review.json"
)
STATIC_REVIEW_MD_NAME = (
    "post_closeout_promotion_readiness_evaluation_preflight_static_review.md"
)
...
PLAN_JSON_NAME = "post_closeout_promotion_readiness_evaluation_plan.json"
PLAN_MD_NAME = "post_closeout_promotion_readiness_evaluation_plan.md"
...
EXPECTED_DECISION_SURFACES = (
    "active_selector_promotion_decision",
    "deployment_fail_closed_decision",
    "safety_or_camp_over_dp_claim_decision",
)
EXPECTED_EVIDENCE_REQUIREMENTS = (
    "fixed_dp_candidate_immutability",
    "formal_split_and_seed_exclusion",
    "shadow_replay_reproducibility",
    "statistical_significance_and_uncertainty",
    "stress_ood_and_failure_mode_coverage",
    "default_off_fail_closed_runtime_boundary",
    "claim_specific_comparator_evidence",
)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Explicit opt-in for the read-only promotion-readiness evaluation plan.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        label=args.label,
        enabled=args.enable_v14_post_closeout_promotion_readiness_evaluation_plan,
    )
...
def build_report(
    *,
    ...
    static_review = _read_json_dict(paths["preflight_static_review_json"])
    ...
    preflight_root_sha256s = _read_sha256sums(preflight_artifact_files["root_sha256s"])
    source_preflight_sha256s_values = _read_sha256sums(paths["source_preflight_sha256s"])
    static_heads = _parse_key_values(_read_text(static_artifact_files["heads"]))
    preflight_heads = _parse_key_values(_read_text(preflight_artifact_files["heads"]))
    v14_text = _read_text(paths["v14_audit_md"])
    status_text = _read_text(paths["current_status_md"])

    checks: list[dict[str, Any]] = [
        _expect("evaluation_plan_enabled", enabled, True),
    ...
    for name, path in preflight_artifact_files.items():
        checks.extend(_path_checks(f"source_preflight_artifact_{name}", path, require_file=True, allow_empty=(name == "stderr")))
    checks.extend(
        [
            _expect("static_review_json_matches_artifact_layout", paths["preflight_static_review_json"], static_artifact_files["review_json"]),
            _expect("static_review_md_matches_artifact_layout", paths["preflight_static_review_md"], static_artifact_files["review_md"]),
            _expect("static_review_sha256s_matches_artifact_layout", paths["preflight_static_review_sha256s"], static_artifact_files["review_sha256s"]),
            _expect("source_preflight_json_matches_artifact_layout", paths["source_preflight_json"], preflight_artifact_files["preflight_json"]),
            _expect("source_preflight_md_matches_artifact_layout", paths["source_preflight_md"], preflight_artifact_files["preflight_md"]),
            _expect("source_preflight_sha256s_matches_artifact_layout", paths["source_preflight_sha256s"], preflight_artifact_files["preflight_sha256s"]),
        ]
    )
    checks.extend(_static_review_artifact_hash_checks(static_artifact_files, static_root_sha256s, static_review_sha256s))
    checks.extend(_source_preflight_artifact_hash_checks(preflight_artifact_files, preflight_root_sha256s, source_preflight_sha256s_values))
    checks.extend(_static_heads_checks(static_heads, static_review, preflight_artifact))
    checks.extend(_source_preflight_heads_checks(preflight_heads, source_preflight))
    ...
    checks.extend(_plan_surface_checks())
    ...
    return {
        ...
        "analysis": {
            ...
            "preflight_static_review_json": str(paths["preflight_static_review_json"]),
            "source_preflight_json": str(paths["source_preflight_json"]),
            ...
            "math_boundary": (
                "This read-only plan evaluates what evidence would be required "
                "before any future selector promotion, deployment, or claim "
                "decision. CAMP remains default-off shadow reranking over fixed "
                f"DP candidate tensors with affine {SCORE_EXPRESSION} over "
                "approved atoms and nonnegative simplex weights."
            ),
        },
        "source_hashes": {
            name: _sha256(path) if path.is_file() else None
            for name, path in {**paths, **static_artifact_files, **preflight_artifact_files}.items()
        },
        ...
        "decision_surfaces": _decision_surfaces(),
...
def write_outputs(output_dir: Path, report: dict[str, Any]) -> None:
    ...
    (output_dir / PLAN_MD_NAME).write_text(render_markdown(report), encoding="utf-8")
    _write_sha256sums(output_dir)


def render_markdown(report: dict[str, Any]) -> str:
    decision = report["final_decision"]
    static_summary = report["source_static_review_summary"]
    preflight_summary = report["source_preflight_summary"]
    lines = [
        ...
        f"- Source static review status: `{static_summary.get('status')}`",
        f"- Source preflight status: `{preflight_summary.get('status')}`",
        f"- Decision surfaces: `{len(report['decision_surfaces'])}`",
        f"- Evidence requirements: `{len(report['evidence_requirements'])}`",
        f"- No-go conditions: `{len(report['no_go_conditions'])}`",
    ...
    for item in report["decision_surfaces"]:
...
def _source_preflight_artifact_hash_checks(
    files: dict[str, Path],
    root_sha256s: dict[str, str],
    ...
    checks = [
        _check("source_preflight_root_sha256s_parseable", bool(root_sha256s), sorted(root_sha256s), "nonempty"),
        _check("source_preflight_sha256s_parseable", bool(preflight_sha256s), sorted(preflight_sha256s), "nonempty"),
    ]
    for name, keys in root_expected.items():
        checks.append(_sha256sums_expect(f"source_preflight_artifact_{name}_root_sha", files[name], root_sha256s, keys))
    checks.extend(
        [
            _sha256sums_expect("source_preflight_json_preflight_sha", files["preflight_json"], preflight_sha256s, (PREFLIGHT_JSON_NAME, f"./{PREFLIGHT_JSON_NAME}")),
            _sha256sums_expect("source_preflight_md_preflight_sha", files["preflight_md"], preflight_sha256s, (PREFLIGHT_MD_NAME, f"./{PREFLIGHT_MD_NAME}")),
            _expect("source_preflight_artifact_run_exit_zero", _read_text(files["run_exit"]).strip(), "0"),
        ]
    )
    return checks
...
def _static_heads_checks(
    ...
    return [
        ...
        _expect("static_review_heads_source_preflight_artifact", heads.get("source_preflight_artifact"), str(source_preflight_artifact)),
        _expect("static_review_analysis_source_preflight_artifact", analysis.get("preflight_artifact_dir"), str(source_preflight_artifact)),
    ]


def _source_preflight_heads_checks(
    heads: dict[str, str],
    source_preflight: dict[str, Any],
) -> list[dict[str, Any]]:
    analysis = _dict(source_preflight.get("analysis"))
    return [
        _expect("source_preflight_heads_dp_fixed", heads.get("dp_head"), FIXED_DP_HEAD),
        _check("source_preflight_heads_camp_head_is_sha", _is_git_sha(str(heads.get("camp_head", ""))), heads.get("camp_head"), "40-char git sha"),
        _check("source_preflight_heads_camp_origin_is_sha", _is_git_sha(str(heads.get("camp_origin_main", ""))), heads.get("camp_origin_main"), "40-char git sha"),
        _expect("source_preflight_heads_camp_matches_origin", heads.get("camp_head"), heads.get("camp_origin_main")),
        _check("source_preflight_heads_plan_static_review_present", bool(heads.get("source_preflight_plan_static_review_artifact")), heads.get("source_preflight_plan_static_review_artifact"), "path"),
        _check("source_preflight_heads_plan_present", bool(heads.get("source_preflight_plan_artifact")), heads.get("source_preflight_plan_artifact"), "path"),
        _expect("source_preflight_analysis_current_dp_fixed", analysis.get("current_dp_head"), FIXED_DP_HEAD),
        _expect("source_preflight_analysis_required_dp_fixed", analysis.get("required_dp_head"), FIXED_DP_HEAD),
    ]
...
def _static_review_contract_checks(static_review: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    checks = [
        ...
        _expect("source_static_review_plan_authorized", decision.get("promotion_readiness_evaluation_plan_authorized"), True),
...
def _source_preflight_contract_checks(source_preflight: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    checks = [
        ...
        _expect("source_preflight_static_review_authorized", decision.get("preflight_static_review_authorized"), True),
        ...
        _expect("source_preflight_evaluation_preflight_count", len(_list(source_preflight.get("evaluation_preflight"))), 5),
        _expect("source_preflight_no_go_status_count", len(no_go), 7),
        _expect("source_preflight_future_review_requirement_count", len(_list(source_preflight.get("future_review_requirements"))), 4),
...
def _plan_surface_checks() -> list[dict[str, Any]]:
    surfaces = _decision_surfaces()
    requirements = _evidence_requirements()
    no_go = _no_go_conditions()
    future = _future_review_requirements()
    return [
        _expect("plan_decision_surfaces", sorted(item["name"] for item in surfaces), sorted(EXPECTED_DECISION_SURFACES)),
        _expect("plan_evidence_requirements", sorted(item["name"] for item in requirements), sorted(EXPECTED_EVIDENCE_REQUIREMENTS)),
        _check("plan_no_go_conditions_present", len(no_go) >= 7, len(no_go), ">=7"),
        _expect("plan_future_review_requirement_count", len(future), 4),
        _expect("plan_score_expression", SCORE_EXPRESSION, "score_k(w)=a_k^T w"),
    ]
...
def _audit_checks(v14_text: str, status_text: str) -> list[dict[str, Any]]:
    ...
    return [
        _expect("audit_latest_eof_authorizes_evaluation_plan", (_latest_value(v14_text, "current_v14_status"), _latest_value(v14_text, "next_work_target")), expected_pair),
        _expect("status_doc_latest_eof_authorizes_evaluation_plan", (_latest_value(status_text, "current_v14_status"), _latest_value(status_text, "next_work_target")), expected_pair),
        _expect("audit_preflight_static_review_passed", _latest_value(v14_text, "post_closeout_promotion_readiness_evaluation_preflight_static_review_passed"), "True"),
        _expect("audit_evaluation_plan_authorized", _latest_value(v14_text, "post_closeout_promotion_readiness_evaluation_plan_authorized"), "True"),
...
def _decision_surfaces() -> list[dict[str, str]]:
    return [
        {
            "name": "active_selector_promotion_decision",
            "status": "future_evidence_required_no_promotion_authorized",
            "scope": "define pass/fail criteria for any later active selector promotion decision",
        },
        {
            "name": "deployment_fail_closed_decision",
            "status": "future_evidence_required_no_deployment_authorized",
            "scope": "define fail-closed runtime and rollback evidence required before deployment discussion",
        },
        {
            "name": "safety_or_camp_over_dp_claim_decision",
            "status": "future_evidence_required_no_claim_authorized",
            "scope": "define claim-specific comparator and statistical evidence before any safety or CAMP-over-DP claim",
        },
    ]
...
def _evidence_requirements() -> list[dict[str, str]]:
    return [
        {
            "name": "fixed_dp_candidate_immutability",
            "requirement": "prove all evaluated candidates are immutable fixed-DP tensors with no CAMP generation or repair",
        },
        {
            "name": "formal_split_and_seed_exclusion",
            "requirement": "prove no Full36 or formal seeds 11/12/13 are used before a future formal gate",
        },
        {
            "name": "shadow_replay_reproducibility",
            "requirement": "pin artifact hashes, commands, heads, stdout/stderr, and deterministic replay manifests",
        },
        {
            "name": "statistical_significance_and_uncertainty",
            "requirement": "predefine metrics, confidence intervals, multiplicity handling, and failure thresholds",
        },
        {
            "name": "stress_ood_and_failure_mode_coverage",
            "requirement": "predefine red routes, scenario buckets, OOD cases, and manual failure review surfaces",
        },
        {
            "name": "default_off_fail_closed_runtime_boundary",
            "requirement": "prove default-off behavior, DP Top-1 fallback, and no online selector activation",
        },
        {
            "name": "claim_specific_comparator_evidence",
            "requirement": "separate promotion evidence from deployment, safety-benefit, and CAMP-over-DP Top-1 claim evidence",
        },
    ]
...
def _no_go_conditions() -> list[str]:
    return [
        ...
        "promotion, deployment, online selector activation, deployable-checkpoint claim, safety claim, or CAMP-over-DP claim is bundled into this gate",
    ]
...
def _future_review_requirements() -> list[dict[str, str]]:
    return [
        {
            "name": "evaluation_plan_static_review",
            "status": "required_before_any_evidence_generation_or_promotion_decision",
        },
        {
            "name": "artifact_hash_and_head_review",
            "status": "must confirm source preflight and static-review artifact immutability",
        },
        {
        ...
        {
            "name": "mathematical_contract_review",
            "status": "must confirm affine scoring and convex nonnegative simplex weight boundary",
        },
    ]
...
def _source_preflight_summary(source_preflight: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "evaluation_preflight_count": len(_list(source_preflight.get("evaluation_preflight"))),
...
def _decision(passed: bool, checks: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    decision = {
        ...
        "post_closeout_promotion_readiness_evaluation_plan_ready": bool(passed),
        "evaluation_plan_static_review_authorized": bool(passed),
        "recommendation": "static_review_this_evaluation_plan_only",
        "immediate_action": "static_review_promotion_readiness_evaluation_plan_only",
...
def _failure_class(failed: list[str]) -> str:
    ...
    if "evaluation_plan_enabled" in failed_set:
        return "explicit_evaluation_plan_authorization_missing"
    if {"current_dp_head_fixed", "required_dp_head_fixed", "static_review_heads_dp_fixed", "source_preflight_heads_dp_fixed"} & failed_set:
    ...
    if any(name.startswith("plan_") for name in failed):
        return "evaluation_plan_surface_contract_failure"
    if any(name.endswith("_exists") or name.endswith("_nonempty") for name in failed):
        return "source_file_missing_or_empty"
    return "promotion_readiness_evaluation_plan_failure"
