#!/usr/bin/env python3
"""Static review for the v14 promotion-readiness runbook execution artifact."""
...
REJECT_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_readiness_evaluation_runbook_execution_static_review_rejected"
)
...
REVIEW_JSON_NAME = "post_closeout_promotion_readiness_evaluation_runbook_execution_static_review.json"
REVIEW_MD_NAME = "post_closeout_promotion_readiness_evaluation_runbook_execution_static_review.md"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--runbook_execution_artifact_dir", type=Path, required=True)
    parser.add_argument("--runbook_execution_json", type=Path, required=True)
    parser.add_argument("--runbook_execution_md", type=Path, required=True)
    parser.add_argument("--runbook_execution_sha256s", type=Path, required=True)
    parser.add_argument("--execution_script_py", type=Path, required=True)
    parser.add_argument("--execution_test_py", type=Path, required=True)
    ...
    parser.add_argument(
        "--enable_v14_post_closeout_promotion_readiness_evaluation_runbook_execution_static_review",
        action="store_true",
        help="Explicit opt-in for static review of the read-only runbook execution artifact.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        runbook_execution_artifact_dir=args.runbook_execution_artifact_dir,
        runbook_execution_json=args.runbook_execution_json,
        runbook_execution_md=args.runbook_execution_md,
        runbook_execution_sha256s=args.runbook_execution_sha256s,
        execution_script_py=args.execution_script_py,
        execution_test_py=args.execution_test_py,
        ...
        label=args.label,
        enabled=args.enable_v14_post_closeout_promotion_readiness_evaluation_runbook_execution_static_review,
    )
...
def build_report(
    *,
    runbook_execution_artifact_dir: Path,
    runbook_execution_json: Path,
    runbook_execution_md: Path,
    runbook_execution_sha256s: Path,
    execution_script_py: Path,
    execution_test_py: Path,
    v14_audit_md: Path,
    ...
    artifact_dir = runbook_execution_artifact_dir.resolve()
    paths = {
        "runbook_execution_json": runbook_execution_json.resolve(),
        "runbook_execution_md": runbook_execution_md.resolve(),
        "runbook_execution_sha256s": runbook_execution_sha256s.resolve(),
        "execution_script_py": execution_script_py.resolve(),
        "execution_test_py": execution_test_py.resolve(),
    ...
    artifact_files = {
        ...
        "execution_json": artifact_dir / "execution" / EXECUTION_JSON_NAME,
        "execution_md": artifact_dir / "execution" / EXECUTION_MD_NAME,
        "execution_sha256s": artifact_dir / "execution" / "SHA256SUMS",
    }
    source_execution = _read_json_dict(paths["runbook_execution_json"])
    root_sha256s = _read_sha256sums(artifact_files["root_sha256s"])
    execution_sha256s = _read_sha256sums(paths["runbook_execution_sha256s"])
    heads = _parse_key_values(_read_text(artifact_files["heads"]))
    script_text = _read_text(paths["execution_script_py"])
    test_text = _read_text(paths["execution_test_py"])
    ...
    checks: list[dict[str, Any]] = [
        ...
        _check("runbook_execution_artifact_dir_exists", artifact_dir.is_dir(), str(artifact_dir), "directory"),
    ]
    ...
    checks.extend(
        [
            _expect("execution_json_matches_artifact_layout", paths["runbook_execution_json"], artifact_files["execution_json"]),
            _expect("execution_md_matches_artifact_layout", paths["runbook_execution_md"], artifact_files["execution_md"]),
            _expect("execution_sha256s_matches_artifact_layout", paths["runbook_execution_sha256s"], artifact_files["execution_sha256s"]),
        ]
    )
    checks.extend(_artifact_hash_checks(artifact_files, root_sha256s, execution_sha256s))
    checks.extend(_heads_checks(heads, source_execution))
    ...
    return {
        ...
        "analysis": {
            ...
            "read_only": True,
            "runbook_execution_artifact_dir": str(artifact_dir),
            "runbook_execution_json": str(paths["runbook_execution_json"]),
            "runbook_execution_md": str(paths["runbook_execution_md"]),
            "runbook_execution_sha256s": str(paths["runbook_execution_sha256s"]),
        ...
        "source_execution_summary": _source_summary(source_execution),
...
def _artifact_hash_checks(
    ...
    return [
        ...
        _sha256sums_expect("artifact_execution_json_root_sha", artifact_files["execution_json"], root_sha256s, (f"execution/{EXECUTION_JSON_NAME}", f"./execution/{EXECUTION_JSON_NAME}", EXECUTION_JSON_NAME)),
        _sha256sums_expect("artifact_execution_md_root_sha", artifact_files["execution_md"], root_sha256s, (f"execution/{EXECUTION_MD_NAME}", f"./execution/{EXECUTION_MD_NAME}", EXECUTION_MD_NAME)),
        _sha256sums_expect("artifact_execution_sha256s_root_sha", artifact_files["execution_sha256s"], root_sha256s, ("execution/SHA256SUMS", "./execution/SHA256SUMS", "SHA256SUMS")),
        _sha256sums_expect("artifact_execution_json_execution_sha", artifact_files["execution_json"], execution_sha256s, (EXECUTION_JSON_NAME, f"./{EXECUTION_JSON_NAME}")),
        _sha256sums_expect("artifact_execution_md_execution_sha", artifact_files["execution_md"], execution_sha256s, (EXECUTION_MD_NAME, f"./{EXECUTION_MD_NAME}")),
        _expect("artifact_run_exit_zero", _read_text(artifact_files["run_exit"]).strip(), "0"),
    ]


def _heads_checks(heads: dict[str, str], source_execution: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        _expect(
            "artifact_heads_source_static_review_path",
            heads.get("source_runbook_execution_preflight_static_review_artifact"),
            analysis.get("runbook_execution_preflight_static_review_artifact_dir"),
        ),
        _expect(
            "artifact_heads_source_preflight_path",
            heads.get("source_runbook_execution_preflight_artifact"),
            analysis.get("source_runbook_execution_preflight_artifact_dir"),
        ),
    ]
...
def _source_execution_contract_checks(source_execution: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    checks = [
        ...
        _expect("source_execution_authorized_next_work", decision.get("authorized_next_work"), AUTHORIZED_CURRENT_WORK),
...
def _source_surface_checks(script: str, test: str) -> list[dict[str, Any]]:
    return [
        _contains("source_surface_script_schema", script, "promotion_readiness_evaluation_runbook_execution_v1"),
        _contains("source_surface_script_authorized_next", script, "runbook_execution_static_review_only"),
        ...
        _contains("source_surface_script_nonclaim_matrix", script, "materializes_nonclaim_evidence_matrix"),
        _contains("source_surface_test_pass_case", test, "test_promotion_readiness_evaluation_runbook_execution_passes"),
        _contains("source_surface_test_requires_enable", test, "test_promotion_readiness_evaluation_runbook_execution_requires_enable"),
        _contains("source_surface_test_rejects_wrong_eof", test, "test_promotion_readiness_evaluation_runbook_execution_rejects_wrong_eof"),
        _contains("source_surface_test_rejects_source_leak", test, "test_promotion_readiness_evaluation_runbook_execution_rejects_source_static_review_leak"),
    ]


def _audit_checks(v14_text: str, status_text: str) -> list[dict[str, Any]]:
    expected_pair = (SOURCE_EXECUTION_STATUS, AUTHORIZED_CURRENT_WORK)
    return [
        _expect("audit_latest_eof_authorizes_execution_static_review", (_latest_value(v14_text, "current_v14_status"), _latest_value(v14_text, "next_work_target")), expected_pair),
        _expect("status_doc_latest_eof_authorizes_execution_static_review", (_latest_value(status_text, "current_v14_status"), _latest_value(status_text, "next_work_target")), expected_pair),
        _expect("audit_runbook_execution_passed", _latest_value(v14_text, "post_closeout_promotion_readiness_evaluation_runbook_execution_passed"), "True"),
        _expect("audit_runbook_execution_static_review_authorized", _latest_value(v14_text, "post_closeout_promotion_readiness_evaluation_runbook_execution_static_review_authorized"), "True"),
...
def _source_summary(source_execution: dict[str, Any]) -> dict[str, Any]:
    decision = _dict(source_execution.get("final_decision"))
    return {
...
def _decision(passed: bool, checks: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    decision = {
        ...
        "post_closeout_promotion_readiness_evaluation_runbook_execution_static_review_passed": bool(passed),
        "evaluation_runbook_execution_result_review_authorized": bool(passed),
        "evaluation_runbook_execution_authorized": False,
        "recommendation": "review_read_only_promotion_readiness_evaluation_runbook_execution_result_only",
        "immediate_action": "review_promotion_readiness_evaluation_runbook_execution_result_only",
...
def _failure_class(failed: list[str]) -> str:
    ...
    if "static_review_enabled" in failed_set:
        return "explicit_runbook_execution_static_review_authorization_missing"
    ...
    if any(name.endswith("_sha") or name.endswith("_root_sha") for name in failed):
        return "runbook_execution_artifact_sha256_mismatch"
    if any(name.startswith("source_execution") for name in failed):
        return "source_runbook_execution_contract_failure"
    return "promotion_readiness_evaluation_runbook_execution_static_review_failure"
