#!/usr/bin/env python3
"""Plan-only closure gate for v14 uncertainty/coverage evidence gaps."""
...
SCHEMA_VERSION = (
    "dp_camp_v14_public_simulator_post_closeout_"
    "promotion_readiness_uncertainty_coverage_evidence_gap_closure_plan_v1"
)
...
READY_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_readiness_uncertainty_coverage_evidence_gap_closure_plan_ready"
)
REJECT_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_readiness_uncertainty_coverage_evidence_gap_closure_plan_rejected"
)
AUTHORIZED_NEXT_WORK = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_readiness_uncertainty_coverage_evidence_gap_closure_plan_static_review_only"
)
...
PLAN_JSON_NAME = (
    "post_closeout_promotion_readiness_uncertainty_coverage_evidence_gap_closure_plan.json"
)
PLAN_MD_NAME = (
    "post_closeout_promotion_readiness_uncertainty_coverage_evidence_gap_closure_plan.md"
)
...
EXPECTED_SOURCE = {
    "static_review_check_count": 134,
    "source_review_check_count": 227,
    "source_review_item_count": 7,
    "source_evidence_gap_count": 5,
}
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v14_post_closeout_promotion_readiness_uncertainty_coverage_evidence_gap_closure_plan",
        action="store_true",
        help="Explicit opt-in for read-only uncertainty/coverage evidence-gap closure planning.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        label=args.label,
        enabled=args.enable_v14_post_closeout_promotion_readiness_uncertainty_coverage_evidence_gap_closure_plan,
    )
...
def build_report(
    *,
    ...
    artifact_files = {
        ...
        "review_json": artifact_dir / "review" / SOURCE_STATIC_REVIEW_JSON_NAME,
        "review_md": artifact_dir / "review" / SOURCE_STATIC_REVIEW_MD_NAME,
    ...
    checks: list[dict[str, Any]] = [
        _expect("evidence_gap_closure_plan_enabled", enabled, True),
    ...
    closure_items = _closure_plan_items()
    checks.extend(
        [
            _expect("closure_plan_gap_names", [item["source_gap"] for item in closure_items], list(EXPECTED_EVIDENCE_GAPS)),
            _expect("closure_plan_item_count", len(closure_items), len(EXPECTED_EVIDENCE_GAPS)),
            _expect("closure_plan_no_execution", [item["authorizes_execution"] for item in closure_items], [False] * len(EXPECTED_EVIDENCE_GAPS)),
            _expect("closure_plan_no_claim", [item["authorizes_claim"] for item in closure_items], [False] * len(EXPECTED_EVIDENCE_GAPS)),
            _expect("closure_plan_requires_static_review_next", AUTHORIZED_NEXT_WORK.endswith("_static_review_only"), True),
        ]
    )
    ...
    return {
        ...
        "analysis": {
            ...
            "plan_only": True,
            "read_only": True,
            "uncertainty_coverage_evidence_gap_closure_plan_only": True,
        ...
        "evidence_gap_closure_plan": closure_items,
...
def _markdown(report: dict[str, Any]) -> str:
    ...
    lines.extend(["", "## Closure Plan"])
    for item in report["evidence_gap_closure_plan"]:
        lines.extend(
            [
                f"- `{item['source_gap']}` -> `{item['planned_artifact']}`",
                f"  - scope: `{item['scope']}`",
                f"  - acceptance_checks: `{', '.join(item['acceptance_checks'])}`",
                f"  - no_go_conditions: `{', '.join(item['no_go_conditions'])}`",
            ]
        )
    lines.extend(["", "## Checks"])
    ...
    lines.extend(
        [
            ...
            "closure plan and does not materialize evidence manifests, run replay, "
            "train CAMP, generate candidates, modify DP, promote, deploy, activate "
            "an online selector, or make safety/CAMP-over-DP claims.",
        ]
    )
    return "\n".join(lines) + "\n"


def _closure_plan_items() -> list[dict[str, Any]]:
    return [
        {
            "source_gap": "future_uncertainty_input_manifest",
            "planned_artifact": "uncertainty_input_manifest",
            "scope": "enumerate read-only uncertainty inputs derived from fixed DP candidate tensors and audited selector features",
            "required_inputs": ["fixed_dp_candidate_tensor_manifest", "selector_feature_manifest", "score_margin_table"],
            "acceptance_checks": ["fixed_dp_head", "no_closed_loop_outcomes", "affine_score_terms_only"],
            "no_go_conditions": ["dp_head_drift", "candidate_generation_or_repair", "non_affine_score"],
            "authorizes_execution": False,
            "authorizes_claim": False,
        },
        {
            "source_gap": "future_coverage_slice_manifest",
            "planned_artifact": "coverage_slice_manifest",
            "scope": "define simulator split and scenario slices for evidence coverage without consuming Full36 or formal seeds 11/12/13",
            "required_inputs": ["audited_split_manifest", "scenario_bucket_manifest", "source_candidate_manifest"],
            "acceptance_checks": ["zero_overlap", "formal_seed_exclusion", "slice_count_is_audited"],
            "no_go_conditions": ["split_overlap", "full36_use", "formal_seed_11_12_13_use"],
            "authorizes_execution": False,
            "authorizes_claim": False,
        },
        {
            "source_gap": "future_atom_stability_manifest",
            "planned_artifact": "atom_stability_manifest",
            "scope": "predeclare stability checks for approved atoms under non-negative simplex weights",
            "required_inputs": ["approved_atom_registry", "simplex_weight_contract", "shadow_selector_score_terms"],
            "acceptance_checks": ["approved_atoms_only", "nonnegative_simplex_weights", "convex_master_contract"],
            "no_go_conditions": ["unapproved_atom", "negative_weight", "nonconvex_master"],
            "authorizes_execution": False,
            "authorizes_claim": False,
        },
        {
            "source_gap": "future_no_go_summary",
            "planned_artifact": "no_go_summary",
            "scope": "compile explicit stop conditions before any follow-on evidence materialization or review",
            "required_inputs": ["fixed_dp_boundary", "split_boundary", "selector_boundary", "claim_boundary"],
            "acceptance_checks": ["all_hard_boundaries_present", "promotion_not_authorized", "deployment_not_authorized"],
            "no_go_conditions": ["dp_modification", "online_selector_change", "promotion_or_deployment_bundled"],
            "authorizes_execution": False,
            "authorizes_claim": False,
        },
        {
            "source_gap": "future_claim_boundary_summary",
            "planned_artifact": "claim_boundary_summary",
            "scope": "separate allowed evidence statements from prohibited safety or CAMP-over-DP Top-1 claims",
            "required_inputs": ["review_evidence_gap_matrix", "promotion_readiness_boundary", "current_evidence_package_summary"],
            "acceptance_checks": ["no_safety_benefit_claim", "no_camp_over_dp_top1_claim", "no_deployable_checkpoint_claim"],
            "no_go_conditions": ["safety_claim", "camp_over_dp_top1_claim", "deployable_checkpoint_claim"],
            "authorizes_execution": False,
            "authorizes_claim": False,
        },
    ]
...
def _artifact_hash_checks(
    ...
    return [
        ...
        _sha256sums_expect("artifact_review_json_root_sha", artifact_files["review_json"], root_sha256s, (f"review/{SOURCE_STATIC_REVIEW_JSON_NAME}", f"./review/{SOURCE_STATIC_REVIEW_JSON_NAME}", SOURCE_STATIC_REVIEW_JSON_NAME)),
        _sha256sums_expect("artifact_review_md_root_sha", artifact_files["review_md"], root_sha256s, (f"review/{SOURCE_STATIC_REVIEW_MD_NAME}", f"./review/{SOURCE_STATIC_REVIEW_MD_NAME}", SOURCE_STATIC_REVIEW_MD_NAME)),
        ...
        _sha256sums_expect("source_review_json_review_sha", artifact_files["review_json"], review_sha256s, (SOURCE_STATIC_REVIEW_JSON_NAME, f"./{SOURCE_STATIC_REVIEW_JSON_NAME}")),
        _sha256sums_expect("source_review_md_review_sha", artifact_files["review_md"], review_sha256s, (SOURCE_STATIC_REVIEW_MD_NAME, f"./{SOURCE_STATIC_REVIEW_MD_NAME}")),
...
def _source_static_review_contract_checks(source_static_review: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    analysis = _dict(source_static_review.get("analysis"))
    source_summary = _dict(source_static_review.get("source_review_summary"))
    checks = [
        ...
        _expect("source_static_review_authorizes_closure_plan", decision.get("uncertainty_coverage_evidence_gap_closure_plan_authorized"), True),
        _expect("source_static_review_direct_promotion", decision.get("direct_promotion_recommendation"), False),
        _expect("source_static_review_promotion_plan_next", decision.get("promotion_decision_plan_authorized_next"), False),
        ...
        _expect("source_review_summary_schema", source_summary.get("schema_version"), "dp_camp_v14_public_simulator_post_closeout_promotion_readiness_uncertainty_coverage_review_v1"),
        _expect("source_review_summary_status", source_summary.get("status"), SOURCE_REVIEW_STATUS),
        _expect("source_review_summary_passed", source_summary.get("passed"), True),
        _expect("source_review_summary_authorized_next_work", source_summary.get("authorized_next_work"), SOURCE_REVIEW_AUTHORIZED_NEXT_WORK),
        _expect("source_review_summary_review_check_count", source_summary.get("review_check_count"), EXPECTED_SOURCE["source_review_check_count"]),
        _expect("source_review_summary_review_item_count", source_summary.get("review_item_count"), EXPECTED_SOURCE["source_review_item_count"]),
        _expect("source_review_summary_evidence_gap_count", source_summary.get("evidence_gap_count"), EXPECTED_SOURCE["source_evidence_gap_count"]),
...
def _audit_checks(v14_text: str, status_text: str) -> list[dict[str, Any]]:
    return [
        ...
        _expect("audit_latest_eof_authorizes_closure_plan", _latest_value(v14_text, "next_work_target"), AUTHORIZED_CURRENT_WORK),
        _expect("audit_closure_plan_authorized_flag", _latest_value(v14_text, "uncertainty_coverage_evidence_gap_closure_plan_authorized"), "True"),
        ...
        _expect("status_doc_latest_eof_authorizes_closure_plan", _latest_value(status_text, "next_work_target"), AUTHORIZED_CURRENT_WORK),
        _check("status_doc_mentions_evidence_gap_closure_plan", AUTHORIZED_CURRENT_WORK in status_text, AUTHORIZED_CURRENT_WORK, "present"),
    ]


def _source_summary(source_static_review: dict[str, Any]) -> dict[str, Any]:
    decision = _dict(source_static_review.get("final_decision"))
    summary = _dict(source_static_review.get("source_review_summary"))
    return {
        ...
        "source_review_item_count": summary.get("review_item_count"),
        "source_evidence_gap_count": summary.get("evidence_gap_count"),
    }
...
def _decision(passed: bool, checks: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if passed:
        failure_class = None
    elif "evidence_gap_closure_plan_enabled" in failed:
        failure_class = "explicit_uncertainty_coverage_evidence_gap_closure_plan_authorization_missing"
    elif any(name.startswith(("audit_", "status_doc_")) for name in failed):
        failure_class = "v14_eof_contract_mismatch"
    elif any(name.startswith(("source_", "source_review_summary")) for name in failed):
        failure_class = "source_uncertainty_coverage_static_review_contract_failure"
    ...
    decision: dict[str, Any] = {
        ...
        "post_closeout_promotion_readiness_uncertainty_coverage_evidence_gap_closure_plan_ready": passed,
        "uncertainty_coverage_evidence_gap_closure_plan_static_review_authorized": passed,
        "direct_promotion_recommendation": False,
        "promotion_decision_plan_authorized_next": False,
        "score_expression": SCORE_EXPRESSION,
        "recommendation": "static_review_this_evidence_gap_closure_plan_only" if passed else "repair_contract_before_rerun",
        "immediate_action": "static_review_evidence_gap_closure_plan_only" if passed else "inspect_failed_checks",
    }
    for action in BLOCKED_ACTIONS:
