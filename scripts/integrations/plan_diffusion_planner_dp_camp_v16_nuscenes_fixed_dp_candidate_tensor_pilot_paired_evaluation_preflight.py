#!/usr/bin/env python3
"""Plan the v16 fixed-DP pilot paired-evaluation preflight."""
...
READY_STATUS = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_paired_evaluation_preflight_plan_ready"
REJECT_STATUS = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_paired_evaluation_preflight_plan_rejected"
AUTHORIZED_NEXT_WORK = (
    "v16_nuscenes_fixed_dp_candidate_tensor_pilot_paired_evaluation_preflight_plan_static_review_only"
)
SCHEMA_VERSION = "dp_camp_v16_nuscenes_fixed_dp_candidate_tensor_pilot_paired_evaluation_preflight_plan_v1"
PLAN_JSON_NAME = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_paired_evaluation_preflight_plan.json"
PLAN_MD_NAME = "v16_nuscenes_fixed_dp_candidate_tensor_pilot_paired_evaluation_preflight_plan.md"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v16_nuscenes_fixed_dp_candidate_tensor_pilot_paired_evaluation_preflight_plan",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        enabled=args.enable_v16_nuscenes_fixed_dp_candidate_tensor_pilot_paired_evaluation_preflight_plan,
    )
...
def build_report(
    *,
    ...
    checks = [
        _expect("pilot_paired_evaluation_preflight_plan_enabled", enabled, True),
        ...
        _expect("train_records_863", review.get("train_records"), EXPECTED_COUNTS["train"]),
        _expect("calibration_records_14", review.get("calibration_records"), EXPECTED_COUNTS["calibration"]),
        _expect("holdout_records_147", review.get("holdout_records"), EXPECTED_COUNTS["holdout"]),
        ...
        _expect("plan_primary_eval_total", plan["paired_rows_by_split"]["primary_eval_total"], 161),
        _expect("plan_comparison_baseline", plan["comparison"]["baseline"], "dp_top1"),
        _expect("plan_smoke_only", plan["pilot_eval_smoke_only"], True),
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- Smoke-only: `{plan['pilot_eval_smoke_only']}`",
            "",
        ]
    )
