#!/usr/bin/env python3
"""Static review for the actual-SafetyCost outcome-materialization preflight plan."""
...
def _load_plan_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "v14_post_closeout_promotion_evidence_acquisition_actual_safetycost_outcome_materialization_preflight_plan",
        plan_path,
    )
...
HELPER_MODULE = PLAN_MODULE.HELPER_MODULE
...
SCHEMA_VERSION = (
    ...
    "actual_safetycost_outcome_materialization_preflight_plan_static_review_v1"
)
...
READY_STATUS = (
    ...
    "actual_safetycost_outcome_materialization_preflight_plan_static_review_passed"
)
REJECT_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_paired_evaluation_"
    "actual_safetycost_outcome_materialization_preflight_plan_static_review_rejected"
)
...
REVIEW_JSON_NAME = (
    "post_closeout_promotion_evidence_acquisition_paired_evaluation_"
    "actual_safetycost_outcome_materialization_preflight_plan_static_review.json"
)
REVIEW_MD_NAME = (
    "post_closeout_promotion_evidence_acquisition_paired_evaluation_"
    "actual_safetycost_outcome_materialization_preflight_plan_static_review.md"
)


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--outcome_materialization_preflight_plan_artifact_dir", type=Path, required=True)
    parser.add_argument("--outcome_materialization_preflight_plan_json", type=Path, required=True)
    parser.add_argument("--outcome_materialization_preflight_plan_md", type=Path, required=True)
    parser.add_argument("--outcome_materialization_preflight_plan_sha256s", type=Path, required=True)
    parser.add_argument("--outcome_materialization_preflight_plan_script_py", type=Path, required=True)
    parser.add_argument("--outcome_materialization_preflight_plan_test_py", type=Path, required=True)
    ...
    parser.add_argument(
        "--enable_v14_post_closeout_promotion_evidence_acquisition_actual_safetycost_outcome_materialization_preflight_plan_static_review",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        outcome_materialization_preflight_plan_artifact_dir=args.outcome_materialization_preflight_plan_artifact_dir,
        outcome_materialization_preflight_plan_json=args.outcome_materialization_preflight_plan_json,
        outcome_materialization_preflight_plan_md=args.outcome_materialization_preflight_plan_md,
        outcome_materialization_preflight_plan_sha256s=args.outcome_materialization_preflight_plan_sha256s,
        outcome_materialization_preflight_plan_script_py=args.outcome_materialization_preflight_plan_script_py,
        outcome_materialization_preflight_plan_test_py=args.outcome_materialization_preflight_plan_test_py,
        ...
        enabled=(
            args.enable_v14_post_closeout_promotion_evidence_acquisition_actual_safetycost_outcome_materialization_preflight_plan_static_review
        ),
    )
...
def build_report(
    *,
    outcome_materialization_preflight_plan_artifact_dir: Path,
    outcome_materialization_preflight_plan_json: Path,
    outcome_materialization_preflight_plan_md: Path,
    outcome_materialization_preflight_plan_sha256s: Path,
    outcome_materialization_preflight_plan_script_py: Path,
    outcome_materialization_preflight_plan_test_py: Path,
    v14_audit_md: Path,
    ...
    artifact_dir = outcome_materialization_preflight_plan_artifact_dir.resolve()
    plan_json = outcome_materialization_preflight_plan_json.resolve()
    plan_md = outcome_materialization_preflight_plan_md.resolve()
    plan_sha256s = outcome_materialization_preflight_plan_sha256s.resolve()
    script_py = outcome_materialization_preflight_plan_script_py.resolve()
    test_py = outcome_materialization_preflight_plan_test_py.resolve()
    ...
    return {
        ...
        "analysis": {
            ...
            "read_only": True,
            "actual_safetycost_outcome_materialization_preflight_plan_static_review_only": True,
        ...
        "inputs": {
            "outcome_materialization_preflight_plan_artifact_dir": str(artifact_dir),
            "outcome_materialization_preflight_plan_json": str(plan_json),
            "outcome_materialization_preflight_plan_md": str(plan_md),
            "outcome_materialization_preflight_plan_sha256s": str(plan_sha256s),
            "outcome_materialization_preflight_plan_script_py": str(script_py),
            "outcome_materialization_preflight_plan_test_py": str(test_py),
        ...
        "preflight_scope_summary": source_plan.get("outcome_materialization_preflight_summary", {}),
...
def _checks(
    *,
    ...
    expect("source_plan_ready_flag", decision.get("post_closeout_promotion_evidence_acquisition_paired_evaluation_actual_safetycost_outcome_materialization_preflight_plan_ready"), True)
    expect("source_plan_static_review_authorized", decision.get("post_closeout_promotion_evidence_acquisition_paired_evaluation_actual_safetycost_outcome_materialization_preflight_plan_static_review_authorized"), True)
    ...
    expect("source_plan_no_closed_loop_training_input", decision.get("closed_loop_outcome_training_or_online_input_authorized"), False)
    ...
    expect("source_plan_check_count", len(_list(source_plan.get("preflight_plan_checks"))), 74)
    expect("source_plan_failed_check_count", len(_list(decision.get("failed_checks"))), 0)
    ...
    expect("source_preflight_plan_names", [item.get("name") for item in _list(source_plan.get("preflight_plan"))], list(PLAN_MODULE.EXPECTED_PREFLIGHT_ITEMS))
    expect("source_preflight_plan_no_materialization", sorted({item.get("materializes_outcomes") for item in _list(source_plan.get("preflight_plan"))}), [False])
    ...
    static_summary = HELPER_MODULE._dict(source_plan.get("source_static_review_summary"))
    expect("source_static_review_check_count", static_summary.get("static_review_check_count"), 82)
    expect("source_static_review_failed_count", static_summary.get("failed_check_count"), 0)
    expect("source_static_review_plan_check_count", static_summary.get("source_plan_check_count"), 71)
    expect("source_static_review_paired_record_count", static_summary.get("source_paired_record_count"), 3200)
    expect("source_static_review_shadow_diff_records", static_summary.get("source_shadow_diff_records"), 2832)
    scope = HELPER_MODULE._dict(source_plan.get("outcome_materialization_preflight_summary"))
...
def _source_surface_checks(script_text: str, test_text: str) -> list[dict[str, Any]]:
    expected_script_tokens = [
        ...
        "closed_loop_outcome_training_or_online_input_authorized",
        "materializes_outcomes",
        "score_expression",
    ]
    expected_test_tokens = [
        "test_actual_safetycost_outcome_materialization_preflight_plan_passes",
        "test_actual_safetycost_outcome_materialization_preflight_plan_rejects_source_claim_leak",
        "test_actual_safetycost_outcome_materialization_preflight_plan_rejects_hash_drift",
    ]
    return [
        {
...
def _decision(*, passed: bool, checks: list[dict[str, Any]], source_plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if passed:
        ...
        failure_class = "explicit_actual_safetycost_outcome_materialization_preflight_plan_static_review_authorization_missing"
    ...
    decision = {
        ...
        "post_closeout_promotion_evidence_acquisition_paired_evaluation_actual_safetycost_outcome_materialization_preflight_plan_static_review_passed": bool(passed),
        "actual_safetycost_outcome_materialization_preflight_authorized": bool(passed),
        ...
        "recommendation": "execute_actual_safetycost_outcome_materialization_preflight_only" if passed else "repair_or_rerun_same_static_review_gate",
        "score_expression": SCORE_EXPRESSION,
    }
    for action in BLOCKED_ACTIONS:
...
def _source_plan_summary(source_plan: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "preflight_plan_check_count": len(_list(source_plan.get("preflight_plan_checks"))),
        ...
        "source_static_review_check_count": static_summary.get("static_review_check_count"),
        "source_paired_record_count": static_summary.get("source_paired_record_count"),
        "source_shadow_diff_records": static_summary.get("source_shadow_diff_records"),
    }
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# v14 Actual-SafetyCost Outcome-Materialization Preflight Plan Static Review",
        "",
        ...
        "## Source Plan",
        "",
        f"- Preflight-plan checks: `{summary['preflight_plan_check_count']}`",
        f"- Failed checks: `{summary['failed_check_count']}`",
        f"- Required inputs / preflight items / planned outputs / no-go entries: `{summary['required_input_count']} / {summary['preflight_plan_count']} / {summary['planned_output_count']} / {summary['no_go_count']}`",
