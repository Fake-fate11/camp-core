...
SUMMARY_FIELD = "camp_lane_hard_violation_support_logging"
LATENCY_FIELDS = LANE_HARD_VIOLATION_SUPPORT_LATENCY_KEYS
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Audit a paired default-off lane/hard-violation support logging "
            "smoke. The baseline run must keep logging disabled and the "
            "candidate run must enable logging without collecting future outcomes."
        )
    )
...
def analyze(
    *,
    ...
    for key in sorted(set(baseline_logs) & set(candidate_logs)):
        ...
        for idx, (baseline, candidate) in enumerate(zip(baseline_rows, candidate_rows)):
            if not isinstance(baseline, dict) or not isinstance(candidate, dict):
                ...
                continue
            baseline_payload = baseline.get(LOGGED_FIELD)
            candidate_payload = candidate.get(LOGGED_FIELD)
    ...
    return {
        "analysis": {
            "name": "dp_camp_lane_hard_violation_support_logging_smoke_audit_v1",
            ...
            "math_boundary": (
                "Lane/hard-violation support payloads are current-tick "
                "finite-candidate route geometry diagnostics. Logged atoms are "
                "fixed nonnegative candidate coefficients and preserve affine "
                "CAMP scores in weights."
            ),
        },
        "counts": {
        ...
        "final_decision": {
            "status": (
                ...
                if not errors
                else "lane_hard_violation_support_logging_smoke_rejected"
            ),
            "passed": not errors,
            "authorized_next_work": (
                "lane_hard_violation_support_logging_smoke_result_documentation_only"
                if not errors
                else None
            ),
...
def _validate_summary(
    ...
    *,
    ...
    metadata = summary.get(SUMMARY_FIELD)
    if not isinstance(metadata, dict):
        errors.append(f"{key}: missing {SUMMARY_FIELD} summary")
        return
    expected = {
        "schema_version": LANE_HARD_VIOLATION_SUPPORT_LOGGING_SCHEMA_VERSION,
    ...
    if expected_enabled and int(metadata.get("records", -1)) <= 0:
        errors.append(f"{key}: enabled summary has no lane/hard support records")
    if expected_enabled and metadata.get("fields") != list(
        LANE_HARD_VIOLATION_SUPPORT_FIELD_NAMES
    ):
        errors.append(f"{key}: summary field list mismatch")
    if expected_enabled and metadata.get("atom_names") != list(
        LANE_HARD_VIOLATION_SUPPORT_ATOM_NAMES
    ):
...
def _validate_payload(
    ...
    *,
    ...
    if not isinstance(finite_checks, dict):
        ...
        missing = sorted(
            set(LANE_HARD_VIOLATION_SUPPORT_FIELD_NAMES) - set(finite_checks)
        )
        if missing:
    ...
    if payload.get("lane_hard_violation_support_atom_names") != list(
        LANE_HARD_VIOLATION_SUPPORT_ATOM_NAMES
    ):
        errors.append(f"{prefix}: lane_hard_violation_support_atom_names mismatch")
    atoms = np.asarray(
    ...
    if atoms.shape != (
        ...
        errors.append(
            f"{prefix}: lane_hard_violation_support_atoms shape={list(atoms.shape)}"
        )
    elif not np.all(np.isfinite(atoms)):
        errors.append(f"{prefix}: lane_hard_violation_support_atoms not finite")
    elif not np.all(atoms >= -1e-12):
        errors.append(f"{prefix}: lane_hard_violation_support_atoms has negative values")
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Lane/Hard-Violation Support Logging Smoke Audit",
        "",
