...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Fail-closed audit for matched DP-CAMP lane/hard-violation support "
            "payloads and candidate outcome labels collected in the same "
            "nonformal replay records. This is read-only and does not run "
            "Diffusion Planner."
        )
    )
...
def analyze(
    ...
    *,
    ...
    candidate_rows = 0
    max_latency_ms = {key: 0.0 for key in LANE_HARD_VIOLATION_SUPPORT_LATENCY_KEYS}
    ...
    for log_path in log_paths:
        ...
        for record_index, raw in enumerate(payload):
            ...
            lane_hard_ok = _validate_lane_hard_payload(
                raw,
            ...
            lane_hard_records += int(lane_hard_ok)
            outcome_records += int(outcome_ok)
            log_lane_hard_records += int(lane_hard_ok)
            log_outcome_records += int(outcome_ok)
            if lane_hard_ok and outcome_ok:
                candidate_rows += int(expected_candidates)
        per_log.append(
            {
                "selection_log": str(log_path),
                "records": len(payload),
                "lane_hard_support_records": log_lane_hard_records,
    ...
    if records_total and lane_hard_records != records_total:
        errors.append(
            f"lane_hard_support_records={lane_hard_records} "
    ...
    return {
        "analysis": {
            "name": "dp_camp_matched_lane_hard_violation_support_outcome_contract_v1",
            "label": label,
            "training": False,
            ...
            "math_boundary": (
                "Lane/hard-violation support descriptors are current-tick "
                ...
                "affine score_k(w)=a_k^T w over fixed atoms, preserving the "
        ...
        "counts": {
            ...
            "lane_hard_support_records": lane_hard_records,
        ...
        "final_decision": {
            "status": (
                ...
                if passed
                else "matched_lane_hard_violation_support_outcome_contract_rejected"
            ),
            "passed": passed,
            "authorized_next_work": (
                "offline_lane_hard_violation_support_descriptor_separability_screen_only"
                if passed
                else "fix_matched_lane_hard_violation_support_label_collection_before_separability"
            ),
...
def _validate_lane_hard_payload(
    record: dict[str, Any],
    *,
    record_prefix: str,
    ...
    payload = record.get("lane_hard_violation_support_logging")
    if not isinstance(payload, dict):
        errors.append(f"{record_prefix}: lane_hard_violation_support_logging missing")
        return False
    for field, expected in EXPECTED_LANE_HARD_FLAGS.items():
        if payload.get(field) != expected:
            errors.append(
                f"{record_prefix}: lane_hard_violation_support_logging.{field}="
                f"{payload.get(field)!r}"
            )
    if "candidate_closed_loop_outcomes" in payload:
        errors.append(
            f"{record_prefix}: lane/hard payload contains candidate outcomes"
        )
    ...
    if candidate_count != int(expected_candidates):
        errors.append(
            f"{record_prefix}: lane/hard candidate_count={candidate_count} "
    ...
    if not isinstance(finite_checks, dict) or not finite_checks:
        errors.append(f"{record_prefix}: lane/hard finite_checks missing")
    else:
        for field in (
            *LANE_HARD_VIOLATION_SUPPORT_FIELD_NAMES,
    ...
    for field in LANE_HARD_VIOLATION_SUPPORT_FIELD_NAMES:
        if field not in payload:
            errors.append(f"{record_prefix}: payload missing {field}")
        elif not _finite_nested(payload[field]):
            errors.append(f"{record_prefix}: payload field {field} is not finite")

    expected_atom_names = list(LANE_HARD_VIOLATION_SUPPORT_ATOM_NAMES)
    if payload.get("lane_hard_violation_support_atom_names") != expected_atom_names:
        errors.append(f"{record_prefix}: lane_hard_violation_support_atom_names mismatch")
    atoms = np.asarray(payload.get("lane_hard_violation_support_atoms"), dtype=np.float64)
    if atoms.shape != (int(expected_candidates), len(expected_atom_names)):
        errors.append(
            f"{record_prefix}: lane_hard_violation_support_atoms shape={list(atoms.shape)}"
        )
    elif not np.all(np.isfinite(atoms)):
        errors.append(f"{record_prefix}: lane_hard_violation_support_atoms not finite")
    elif not np.all(atoms >= -1e-12):
        errors.append(
            f"{record_prefix}: lane_hard_violation_support_atoms has negative values"
        )
        ...
        for key in LANE_HARD_VIOLATION_SUPPORT_LATENCY_KEYS:
            ...
            if record_value is not None and value is not None:
                if abs(record_value - value) > 1e-9:
                    errors.append(
                        f"{record_prefix}: record/payload latency mismatch {key}"
                    )
...
def render_markdown(report: dict[str, Any]) -> str:
    lines = [
        "# DP CAMP Matched Lane/Hard-Violation Support Outcome Contract Audit",
        "",
        "This read-only audit requires every checked selection-log record to "
        "carry both a no-leak lane/hard-violation support payload and offline "
        "candidate outcome labels in the same fixed candidate ordering.",
        "",
        "## Counts",
        "",
        "```json",
