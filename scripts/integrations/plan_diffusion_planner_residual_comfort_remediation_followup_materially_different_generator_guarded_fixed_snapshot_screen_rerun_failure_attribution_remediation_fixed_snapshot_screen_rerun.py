...
DEFAULT_POST_REVIEW_ROOT = (
    "/root/autodl-tmp/"
    "camp_dp_material_generator_failure_attribution_remediation_post_impl_static_review_bff8f8b"
)
DEFAULT_EXECUTION_ROOT = (
    "/root/autodl-tmp/"
    "camp_dp_material_generator_failure_attribution_remediation_fixed_snapshot_screen_rerun_bff8f8b"
)
...
POST_REVIEW_JSON = "post_implementation_static_contract_review.json"
POST_REVIEW_MD = "post_implementation_static_contract_review.md"
...
GUARD_ENV_VAR = "CAMP_MATERIAL_GENERATOR_REMEDIATION_FIXED_SNAPSHOT_RERUN_APPROVED"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Plan-only guarded fixed-snapshot screen rerun for the material "
            "generator failure-attribution remediation v2."
        )
    )
    parser.add_argument(
        "--post_review_root",
    ...
    return {
        "analysis": {
            "name": (
                ...
                "fixed_snapshot_screen_rerun_plan_v1"
            ),
            "label": label,
            ...
            "math_boundary": (
                ...
                "run replay, use formal seeds, define or promote runtime atoms, "
                "choose lambda online, alter score_k(w)=a_k^T w, mutate the "
                "convex simplex/CVaR/L2 master, train CAMP, change online "
                "selection, modify DP weights/code/config, claim safety benefit, "
                "claim CAMP over DP Top-1, or claim a DP-side classical Benders "
                "decomposition."
            ),
        },
        "head_audit": {
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Material Generator Remediation Fixed-Snapshot Screen Rerun Plan",
        "",
...
"${{PY}}" scripts/integrations/analyze_diffusion_planner_route_topology_candidate_screen.py \\
  ...
  --label material_generator_remediation_fixed_snapshot_screen \\
...
cd "${{OUTPUT_ROOT}}"
sha256sum \\
  material_generator_remediation_fixed_snapshot_screen.json \\
  material_generator_remediation_fixed_snapshot_screen.md \\
...
def _screen_rerun_plan(execution_root: Path) -> dict[str, Any]:
    ...
    return {
        "selection_type": (
            "material_generator_remediation_fixed_snapshot_screen_rerun_plan_only"
        ),
        ...
        "execution_artifacts": {
            "output_root": root,
            "screen_json": (
                f"{root}/material_generator_remediation_fixed_snapshot_screen.json"
            ),
            "screen_md": (
                f"{root}/material_generator_remediation_fixed_snapshot_screen.md"
            ),
            "heads": f"{root}/HEADS.txt",
            "sha256sums": f"{root}/SHA256SUMS",
            "runbook": (
                f"{root}/material_generator_remediation_fixed_snapshot_screen_runbook.sh"
            ),
        },
        "accept_criteria": [
            ...
            "future evidence must report hard/precheck, progress, comfort, and material descriptor coverage",
...
def _post_review_summary(payload: dict[str, Any], markdown_text: str) -> dict[str, Any]:
    ...
    return {
        ...
        "plan_authorized": bool(
            decision.get("remediation_fixed_snapshot_screen_rerun_plan_authorized")
        ),
...
def _source_checks(source: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        ...
        _check("post_review_markdown_records_authorized_next_work", source["markdown_records_authorized_next_work"]),
    ]
...
def _plan_checks(plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    config = plan["candidate_config"]
    return [
        _check("plan_selection_type", plan["selection_type"] == "material_generator_remediation_fixed_snapshot_screen_rerun_plan_only"),
        ...
        _check("plan_uses_v2_policy", config["generator_policy"] == PLANNED_POLICY),
        _check("plan_uses_v2_profile", config["default_off_remediation_profile"] == REMEDIATION_PROFILE),
...
def _runbook_checks(runbook_text: str) -> list[dict[str, Any]]:
    ...
    return [
        ...
        _check("runbook_uses_v2_policy", f"--generator_policy {PLANNED_POLICY}" in runbook_text),
        _check("runbook_uses_v2_profile", f"--default_off_remediation_profile {REMEDIATION_PROFILE}" in runbook_text),
        _check("runbook_pins_candidate_cap", f"--max_remediation_candidates {DEFAULT_MAX_REMEDIATION_CANDIDATES}" in runbook_text),
        _check("runbook_has_prefix_bridge", "--prefix_step 3" in runbook_text and "--bridge_step 10" in runbook_text),
