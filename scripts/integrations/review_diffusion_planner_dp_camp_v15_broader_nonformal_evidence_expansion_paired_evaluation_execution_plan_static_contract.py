#!/usr/bin/env python3
"""Static-review the v15 paired-evaluation execution plan."""
...
def _load_plan_module():
    path = Path(__file__).resolve().with_name(
        "plan_diffusion_planner_dp_camp_v15_broader_nonformal_evidence_expansion_paired_evaluation_execution.py"
    )
    spec = importlib.util.spec_from_file_location("v15_paired_evaluation_execution_plan", path)
...
SCHEMA_VERSION = "dp_camp_v15_broader_nonformal_evidence_expansion_paired_evaluation_execution_plan_static_review_v1"
AUTHORIZED_CURRENT_WORK = PLAN_MODULE.AUTHORIZED_NEXT_WORK
READY_STATUS = "v15_broader_nonformal_evidence_expansion_paired_evaluation_execution_plan_static_review_passed"
REJECT_STATUS = "v15_broader_nonformal_evidence_expansion_paired_evaluation_execution_plan_static_review_rejected"
AUTHORIZED_NEXT_WORK = "v15_broader_nonformal_evidence_expansion_paired_evaluation_execution_preflight_only"
REVIEW_JSON_NAME = "v15_broader_nonformal_evidence_expansion_paired_evaluation_execution_plan_static_review.json"
REVIEW_MD_NAME = "v15_broader_nonformal_evidence_expansion_paired_evaluation_execution_plan_static_review.md"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v15_broader_nonformal_evidence_expansion_paired_evaluation_execution_plan_static_review",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        enabled=args.enable_v15_broader_nonformal_evidence_expansion_paired_evaluation_execution_plan_static_review,
    )
...
def build_report(
    *,
    ...
    plan = source["paired_evaluation_execution_plan"]
    ...
    checks = [
        ...
        _expect("source_plan_not_executed", decision.get("paired_evaluation_execution_plan_executed"), False),
        ...
        _expect("plan_online_latency_fields", tuple(plan["timing_contract"]["online_selector_latency_required_fields"]), PLAN_MODULE.PLAN_MODULE.LATENCY_FIELDS),
        _expect("plan_fallback_latency_fields", tuple(plan["timing_contract"]["fallback_latency_required_fields"]), PLAN_MODULE.PLAN_MODULE.LATENCY_FIELDS),
        ...
        _expect("plan_train_split_for_evaluation", plan["blocked_inputs"]["train_split_for_evaluation"], False),
        ...
        _check("planned_command_is_execution", "execute_diffusion_planner_dp_camp_v15" in plan["planned_command"], plan["planned_command"], "execute script"),
    ]
    ...
    return _stable(
        {
            ...
            "final_decision": {
                ...
                "reviewed_paired_evaluation_execution_plan": True,
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# V15 Paired Evaluation Execution Plan Static Review",
            "",
