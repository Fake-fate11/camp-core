...
READY_STATUS = (
    ...
    "default_off_remediation_fixed_snapshot_screen_rerun_failure_attribution_"
    "plan_ready"
)
REJECT_STATUS = (
    "candidate_set_consensus_lane_projected_jerk_progress_support_"
...
SCREEN_MD = "route_topology_default_off_remediation_rerun_screen.md"
ABSOLUTE_JSON = "route_topology_default_off_remediation_absolute_lateral_guard.json"
ABSOLUTE_MD = "route_topology_default_off_remediation_absolute_lateral_guard.md"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Plan-only failure attribution for the default-off remediation "
            "fixed-snapshot screen rerun result. It reads only existing "
            "screen/guard artifacts and authorizes only read-only analysis."
        )
    )
...
def build_report(
    *,
    ...
    plan = _failure_attribution_plan(screen, absolute)
    checks = [
    ...
    return {
        "analysis": {
            "name": (
                ...
                "failure_attribution_plan_v1"
            ),
            "label": label,
            "role": "plan-only read-only attribution for rerun failures",
            "plan_only": True,
            "read_only": True,
            ...
            "math_boundary": (
                "This plan reads only existing fixed-snapshot screen artifacts "
                "and fixed-head audit. It does not generate candidates, rerun "
                "the screen, run DP, run replay, recompute outcomes, define "
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    absolute = report["absolute_summary"]
    lines = [
        "# Default-Off Remediation Fixed-Snapshot Rerun Failure Attribution Plan",
        "",
        ...
        "## Evidence Summary",
        "",
        f"- Screen status: `{screen['status']}`",
        f"- Generated rows: `{screen['generated_candidate_rows']}`",
        f"- Hard support rate: `{screen['hard_support_rate']}`",
        f"- Comfort support rate: `{screen['comfort_support_rate']}`",
        f"- Candidate-build p95 ms: `{screen['candidate_build_p95_ms']}`",
        f"- Total p95 ms: `{screen['total_p95_ms']}`",
        f"- Absolute guard status: `{absolute['status']}`",
        f"- Absolute guard support rate: `{absolute['absolute_support_rate']}`",
        "",
...
def _failure_attribution_plan(
    ...
    return {
        ...
        "required_output_contract": [
            ...
            "preserve no-promotion and no-safety-claim boundaries",
            "do not create new candidate trajectories, rerun the screen, run replay, train CAMP, or modify DP",
        ],
        "accept_criteria_for_diagnosis": [
            ...
            "diagnosis plan includes comfort, hard, latency, absolute-guard, and overlap axes",
            "diagnosis plan preserves no-promotion and no-replay decisions",
        ],
        "blocked_boundaries": [
...
def _evidence_checks(
    ...
    return [
        ...
        _check_equal(
            ...
            all(
                key in hard_reasons
                for key in ("dp_kinematic", "dp_red_light", "dp_road_border")
            ),
            True,
        ),
        _check_equal(
...
def _plan_checks(plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        _check_equal("plan_mentions_overlap", "overlap" in text, True),
