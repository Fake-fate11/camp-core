...
READY_STATUS = "progress_support_matched_outcome_label_pass_plan_ready"
REJECT_STATUS = "progress_support_matched_outcome_label_pass_plan_rejected"
SOURCE_STATUS = "progress_support_broader_nonformal_smoke_passed"
AUTHORIZED_NEXT_WORK = "progress_support_matched_outcome_label_nonformal_smoke_only"
MIN_SOURCE_RECORDS = 48
MAX_SOURCE_LOGGING_MS = 25.0


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Design-only gate for a nonformal matched progress-support plus "
    ...
    *,
    ...
    if spec is None:
        spec = replace(
            ...
            root=(
                "/root/autodl-tmp/"
                "camp_dp_progress_support_matched_outcome_labels_nonformal_v1"
            ),
        )
    source_checks = [
    ...
    return {
        "analysis": {
            "name": "dp_camp_progress_support_matched_outcome_label_pass_plan_v1",
            "label": label,
        ...
        "final_decision": {
            ...
            "paired_smoke_execution_scope": (
                ...
                "branch collects progress_support_logging and "
...
def _source_artifact_checks(
    *,
    ...
    return [
        {
            "name": "broader_progress_support_smoke_passed",
        ...
        {
            "name": "broader_progress_support_records_material",
        ...
        {
            "name": "broader_progress_support_latency_within_budget",
...
def _source_text_checks(
    *,
    ...
    matched_progress_text = _read_text(matched_progress_contract_audit_source)
    return [
        _check_tokens(
            "replay_supports_progress_support_and_outcome_labels",
            replay_text,
            (
        ...
        _check_order(
            "replay_computes_progress_support_payload_before_outcomes",
            replay_text,
            "build_progress_support_logging_payload(",
...
def _commands(spec: BroaderSmokeSpec) -> dict[str, Any]:
    ...
    matched_root = f"{spec.root}/matched_progress_support_outcomes"
    ...
    for run in spec.runs:
        ...
        replays.append(
            {
                "run_id": run.run_id,
                "variant": "matched_progress_support_outcomes",
                "command": _replay_command(
                    spec,
                    run,
...
def _matched_progress_contract_command(
    ...
    return [
        ...
        str(spec.num_candidates),
        "--label",
        "progress_support_matched_outcome_labels_nonformal_v1",
        "--output_json",
...
def _accept_criteria(spec: BroaderSmokeSpec) -> list[str]:
    return [
        ...
        "matched records contain progress_support_logging payloads and candidate_closed_loop_outcomes in the same record",
        "progress-support payloads report selection_effect=false and future_outcome_leakage=false",
        "progress-support payloads do not embed candidate_closed_loop_outcomes",
        ...
        "matched progress-support contract audit passes for all records and candidates",
...
def _reject_criteria() -> list[str]:
    return [
        ...
        "any matched record lacks progress_support_logging or candidate_closed_loop_outcomes",
        "any selected index, feasibility mask, atom, score, or weight changes under logging/outcome collection",
        "any runtime progress-support descriptor uses closed-loop outcome labels or reports future_outcome_leakage=true",
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Progress-Support Matched Outcome Label Pass Plan",
        "",
