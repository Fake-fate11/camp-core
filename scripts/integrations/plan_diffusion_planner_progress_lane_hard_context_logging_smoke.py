...
PAYLOAD_AUDIT = (
    ...
    "analyze_diffusion_planner_progress_lane_hard_context_logging_smoke.py"
)
...
READY_STATUS = "progress_lane_hard_context_logging_nonformal_smoke_plan_ready"
REJECT_STATUS = "progress_lane_hard_context_logging_nonformal_smoke_plan_rejected"
SOURCE_STATUS = "progress_lane_hard_context_logging_unit_wired"
AUTHORIZED_NEXT_WORK = (
    "default_off_progress_lane_hard_context_logging_paired_three_step_smoke_only"
)
...
class SmokeSpec:
    root: str = "/root/autodl-tmp/camp_dp_progress_lane_hard_context_logging_smoke"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Design-only preflight for the first default-off progress+lane/hard "
            "context logging smoke. It emits paired nonformal replay commands "
...
def build_report(
    *,
    ...
    return {
        "analysis": {
            "name": "dp_camp_progress_lane_hard_context_logging_nonformal_smoke_plan_v1",
            "label": label,
            ...
            "math_boundary": (
                ...
                "fixed finite-candidate progress+lane/hard context descriptors "
                "and nonnegative candidate atom coefficients. It must not "
                "change CAMP scores, feasibility, selected indices, DP "
                "candidates, or PerfectTracker execution. If these atoms later "
                "enter CAMP, they are fixed coefficients a_k preserving affine "
...
def _source_checks(
    *,
    ...
    return [
        _check_tokens(
            "replay_default_off_context_logging_cli",
            replay_text,
            (
        ...
        _check_tokens(
            ...
            payload_text,
            (
                "dp_camp_progress_lane_hard_context_logging_smoke_audit_v1",
                "progress_lane_hard_context_logging",
...
def _plan_checks(smoke: SmokeSpec) -> list[dict[str, Any]]:
    return [
        ...
        {
            "name": "context_horizon_valid",
            "passed": smoke.context_steps >= 2,
            "value": smoke.context_steps,
        },
        {
            "name": "context_dt_valid",
            "passed": smoke.context_dt_s > 0.0,
            "value": smoke.context_dt_s,
        },
        {
            "name": "context_corridor_budget_valid",
            "passed": smoke.corridor_half_width_m > 0.0
            and smoke.corridor_safety_margin_m >= 0.0,
            "value": {
                "corridor_half_width_m": smoke.corridor_half_width_m,
                "corridor_safety_margin_m": smoke.corridor_safety_margin_m,
            },
        },
    ]
    ...
    if logging:
        command.extend(
            [
                ...
                str(smoke.context_steps),
                "--camp_progress_lane_hard_context_dt_s",
                str(smoke.context_dt_s),
                ...
                str(smoke.corridor_safety_margin_m),
            ]
        )
    return command
...
def _payload_audit_command(
    ...
    return [
        ...
        str(smoke.num_candidates),
        "--output_json",
        f"{audit_dir}/progress_lane_hard_context_logging_smoke.json",
        "--output_md",
        f"{audit_dir}/progress_lane_hard_context_logging_smoke.md",
        "--require_pass",
    ]
...
def _accept_criteria(smoke: SmokeSpec) -> list[str]:
    return [
        ...
        "progress_lane_hard_context_atoms are finite and nonnegative for all candidates",
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Progress+Lane/Hard Context Logging Smoke Plan",
        "",
