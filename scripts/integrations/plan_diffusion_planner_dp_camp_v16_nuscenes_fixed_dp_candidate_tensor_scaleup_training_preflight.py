#!/usr/bin/env python3
"""Plan the v16 fixed-DP scale-up candidate-tensor training preflight."""
...
EXPECTED_RECORD_COUNTS = SOURCE_REVIEW_MODULE.EXPECTED_RECORD_COUNTS
EXPECTED_SCENE_COUNTS = SOURCE_REVIEW_MODULE.EXPECTED_SCENE_COUNTS
EXPECTED_RECORDS = SOURCE_REVIEW_MODULE.EXPECTED_RECORDS
EXPECTED_SCENES = SOURCE_REVIEW_MODULE.EXPECTED_SCENES
...
READY_STATUS = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_preflight_plan_ready"
REJECT_STATUS = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_preflight_plan_rejected"
AUTHORIZED_NEXT_WORK = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_preflight_plan_static_review_only"
SCHEMA_VERSION = "dp_camp_v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_preflight_plan_v1"
PLAN_JSON_NAME = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_preflight_plan.json"
PLAN_MD_NAME = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_preflight_plan.md"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_preflight_plan",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        enabled=args.enable_v16_nuscenes_fixed_dp_candidate_tensor_scaleup_training_preflight_plan,
    )
...
def build_report(
    *,
    ...
    checks = [
        _expect("scaleup_training_preflight_plan_enabled", enabled, True),
        ...
        _expect("review_records_10000", split_summary.get("records") or split_summary.get("total_records"), EXPECTED_RECORDS),
        _expect("review_scenes_50", split_summary.get("scenes"), EXPECTED_SCENES),
        _expect("review_scene_counts", _counts(split_summary, "scene_counts"), EXPECTED_SCENE_COUNTS),
        _expect("review_record_counts", _counts(split_summary, "record_counts"), EXPECTED_RECORD_COUNTS),
        ...
        _expect("review_record_level_hard_split_not_executed", split_summary.get("record_level_hard_split_executed"), False),
        ...
        _expect("review_unassigned_record_count_zero", split_summary.get("unassigned_record_count"), 0),
        _expect("train_records_6263", record_checks["counts"]["train"], EXPECTED_RECORD_COUNTS["train"]),
        _expect("calibration_records_2156", record_checks["counts"]["calibration"], EXPECTED_RECORD_COUNTS["calibration"]),
        _expect("holdout_records_1581", record_checks["counts"]["holdout"], EXPECTED_RECORD_COUNTS["holdout"]),
        _expect("scene_zero_overlap", record_checks["scene_zero_overlap"], True),
        _expect("sample_zero_overlap", record_checks["sample_zero_overlap"], True),
        ...
        _expect("nonnegative_simplex", plan["math_contract"]["nonnegative_simplex"], True),
        _expect("approved_atoms_only", plan["math_contract"]["approved_atoms_only"], True),
        _expect(
            "no_closed_loop_outcomes_as_training_input",
            plan["math_contract"]["no_closed_loop_outcomes_as_training_input"],
            True,
        ),
        _expect("no_candidate_tensor_mutation", plan["math_contract"]["no_candidate_tensor_mutation"], True),
        _expect("no_dp_modification", plan["math_contract"]["no_dp_modification"], True),
        _expect("scaleup_corpus_artifact", plan["training_inputs"]["scaleup_corpus_artifact"], _corpus(split_execution).get("path")),
        _expect("scaleup_corpus_root_sha256", plan["training_inputs"]["scaleup_corpus_root_sha256"], SCALEUP_CORPUS_ROOT_SHA),
    ]
    checks.extend(_no_forbidden_work_checks(review_decision, "source_review"))
    checks.extend(_no_forbidden_work_checks(split_decision, "source_split_execution"))
    ...
    return _stable(
        {
            ...
            "source_artifacts": {
                ...
                "scaleup_corpus": {
                    "path": plan["training_inputs"]["scaleup_corpus_artifact"],
                    "root_sha256": plan["training_inputs"]["scaleup_corpus_root_sha256"],
                },
            },
            "heads": {
            ...
            "scaleup_training_preflight_plan": plan,
            "checks": checks,
            "final_decision": {
                "passed": passed,
                "status": READY_STATUS if passed else REJECT_STATUS,
...
def _training_preflight_plan(
    *,
    ...
    corpus = _corpus(split_execution)
    return {
        "camp_head": current_camp_head,
        ...
        "training_inputs": {
            ...
            "scaleup_corpus_artifact": corpus.get("path") or SCALEUP_CORPUS_ARTIFACT,
            "scaleup_corpus_root_sha256": corpus.get("root_sha256"),
            "dp_head": current_dp_head,
        ...
        "pass_conditions": {
            "train_records": EXPECTED_RECORD_COUNTS["train"],
            "calibration_records_used_for_training": 0,
            "holdout_records_used_for_training": 0,
            "score_expression": SCORE_EXPRESSION,
            "weights_nonnegative": True,
...
def _record_checks(records: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    all_records = [record for split in ordered_splits for record in records[split]]
    sample_sets = {
        split: {str(record.get("source_sample_id") or record.get("sample_id")) for record in records[split]}
        for split in ordered_splits
    }
    scene_sets = {
        split: {str(record.get("source_scene_id") or record.get("scene_id")) for record in records[split]}
        for split in ordered_splits
    }
    return {
        "counts": {split: len(records[split]) for split in ordered_splits},
...
def _counts(summary: dict[str, Any], key: str) -> dict[str, int] | None:
    value = summary.get(key)
    if value is None and key == "record_counts":
        value = summary.get("counts")
    return value


def _corpus(split_execution: dict[str, Any]) -> dict[str, Any]:
    source_artifacts = split_execution.get("source_artifacts", {})
    return source_artifacts.get("corpus") or source_artifacts.get("scaleup_corpus") or {}
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# V16 nuScenes Fixed-DP Scale-Up Training Preflight Plan",
            "",
            f"- Status: `{decision['status']}`",
            f"- Passed: `{decision['passed']}`",
            f"- Authorized next work: `{decision['authorized_next_work']}`",
            "- Scope: scale-up train-split training plan only; no training, evaluation, or performance claim.",
...
def _render_heads(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"SOURCE_SPLIT_RESULT_REVIEW_ROOT_SHA256={artifacts['split_result_review']['root_sha256']}",
            f"SOURCE_SPLIT_EXECUTION_ROOT_SHA256={artifacts['split_execution']['root_sha256']}",
            f"SOURCE_SCALEUP_CORPUS_ROOT_SHA256={artifacts['scaleup_corpus']['root_sha256']}",
