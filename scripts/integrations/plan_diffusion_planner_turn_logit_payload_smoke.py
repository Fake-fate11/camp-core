...
READY_STATUS = "turn_logit_payload_nonformal_smoke_plan_ready"
REJECT_STATUS = "turn_logit_payload_nonformal_smoke_plan_rejected"
SOURCE_STATUS = "turn_logit_payload_default_off_unit_wired"
AUTHORIZED_NEXT_WORK = "turn_logit_payload_paired_three_step_smoke_only"
FORMAL_SEEDS = frozenset({11, 12, 13})


@dataclass(frozen=True)
class SmokeSpec:
    root: str = "/root/autodl-tmp/camp_dp_turn_logit_payload_smoke"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Design-only preflight for the first default-off turn-logit payload "
...
def build_report(
    *,
    ...
    return {
        "analysis": {
            "name": "dp_camp_turn_logit_payload_nonformal_smoke_plan_v1",
            "label": label,
            ...
            "math_boundary": (
                "The smoke only enables default-off logging of optional "
                "current-tick per-candidate turn-indicator logits returned by "
                "fixed DP before selection. It must not change CAMP scores, "
                "feasibility, selected indices, DP candidates, turn-indicator "
                "behavior, or PerfectTracker execution. If later atomized, "
                "these are fixed coefficients a_k preserving affine "
...
def _source_checks(
    *,
    ...
    return [
        _check_tokens(
            "replay_default_off_turn_logit_payload_cli",
            replay_text,
            (
...
def _plan_checks(smoke: SmokeSpec) -> list[dict[str, Any]]:
    return [
        ...
        {"name": "tiny_three_step_scope", "passed": smoke.steps == 3, "value": smoke.steps},
        {
        ...
        {
            "name": "traffic_light_static_scope",
            "passed": smoke.traffic_lights in {"off", "on"},
            "value": smoke.traffic_lights,
        },
        {
...
def _payload_audit_command(
    ...
    return [
        ...
        str(smoke.num_candidates),
        "--output_json",
        f"{audit_dir}/turn_logit_payload_smoke.json",
        "--output_md",
        f"{audit_dir}/turn_logit_payload_smoke.md",
        "--require_pass",
    ]
...
def _accept_criteria(smoke: SmokeSpec) -> list[str]:
    return [
        ...
        "candidate records contain non-null turn_logit_payload_logging payloads",
        "payload schema, null behavior, finite checks, and latency fields pass audit",
