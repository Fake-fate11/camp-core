"""Seal the outcome-independent V25 batch8-primary generator contract."""
...
from camp_core.integrations.diffusion_planner_v25_batch8_primary_generator_contract import (  # noqa: E402
    ARTIFACT_SCHEMA,
    EXACT_DIR_KEYS,
    contract_amendment,
...
SOURCE_PATHS = {
    "producer": (
        ...
        "diffusion_planner_v25_batch8_primary_generator_contract.py"
    ),
    "reviewer": (
        "camp_core/camp_core/integrations/"
    ...
    "freeze_script": (
        ...
        "freeze_diffusion_planner_v25_batch8_primary_generator_contract.py"
    ),
    "review_script": (
        "scripts/integrations/"
        "review_diffusion_planner_v25_batch8_primary_generator_contract.py"
    ),
    "tests": (
        "camp_core/tests/"
        "test_diffusion_planner_v25_batch8_primary_generator_contract.py"
    ),
}
...
def freeze(*, output: Path, fixed_dp_repo: Path, exact_dirs: dict[str, str]) -> str:
    ...
    contract = validate_contract_amendment(
        contract_amendment(
    ...
    report = {
        ...
        "status": "sealed_outcome_independent_batch8_primary_contract",
        "contract": contract,
        "fixed_dp": {
    ...
    return _atomic(output, report, "V25 batch8-primary contract")
...
def _atomic(output: Path, report: dict[str, Any], label: str) -> str:
    ...
    try:
        ...
        (staging / "HEADS.json").write_bytes(
            _bytes(
                {
                    "role": "batch8_primary_generator_contract",
...
def main() -> int:
    ...
    print(freeze(output=args.output, fixed_dp_repo=args.fixed_dp_repo, exact_dirs=exact_dirs))
    return 0
