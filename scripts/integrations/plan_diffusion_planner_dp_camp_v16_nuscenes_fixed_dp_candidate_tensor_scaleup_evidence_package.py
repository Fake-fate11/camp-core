#!/usr/bin/env python3
"""Plan the v16 fixed-DP scale-up evidence package."""
...
SOURCE_MODULE = _load_module(
    ...
    "v16_scaleup_paired_eval_result_review",
)
...
READY_STATUS = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_evidence_package_plan_ready"
REJECT_STATUS = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_evidence_package_plan_rejected"
AUTHORIZED_NEXT_WORK = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_evidence_package_plan_static_review_only"
SCHEMA_VERSION = "dp_camp_v16_nuscenes_fixed_dp_candidate_tensor_scaleup_evidence_package_plan_v1"
PLAN_JSON_NAME = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_evidence_package_plan.json"
PLAN_MD_NAME = "v16_nuscenes_fixed_dp_candidate_tensor_scaleup_evidence_package_plan.md"
REQUIRED_FILES = (
    "JSON summaries",
    "rows JSONL",
...
EXPECTED_SCENES = 50
EXPECTED_SPLIT_ROWS = {"train": 6263, "calibration": 2156, "holdout": 1581}
EXPECTED_PAIRED_ROWS = 3737
EXPECTED_METRICS = {
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable_v16_nuscenes_fixed_dp_candidate_tensor_scaleup_evidence_package_plan",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        enabled=args.enable_v16_nuscenes_fixed_dp_candidate_tensor_scaleup_evidence_package_plan,
    )
...
def build_report(
    *,
    ...
    checks = [
        ...
        _expect("records_10000", plan["package_report"]["records"], EXPECTED_RECORDS),
        _expect("scenes_50", plan["package_report"]["scenes"], EXPECTED_SCENES),
        _expect("split_rows", plan["package_report"]["split_rows"], EXPECTED_SPLIT_ROWS),
        _expect("paired_eval_rows_3737", review.get("primary_eval_rows"), EXPECTED_PAIRED_ROWS),
        _expect("calibration_rows_2156", review.get("calibration_rows"), EXPECTED_SPLIT_ROWS["calibration"]),
        _expect("holdout_rows_1581", review.get("holdout_rows"), EXPECTED_SPLIT_ROWS["holdout"]),
        _expect("train_reporting_only_rows_6263", review.get("train_reporting_only_rows"), EXPECTED_SPLIT_ROWS["train"]),
        ...
        _expect("metrics_summary", plan["package_report"]["metrics_summary"], EXPECTED_METRICS),
        _expect("descriptive_paired_metrics_only", review.get("descriptive_paired_metrics_only"), True),
        _check("latency_summary_present", bool(review.get("latency_summary")), review.get("latency_summary"), "latency summary"),
        ...
        _expect("candidate_tensor_unmodified", plan["pass_checks"]["candidate_tensor_unmodified"], True),
        ...
        _expect("direct_claim_not_allowed", plan["package_report"]["recommended_next_path"]["direct_claim_allowed"], False),
    ]
    checks.extend(_required_file_checks(plan["required_files"]))
    ...
    return _stable(
        {
            ...
            "scaleup_evidence_package_plan": plan,
            "checks": checks,
            "final_decision": {
                "passed": passed,
                "status": READY_STATUS if passed else REJECT_STATUS,
...
def _evidence_package_plan(artifacts: list[dict[str, Any]], review: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "package_report": {
            ...
            "paired_eval_rows": review.get("primary_eval_rows"),
            "metrics_summary": _metrics_summary(review.get("primary_metrics", {})),
            "no_performance_claim": True,
            "no_safety_claim": True,
            "no_camp_over_dp_claim": True,
        ...
        "pass_checks": {
            ...
            "dp_head_fixed": FIXED_DP_HEAD,
            "camp_head_chain_recorded": all(artifact["heads"].get("CAMP_HEAD") for artifact in artifacts),
            "candidate_tensor_unmodified": review.get("candidate_tensor_mutated_count") == 0,
...
def _source_artifact(entry: dict[str, Any]) -> dict[str, Any]:
    ...
    files = _read_sha_manifest(sha_path)
...
def _metrics_summary(metrics: dict[str, Any]) -> dict[str, Any]:
    return {key: metrics.get(key) for key in EXPECTED_METRICS}


def _required_file_checks(files: list[str]) -> list[dict[str, Any]]:
    return [_check(f"required_file_{name}", name in files, "present" if name in files else "missing", name) for name in REQUIRED_FILES]
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    report_body = plan["package_report"]
    return "\n".join(
        [
            "# V16 nuScenes Fixed-DP Scale-Up Evidence Package Plan",
            "",
            f"- Status: `{decision['status']}`",
            f"- Passed: `{decision['passed']}`",
            f"- Authorized next work: `{decision['authorized_next_work']}`",
            "- Scope: plan only; no package construction, claim, promotion, deployment, training, or new paired evaluation.",
            f"- Source artifacts: `{[artifact['id'] for artifact in plan['source_artifacts']]}`",
            f"- Required files: `{plan['required_files']}`",
            f"- Planned package report: `{report_body}`",
            f"- Pass checks: `{plan['pass_checks']}`",
            "",
        ]
    )
...
def _read_sha_manifest(path: Path) -> list[dict[str, str]]:
    if not path.is_file():
        return []
    rows = []
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        rows.append({"path": rel.strip(), "sha256": sha})
    return rows
