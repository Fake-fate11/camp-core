#!/usr/bin/env python3
"""Static review for the v14 promotion-readiness evaluation preflight plan.

This gate reviews a previously materialized preflight plan. It verifies the
plan artifact, fixed-DP boundary, no-go conditions, and current EOF contract. It
does not execute evaluation, replay, training, candidate generation, promotion,
...
REJECT_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_readiness_evaluation_preflight_plan_static_review_rejected"
)
...
PLAN_MD_NAME = "post_closeout_promotion_readiness_evaluation_preflight_plan.md"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--preflight_plan_script_py", type=Path, required=True)
    parser.add_argument("--preflight_plan_test_py", type=Path, required=True)
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Explicit opt-in for read-only static review of the preflight plan.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        preflight_plan_script_py=args.preflight_plan_script_py,
        preflight_plan_test_py=args.preflight_plan_test_py,
        ...
        label=args.label,
        enabled=args.enable_v14_post_closeout_promotion_readiness_evaluation_preflight_plan_static_review,
    )
...
def build_report(
    *,
    ...
    preflight_plan_script_py: Path,
    preflight_plan_test_py: Path,
    v14_audit_md: Path,
    ...
    paths = {
        ...
        "preflight_plan_script_py": preflight_plan_script_py.resolve(),
        "preflight_plan_test_py": preflight_plan_test_py.resolve(),
    ...
    source_plan = _read_json_dict(paths["preflight_plan_json"])
    root_sha256s = _read_sha256sums(artifact_files["root_sha256s"])
    plan_sha256s = _read_sha256sums(paths["preflight_plan_sha256s"])
    heads = _parse_key_values(_read_text(artifact_files["heads"]))
    script_text = _read_text(paths["preflight_plan_script_py"])
    test_text = _read_text(paths["preflight_plan_test_py"])
    ...
    checks: list[dict[str, Any]] = [
        ...
        _check("preflight_plan_artifact_dir_exists", artifact_dir.is_dir(), str(artifact_dir), "directory"),
    ]
    ...
    checks.extend(
        [
            _expect("preflight_plan_json_matches_artifact_layout", paths["preflight_plan_json"], artifact_files["plan_json"]),
            _expect("preflight_plan_md_matches_artifact_layout", paths["preflight_plan_md"], artifact_files["plan_md"]),
            _expect("preflight_plan_sha256s_matches_artifact_layout", paths["preflight_plan_sha256s"], artifact_files["plan_sha256s"]),
        ]
    )
    ...
    checks.extend(_plan_contract_checks(source_plan))
    ...
    return {
        ...
        "analysis": {
            ...
            "preflight_plan_json": str(paths["preflight_plan_json"]),
            "preflight_plan_md": str(paths["preflight_plan_md"]),
            "preflight_plan_sha256s": str(paths["preflight_plan_sha256s"]),
...
def write_outputs(output_dir: Path, report: dict[str, Any]) -> None:
    ...
    _write_json(output_dir / "post_closeout_promotion_readiness_evaluation_preflight_plan_static_review.json", report)
    (output_dir / "post_closeout_promotion_readiness_evaluation_preflight_plan_static_review.md").write_text(
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- No-go conditions: `{summary.get('no_go_count')}`",
        "",
...
def _artifact_hash_checks(files: dict[str, Path], root_sha256s: dict[str, str], plan_sha256s: dict[str, str]) -> list[dict[str, Any]]:
    root_expected = {
...
def _heads_checks(heads: dict[str, str], source_plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        _check("artifact_heads_source_static_review_present", bool(heads.get("source_gap_analysis_static_review_artifact")), heads.get("source_gap_analysis_static_review_artifact"), "path"),
        _check("artifact_heads_source_gap_present", bool(heads.get("source_gap_analysis_artifact")), heads.get("source_gap_analysis_artifact"), "path"),
...
def _plan_contract_checks(source_plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    checks = [
        ...
        _expect("source_plan_check_failures", _failed_source_checks(source_plan), []),
        _expect("source_plan_preflight_plan_count", len(_list(source_plan.get("preflight_plan"))), 4),
...
def _source_surface_checks(script: str, test: str) -> list[dict[str, Any]]:
    return [
        _contains("source_surface_script_schema", script, "promotion_readiness_evaluation_preflight_plan_v1"),
        _contains("source_surface_script_static_review_next", script, "promotion_readiness_evaluation_preflight_plan_static_review_only"),
        ...
        _contains("source_surface_test_pass_case", test, "test_promotion_readiness_evaluation_preflight_plan_passes"),
        _contains("source_surface_test_requires_enable", test, "test_promotion_readiness_evaluation_preflight_plan_requires_enable"),
        _contains("source_surface_test_rejects_wrong_eof", test, "test_promotion_readiness_evaluation_preflight_plan_rejects_wrong_eof"),
    ]
...
def _audit_checks(v14_text: str, status_text: str) -> list[dict[str, Any]]:
    ...
    return [
        _expect("audit_latest_eof_authorizes_plan_static_review", (_latest_value(v14_text, "current_v14_status"), _latest_value(v14_text, "next_work_target")), expected_pair),
        _expect("status_doc_latest_eof_authorizes_plan_static_review", (_latest_value(status_text, "current_v14_status"), _latest_value(status_text, "next_work_target")), expected_pair),
        _expect("audit_preflight_plan_ready", _latest_value(v14_text, "post_closeout_promotion_readiness_evaluation_preflight_plan_ready"), "True"),
        _expect("audit_plan_static_review_authorized", _latest_value(v14_text, "post_closeout_promotion_readiness_evaluation_preflight_plan_static_review_authorized"), "True"),
...
def _decision(passed: bool, checks: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    decision = {
        ...
        "post_closeout_promotion_readiness_evaluation_preflight_plan_static_review_passed": bool(passed),
        "promotion_readiness_evaluation_preflight_authorized": bool(passed),
        "recommendation": "run_promotion_readiness_evaluation_preflight_only",
        "immediate_action": "execute_read_only_promotion_readiness_evaluation_preflight",
...
def _failure_class(failed: list[str]) -> str:
    ...
    if "static_review_enabled" in failed_set:
        return "explicit_preflight_plan_static_review_authorization_missing"
    ...
    if any(name.endswith("_sha") or name.endswith("_root_sha") for name in failed):
        return "preflight_plan_artifact_sha256_mismatch"
    ...
    return "promotion_readiness_evaluation_preflight_plan_static_review_failure"
...
def _failed_source_checks(source_plan: dict[str, Any]) -> list[str]:
    return [
        str(check.get("name"))
        for check in _list(source_plan.get("plan_checks"))
