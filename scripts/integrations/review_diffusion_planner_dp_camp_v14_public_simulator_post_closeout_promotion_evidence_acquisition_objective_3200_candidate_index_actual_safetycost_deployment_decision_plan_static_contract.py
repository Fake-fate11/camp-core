#!/usr/bin/env python3
"""Read-only static review for the objective-3200 deployment decision plan."""
...
def _load_plan_module():
    plan_path = Path(__file__).resolve().with_name(
        ...
        "actual_safetycost_deployment_decision.py"
    )
...
EXPECTED_PLAN_CHECK_COUNT = 69
DEPLOYMENT_DECISION_ITEMS = PLAN_MODULE.DEPLOYMENT_DECISION_ITEMS

SCHEMA_VERSION = (
    "dp_camp_v14_public_simulator_post_closeout_promotion_evidence_acquisition_"
    "objective_3200_candidate_index_actual_safetycost_deployment_decision_plan_static_review_v1"
)
READY_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_deployment_decision_plan_static_review_passed"
)
REJECT_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_deployment_decision_plan_static_review_rejected"
)
AUTHORIZED_NEXT_WORK = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_deployment_decision_only"
)

REVIEW_JSON_NAME = (
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_deployment_decision_plan_static_review.json"
)
REVIEW_MD_NAME = (
    "post_closeout_promotion_evidence_acquisition_objective_3200_"
    "candidate_index_actual_safetycost_deployment_decision_plan_static_review.md"
)


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--source_deployment_decision_plan_artifact_dir", type=Path, required=True)
    parser.add_argument("--source_deployment_decision_plan_json", type=Path, required=True)
    parser.add_argument("--source_deployment_decision_plan_md", type=Path, required=True)
    parser.add_argument("--source_deployment_decision_plan_sha256s", type=Path, required=True)
    parser.add_argument("--deployment_decision_plan_script", type=Path, required=True)
    parser.add_argument("--deployment_decision_plan_test", type=Path, required=True)
    ...
    parser.add_argument(
        "--enable_v14_post_closeout_promotion_evidence_acquisition_objective_3200_candidate_index_actual_safetycost_deployment_decision_plan_static_review",
        action="store_true",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        source_deployment_decision_plan_artifact_dir=args.source_deployment_decision_plan_artifact_dir,
        source_deployment_decision_plan_json=args.source_deployment_decision_plan_json,
        source_deployment_decision_plan_md=args.source_deployment_decision_plan_md,
        source_deployment_decision_plan_sha256s=args.source_deployment_decision_plan_sha256s,
        deployment_decision_plan_script=args.deployment_decision_plan_script,
        deployment_decision_plan_test=args.deployment_decision_plan_test,
        ...
        enabled=(
            args.enable_v14_post_closeout_promotion_evidence_acquisition_objective_3200_candidate_index_actual_safetycost_deployment_decision_plan_static_review
        ),
    )
...
def build_report(
    *,
    source_deployment_decision_plan_artifact_dir: Path,
    source_deployment_decision_plan_json: Path,
    source_deployment_decision_plan_md: Path,
    source_deployment_decision_plan_sha256s: Path,
    deployment_decision_plan_script: Path,
    deployment_decision_plan_test: Path,
    v14_audit_md: Path,
    ...
    artifact_dir = source_deployment_decision_plan_artifact_dir.resolve()
    paths = {
        "source_deployment_decision_plan_json": source_deployment_decision_plan_json.resolve(),
        "source_deployment_decision_plan_md": source_deployment_decision_plan_md.resolve(),
        "source_deployment_decision_plan_sha256s": source_deployment_decision_plan_sha256s.resolve(),
        "deployment_decision_plan_script": deployment_decision_plan_script.resolve(),
        "deployment_decision_plan_test": deployment_decision_plan_test.resolve(),
        "v14_audit_md": v14_audit_md.resolve(),
        "current_status_md": current_status_md.resolve(),
    }
    files = _artifact_files(artifact_dir)
    source_plan = BASE_MODULE._read_json_dict(paths["source_deployment_decision_plan_json"])
    ...
    nested_sha256s = BASE_MODULE._read_sha256sums(paths["source_deployment_decision_plan_sha256s"])
    script_text = BASE_MODULE._read_text(paths["deployment_decision_plan_script"])
    test_text = BASE_MODULE._read_text(paths["deployment_decision_plan_test"])
    checks = _checks(
        enabled=enabled,
    ...
    return {
        ...
        "inputs": {
            "source_deployment_decision_plan_artifact_dir": str(artifact_dir),
        ...
        "source_deployment_decision_plan_summary": _source_plan_summary(source_plan),
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    source = report["source_deployment_decision_plan_summary"]
    failed = decision["failed_checks"] or ["none"]
    return "\n".join(
        [
            "# Objective-3200 Candidate-Index Actual-SafetyCost Deployment Decision Plan Static Review",
            "",
            ...
            f"- Deployment decision item count: `{source['deployment_decision_item_count']}`",
            "",
            "This static review authorizes only the next deployment decision gate.",
            "",
        ]
    )
...
def _checks(
    *,
    ...
    deployment_plan = _list(source_plan.get("deployment_decision_plan"))
    checks = [
        BASE_MODULE._expect("static_review_enabled", enabled, True),
        BASE_MODULE._check("source_deployment_decision_plan_artifact_dir_exists", artifact_dir.is_dir(), str(artifact_dir), "directory"),
        BASE_MODULE._expect("source_deployment_decision_plan_json_path_matches_artifact", paths["source_deployment_decision_plan_json"], files["plan_json"]),
        BASE_MODULE._expect("source_deployment_decision_plan_md_path_matches_artifact", paths["source_deployment_decision_plan_md"], files["plan_md"]),
        BASE_MODULE._expect("source_deployment_decision_plan_sha256s_path_matches_artifact", paths["source_deployment_decision_plan_sha256s"], files["plan_sha256s"]),
        ...
        BASE_MODULE._expect("audit_plan_ready", BASE_MODULE._latest_value(v14_text, "objective_3200_candidate_index_actual_safetycost_deployment_decision_plan_ready"), "True"),
        BASE_MODULE._expect("audit_static_review_authorized", BASE_MODULE._latest_value(v14_text, "objective_3200_candidate_index_actual_safetycost_deployment_decision_plan_static_review_authorized"), "True"),
        ...
        BASE_MODULE._expect("source_plan_static_review_authorized", decision.get("objective_3200_candidate_index_actual_safetycost_deployment_decision_plan_static_review_authorized"), True),
        ...
        BASE_MODULE._expect("deployment_decision_item_count", len(deployment_plan), len(DEPLOYMENT_DECISION_ITEMS)),
        ...
        BASE_MODULE._check("plan_script_schema_token", PLAN_SCHEMA in script_text or "deployment_decision_plan_v1" in script_text, PLAN_SCHEMA, "present"),
        BASE_MODULE._check("plan_script_static_review_next_token", AUTHORIZED_CURRENT_WORK in script_text or "deployment_decision_plan_static_review_only" in script_text, AUTHORIZED_CURRENT_WORK, "present"),
        BASE_MODULE._check("plan_script_no_deployment_now_token", "authorizes_deployment_now" in script_text, "authorizes_deployment_now", "present"),
        BASE_MODULE._check("plan_test_pass_test", "deployment_decision_plan_passes" in test_text, "deployment_decision_plan_passes", "present"),
        BASE_MODULE._check("plan_test_hash_drift_test", "rejects_hash_drift" in test_text, "rejects_hash_drift", "present"),
        BASE_MODULE._check("plan_test_deployment_leak_test", "rejects_source_deployment_leak" in test_text, "rejects_source_deployment_leak", "present"),
    ]
...
def _source_plan_summary(source_plan: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "deployment_decision_item_count": len(_list(source_plan.get("deployment_decision_plan"))),
    }
...
def _decision(*, passed: bool, checks: list[dict[str, Any]], source_plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if passed:
        ...
        failure_class = "explicit_candidate_index_actual_safetycost_deployment_decision_plan_static_review_authorization_missing"
        ...
        failure_class = "source_deployment_decision_plan_contract_failure"
    else:
        failure_class = "deployment_decision_plan_static_review_contract_failure"
    return {
        "passed": bool(passed),
        ...
        "objective_3200_candidate_index_actual_safetycost_deployment_decision_plan_static_review_passed": bool(passed),
        "objective_3200_candidate_index_actual_safetycost_deployment_decision_authorized": bool(passed),
        "source_plan_passed": source_decision.get("passed"),
        "source_plan_ready": source_decision.get("objective_3200_candidate_index_actual_safetycost_deployment_decision_plan_ready"),
