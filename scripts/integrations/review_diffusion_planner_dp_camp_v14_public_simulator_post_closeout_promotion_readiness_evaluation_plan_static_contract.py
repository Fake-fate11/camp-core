#!/usr/bin/env python3
"""Static review for the v14 promotion-readiness evaluation plan.

This gate reviews a previously materialized read-only evaluation plan artifact.
It verifies fixed-DP provenance, artifact hashes, source contract boundaries,
EOF authorization, and the source surface for case-insensitive HEADS parsing.
It does not run evaluation, replay, training, candidate generation, promotion,
...
REJECT_STATUS = (
    "public_simulator_fixed_dp_candidate_generation_trained_default_off_"
    "shadow_replay_evaluation_default_off_shadow_selector_runtime_"
    "post_closeout_promotion_readiness_evaluation_plan_static_review_rejected"
)
...
REVIEW_JSON_NAME = "post_closeout_promotion_readiness_evaluation_plan_static_review.json"
REVIEW_MD_NAME = "post_closeout_promotion_readiness_evaluation_plan_static_review.md"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--plan_md", type=Path, required=True)
    parser.add_argument("--plan_sha256s", type=Path, required=True)
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Explicit opt-in for read-only static review of the evaluation plan.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        plan_sha256s=args.plan_sha256s,
        ...
        label=args.label,
        enabled=args.enable_v14_post_closeout_promotion_readiness_evaluation_plan_static_review,
    )
...
def build_report(
    *,
    ...
    paths = {
        ...
        "plan_md": plan_md.resolve(),
        "plan_sha256s": plan_sha256s.resolve(),
    ...
    source_plan = _read_json_dict(paths["plan_json"])
    root_sha256s = _read_sha256sums(artifact_files["root_sha256s"])
    plan_sha256s_values = _read_sha256sums(paths["plan_sha256s"])
    ...
    checks.extend(_artifact_hash_checks(artifact_files, root_sha256s, plan_sha256s_values))
    ...
    return {
        ...
        "analysis": {
            ...
            "plan_sha256s": str(paths["plan_sha256s"]),
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Source decision surfaces: `{summary.get('decision_surface_count')}`",
        f"- Source evidence requirements: `{summary.get('evidence_requirement_count')}`",
        "",
...
def _heads_checks(heads: dict[str, str], source_plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        _check("artifact_heads_source_static_review_present", bool(heads.get("source_preflight_static_review_artifact")), heads.get("source_preflight_static_review_artifact"), "path"),
        _check("artifact_heads_source_preflight_present", bool(heads.get("source_preflight_artifact")), heads.get("source_preflight_artifact"), "path"),
...
def _source_surface_checks(script: str, test: str) -> list[dict[str, Any]]:
    return [
        _contains("source_surface_script_schema", script, "promotion_readiness_evaluation_plan_v1"),
        _contains("source_surface_script_static_review_next", script, "promotion_readiness_evaluation_plan_static_review_only"),
        _contains("source_surface_script_static_review_flag", script, "evaluation_plan_static_review_authorized"),
        ...
        _contains("source_surface_test_pass_case", test, "test_promotion_readiness_evaluation_plan_passes"),
        _contains("source_surface_test_requires_enable", test, "test_promotion_readiness_evaluation_plan_requires_enable"),
        _contains("source_surface_test_rejects_wrong_eof", test, "test_promotion_readiness_evaluation_plan_rejects_wrong_eof"),
        _contains("source_surface_test_accepts_uppercase_dp_head", test, "test_promotion_readiness_evaluation_plan_accepts_uppercase_source_dp_head"),
    ]
...
def _audit_checks(v14_text: str, status_text: str) -> list[dict[str, Any]]:
    ...
    return [
        ...
        _expect("audit_plan_ready", _latest_value(v14_text, "post_closeout_promotion_readiness_evaluation_plan_ready"), "True"),
        _expect("audit_plan_static_review_authorized", _latest_value(v14_text, "post_closeout_promotion_readiness_evaluation_plan_static_review_authorized"), "True"),
...
def _decision(passed: bool, checks: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    decision = {
        ...
        "post_closeout_promotion_readiness_evaluation_plan_static_review_passed": bool(passed),
        "evaluation_runbook_preflight_plan_authorized": bool(passed),
        "recommendation": "plan_follow_on_evaluation_runbook_preflight_only",
        "immediate_action": "plan_read_only_evaluation_runbook_preflight_only",
...
def _failure_class(failed: list[str]) -> str:
    ...
    if "static_review_enabled" in failed_set:
        return "explicit_evaluation_plan_static_review_authorization_missing"
    ...
    if any(name.endswith("_sha") or name.endswith("_root_sha") for name in failed):
        return "plan_artifact_sha256_mismatch"
    ...
    return "promotion_readiness_evaluation_plan_static_review_failure"
